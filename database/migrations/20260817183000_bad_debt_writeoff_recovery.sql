...
-- Adds additive write-off/reversal and payment-backed recovery/reversal evidence.
-- Issued invoices remain immutable and receivable state remains derived.
...
VALUES
    (NULL, 'finance.bad_debt.view', 'View bad debt', 'View controlled receivable write-off and recovery evidence.', TRUE),
    (NULL, 'finance.bad_debt.write_off', 'Write off receivable', 'Record a controlled partial or full receivable write-off.', TRUE),
    (NULL, 'finance.bad_debt.write_off.reverse', 'Reverse receivable write-off', 'Reverse an existing receivable write-off with evidence.', TRUE),
    (NULL, 'finance.bad_debt.recovery', 'Record bad-debt recovery', 'Apply otherwise unallocated customer cash as recovery against written-off debt.', TRUE),
    (NULL, 'finance.bad_debt.recovery.reverse', 'Reverse bad-debt recovery', 'Reverse bad-debt recovery evidence and restore payment availability.', TRUE)
...
CREATE TABLE receivable_write_off_reversals (
    ...
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    UNIQUE KEY uq_receivable_write_off_reversals_public (organisation_id, public_id),
...
CREATE TABLE receivable_recoveries (
    ...
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    UNIQUE KEY uq_receivable_recoveries_public (organisation_id, public_id),
    UNIQUE KEY uq_receivable_recoveries_id_organisation (id, organisation_id),
...
CREATE TABLE receivable_recovery_reversals (
    ...
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    UNIQUE KEY uq_receivable_recovery_reversals_public (organisation_id, public_id),

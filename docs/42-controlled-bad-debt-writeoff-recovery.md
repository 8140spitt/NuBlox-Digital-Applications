...
Status: implemented application boundary on Package 004J feature branch pending final release gate.
...
Package 004J does not itself post a tax adjustment or general-ledger entry. Where tax adjustment is required, that remains an explicit later accounting boundary.
...
Recovery uses the payment row as the first cash mutex before calculating available payment capacity and recovery headroom.
...
The exact final integration-test and Svelte totals are recorded only after the documentation-synchronised release head has passed CI.

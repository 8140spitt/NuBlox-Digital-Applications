...
# Period close now includes bank settlement readiness.
period_path = "app/src/lib/server/finance/accounting-period-service.ts"
replace_once(
    period_path,
...
# Period UI exposes the additional readiness signal.
...
replace_once(
    ...
    "\t\t\t\t\t\t\t<td>\n",
    "\t\t\t\t\t\t\t<td>{period.unexportedJournalCount}</td>\n"
    "\t\t\t\t\t\t\t<td>{period.unreconciledSupplierPaymentCount}</td>\n"
    "\t\t\t\t\t\t\t<td>\n",
)
replace_once(
    period_page,
...
# Supplier payment workspace points to settlement evidence.
...
# Capability registry reflects the delivered finance thread and exposes the workspace.
...
# Remove a limit-dependent workspace classification: use database active-match predicates directly.
...
replace_once(
    ...
    "\t\t\t\t\tselect 1\n"
    "\t\t\t\t\tfrom bank_reconciliation_matches as active_match\n"
...
if active_sets in text:
    write(bank_service, text.replace(active_sets, "", 1))
replace_once(
    bank_service,
    "\t\t\t.where(\n"
    "\t\t\t\tsql<boolean>`exists (\n"
...
# Extend the existing canonical AP journey into bank settlement and hard-close readiness.
...
text = read(ap_test)
marker = "requires active bank settlement evidence before hard close and freezes that evidence after close"
if marker not in text:
	...
	it('requires active bank settlement evidence before hard close and freezes that evidence after close', async () => {
		...
		await expect(
			periodService.hardClose(actorMaker, september.publicId, 'Attempt close before settlement evidence.')
		...
		expect(
			bankWorkspace.unsettledSupplierPayments.some((payment) => payment.publicId === paymentPublicId)
		).toBe(true);
...
'''
    write(ap_test, text[: -len(suffix)] + block + suffix)

# Delete this one-time patcher from the branch commit produced by CI.

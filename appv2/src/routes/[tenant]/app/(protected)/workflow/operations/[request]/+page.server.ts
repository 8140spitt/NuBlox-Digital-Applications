...
async function actorFor(request: Request, params: { tenant: string }, returnTo: string) {
	...
	return { organisationId: access.organisationId, userId: access.userId, memberId: access.memberId };
}
...
export const load: PageServerLoad = async ({ parent, params }) => {
	...
	try {
		...
		if (cause instanceof WorkflowOperationsValidationError) redirect(303, `/${tenant.slug}/app/workflow/operations`);
		throw cause;
	}
};

function handled(cause: unknown) {
	if (cause instanceof WorkflowOperationsValidationError) return fail(400, { formError: cause.message });
	if (cause instanceof WorkflowOperationsAccessError) return fail(403, { formError: cause.message });
	throw cause;
}
...
export const actions = {
	priority: async ({ request, params, url }) => {
		...
		try {
			await setWorkflowPriority({ actor, requestPublicId: params.request, priority: String(data.get('priority')) as 'low' | 'normal' | 'high' | 'urgent' | 'critical', reason: String(data.get('reason') ?? '') });
			return { success: 'Workflow priority updated.' };
		} catch (cause) { return handled(cause); }
	},
	status: async ({ request, params, url }) => {
		const actor = await actorFor(request, params, `${url.pathname}${url.search}`);
		const data = await request.formData();
		try {
			await setWorkflowWorkStatus({ actor, requestPublicId: params.request, status: String(data.get('status')) as 'open' | 'in_progress' | 'blocked', reason: String(data.get('reason') ?? '') });
	...
	reassign: async ({ request, params, url }) => {
		...
		const data = await request.formData();
		try {
			await reassignWorkflowWorkItem({ actor, requestPublicId: params.request, memberId: Number(data.get('memberId')), reason: String(data.get('reason') ?? '') });
	...
	cancel: async ({ request, params, url }) => {
		...
		const data = await request.formData();
		try {
			await cancelWorkflowOperation({ actor, requestPublicId: params.request, reason: String(data.get('reason') ?? '') });

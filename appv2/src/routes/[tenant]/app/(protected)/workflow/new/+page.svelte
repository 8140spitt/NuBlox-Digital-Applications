...
<div class="nb-page workflow-create">
	...
	/>
	{#if form?.formError}<Alert tone="danger" title="Template not created">{form.formError}</Alert>{/if}
	<Panel title="Template identity" description="Keep the key stable; business-event bindings follow the workflow lineage." padding="spacious">
		<form method="POST" use:enhance class="form-stack">
			<Field id="templateKey" label="Template key" hint="For example f01.strategy-approval." required>
				<input class="nb-control" id="templateKey" name="templateKey" value={form?.values?.templateKey ?? ''} required />
			</Field>
...
<style>
	.workflow-create { max-width: 760px; padding-top: var(--nb-space-5); padding-bottom: var(--nb-space-10); }
	.form-stack { display: grid; gap: var(--nb-space-4); margin-top: var(--nb-space-2); }
	.actions { display: flex; justify-content: flex-end; align-items: center; gap: var(--nb-space-3); padding-top: var(--nb-space-2); }
	.cancel { color: var(--nb-color-text-secondary); text-decoration: none; font-weight: var(--nb-weight-semibold); }
</style>

...
export const load: PageServerLoad = async ({ parent, params }) => {
	...
	try {
		const workspace = await getOperatingModelWorkspace({
		...
		return {
			workspace,
			draftPlans: workspace.plans.filter((plan) => plan.status === 'draft')
		};
	} catch (cause) {
...
export const actions = {
	default: async ({ request, params, url }) => {
		...
		try {
			await createOperatingModelComponent({
				actor,
				frameworkPublicId: params.strategy,
				planPublicId: text(data, 'planPublicId'),
				componentCode: text(data, 'componentCode'),
				componentType: text(data, 'componentType') as OperatingModelComponentType,
				title: text(data, 'title'),
				currentState: text(data, 'currentState') || null,
				targetState: text(data, 'targetState'),
				parentComponentPublicId: text(data, 'parentComponentPublicId') || null
			});
			redirect(303, routes.strategyOperatingModel(access.organisationRouteSlug, params.strategy));
		} catch (cause) {

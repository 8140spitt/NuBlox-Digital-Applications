...
<div class="nb-page-form">
	<Breadcrumbs
		items={[
			...
			},
			{
				...
				href: routes.strategyForesight(data.tenant.slug, data.workspace.framework.publicId)
			},
	...
	{#if form?.formError}<Alert tone="danger" title="Scenario not created">{form.formError}</Alert
		>{/if}

	<Panel
		title="Scenario definition"
		description="Use one stable scenario code for a lineage. Controlled revisions increment the version while preserving approved history."
	>
		<form method="POST" use:enhance class="scenario-form">
			<div class="two-column">
				...
				<Field id="scenarioType" label="Scenario type" required>
					<select
						...
						name="scenarioType"
						required
						value={value('scenarioType', 'baseline')}
					>
			...
			<div class="two-column">
				<Field id="horizonStart" label="Horizon start" required>
					<input
						...
						name="horizonStart"
						type="date"
						min={data.workspace.framework.horizonStart}
						max={data.workspace.framework.horizonEnd}
						value={value('horizonStart', data.workspace.framework.horizonStart)}
						required
					/>
				</Field>
				<Field id="horizonEnd" label="Horizon end" required>
					<input
						...
						min={data.workspace.framework.horizonStart}
						max={data.workspace.framework.horizonEnd}
						value={value('horizonEnd', data.workspace.framework.horizonEnd)}
						required
					/>
				</Field>
			</div>
			<Field
				...
				hint="Describe the coherent future state, not just a list of risks."
				required
			>
				<textarea class="nb-control" id="narrative" name="narrative" rows="7" required
					>{value('narrative')}</textarea
				>
			</Field>

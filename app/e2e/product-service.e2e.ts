...
test('F05 governs portfolio, need, idea, investment and lifecycle design', async ({ page }) => {
	...
	await offering.getByLabel('Portfolio').selectOption({ label: 'F05-E2E · E2E Product & Service Portfolio' });
	...
	await idea.locator('textarea[name="problemStatement"]').fill('Lifecycle evidence is fragmented across functions.');
	await idea.locator('textarea[name="proposedValue"]').fill('Create a governed cross-domain lifecycle control thread.');
	...
	const score = page.locator('form[action="?/scoreIdea"]').filter({ has: page.locator('input[value]') }).first();
	...
	await businessCase.locator('select[name="offeringPublicId"]').selectOption({ label: /OFFER-E2E/ });
	...
	await businessCase.locator('textarea[name="riskSummary"]').fill('Controlled implementation and adoption risk.');
	...
	const approval = page.locator('form[action="?/approveBusinessCase"]').filter({ hasText: 'Approve' }).first();
	...
	await design.locator('textarea[name="functionalRequirements"]').fill('Govern each lifecycle gate with evidence.');
	await design.locator('textarea[name="acceptanceCriteria"]').fill('Design review passes and approval is attributable.');
	...
	await review.locator('textarea[name="findings"]').fill('Design satisfies the governed lifecycle gate.');

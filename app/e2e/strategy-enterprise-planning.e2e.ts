...
test('F01 strategy moves from intent and analysis to approved immutable evidence and revision', async ({
	...
	await expect(page.getByRole('heading', { name: 'Strategy & enterprise planning', level: 1 })).toBeVisible();
	...
	await expect(page.getByRole('heading', { name: 'Lead with governed digital thread' })).toBeVisible();

	const decision = page.locator('form[action="?/decideOption"]');
	await decision.getByRole('combobox').selectOption('selected');
	await decision.getByPlaceholder('Decision rationale').fill('Best route to differentiated customer value.');
	await decision.getByRole('button', { name: 'Record decision' }).click();
	await expect(page.getByText('Decision: Best route to differentiated customer value.')).toBeVisible();
	...
	await expect(page.getByRole('heading', { name: 'Prove enterprise-to-asset continuity' })).toBeVisible();

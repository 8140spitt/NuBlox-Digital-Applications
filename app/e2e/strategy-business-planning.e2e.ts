...
test('F01 business plan governs objective through operating model to approved revision evidence', async ({ page }) => {
	await signIn(page);
	...
	await createStrategy.getByLabel('Purpose').fill('Convert strategy into accountable enterprise execution.');
	await createStrategy.getByLabel('Vision').fill('Operate a coherent strategy-to-performance system.');
	await createStrategy.getByLabel('Mission').fill('Govern objectives, plans, operating model and execution as one thread.');
	...
	await factor.getByLabel('Analysis').fill('Customers expect joined-up enterprise delivery evidence.');
	...
	await option.getByLabel('Description').fill('Translate strategic intent into funded cross-functional execution.');
	...
	await objective.getByLabel('Description').fill('Mobilise accountable initiatives against approved strategic intent.');
	...
	await expect(page.getByRole('heading', { name: 'Business planning & operating model', level: 1 })).toBeVisible();
	...
	await createPlan.getByLabel('Plan narrative').fill('Execute approved strategy through funded initiatives and an accountable target operating model.');
	...
	await initiative.getByLabel('Target outcome').fill('Cross-functional teams execute approved strategy through shared canonical records.');
	await initiative.getByLabel('Benefit statement').fill('Faster decisions with less duplicated enterprise data.');
	...
	await expect(page.getByRole('heading', { name: 'Mobilise integrated enterprise delivery' })).toBeVisible();
	...
	await component.getByLabel('Current state').fill('Function-led handoffs and duplicated reporting.');
	await component.getByLabel('Target state').fill('Value streams coordinate through canonical records and explicit accountability.');
	...
	await accountability.getByLabel('Notes').fill('Business accountability does not grant access authority.');

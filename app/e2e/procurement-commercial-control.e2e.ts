...
test('owner procures, commits, values and controls project cost through the browser', async ({
	page
}) => {
	await signIn(page);
	...
	await packagePanel
		...
		.fill('Galvanised containment installation package');
	...
	const packageCard = page
		.locator('#package-register .record-card')
	...
	await poPanel
		.getByLabel('Order line description')
		.fill('Galvanised containment installation package');
	...
	await budgetPanel
		...
		.selectOption({ label: `${costCode} · Containment materials` });
	...
	let budgetCard = page
		...
		.filter({ hasText: `Approved baseline ${suffix}` });
	...
	await variationPanel
		.getByLabel('Description')
		.fill('Additional containment supports required by coordination.');
	...
	await variationCard.locator('select[name="decision"]').selectOption('accepted');
	...
	await expect(
		page.locator('.metrics article').filter({ hasText: 'Approved baseline budget' })
	).toContainText('£5,000.00');
	await expect(
		page.locator('.metrics article').filter({ hasText: 'Issued PO commitment' })
	).toContainText('£1,250.00');
	await expect(
		page.locator('.metrics article').filter({ hasText: 'Cost-code classified' })
	).toContainText('£1,250.00');
	await expect(
		page.locator('.metrics article').filter({ hasText: 'Accepted receipt cost' })
	).toContainText('£500.00');
	await expect(
		page.locator('.metrics article').filter({ hasText: 'Approved change' })
	).toContainText('£250.00');

	await page.goto(
		`/commercial/valuations?project=${encodeURIComponent(projectPublicId)}#create-valuation`
	);
	...
	await valuationPanel
		.getByLabel('Description')
		.fill('Supplier application for installed containment to date.');
	...
	let valuationCard = page
		.locator('#valuation-register .valuation-card')
		.filter({ hasText: purchaseOrderNumber });

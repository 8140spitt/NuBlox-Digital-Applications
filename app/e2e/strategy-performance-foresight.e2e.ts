...
test('F01 performance closes the strategy loop through KPI, review and foresight', async ({ page }) => {
	await signIn(page);
	...
	await createStrategy.getByLabel('Purpose').fill('Turn approved intent into measurable enterprise outcomes.');
	await createStrategy.getByLabel('Vision').fill('See strategy, actual performance and future scenarios as one evidence chain.');
	await createStrategy.getByLabel('Mission').fill('Govern targets, actuals, actions, reviews and foresight without duplicate truth.');
	...
	await factor.getByLabel('Analysis').fill('Demand and input costs require active performance management.');
	...
	await option.getByLabel('Description').fill('Use integrated cost, revenue and delivery evidence to improve margin.');
	...
	await decision.getByPlaceholder('Decision rationale').fill('Creates an executable strategy-to-performance thread.');
	...
	await objective.getByLabel('Description').fill('Increase enterprise operating margin through accountable execution.');
	...
	await expect(page.getByRole('heading', { name: 'Strategy performance & foresight', level: 1 })).toBeVisible();

	const kpi = page.locator('form[action="?/createKpi"]');
	await kpi.getByLabel('Strategic objective').selectOption({ label: 'OBJ-PERF-E2E · Improve operating margin' });
	...
	await kpi.getByLabel('KPI description').fill('Operating margin from canonical finance reporting.');
	...
	await observation.getByLabel('Approved KPI').selectOption({ label: 'KPI-PERF-E2E · Operating margin' });
	...
	await action.getByLabel('Approved KPI').selectOption({ label: 'KPI-PERF-E2E · Operating margin' });
	...
	await action.getByLabel('Action', { exact: true }).fill('Close the margin gap through accountable cost actions.');
	...
	await completion.getByLabel('Completion note ACT-PERF-E2E').fill('Cost actions approved and mobilised.');
	...
	await review.getByLabel('Review summary').fill('Review margin performance and agree corrective action.');
	...
	await snapshot.getByLabel('Draft review').selectOption({ label: 'REV-PERF-E2E · H1 strategic performance review' });
	await snapshot.getByLabel('Approved KPI').selectOption({ label: 'KPI-PERF-E2E · Operating margin' });
	...
	await scenario.getByLabel('Scenario narrative').fill('Test margin resilience under weaker demand.');
	...
	await assumption.getByLabel('Assumption description').fill('Demand growth falls below the approved baseline.');
	...
	await projection.getByLabel('Approved KPI').selectOption({ label: 'KPI-PERF-E2E · Operating margin' });
	await projection.getByLabel('Projection date').fill('2028-12-31');
	await projection.getByLabel('Projected value').fill('8.2');
	await projection.getByLabel('Projection rationale').fill('Downside demand compresses operating margin.');

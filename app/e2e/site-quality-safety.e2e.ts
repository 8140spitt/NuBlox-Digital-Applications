...
test('owner records and closes controlled site, quality and safety workflows through the browser', async ({
	...
	await page.getByLabel(/Description/).fill('Browser acceptance project for site, quality and safety.');
	...
	await expect(page.getByRole('heading', { name: 'Site, quality & safety', level: 1 })).toBeVisible();
	...
	await expect(page.locator('#site-register .record-card').filter({ hasText: siteName })).toBeVisible();
	...
	await diaryPanel.getByLabel('Activity description').fill('Installed containment to the east corridor.');
	...
	await expect(page).toHaveURL(siteUrl);
	await expect(page.locator('#template-register .record-card').filter({ hasText: templateName })).toContainText('published');
	...
	await expect(page).toHaveURL(siteUrl);
	let inspectionCard = page.locator('#inspection-register .inspection-card').filter({ hasText: inspectionTitle });
	...
	await expect(page).toHaveURL(siteUrl);
	inspectionCard = page.locator('#inspection-register .inspection-card').filter({ hasText: inspectionTitle });
	...
	await defectPanel.getByLabel('Description').fill('Add one containment support at the corridor transition.');
	...
	await safetyPanel.getByLabel('Description').fill('Temporary lead crossed the pedestrian access route.');
	await safetyPanel.getByLabel('Location').fill('Level 01 access route');
	await safetyPanel.getByLabel('Immediate action taken').fill('Lead moved clear pending protected reroute.');

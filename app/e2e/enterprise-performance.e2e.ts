...
test('F03 exposes a governed enterprise performance command centre', async ({ page }) => {
	...
	await expect(page.getByRole('heading', { name: 'Performance command centre', level: 1 })).toBeVisible();
	...
	await form.getByLabel('Purpose').fill('Operate enterprise performance through one governed evidence chain.');
	await form.getByLabel('Scope').fill('Executive KPIs, reporting, intervention, review, benchmarking and benefits.');

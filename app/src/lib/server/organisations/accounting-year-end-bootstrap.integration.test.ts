...
async function cleanup() {
	...
	if (organisationId) {
		await db
			...
			.execute();
		await db
			.deleteFrom('accounting_year_end_closes')
			.where('organisation_id', '=', organisationId)
			.execute();
		await db
...
describe('Package 004O future organisation bootstrap parity', () => {
	it('grants Owner/Admin year-end authority, keeps Finance/Commercial unprivileged and honours explicit prepare deny', async () => {
		...
		const actor: TenantActorContext = {
			...
			userId,
			memberId,
			correlationId: `year-end-bootstrap-${randomUUID()}`
		};
		const permission = await db
			.selectFrom('permissions')
			.select('id')
			.where('permission_key', '=', 'finance.accounting.year_end.prepare')
			.executeTakeFirstOrThrow();
		await db
			...
			.values({
				...
				effect: 'deny',
				reason: 'Package 004O explicit-deny precedence test'
			})
			.executeTakeFirstOrThrow();

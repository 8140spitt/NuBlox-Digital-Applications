import { randomUUID } from 'node:crypto';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

import type { TenantActorContext } from '$lib/server/auth/tenant-actor-context';
import { closeDatabase, getDatabase, type Database } from '$lib/server/db/database';
import { TenantAccessError } from '$lib/server/kernel/errors';
import {
	OrganisationLocationService,
	OrganisationLocationValidationError
} from './organisation-location-service';

const PREFIX = 'Organisation Location Integration ';

let db: Database;
let organisationId: string;
let otherOrganisationId: string;
let managerUserId: string;
let managerMemberId: string;
let memberUserId: string;
let memberId: string;
let roleId: string;
let organisationManagePermissionId: string;

function insertedId(result: { insertId?: bigint }): string {
	if (result.insertId === undefined) throw new Error('Expected an AUTO_INCREMENT insert ID.');
	return result.insertId.toString();
}

function actor(userId: string, organisationMemberId: string): TenantActorContext {
	return {
		organisationId,
		userId,
		memberId: organisationMemberId,
		correlationId: randomUUID()
	};
}

async function createUser(name: string): Promise<string> {
	return insertedId(
		await db
			.insertInto('users')
			.values({ public_id: randomUUID(), display_name: `${PREFIX}${name}`, status: 'active' })
			.executeTakeFirstOrThrow()
	);
}

async function createMember(organisation: string, userId: string): Promise<string> {
	return insertedId(
		await db
			.insertInto('organisation_members')
			.values({
				organisation_id: organisation,
				user_id: userId,
				public_id: randomUUID(),
				status: 'active',
				joined_at: new Date()
			})
			.executeTakeFirstOrThrow()
	);
}

async function cleanup(): Promise<void> {
	if (!db) return;
	const organisationIds = [organisationId, otherOrganisationId].filter(Boolean);
	if (organisationIds.length > 0) {
		await db.deleteFrom('outbox_events').where('organisation_id', 'in', organisationIds).execute();
		await db
			.deleteFrom('audit_events')
			.where('acting_organisation_id', 'in', organisationIds)
			.execute();
		await db
			.deleteFrom('organisation_locations')
			.where('organisation_id', 'in', organisationIds)
			.execute();
		await db.deleteFrom('addresses').where('organisation_id', 'in', organisationIds).execute();
		await db.deleteFrom('member_roles').where('organisation_id', 'in', organisationIds).execute();
		await db
			.deleteFrom('role_permissions')
			.where('organisation_id', 'in', organisationIds)
			.execute();
		await db
			.deleteFrom('organisation_roles')
			.where('organisation_id', 'in', organisationIds)
			.execute();
		await db
			.deleteFrom('organisation_members')
			.where('organisation_id', 'in', organisationIds)
			.execute();
		await db.deleteFrom('organisations').where('id', 'in', organisationIds).execute();
	}
	const userIds = [managerUserId, memberUserId].filter(Boolean);
	if (userIds.length > 0) await db.deleteFrom('users').where('id', 'in', userIds).execute();
}

async function createFixture(): Promise<void> {
	organisationManagePermissionId = (
		await db
			.selectFrom('permissions')
			.select('id')
			.where('permission_key', '=', 'organisation.manage')
			.where('is_active', '=', 1)
			.executeTakeFirstOrThrow()
	).id;

	organisationId = insertedId(
		await db
			.insertInto('organisations')
			.values({ public_id: randomUUID(), legal_name: `${PREFIX}Organisation`, status: 'active' })
			.executeTakeFirstOrThrow()
	);
	otherOrganisationId = insertedId(
		await db
			.insertInto('organisations')
			.values({
				public_id: randomUUID(),
				legal_name: `${PREFIX}Other Organisation`,
				status: 'active'
			})
			.executeTakeFirstOrThrow()
	);

	managerUserId = await createUser('Manager');
	memberUserId = await createUser('Member');
	managerMemberId = await createMember(organisationId, managerUserId);
	memberId = await createMember(organisationId, memberUserId);

	roleId = insertedId(
		await db
			.insertInto('organisation_roles')
			.values({
				organisation_id: organisationId,
				public_id: randomUUID(),
				name: `${PREFIX}Manager`,
				is_active: 1
			})
			.executeTakeFirstOrThrow()
	);
	await db
		.insertInto('role_permissions')
		.values({
			organisation_id: organisationId,
			organisation_role_id: roleId,
			permission_id: organisationManagePermissionId
		})
		.executeTakeFirstOrThrow();
	await db
		.insertInto('member_roles')
		.values({
			organisation_id: organisationId,
			organisation_member_id: managerMemberId,
			organisation_role_id: roleId
		})
		.executeTakeFirstOrThrow();
}

describe('organisation location governance', () => {
	beforeAll(async () => {
		db = getDatabase();
		await cleanup();
		await createFixture();
	});

	afterAll(async () => {
		await cleanup();
		await closeDatabase();
	});

	it('creates and corrects a location without mutating the previous address snapshot', async () => {
		const manager = actor(managerUserId, managerMemberId);
		const service = new OrganisationLocationService(db);
		const locationPublicId = await service.createLocation(manager, {
			name: '  London Office  ',
			locationType: 'OFFICE',
			timezone: 'Europe/London',
			address: {
				line1: '1 Original Street',
				city: 'London',
				postalCode: 'SW1A 1AA',
				countryCode: 'gb'
			}
		});

		const first = (await service.load(manager)).locations[0]!;
		expect(first).toMatchObject({
			publicId: locationPublicId,
			name: 'London Office',
			locationType: 'office',
			timezone: 'Europe/London',
			isActive: true,
			address: {
				line1: '1 Original Street',
				city: 'London',
				postalCode: 'SW1A 1AA',
				countryCode: 'GB'
			}
		});
		const originalAddressId = first.address!.id;

		await service.updateLocation(manager, locationPublicId, {
			name: 'London Headquarters',
			locationType: 'headquarters',
			timezone: 'Europe/London',
			isActive: true,
			address: {
				line1: '2 Corrected Street',
				city: 'London',
				postalCode: 'SW1A 2AA',
				countryCode: 'GB'
			}
		});

		const corrected = (await service.load(manager)).locations[0]!;
		expect(corrected).toMatchObject({
			publicId: locationPublicId,
			name: 'London Headquarters',
			locationType: 'headquarters',
			address: {
				line1: '2 Corrected Street',
				postalCode: 'SW1A 2AA'
			}
		});
		expect(corrected.address!.id).not.toBe(originalAddressId);

		const preservedAddress = await db
			.selectFrom('addresses')
			.select(['line_1', 'postal_code'])
			.where('id', '=', originalAddressId)
			.where('organisation_id', '=', organisationId)
			.executeTakeFirstOrThrow();
		expect(preservedAddress).toEqual({ line_1: '1 Original Street', postal_code: 'SW1A 1AA' });

		const auditActions = await db
			.selectFrom('audit_events')
			.select('action_key')
			.where('acting_organisation_id', '=', organisationId)
			.where('subject_public_id', '=', locationPublicId)
			.orderBy('id', 'asc')
			.execute();
		expect(auditActions.map((row) => row.action_key)).toEqual([
			'organisation.location.create',
			'organisation.location.update'
		]);

		const topics = await db
			.selectFrom('outbox_events')
			.select('topic')
			.where('organisation_id', '=', organisationId)
			.where('aggregate_type', '=', 'organisation')
			.orderBy('id', 'asc')
			.execute();
		expect(topics.map((row) => row.topic)).toEqual([
			'organisation.location.created',
			'organisation.location.changed'
		]);
	});

	it('rejects duplicate names and invalid address or timezone values', async () => {
		const manager = actor(managerUserId, managerMemberId);
		const service = new OrganisationLocationService(db);
		await service.createLocation(manager, {
			name: 'Manchester Depot',
			locationType: 'depot'
		});

		await expect(
			service.createLocation(manager, {
				name: 'Manchester Depot',
				locationType: 'office'
			})
		).rejects.toBeInstanceOf(OrganisationLocationValidationError);
		await expect(
			service.createLocation(manager, {
				name: 'Invalid timezone',
				locationType: 'office',
				timezone: 'Not/A-Timezone'
			})
		).rejects.toBeInstanceOf(OrganisationLocationValidationError);
		await expect(
			service.createLocation(manager, {
				name: 'Incomplete address',
				locationType: 'office',
				address: { line1: '1 Test Street', countryCode: 'GBR' }
			})
		).rejects.toBeInstanceOf(OrganisationLocationValidationError);
	});

	it('requires organisation.manage and the full active tenant membership tuple', async () => {
		const service = new OrganisationLocationService(db);
		await expect(
			service.createLocation(actor(memberUserId, memberId), {
				name: 'Unauthorised Office',
				locationType: 'office'
			})
		).rejects.toBeInstanceOf(TenantAccessError);

		await expect(
			service.createLocation(
				{
					organisationId: otherOrganisationId,
					userId: managerUserId,
					memberId: managerMemberId,
					correlationId: randomUUID()
				},
				{ name: 'Forged Office', locationType: 'office' }
			)
		).rejects.toBeInstanceOf(TenantAccessError);
	});
});

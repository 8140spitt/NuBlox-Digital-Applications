...
async function permissionKeys(roleName: string): Promise<string[]> {
	const rows = await db.selectFrom('role_permissions as grant')
		...
		.select('permission.permission_key as permissionKey').where('grant.organisation_id', '=', organisationId).where('role.name', '=', roleName).where('permission.permission_key', 'like', 'finance.bad_debt.%').orderBy('permission.permission_key').execute();
...
beforeAll(async () => {
	...
	if (result.insertId === undefined) throw new Error('Expected user insert ID.'); userId = result.insertId.toString();
});
...
describe('Package 004J future organisation bootstrap parity', () => {
	it('persists stronger write-off authority separately from delegated assessment/recovery', async () => {
		const created = await new OrganisationBootstrapService(db).createForExistingUser({ userId, correlationId: `bad-debt-bootstrap-${randomUUID()}` }, { legalName: `${PREFIX}Organisation`, defaultTimezone: 'Europe/London', defaultCurrencyCode: 'GBP' });
		organisationId = created.organisationId;
		const allSeven = ['finance.bad_debt.case.manage','finance.bad_debt.recommend','finance.bad_debt.recovery.record','finance.bad_debt.recovery.reverse','finance.bad_debt.view','finance.bad_debt.write_off.authorise','finance.bad_debt.write_off.reverse'].sort();
		const delegated = ['finance.bad_debt.case.manage','finance.bad_debt.recommend','finance.bad_debt.recovery.record','finance.bad_debt.recovery.reverse','finance.bad_debt.view'].sort();

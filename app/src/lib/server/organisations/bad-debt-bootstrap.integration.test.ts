...
describe('Package 004J future organisation bootstrap parity', () => {
	it('persists all five bad-debt permissions for Owner, Administrator and Finance/Commercial', async () => {
		...
		await expect(permissionKeys('Owner')).resolves.toEqual(allFive);
		await expect(permissionKeys('Administrator')).resolves.toEqual(allFive);
		await expect(permissionKeys('Finance/Commercial')).resolves.toEqual(allFive);
	});
});
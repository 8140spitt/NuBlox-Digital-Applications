...
export type BenefitInput = {
	...
	title: string;
	benefitType:
		'financial' | 'operational' | 'customer' | 'people' | 'risk' | 'sustainability' | 'other';
	unitLabel: string;
...
function coded(value: string, label: string): string {
	...
	if (!CODE.test(normalized))
		throw new EnterprisePerformanceValidationError(`${label} has an invalid format.`);
	return normalized;
}
...
function dateOnly(value: string | Date, label: string): Date {
	...
	if (Number.isNaN(date.getTime()))
		throw new EnterprisePerformanceValidationError(`${label} is invalid.`);
	return date;
}
...
export class EnterprisePerformanceService {
	...
	private async requirePermission(
		...
		if (!decision.allowed)
			throw new TenantAccessError('Enterprise performance action is not permitted.');
	}
	...
	private async activeMember(
		...
		if (!row)
			throw new EnterprisePerformanceValidationError(
				`${label} must be an active organisation member.`
			);
		return row.id;
	}
	...
	async getWorkspace(
	...
	): Promise<EnterprisePerformanceWorkspace> {
		...
		let selectedFramework: PerformanceFrameworkRecord | null =
			frameworks.find((row) => row.lifecycle_status === 'approved') ?? frameworks[0] ?? null;
		...
		const observations = (
			await Promise.all(
				kpis
					.filter((row) => selectedKpiIds.has(row.id))
					.map((row) => repository.listObservationsForKpi(row.id))
			)
		).flat();
	...
	async createFramework(
		actor: TenantActorContext,
		input: PerformanceFrameworkInput
	): Promise<PerformanceFrameworkRecord> {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (await repository.findLatestFrameworkVersion(actor.organisationId, code))
				throw new EnterprisePerformanceValidationError(
					'A performance framework with this code already exists; create a controlled revision instead.'
				);
			const owner = await this.activeMember(
				trx,
			...
			await this.evidence(
				trx,
				actor,
				'performance.framework.create',
				'strategy_performance_framework',
				row.public_id,
				{ frameworkCode: row.framework_code, versionNumber: row.version_number },
				['F03.01']
			);
			return row;
		});
	...
	async createFrameworkRevision(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (source.lifecycle_status !== 'approved')
				throw new EnterprisePerformanceValidationError(
					'Only an approved framework can be revised.'
				);
			...
			if (!latest || latest.id !== source.id)
				throw new EnterprisePerformanceValidationError(
					'Only the latest approved framework version can be revised.'
				);
			const revision = await repository.insertFramework({
				...
				supersedes_performance_framework_id: source.id,
				created_by_member_id: actor.memberId,
				approved_by_member_id: null,
				approved_at: null
			});
			for (const link of await repository.listFrameworkKpis(source.id)) {
				await repository.insertFrameworkKpi({
					organisation_id: actor.organisationId,
					performance_framework_id: revision.id,
					strategy_kpi_id: link.strategy_kpi_id,
					display_order: link.display_order,
					materiality_threshold_percent: link.materiality_threshold_percent,
					commentary_required: link.commentary_required,
					created_by_member_id: actor.memberId
				});
			}
			await this.evidence(
	...
	async addFrameworkKpi(
		actor: TenantActorContext,
		input: FrameworkKpiInput
	): Promise<PerformanceFrameworkKpiRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new EnterprisePerformanceRepository(trx);
			const framework = await repository.findFrameworkByPublicId(
				actor.organisationId,
				input.frameworkPublicId.trim()
			);
			if (!framework) throw new RecordNotFoundError('Performance framework not found.');
			if (framework.lifecycle_status !== 'draft')
				throw new EnterprisePerformanceValidationError(
					'KPIs can only be composed into a draft performance framework.'
				);
			...
			if (!kpi || kpi.lifecycle_status !== 'approved')
				throw new RecordNotFoundError('Approved canonical KPI not found.');
			const link = await repository.insertFrameworkKpi({
				...
				materiality_threshold_percent: optionalDecimal(
			...
			await this.evidence(
				trx,
				actor,
				'performance.framework.kpi.add',
				'strategy_performance_framework',
				framework.public_id,
				{ kpiPublicId: kpi.public_id, kpiCode: kpi.kpi_code },
				['F03.01']
			);
			return link;
		});
	...
	async approveFramework(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (framework.lifecycle_status !== 'draft')
				throw new EnterprisePerformanceValidationError(
					'Only a draft performance framework can be approved.'
				);
			if ((await repository.listFrameworkKpis(framework.id)).length === 0)
				throw new EnterprisePerformanceValidationError(
					'A performance framework requires at least one approved canonical KPI.'
				);
			if (framework.supersedes_performance_framework_id) {
				await repository.updateFramework(
					actor.organisationId,
					framework.supersedes_performance_framework_id,
			...
			await this.evidence(
				trx,
				actor,
				'performance.framework.approve',
				'strategy_performance_framework',
				approved.public_id,
				{ frameworkCode: approved.framework_code, versionNumber: approved.version_number },
				['F03.01']
			);
			return approved;
		});
	...
	): Promise<PerformancePeriodRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new EnterprisePerformanceRepository(trx);
			const framework = await repository.findFrameworkByPublicId(
			...
			if (!framework || framework.lifecycle_status !== 'approved')
				throw new RecordNotFoundError('Approved performance framework not found.');
			...
			if (end < start || reportingDate < end)
				throw new EnterprisePerformanceValidationError('Reporting period dates are invalid.');
			const row = await repository.insertPeriod({
				organisation_id: actor.organisationId,
				performance_framework_id: framework.id,
				public_id: this.publicIdFactory(),
				period_code: coded(input.periodCode, 'Period code'),
				title: requiredText(input.title, 'Period title', 255),
				period_start: start,
				period_end: end,
	...
	async createPack(
		actor: TenantActorContext,
		input: PerformancePackInput
	): Promise<PerformancePackRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new EnterprisePerformanceRepository(trx);
			const period = await repository.findPeriodByPublicId(
				actor.organisationId,
				input.periodPublicId.trim()
			);
			if (!period || period.lifecycle_status === 'closed')
				throw new RecordNotFoundError('Open performance period not found.');
			const framework = await trx
				.selectFrom('strategy_performance_frameworks')
				.selectAll()
				.where('organisation_id', '=', actor.organisationId)
				.where('id', '=', period.performance_framework_id)
				.executeTakeFirst();
			if (!framework || framework.lifecycle_status !== 'approved')
				throw new EnterprisePerformanceValidationError(
					'Performance packs require an approved performance framework.'
				);
			const links = await repository.listFrameworkKpis(framework.id);
			if (!links.length)
				throw new EnterprisePerformanceValidationError('Performance framework contains no KPIs.');
			const pack = await repository.insertPack({
				organisation_id: actor.organisationId,
				performance_period_id: period.id,
				public_id: this.publicIdFactory(),
				pack_code: coded(input.packCode, 'Pack code'),
				version_number: 1,
				title: requiredText(input.title, 'Pack title', 255),
				executive_summary: requiredText(input.executiveSummary, 'Executive summary', 20_000),
				lifecycle_status: 'draft',
				supersedes_performance_pack_id: null,
				created_by_member_id: actor.memberId,
				approved_by_member_id: null,
				approved_at: null
			});
			for (const link of links) {
				...
				if (!observation)
					throw new EnterprisePerformanceValidationError(
						`KPI ${kpi.kpi_code} has no authoritative observation for the reporting period.`
					);
				const delta = variance(kpi.target_value, observation.actual_value);
				await repository.insertPackKpi({
					organisation_id: actor.organisationId,
					performance_pack_id: pack.id,
					strategy_kpi_id: kpi.id,
					strategy_kpi_observation_id: observation.id,
					target_value_snapshot: kpi.target_value,
					actual_value_snapshot: observation.actual_value,
					forecast_value_snapshot: observation.forecast_value,
					variance_value: delta.value,
					variance_percent: delta.percent,
					assessment: assess(kpi, observation.actual_value, link.materiality_threshold_percent),
					...
					source_public_id: observation.source_public_id,
					created_by_member_id: actor.memberId
				});
			}
			await repository.updatePeriod(actor.organisationId, period.id, {
				lifecycle_status: 'reported'
			});
			await this.evidence(
				trx,
				actor,
				'performance.pack.create',
				'strategy_performance_pack',
				pack.public_id,
				{ packCode: pack.pack_code, periodPublicId: period.public_id, kpiCount: links.length },
				['F03.02']
			);
			return pack;
		});
	...
	async approvePack(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (pack.lifecycle_status !== 'draft')
				throw new EnterprisePerformanceValidationError(
					'Only a draft performance pack can be approved.'
				);
			const approved = await repository.updatePack(actor.organisationId, pack.id, {
				lifecycle_status: 'approved',
				approved_by_member_id: actor.memberId,
				approved_at: new Date()
			});
			await this.evidence(
				trx,
				actor,
				'performance.pack.approve',
				'strategy_performance_pack',
				approved.public_id,
				{ packCode: approved.pack_code },
				['F03.02']
			);
			return approved;
		});
	...
	async createVariance(
		actor: TenantActorContext,
		input: VarianceInput
	): Promise<PerformanceVarianceRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const packKpi = await trx
				.selectFrom('strategy_performance_pack_kpis')
				.selectAll()
				.where('organisation_id', '=', actor.organisationId)
				.where('id', '=', input.packKpiId.trim())
				.executeTakeFirst();
			if (!packKpi) throw new RecordNotFoundError('Performance pack KPI not found.');
			if (packKpi.assessment === 'on_track')
				throw new EnterprisePerformanceValidationError(
					'An on-track KPI does not require a variance case.'
				);
			const owner = await this.activeMember(
				trx,
				actor.organisationId,
			...
			const repository = new EnterprisePerformanceRepository(trx);
			const row = await repository.insertVariance({
				organisation_id: actor.organisationId,
				performance_pack_kpi_id: packKpi.id,
				public_id: this.publicIdFactory(),
				variance_code: coded(input.varianceCode, 'Variance code'),
				materiality: input.materiality,
				cause_category: input.causeCategory,
				root_cause_text: requiredText(input.rootCauseText, 'Root cause', 20_000),
				impact_text: requiredText(input.impactText, 'Impact', 20_000),
				owner_member_id: owner,
				lifecycle_status: 'open',
			...
			await this.evidence(
				trx,
				actor,
				'performance.variance.create',
				'strategy_performance_variance',
				row.public_id,
				{ varianceCode: row.variance_code, assessment: packKpi.assessment },
				['F03.03']
			);
			return row;
		});
	...
	): Promise<PerformanceActionRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new EnterprisePerformanceRepository(trx);
			const varianceRow = await repository.findVarianceByPublicId(
				actor.organisationId,
				input.variancePublicId.trim()
			);
			if (!varianceRow || varianceRow.lifecycle_status === 'closed')
				throw new RecordNotFoundError('Open performance variance not found.');
			const owner = await this.activeMember(
				trx,
				actor.organisationId,
				input.ownerMemberId,
			...
			const row = await repository.insertAction({
				organisation_id: actor.organisationId,
				performance_variance_id: varianceRow.id,
				public_id: this.publicIdFactory(),
				action_code: coded(input.actionCode, 'Action code'),
				title: requiredText(input.title, 'Action title', 255),
				action_text: requiredText(input.actionText, 'Action', 20_000),
				owner_member_id: owner,
			...
			await repository.updateVariance(actor.organisationId, varianceRow.id, {
				lifecycle_status: 'actioned'
			});
			await this.evidence(
				trx,
				actor,
				'performance.action.create',
				'strategy_performance_action',
				row.public_id,
				{ actionCode: row.action_code, variancePublicId: varianceRow.public_id },
				['F03.03']
			);
			return row;
		});
	...
	async completeCorrectiveAction(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (action.lifecycle_status === 'completed')
				throw new EnterprisePerformanceValidationError('Performance action is already completed.');
			...
			await this.evidence(
				trx,
				actor,
				'performance.action.complete',
				'strategy_performance_action',
				completed.public_id,
				{ actionCode: completed.action_code },
				['F03.03']
			);
			return completed;
		});
	...
	async closeVariance(
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new EnterprisePerformanceRepository(trx);
			const row = await repository.findVarianceByPublicId(
				actor.organisationId,
				variancePublicId.trim()
			);
			if (!row) throw new RecordNotFoundError('Performance variance not found.');
			const openActions = (await repository.listActions(actor.organisationId)).filter(
				(action) =>
					action.performance_variance_id === row.id &&
					!['completed', 'cancelled'].includes(action.lifecycle_status)
			);
			if (openActions.length)
				throw new EnterprisePerformanceValidationError(
					'Complete or cancel all corrective actions before closing the variance.'
				);
			const closed = await repository.updateVariance(actor.organisationId, row.id, {
			...
			await this.evidence(
				trx,
				actor,
				'performance.variance.close',
				'strategy_performance_variance',
				closed.public_id,
				{ varianceCode: closed.variance_code },
				['F03.03']
			);
			return closed;
		});
	...
	async createReview(
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new EnterprisePerformanceRepository(trx);
			const pack = await repository.findPackByPublicId(
				actor.organisationId,
				input.packPublicId.trim()
			);
			if (!pack || pack.lifecycle_status !== 'approved')
				throw new RecordNotFoundError('Approved performance pack not found.');
			...
			if (meetingPublicId) {
				const meeting = await trx
					...
					.where('organisation_id', '=', actor.organisationId)
					.where('public_id', '=', meetingPublicId)
					.executeTakeFirst();
			...
			const row = await repository.insertReview({
				organisation_id: actor.organisationId,
				performance_pack_id: pack.id,
				public_id: this.publicIdFactory(),
				review_code: coded(input.reviewCode, 'Review code'),
				review_date: dateOnly(input.reviewDate, 'Review date'),
				title: requiredText(input.title, 'Review title', 255),
				summary: requiredText(input.summary, 'Review summary', 20_000),
				decision_text: optionalText(input.decisionText, 20_000),
				governance_meeting_public_id: meetingPublicId,
				governance_decision_public_id: decisionPublicId,
				lifecycle_status: 'draft',
				created_by_member_id: actor.memberId,
				approved_by_member_id: null,
				approved_at: null
			...
			await this.evidence(
				...
				row.public_id,
				{
					...
					packPublicId: pack.public_id,
					governanceMeetingPublicId: meetingPublicId,
					governanceDecisionPublicId: decisionPublicId
				},
				['F03.04']
			);
			return row;
	...
	async approveReview(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const review = await trx
				.selectFrom('strategy_performance_reviews')
				.selectAll()
				.where('organisation_id', '=', actor.organisationId)
				.where('public_id', '=', reviewPublicId.trim())
				.executeTakeFirst();
			if (!review) throw new RecordNotFoundError('Performance review not found.');
			if (review.lifecycle_status !== 'draft')
				throw new EnterprisePerformanceValidationError(
					'Only a draft performance review can be approved.'
				);
			const approved = await repository.updateReview(actor.organisationId, review.id, {
				lifecycle_status: 'approved',
				approved_by_member_id: actor.memberId,
				approved_at: new Date()
			});
			await this.evidence(
				...
				'strategy_performance_review',
				approved.public_id,
				{ reviewCode: approved.review_code },
				['F03.04']
			);
			return approved;
		});
	...
	async createBenchmark(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (!kpi || kpi.lifecycle_status !== 'approved')
				throw new RecordNotFoundError('Approved canonical KPI not found.');
			if (requiredText(input.unitLabel, 'Benchmark unit', 64) !== kpi.unit_label)
				throw new EnterprisePerformanceValidationError(
					'Benchmark unit must match the canonical KPI unit.'
				);
			const start = dateOnly(input.periodStart, 'Benchmark period start');
			const end = dateOnly(input.periodEnd, 'Benchmark period end');
			if (end < start)
				throw new EnterprisePerformanceValidationError(
					'Benchmark period end must not precede period start.'
				);
			const row = await repository.insertBenchmark({
				organisation_id: actor.organisationId,
				strategy_kpi_id: kpi.id,
				public_id: this.publicIdFactory(),
				benchmark_code: coded(input.benchmarkCode, 'Benchmark code'),
				benchmark_type: input.benchmarkType,
				title: requiredText(input.title, 'Benchmark title', 255),
				scope_text: requiredText(input.scopeText, 'Benchmark scope', 20_000),
				unit_label: kpi.unit_label,
				period_start: start,
				period_end: end,
				benchmark_value: decimal(input.benchmarkValue, 'Benchmark value'),
				provenance_text: requiredText(input.provenanceText, 'Benchmark provenance', 20_000),
				source_reference: optionalText(input.sourceReference, 512),
				created_by_member_id: actor.memberId
			});
			await this.evidence(
				trx,
				...
				'strategy_performance_benchmark',
				row.public_id,
				{ benchmarkCode: row.benchmark_code, kpiPublicId: kpi.public_id },
				['F03.05']
			);
			return row;
		});
	...
	async compareBenchmark(
		actor: TenantActorContext,
		benchmarkPublicId: string,
		observationPublicId: string,
		interpretation: string
	): Promise<PerformanceBenchmarkResultRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new EnterprisePerformanceRepository(trx);
			const benchmark = await repository.findBenchmarkByPublicId(
				actor.organisationId,
				benchmarkPublicId.trim()
			);
			const observation = await repository.findObservationByPublicId(
				actor.organisationId,
				observationPublicId.trim()
			);
			...
			const row = await repository.insertBenchmarkResult({
				organisation_id: actor.organisationId,
				performance_benchmark_id: benchmark.id,
				strategy_kpi_observation_id: observation.id,
				public_id: this.publicIdFactory(),
				gap_value: delta.value,
				gap_percent: delta.percent,
				interpretation: requiredText(interpretation, 'Benchmark interpretation', 20_000),
				created_by_member_id: actor.memberId
			});
			await this.evidence(
				trx,
				...
				'strategy_performance_benchmark_result',
				row.public_id,
				{ benchmarkPublicId: benchmark.public_id, observationPublicId: observation.public_id },
				['F03.05']
			);
			return row;
		});
	...
	async createBenefit(
		actor: TenantActorContext,
		input: BenefitInput
	): Promise<PerformanceBenefitRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new EnterprisePerformanceRepository(trx);
			const kpi = await repository.findKpiByPublicId(
			...
			const row = await repository.insertBenefit({
				organisation_id: actor.organisationId,
				strategy_kpi_id: kpi.id,
				public_id: this.publicIdFactory(),
				benefit_code: coded(input.benefitCode, 'Benefit code'),
				title: requiredText(input.title, 'Benefit title', 255),
				benefit_type: input.benefitType,
				unit_label: requiredText(input.unitLabel, 'Benefit unit', 64),
				baseline_value: decimal(input.baselineValue, 'Benefit baseline'),
				target_value: decimal(input.targetValue, 'Benefit target'),
				target_date: dateOnly(input.targetDate, 'Benefit target date'),
				owner_member_id: owner,
				review_cadence: input.reviewCadence,
				source_domain: token(input.sourceDomain, 'Source domain', true)!,
				source_record_type: token(input.sourceRecordType, 'Source record type', true)!,
				source_public_id: requiredText(input.sourcePublicId, 'Source public ID', 128),
			...
			await this.evidence(
				trx,
				actor,
				'performance.benefit.create',
				'strategy_performance_benefit',
				row.public_id,
				{
					benefitCode: row.benefit_code,
					kpiPublicId: kpi.public_id,
					source: `${row.source_domain}:${row.source_record_type}:${row.source_public_id}`
				},
				['F03.06']
			);
			return row;
	...
	async recordBenefitMeasurement(
		actor: TenantActorContext,
		input: BenefitMeasurementInput
	): Promise<PerformanceBenefitMeasurementRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new EnterprisePerformanceRepository(trx);
			const benefit = await repository.findBenefitByPublicId(
				actor.organisationId,
				input.benefitPublicId.trim()
			);
			...
			const row = await repository.insertBenefitMeasurement({
				organisation_id: actor.organisationId,
				performance_benefit_id: benefit.id,
				public_id: this.publicIdFactory(),
			...
			await this.evidence(
				trx,
				actor,
				'performance.benefit.measure',
				'strategy_performance_benefit_measurement',
				row.public_id,
				{
					benefitPublicId: benefit.public_id,
					realisedValue: row.realised_value,

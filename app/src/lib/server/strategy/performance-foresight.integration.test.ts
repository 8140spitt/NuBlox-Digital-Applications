...
async function createMember(organisationId: string, userId: string): Promise<string> {
	return insertedId(await db.insertInto('organisation_members').values({ organisation_id: organisationId, user_id: userId, public_id: randomUUID(), status: 'active', joined_at: new Date('2026-09-07T08:00:00.000Z') }).executeTakeFirstOrThrow());
}
...
async function cleanup(): Promise<void> {
	...
	await db.deleteFrom('strategy_scenario_kpi_projections').where('organisation_id', 'in', ids).execute();
	await db.deleteFrom('strategy_scenario_assumptions').where('organisation_id', 'in', ids).execute();
	await db.updateTable('strategy_scenarios').set({ supersedes_strategy_scenario_id: null }).where('organisation_id', 'in', ids).execute();
	...
	await db.updateTable('strategy_kpis').set({ supersedes_strategy_kpi_id: null }).where('organisation_id', 'in', ids).execute();
	...
	await db.updateTable('strategy_frameworks').set({ supersedes_strategy_framework_id: null }).where('organisation_id', 'in', ids).execute();
...
beforeAll(async () => {
	...
	await assignPermissionRole(organisationAId, ownerMemberId, 'Strategy owner', ['strategy.view', 'strategy.manage', 'strategy.approve']);
	await assignPermissionRole(organisationBId, viewerMemberId, 'Strategy viewer', ['strategy.view']);
	owner = { organisationId: organisationAId, userId: ownerUserId, memberId: ownerMemberId, correlationId: randomUUID() };
	viewer = { organisationId: organisationBId, userId: viewerUserId, memberId: viewerMemberId, correlationId: randomUUID() };

	frameworkPublicId = randomUUID();
	frameworkId = insertedId(await db.insertInto('strategy_frameworks').values({
	...
	await db.insertInto('strategy_objectives').values({
		organisation_id: organisationAId,
...
describe('F01 strategy performance and foresight', () => {
	it('governs objective → KPI → actual/forecast → action → review → scenario with source evidence', async () => {
		const service = new PerformanceForesightService(db, randomUUID, () => new Date('2027-06-30T12:00:00.000Z'));
		...
		const completedAction = await service.completeAction(owner, action.public_id, 'Programme actions approved and mobilised.');
		...
		const snapshot = await service.addReviewKpi(owner, review.public_id, approvedKpi.public_id, observation.public_id, 'watch', 'Forecast remains below target.');
		...
		await service.addScenarioProjection(owner, scenario.public_id, approvedKpi.public_id, '2028-12-31', '8.2', 'Downside demand and input-cost assumptions compress operating margin.');
		...
		await expect(service.approveReview(owner, approvedReview.public_id)).rejects.toBeInstanceOf(PerformanceForesightValidationError);
		await expect(service.getWorkspace(viewer, frameworkPublicId)).rejects.toBeInstanceOf(RecordNotFoundError);

		const auditActions = await db.selectFrom('audit_events').select('action_key').where('acting_organisation_id', '=', organisationAId).where('action_key', 'like', 'strategy.%').execute();
		expect(new Set(auditActions.map((row) => row.action_key))).toEqual(expect.objectContaining ? expect.any(Set) : new Set());
		...
		const outbox = await db.selectFrom('outbox_events').select('topic').where('organisation_id', '=', organisationAId).where('topic', 'like', 'strategy.%').execute();
	...
	it('requires complete canonical source provenance', async () => {
		...
		await expect(service.createKpi(owner, {
			frameworkPublicId,
			objectivePublicId,

...
export class PerformanceForesightRepository {
	...
	async findKpiByPublicId(
		organisationId: string,
		publicId: string
	): Promise<StrategyKpiRecord | undefined> {
		return this.db
			.selectFrom('strategy_kpis')
			.selectAll()
	...
	async updateKpi(
		organisationId: string,
		id: string,
		values: Updateable<StrategyKpis>
	): Promise<StrategyKpiRecord> {
		await this.db
	...
	async findObservationByPublicId(
		organisationId: string,
		publicId: string
	): Promise<StrategyKpiObservationRecord | undefined> {
		return this.db
			.selectFrom('strategy_kpi_observations')
			.selectAll()
			.where('organisation_id', '=', organisationId)
	...
	async insertObservation(
		values: Insertable<StrategyKpiObservations>
	...
	async findActionByPublicId(
		organisationId: string,
		publicId: string
	): Promise<StrategyKpiActionRecord | undefined> {
		return this.db
			.selectFrom('strategy_kpi_actions')
			.selectAll()
			.where('organisation_id', '=', organisationId)
			.where('public_id', '=', publicId)
			.executeTakeFirst();
	...
	async insertAction(values: Insertable<StrategyKpiActions>): Promise<StrategyKpiActionRecord> {
		const result = await this.db
			.insertInto('strategy_kpi_actions')
			.values(values)
			.executeTakeFirstOrThrow();
		const id = result.insertId?.toString();
	...
	async updateAction(
		organisationId: string,
		id: string,
		values: Updateable<StrategyKpiActions>
	): Promise<StrategyKpiActionRecord> {
		await this.db
			.updateTable('strategy_kpi_actions')
			.set(values)
			.where('organisation_id', '=', organisationId)
			.where('id', '=', id)
			.executeTakeFirstOrThrow();
	...
	async findReviewByPublicId(
		organisationId: string,
		publicId: string
	): Promise<StrategyReviewRecord | undefined> {
		return this.db
			.selectFrom('strategy_reviews')
			.selectAll()
			.where('organisation_id', '=', organisationId)
	...
	async updateReview(
		organisationId: string,
		id: string,
		values: Updateable<StrategyReviews>
	): Promise<StrategyReviewRecord> {
		await this.db
			.updateTable('strategy_reviews')
			.set(values)
			.where('organisation_id', '=', organisationId)
			.where('id', '=', id)
			.executeTakeFirstOrThrow();
	...
	async listReviewKpisForReview(reviewId: string): Promise<StrategyReviewKpiRecord[]> {
		return this.db
			.selectFrom('strategy_review_kpis')
			.selectAll()
			.where('strategy_review_id', '=', reviewId)
			.execute();
	}

	async insertReviewKpi(values: Insertable<StrategyReviewKpis>): Promise<StrategyReviewKpiRecord> {
		const result = await this.db
			.insertInto('strategy_review_kpis')
			.values(values)
			.executeTakeFirstOrThrow();
		const id = result.insertId?.toString();
	...
	async findScenarioByPublicId(
		organisationId: string,
		publicId: string
	): Promise<StrategyScenarioRecord | undefined> {
		return this.db
			.selectFrom('strategy_scenarios')
			.selectAll()
			.where('organisation_id', '=', organisationId)
	...
	async findLatestScenarioVersion(
		organisationId: string,
		scenarioCode: string
	): Promise<StrategyScenarioRecord | undefined> {
		return this.db
	...
	async updateScenario(
		organisationId: string,
		id: string,
		values: Updateable<StrategyScenarios>
	...
	async listScenarioAssumptionsForScenario(
		scenarioId: string
	): Promise<StrategyScenarioAssumptionRecord[]> {
		return this.db
			.selectFrom('strategy_scenario_assumptions')
			.selectAll()
			.where('strategy_scenario_id', '=', scenarioId)
			.orderBy('assumption_code', 'asc')
			.execute();
	}

	async insertScenarioAssumption(
		values: Insertable<StrategyScenarioAssumptions>
	): Promise<StrategyScenarioAssumptionRecord> {
		const result = await this.db
			.insertInto('strategy_scenario_assumptions')
			.values(values)
			.executeTakeFirstOrThrow();
		const id = result.insertId?.toString();
	...
	async listScenarioProjectionsForScenario(
		scenarioId: string
	): Promise<StrategyScenarioKpiProjectionRecord[]> {
		return this.db
			.selectFrom('strategy_scenario_kpi_projections')
			.selectAll()
			.where('strategy_scenario_id', '=', scenarioId)
			.orderBy('projection_date', 'asc')
			.execute();
	}

	async insertScenarioProjection(
		values: Insertable<StrategyScenarioKpiProjections>
	): Promise<StrategyScenarioKpiProjectionRecord> {
		const result = await this.db
			.insertInto('strategy_scenario_kpi_projections')
			.values(values)
			.executeTakeFirstOrThrow();
		const id = result.insertId?.toString();
		if (!id)
			throw new Error('Strategy scenario KPI projection insert did not return an identifier.');
		return this.db
			.selectFrom('strategy_scenario_kpi_projections')
			.selectAll()
			.where('id', '=', id)

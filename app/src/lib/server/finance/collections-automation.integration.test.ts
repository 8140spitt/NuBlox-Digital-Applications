...
function automationService() {
	return new CollectionsAutomationService(db, randomUUID, () => NOW, () => fakeDelivery);
}
...
async function cleanup(): Promise<void> {
	...
	await db.deleteFrom('receivable_collection_reminder_deliveries').where('organisation_id', 'in', ids).execute();
	await db.deleteFrom('receivable_collection_reminders').where('organisation_id', 'in', ids).execute();
	await db.deleteFrom('receivable_collection_policy_stages').where('organisation_id', 'in', ids).execute();
	await db.deleteFrom('receivable_collection_policies').where('organisation_id', 'in', ids).execute();
	await db.deleteFrom('receivable_collection_actions').where('organisation_id', 'in', ids).execute();
...
async function createIssuedInvoice(): Promise<void> {
	...
	invoiceDocumentId = insertedId(await db.insertInto('financial_documents').values({
	...
	await db.insertInto('invoices').values({ financial_document_id: invoiceDocumentId, organisation_id: organisationAId, payment_term_id: null, invoice_type: 'standard', due_date: new Date('2026-06-01T00:00:00.000Z'), customer_purchase_order_reference: null }).executeTakeFirstOrThrow();
	await db.insertInto('financial_document_items').values({ organisation_id: organisationAId, financial_document_id: invoiceDocumentId, source_quotation_item_id: null, sales_item_type_id: salesItemTypeId, sales_catalog_item_id: null, unit_of_measure_id: null, line_number: 1, description: `${PREFIX}Invoice`, quantity: '1.000000', unit_rate: '120.0000' }).executeTakeFirstOrThrow();
	await db.insertInto('financial_document_issue_events').values({ organisation_id: organisationAId, financial_document_id: invoiceDocumentId, issue_sequence: 1, issued_by_member_id: ownerAMemberId, delivery_channel: 'manual', issued_at: new Date('2026-05-01T09:00:00.000Z'), note: null }).executeTakeFirstOrThrow();
}
...
beforeAll(async () => {
	...
	await assignRole(organisationAId, financeAMemberId, 'Finance A', [
		'finance.view', 'crm.view', 'finance.collections.view', 'finance.collections.case.manage',
		'finance.collections.promise.manage', 'finance.collections.dispute.manage',
		'finance.collections.reminder.generate', 'finance.collections.reminder.dispatch'
	]);
	...
	activeCasePublicId = (await new CollectionsService(db, randomUUID, () => NOW).startCase(actorOwnerA, customerPartyPublicId)).publicId;
});
...
describe.sequential('Package 004H collections automation policy', () => {
	it('keeps policy authoring stronger than ordinary reminder generation/dispatch delegation', async () => {
		...
		await expect(service.createDraftPolicy(actorFinanceA, 'Unauthorised policy')).rejects.toBeInstanceOf(TenantAccessError);
		activePolicyPublicId = await service.createDraftPolicy(actorOwnerA, 'Standard collections policy');
		stageOnePublicId = await service.saveDraftStage(actorOwnerA, {
			...
			bodyTemplate: '{{customer_name}} has {{invoice_count}} overdue invoice(s), oldest {{days_overdue}} days overdue.',
		...
		expect(workspace.activePolicy).toMatchObject({ publicId: activePolicyPublicId, versionNumber: 1, status: 'active' });
		expect(workspace.activePolicy?.stages.map((stage) => stage.publicId)).toEqual([stageOnePublicId, stageTwoPublicId]);
	...
	it('freezes activated policy versions and opens the next draft as a new version', async () => {
		const service = automationService();
		await expect(service.saveDraftStage(actorOwnerA, {
			policyPublicId: activePolicyPublicId,
			sequenceNumber: 3,
		...
		expect(workspace.draftPolicy).toMatchObject({ publicId: draftPublicId, versionNumber: 2, status: 'draft' });
	});
	...
	it('derives threshold candidates from live overdue receivables and snapshots a reminder idempotently without touching the ledger', async () => {
		const service = automationService();
		const before = await new ReceivablesReportingService(db, () => NOW).getCustomerStatement(actorFinanceA, customerPartyPublicId);
		const workspace = await service.getWorkspace(actorFinanceA);
		const candidate = workspace.dueReminders.find((item) => item.stagePublicId === stageOnePublicId);
		expect(candidate).toMatchObject({ customerPartyPublicId, canGenerate: true, recipient: { email: 'accounts@example.test' } });
		firstReminderPublicId = await service.generateReminder(actorFinanceA, activeCasePublicId, stageOnePublicId);
		const retry = await service.generateReminder(actorFinanceA, activeCasePublicId, stageOnePublicId);
		expect(retry).toBe(firstReminderPublicId);
		const reminder = await db.selectFrom('receivable_collection_reminders').select(['status', 'recipient_email as recipientEmail', 'subject', 'message_body as messageBody']).where('organisation_id', '=', organisationAId).where('public_id', '=', firstReminderPublicId).executeTakeFirstOrThrow();
		expect(reminder).toMatchObject({ status: 'pending', recipientEmail: 'accounts@example.test', subject: `Payment reminder for ${PREFIX}Client Ltd` });
		expect(reminder.messageBody).toContain('77 days overdue');
		const reminderActions = await db.selectFrom('receivable_collection_actions').select('id').where('organisation_id', '=', organisationAId).where('action_type', '=', 'reminder').execute();
		expect(reminderActions).toHaveLength(0);
		const after = await new ReceivablesReportingService(db, () => NOW).getCustomerStatement(actorFinanceA, customerPartyPublicId);
	...
	it('preserves explicit granular deny precedence over the finance umbrella at dispatch', async () => {
		const permission = await db.selectFrom('permissions').select('id').where('permission_key', '=', 'finance.collections.reminder.dispatch').executeTakeFirstOrThrow();
		await db.insertInto('member_permission_overrides').values({ organisation_id: organisationAId, organisation_member_id: ownerAMemberId, permission_id: permission.id, effect: 'deny', reason: 'Integration deny precedence' }).executeTakeFirstOrThrow();
		await expect(automationService().dispatchReminder(actorOwnerA, firstReminderPublicId)).rejects.toBeInstanceOf(TenantAccessError);
	...
	it('records failed delivery attempts, keeps the snapshot retryable, then records successful dispatch once', async () => {
		...
		expect(result).toMatchObject({ sent: false, errorMessage: 'Provider unavailable for integration test.' });
		let reminder = await db.selectFrom('receivable_collection_reminders').select(['id', 'status', 'sent_at as sentAt']).where('organisation_id', '=', organisationAId).where('public_id', '=', firstReminderPublicId).executeTakeFirstOrThrow();
		expect(reminder).toMatchObject({ status: 'pending', sentAt: null });
		let attempts = await db.selectFrom('receivable_collection_reminder_deliveries').select(['attempt_number as attemptNumber', 'outcome']).where('organisation_id', '=', organisationAId).where('reminder_id', '=', reminder.id).orderBy('attempt_number').execute();
		...
		reminder = await db.selectFrom('receivable_collection_reminders').select(['id', 'status', 'sent_at as sentAt']).where('organisation_id', '=', organisationAId).where('public_id', '=', firstReminderPublicId).executeTakeFirstOrThrow();
		expect(reminder.status).toBe('sent');
		expect(reminder.sentAt).not.toBeNull();
		attempts = await db.selectFrom('receivable_collection_reminder_deliveries').select(['attempt_number as attemptNumber', 'outcome']).where('organisation_id', '=', organisationAId).where('reminder_id', '=', reminder.id).orderBy('attempt_number').execute();
		expect(attempts).toEqual([{ attemptNumber: 1, outcome: 'failed' }, { attemptNumber: 2, outcome: 'sent' }]);
		expect(deliveredMessages.slice(-2).map((message) => message.idempotencyKey)).toEqual([firstReminderPublicId, firstReminderPublicId]);
		const actions = await db.selectFrom('receivable_collection_actions').select(['action_type as actionType', 'delivery_channel as deliveryChannel']).where('organisation_id', '=', organisationAId).where('action_type', '=', 'reminder').execute();
	...
	it('suppresses future-stage generation for a current promise and surfaces overdue promises for review', async () => {
		...
		const currentPromise = await collections.recordPromise(actorFinanceA, { casePublicId: activeCasePublicId, amount: '50', currencyCode: 'GBP', dueOn: '2026-08-24' });
		...
		await collections.resolvePromise(actorFinanceA, { casePublicId: activeCasePublicId, promisePublicId: currentPromise.publicId, status: 'cancelled', note: 'Customer replaced the promise.' });
		const overduePromise = await collections.recordPromise(actorFinanceA, { casePublicId: activeCasePublicId, amount: '40', currencyCode: 'GBP', dueOn: '2026-08-10' });
		workspace = await automationService().getWorkspace(actorFinanceA);
		expect(workspace.promiseReviews).toContainEqual(expect.objectContaining({ promisePublicId: overduePromise.publicId, daysPastDue: 7 }));
		expect(workspace.dueReminders.find((item) => item.stagePublicId === stageTwoPublicId)?.canGenerate).toBe(true);
		secondReminderPublicId = await automationService().generateReminder(actorFinanceA, activeCasePublicId, stageTwoPublicId);
	});

	it('tenant-masks reminder identities and blocks dispatch while a case is paused', async () => {
		await expect(automationService().dispatchReminder(actorOwnerB, secondReminderPublicId)).rejects.toBeInstanceOf(RecordNotFoundError);
		const collections = new CollectionsService(db, randomUUID, () => NOW);
		await collections.setCaseStatus(actorFinanceA, activeCasePublicId, 'paused');
		await expect(automationService().dispatchReminder(actorFinanceA, secondReminderPublicId)).rejects.toThrow('Only reminders for an open collections case can be dispatched');
	...
	it('revalidates the authoritative receivable immediately before dispatch and refuses a reminder after settlement state changes', async () => {
		await db.updateTable('financial_documents').set({ lifecycle_status: 'void', voided_by_member_id: ownerAMemberId, voided_at: NOW, void_reason: 'Integration settlement revalidation' }).where('organisation_id', '=', organisationAId).where('id', '=', invoiceDocumentId).executeTakeFirstOrThrow();
		await expect(automationService().dispatchReminder(actorFinanceA, secondReminderPublicId)).rejects.toBeInstanceOf(FinanceValidationError);
		const reminder = await db.selectFrom('receivable_collection_reminders').select(['id', 'status']).where('organisation_id', '=', organisationAId).where('public_id', '=', secondReminderPublicId).executeTakeFirstOrThrow();
		expect(reminder.status).toBe('pending');
		const attempts = await db.selectFrom('receivable_collection_reminder_deliveries').select('id').where('organisation_id', '=', organisationAId).where('reminder_id', '=', reminder.id).execute();

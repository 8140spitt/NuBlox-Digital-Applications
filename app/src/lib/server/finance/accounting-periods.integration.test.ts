...
beforeAll(async () => {
	...
	const receivable = await accounting.createAccount(actorOwnerA, {
		accountCode: '1100-P',
		name: 'Period Trade Receivables',
		accountType: 'asset'
	});
	const revenue = await accounting.createAccount(actorOwnerA, {
		accountCode: '4000-P',
		name: 'Period Sales Revenue',
		accountType: 'revenue'
	});
	await accounting.assignMapping(actorOwnerA, {
		mappingKey: 'accounts_receivable',
		accountPublicId: receivable.publicId,
		reason: 'Period integration fixture.'
	});
	await accounting.assignMapping(actorOwnerA, {
		mappingKey: 'sales_revenue',
		accountPublicId: revenue.publicId,
		reason: 'Period integration fixture.'
	});
	...
	await db
		...
		.values({
			party_id: customerId,
			organisation_id: organisationAId,
			legal_name: `${PREFIX}Customer`,
			trading_name: null
		})
		.executeTakeFirstOrThrow();
	const salesItemTypeId = Number(
		(
			await db
	...
	const invoiceId = insertedId(
		await db
			...
			.values({
				...
				document_number: 'INV-PERIOD-001',
	...
	await db
		...
		.values({
			...
			line_number: 1,
			description: 'Period governed invoice',
			quantity: '1.000000',
			unit_rate: '50.0000'
		})
		.executeTakeFirstOrThrow();
...
describe.sequential('Package 004M controlled accounting periods and close governance', () => {
	it('keeps Finance/Commercial view-only while Owner creates non-overlapping year and period configuration', async () => {
		...
		await expect(
			service.createFinancialYear(actorFinanceA, {
				yearCode: 'FY26',
				name: 'FY26',
				startsOn: '2026-01-01',
				endsOn: '2026-12-31'
		...
		const year = await service.createFinancialYear(actorOwnerA, {
			yearCode: 'FY26',
		...
		await expect(
			service.createFinancialYear(actorOwnerA, {
				yearCode: 'FY26-OVERLAP',
				name: 'Overlap',
				startsOn: '2026-06-01',
				endsOn: '2027-05-31'
			})
		...
		await expect(
			service.createPeriod(actorOwnerA, {
		...
		const period = await service.createPeriod(actorOwnerA, {
			financialYearPublicId: year.publicId,
			periodNumber: 8,
			name: 'August 2026',
			startsOn: '2026-08-01',
	...
	it('requires an open configured period for source posting and blocks accounting dates outside configured periods', async () => {
		const accounting = new AccountingService(db, randomUUID, () => NOW);
		await expect(
			accounting.postSource(actorOwnerA, {
				sourceType: 'invoice_issue',
				sourcePublicId: invoicePublicId,
				accountingDate: '2026-07-31'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
		const posted = await accounting.postSource(actorOwnerA, {
			sourceType: 'invoice_issue',
			sourcePublicId: invoicePublicId,
	...
	it('blocks export while open, blocks hard close while journals are unexported, then allows exact soft-closed export and hard close', async () => {
		...
		const accounting = new AccountingService(db, randomUUID, () => NOW);
		await expect(
			accounting.createExport(actorOwnerA, {
				periodStart: '2026-08-01',
				periodEnd: '2026-08-31',
				reason: 'Open period export must fail.'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
		...
		await expect(
			periods.hardClose(actorOwnerA, periodPublicId, 'Attempt before export.')
		).rejects.toBeInstanceOf(FinanceValidationError);
		await expect(
			accounting.createExport(actorOwnerA, {
				periodStart: '2026-08-01',
				periodEnd: '2026-08-30',
				reason: 'Non-exact range must fail.'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
		const created = await accounting.createExport(actorOwnerA, {
			periodStart: '2026-08-01',
			periodEnd: '2026-08-31',
			reason: 'August controlled accounting export.'
		});
		exportPublicId = created.publicId;
		expect(created.exportNumber).toBe('AEX-000001');
		await periods.hardClose(
			actorOwnerA,
			periodPublicId,
			'All August journals have active export evidence.'
		);
		const workspace = await periods.getWorkspace(actorOwnerA);
		expect(
			workspace.financialYears
				.find((year) => year.publicId === yearPublicId)
				?.periods.find((period) => period.publicId === periodPublicId)?.status
		).toBe('hard_closed');
	});
	...
	it('blocks export reversal under hard close, then permits it only after reasoned reopen', async () => {
		...
		const accounting = new AccountingService(db, randomUUID, () => NOW);
		await expect(
			accounting.reverseExport(actorOwnerA, {
				exportPublicId,
				reason: 'Cannot alter hard-closed export evidence.'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
		await periods.reopen(actorOwnerA, periodPublicId, 'Correction identified after hard close.');
		await accounting.reverseExport(actorOwnerA, {
			exportPublicId,
			reason: 'Withdraw export after controlled period reopen.'
		});
	...
	it('retains additive period transition evidence for soft close, hard close and reopen', async () => {
		...
		const events = await db
			.selectFrom('accounting_period_status_events')
			.select([
				'from_status as fromStatus',
				'to_status as toStatus',
				'reason',
				'changed_by_member_id as memberId'
			])
			.where('organisation_id', '=', organisationAId)
		...
		expect(
			events.every((event) => event.memberId === ownerAMemberId && event.reason.length > 0)
		).toBe(true);
	});
	...
	it('keeps explicit period-reopen deny above finance.manage and tenant-masks foreign period IDs', async () => {
		...
		const permission = await db
			.selectFrom('permissions')
			.select('id')
			.where('permission_key', '=', 'finance.accounting.period.reopen')
			.executeTakeFirstOrThrow();
		await db
			...
			.values({
				...
				permission_id: permission.id,
				effect: 'deny',
				reason: 'Package 004M explicit deny test.'
			})
			.executeTakeFirstOrThrow();
		await expect(
			service.reopen(actorOwnerA, periodPublicId, 'Denied reopen.')
		).rejects.toBeInstanceOf(TenantAccessError);
		await db
		...
		await expect(
			new AccountingPeriodService(db, randomUUID, () => NOW).reopen(
				actorOwnerB,
				periodPublicId,
				'Foreign tenant must not discover period.'
			)

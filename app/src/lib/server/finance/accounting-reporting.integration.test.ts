...
async function accountId(publicId: string): Promise<string> {
	return (await db.selectFrom('accounting_accounts').select('id').where('organisation_id', '=', organisationAId).where('public_id', '=', publicId).executeTakeFirstOrThrow()).id;
}
...
async function insertJournal(input: {
	...
	const result = await db.insertInto('accounting_journal_entries').values({
	...
	await db.insertInto('accounting_journal_lines').values(input.lines.map((line, index) => ({
		organisation_id: organisationId,
...
beforeAll(async () => {
	...
	const yearA = await periods.createFinancialYear(actorOwnerA, { yearCode: 'FY26-R', name: 'Reporting FY26', startsOn: '2026-01-01', endsOn: '2026-12-31' });
	januaryPublicId = (await periods.createPeriod(actorOwnerA, { financialYearPublicId: yearA.publicId, periodNumber: 1, name: 'January 2026', startsOn: '2026-01-01', endsOn: '2026-01-31' })).publicId;
	februaryPublicId = (await periods.createPeriod(actorOwnerA, { financialYearPublicId: yearA.publicId, periodNumber: 2, name: 'February 2026', startsOn: '2026-02-01', endsOn: '2026-02-28' })).publicId;
	const yearB = await periods.createFinancialYear(actorOwnerB, { yearCode: 'FY26-RB', name: 'Foreign reporting FY26', startsOn: '2026-01-01', endsOn: '2026-12-31' });
	foreignPeriodPublicId = (await periods.createPeriod(actorOwnerB, { financialYearPublicId: yearB.publicId, periodNumber: 1, name: 'Foreign January', startsOn: '2026-01-01', endsOn: '2026-01-31' })).publicId;

	const accounting = new AccountingService(db, randomUUID, () => NOW);
	const receivable = await accounting.createAccount(actorOwnerA, { accountCode: '1100-R', name: 'Trade receivables', accountType: 'asset' });
	const cash = await accounting.createAccount(actorOwnerA, { accountCode: '1000-R', name: 'Cash', accountType: 'asset' });
	const revenue = await accounting.createAccount(actorOwnerA, { accountCode: '4000-R', name: 'Sales revenue', accountType: 'revenue' });
	...
	originalSaleJournalId = (await insertJournal({
		currency: 'GBP', accountingDate: '2026-01-15',
		lines: [{ accountId: receivableAccountId, debit: '100.0000' }, { accountId: revenueAccountId, credit: '100.0000' }]
	})).id;
	await insertJournal({
		currency: 'GBP', accountingDate: '2026-02-10', sourceType: 'payment_receipt',
		lines: [{ accountId: cashAccountId, debit: '100.0000' }, { accountId: receivableAccountId, credit: '100.0000' }]
	});
	await insertJournal({
		currency: 'EUR', accountingDate: '2026-01-20',
		lines: [{ accountId: receivableAccountId, debit: '50.0000' }, { accountId: revenueAccountId, credit: '50.0000' }]
	});
});
...
describe.sequential('Package 004N trial balance and financial reporting', () => {
	it('derives balanced opening, period and closing trial-balance columns plus P&L and balance-sheet views', async () => {
		const workspace = await new AccountingReportingService(db).getWorkspace(actorFinanceA, { periodPublicId: februaryPublicId, currencyCode: 'GBP' });
	...
	it('keeps currencies isolated and never aggregates GBP and EUR balances', async () => {
		const gbp = await new AccountingReportingService(db).getWorkspace(actorFinanceA, { periodPublicId: januaryPublicId, currencyCode: 'GBP' });
		const eur = await new AccountingReportingService(db).getWorkspace(actorFinanceA, { periodPublicId: januaryPublicId, currencyCode: 'EUR' });
	...
	it('preserves historical periods while a later additive reversal changes reporting from its own accounting date onward', async () => {
		const reversal = await insertJournal({
			currency: 'GBP', accountingDate: '2026-02-20', sourceType: 'journal_reversal', sourcePublicId: randomUUID(),
			lines: [{ accountId: revenueAccountId, debit: '100.0000' }, { accountId: receivableAccountId, credit: '100.0000' }]
		});
		await db.insertInto('accounting_journal_entry_reversals').values({
		...
		const january = await new AccountingReportingService(db).getWorkspace(actorFinanceA, { periodPublicId: januaryPublicId, currencyCode: 'GBP' });
		const february = await new AccountingReportingService(db).getWorkspace(actorFinanceA, { periodPublicId: februaryPublicId, currencyCode: 'GBP' });
	...
	it('requires accounting read authority, honours explicit granular deny over finance.manage, and tenant-masks period selection', async () => {
		const permission = await db.selectFrom('permissions').select('id').where('permission_key', '=', 'finance.accounting.view').executeTakeFirstOrThrow();
		...
		await expect(new AccountingReportingService(db).getWorkspace(actorOwnerA, { periodPublicId: januaryPublicId, currencyCode: 'GBP' })).rejects.toBeInstanceOf(TenantAccessError);
		...
		await expect(new AccountingReportingService(db).getWorkspace(actorFinanceA, { periodPublicId: foreignPeriodPublicId, currencyCode: 'GBP' })).rejects.toBeInstanceOf(FinanceValidationError);
	});
});

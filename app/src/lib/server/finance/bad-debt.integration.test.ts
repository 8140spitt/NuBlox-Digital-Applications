...
async function cleanup() {
	...
	const ids = orgs.map((row) => row.id); if (ids.length === 0) return;
...
async function createUser(name: string) { return insertedId(await db.insertInto('users').values({ public_id: randomUUID(), display_name: `${PREFIX}${name}`, status: 'active' }).executeTakeFirstOrThrow()); }
async function createOrganisation(name: string) { return insertedId(await db.insertInto('organisations').values({ public_id: randomUUID(), legal_name: `${PREFIX}${name}`, default_currency_code: 'GBP', default_timezone: 'Europe/London', status: 'active' }).executeTakeFirstOrThrow()); }
async function createMember(organisationId: string, userId: string) { return insertedId(await db.insertInto('organisation_members').values({ organisation_id: organisationId, user_id: userId, public_id: randomUUID(), status: 'active', joined_at: NOW }).executeTakeFirstOrThrow()); }
...
beforeAll(async () => {
	db = getDatabase(); await cleanup();
	ownerAUserId = await createUser('Owner A'); financeAUserId = await createUser('Finance A'); ownerBUserId = await createUser('Owner B');
	organisationAId = await createOrganisation('Tenant A'); organisationBId = await createOrganisation('Tenant B');
	ownerAMemberId = await createMember(organisationAId, ownerAUserId); financeAMemberId = await createMember(organisationAId, financeAUserId); ownerBMemberId = await createMember(organisationBId, ownerBUserId);
	await assignRole(organisationAId, ownerAMemberId, 'Owner', ['finance.view', 'finance.manage']);
	await assignRole(organisationAId, financeAMemberId, 'Finance', ['finance.view', 'finance.bad_debt.view', 'finance.bad_debt.case.manage', 'finance.bad_debt.recommend', 'finance.bad_debt.recovery.record', 'finance.bad_debt.recovery.reverse']);
	await assignRole(organisationBId, ownerBMemberId, 'Owner B', ['finance.view', 'finance.bad_debt.view']);
	...
	customerId = insertedId(await db.insertInto('parties').values({ organisation_id: organisationAId, public_id: customerPublicId, party_kind: 'organisation', account_owner_member_id: ownerAMemberId, status: 'active' }).executeTakeFirstOrThrow());
	...
	invoiceId = insertedId(await db.insertInto('financial_documents').values({ organisation_id: organisationAId, public_id: invoicePublicId, document_kind: 'invoice', document_number: 'INV-BAD-001', customer_party_id: customerId, billing_contact_party_id: null, project_id: null, contract_id: null, currency_code: 'GBP', lifecycle_status: 'issued', created_by_member_id: ownerAMemberId, voided_by_member_id: null, voided_at: null, void_reason: null }).executeTakeFirstOrThrow());
	await db.insertInto('invoices').values({ financial_document_id: invoiceId, organisation_id: organisationAId, payment_term_id: null, invoice_type: 'standard', due_date: new Date('2026-08-01T00:00:00.000Z'), customer_purchase_order_reference: null }).executeTakeFirstOrThrow();
...
describe.sequential('Package 004J controlled bad debt, write-off and recovery', () => {
	it('allows delegated assessment/recommendation but reserves write-off authority', async () => {
		...
		casePublicId = (await mutations.startCase(actorFinanceA, { invoicePublicId, reason: 'Customer insolvency assessment.' })).publicId;
		expect((await mutations.startCase(actorFinanceA, { invoicePublicId, reason: 'Idempotent retry.' })).publicId).toBe(casePublicId);
		recommendationPublicId = (await mutations.recommendWriteOff(actorFinanceA, { casePublicId, amount: '60.0000', reason: 'Recommend partial loss recognition.' })).publicId;
		await expect(mutations.authoriseWriteOff(actorFinanceA, { casePublicId, recommendationPublicId, taxTreatmentPolicy: 'no_tax_adjustment', reason: 'Finance must not authorise.' })).rejects.toBeInstanceOf(TenantAccessError);
		writeOffPublicId = (await mutations.authoriseWriteOff(actorOwnerA, { casePublicId, recommendationPublicId, taxTreatmentPolicy: 'no_tax_adjustment', reason: 'Owner authorises the assessed loss.' })).publicId;
	...
	it('revalidates current receivable before accepting another recommendation', async () => {
		await expect(new BadDebtMutationService(db).recommendWriteOff(actorFinanceA, { casePublicId, amount: '40.0001', reason: 'Must exceed remaining balance.' })).rejects.toBeInstanceOf(FinanceValidationError);
	});

	it('records recovery from available cash without reopening the receivable', async () => {
		const mutations = new BadDebtMutationService(db, randomUUID, () => NOW);
		recoveryPublicId = (await mutations.recordRecovery(actorFinanceA, { casePublicId, writeOffPublicId, paymentPublicId, amount: '30.0000', reason: 'Late recovery after write-off.' })).publicId;
		const payment = await new PaymentControlService(db).getWorkspace(actorFinanceA, paymentPublicId);
		expect(payment.payment.unallocatedAmount).toBe('20.0000');
		expect((await issuedInvoiceOutstanding(db, organisationAId, invoiceId)).outstandingAmount).toBe('40.0000');
		await expect(mutations.reverseWriteOff(actorOwnerA, { casePublicId, writeOffPublicId, reason: 'Must reverse recovery first.' })).rejects.toBeInstanceOf(FinanceValidationError);
		await expect(new PaymentControlService(db).reversePayment(actorOwnerA, { paymentPublicId, reason: 'Must not bypass recovery evidence.' })).rejects.toBeInstanceOf(FinanceValidationError);
	});

	it('reverses recovery and write-off additively, restoring cash and receivable', async () => {
		const mutations = new BadDebtMutationService(db, randomUUID, () => NOW);
		await mutations.reverseRecovery(actorFinanceA, { casePublicId, recoveryPublicId, reason: 'Recovery was posted to the wrong write-off.' });
		expect((await new PaymentControlService(db).getWorkspace(actorFinanceA, paymentPublicId)).payment.unallocatedAmount).toBe('50.0000');
		await mutations.reverseWriteOff(actorOwnerA, { casePublicId, writeOffPublicId, reason: 'Reverse the recognised loss.' });
		const position = await issuedInvoiceOutstanding(db, organisationAId, invoiceId);
	...
	it('honours explicit granular deny above the finance.manage umbrella', async () => {
		const mutations = new BadDebtMutationService(db, randomUUID, () => NOW);
		const recommendation = await mutations.recommendWriteOff(actorFinanceA, { casePublicId, amount: '25.0000', reason: 'Second recommendation after reversal.' });
		const permission = await db.selectFrom('permissions').select('id').where('permission_key', '=', 'finance.bad_debt.write_off.authorise').executeTakeFirstOrThrow();
		...
		await expect(mutations.authoriseWriteOff(actorOwnerA, { casePublicId, recommendationPublicId: recommendation.publicId, taxTreatmentPolicy: 'separate_tax_adjustment_required', reason: 'Explicit deny must win.' })).rejects.toBeInstanceOf(TenantAccessError);
	...
	it('masks bad-debt case identities across tenants', async () => {
		await expect(new BadDebtQueryService(db).getWorkspace(actorOwnerB, casePublicId)).rejects.toBeInstanceOf(RecordNotFoundError);
	});
});

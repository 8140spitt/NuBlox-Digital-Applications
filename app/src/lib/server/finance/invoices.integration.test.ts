...
async function createMember(organisationId: string, userId: string): Promise<string> {
	return insertedId(
		await db.insertInto('organisation_members').values({
			organisation_id: organisationId,
			user_id: userId,
			public_id: randomUUID(),
			status: 'active',
			joined_at: new Date('2026-08-16T08:00:00.000Z')
		}).executeTakeFirstOrThrow()
	);
}
...
async function assignRole(
	...
	const roleId = insertedId(
		await db.insertInto('organisation_roles').values({
			organisation_id: organisationId,
			public_id: randomUUID(),
			name: `${PREFIX}${name}`,
			is_active: 1
...
async function createOrganisationParty(): Promise<void> {
	...
	customerPartyId = insertedId(await db.insertInto('parties').values({
	...
	await db.insertInto('party_organisations').values({
		party_id: customerPartyId,
		organisation_id: organisationAId,
		legal_name: `${PREFIX}Client Ltd`,
		trading_name: null
	}).executeTakeFirstOrThrow();
	await db.insertInto('party_email_addresses').values({
		party_id: customerPartyId,
		organisation_id: organisationAId,
	...
	const addressId = insertedId(await db.insertInto('addresses').values({
		organisation_id: organisationAId,
		line_1: '10 Invoice Street',
		city: 'London',
		postal_code: 'EC1A 1AA',
		country_code: 'GB'
	}).executeTakeFirstOrThrow());
	await db.insertInto('party_addresses').values({
		organisation_id: organisationAId,
		party_id: customerPartyId,
	...
	billingContactPartyId = insertedId(await db.insertInto('parties').values({
		organisation_id: organisationAId,
	...
	await db.insertInto('party_persons').values({
		party_id: billingContactPartyId,
		organisation_id: organisationAId,
		given_names: 'Alex',
	...
	await db.insertInto('party_organisation_contacts').values({
		organisation_id: organisationAId,
...
async function createContract(contractNumber: string, executed: boolean): Promise<string> {
	...
	const contractId = insertedId(await db.insertInto('contracts').values({
		organisation_id: organisationAId,
		public_id: publicId,
		...
		title: `${PREFIX}${contractNumber}`,
		currency_code: 'GBP',
		lifecycle_status: executed ? 'active' : 'draft',
		started_on: executed ? new Date('2026-08-01T00:00:00.000Z') : null
	}).executeTakeFirstOrThrow());
	const versionId = insertedId(await db.insertInto('contract_versions').values({
		organisation_id: organisationAId,
		contract_id: contractId,
		version_number: 1,
		title: `${PREFIX}${contractNumber}`,
		customer_reference: null,
		version_status: executed ? 'executed' : 'draft',
		created_by_member_id: ownerAMemberId,
		locked_by_member_id: executed ? ownerAMemberId : null,
		locked_at: executed ? new Date('2026-08-01T09:00:00.000Z') : null
	}).executeTakeFirstOrThrow());
	await db.insertInto('contract_version_parties').values({
		organisation_id: organisationAId,
	...
	if (executed) {
		await db.insertInto('contract_version_value_components').values({
			organisation_id: organisationAId,
			contract_version_id: versionId,
...
async function createTaxFixture(): Promise<void> {
	...
	const taxCategoryId = insertedId(await db.insertInto('tax_categories').values({
		organisation_id: organisationAId,
		public_id: taxCategoryPublicId,
		code: `${PREFIX.replaceAll(' ', '_').toUpperCase()}VAT`,
		name: `${PREFIX}VAT`,
		treatment: 'taxable',
		is_active: 1
	}).executeTakeFirstOrThrow());
	// Effective lookups use instants against DATE boundaries. The previous rate
	// therefore closes at the start of the day on which the replacement begins.
	await db.insertInto('tax_category_rates').values([
		{
			...
			valid_to: new Date('2026-08-16T00:00:00.000Z')
		},
		{
			organisation_id: organisationAId,
			tax_category_id: taxCategoryId,
...
describe('Package 004 operational accounts receivable invoices', () => {
	it('manages payment terms and customer billing defaults through the finance umbrella', async () => {
		...
		paymentTermPublicId = await service.createPaymentTerm(actorOwnerA, {
			name: 'Net 30', calculationBasis: 'invoice_date', daysOffset: 30, isDefault: true
		});
		await service.setPartyBillingSettings(actorOwnerA, {
			partyPublicId: customerPartyPublicId,
			defaultPaymentTermPublicId: paymentTermPublicId,
			defaultCurrencyCode: 'GBP',
			customerAccountReference: 'CUST-AR-001',
		...
		expect(workspace.parties.find((party) => party.partyPublicId === customerPartyPublicId)).toMatchObject({
			defaultPaymentTermPublicId: paymentTermPublicId,
			defaultCurrencyCode: 'GBP',
			customerAccountReference: 'CUST-AR-001',
			purchaseOrderRequired: true
		});
		await expect(service.createPaymentTerm(actorReadOnlyA, {
			name: 'Forbidden', calculationBasis: 'manual', daysOffset: 0, isDefault: false
	...
	it('creates only from an active executed contract and keeps draft invoices unnumbered', async () => {
		...
		await expect(service.createFromContract(actorOwnerA, {
			contractPublicId: draftContractPublicId, invoiceType: 'standard'
		})).rejects.toBeInstanceOf(FinanceValidationError);
		const created = await service.createFromContract(actorOwnerA, {
			contractPublicId: executedContractPublicId, invoiceType: 'interim'
		});
	...
	it('refreshes tax at issue, enforces customer PO policy, snapshots evidence and freezes the invoice', async () => {
		...
		await draftService.addLine(actorOwnerA, {
			...
			salesItemTypeCode,
			unitCode,
			description: 'Interim works valuation',
			quantity: '1',
			unitRate: '500.0000',
			taxCategoryPublicId
		});
		...
		await expect(issueService.issue(actorOwnerA, {
			invoicePublicId: firstInvoicePublicId, deliveryChannel: 'manual'
		})).rejects.toThrow('purchase order/reference');
		await draftService.updateDraft(actorOwnerA, {
			invoicePublicId: firstInvoicePublicId,
			invoiceType: 'interim',
			paymentTermPublicId,
			customerPurchaseOrderReference: 'PO-CLIENT-1001'
		});
		await issueService.issue(actorOwnerA, {
			invoicePublicId: firstInvoicePublicId,
			deliveryChannel: 'manual',
			note: 'Issued by controlled AR workflow.'
		});
		...
		expect(issued.partySnapshots.find((snapshot) => snapshot.snapshotRole === 'customer')).toMatchObject({
			displayName: `${PREFIX}Client Ltd`, referenceIdentifier: 'CUST-AR-001'
		});
		expect(issued.issueEvents[0]).toMatchObject({
			issueSequence: 1,
			deliveryChannel: 'manual',
			recipientName: 'Alex Accounts',
			recipientEmail: 'accounts@example.test',
			deliveryStatus: 'acknowledged'
		});
		const address = await db.selectFrom('financial_document_party_snapshot_addresses')
			.select(['line_1 as line1', 'city', 'postal_code as postalCode'])
			.where('organisation_id', '=', organisationAId)
			.executeTakeFirstOrThrow();
		expect(address).toEqual({ line1: '10 Invoice Street', city: 'London', postalCode: 'EC1A 1AA' });
		await expect(draftService.addLine(actorOwnerA, {
			invoicePublicId: firstInvoicePublicId,
			salesItemTypeCode,
			description: 'Forbidden issued line',
			quantity: '1',
			unitRate: '1.0000',
			taxCategoryPublicId
		})).rejects.toBeInstanceOf(FinanceValidationError);
	...
	it('lets an explicit granular deny override finance.manage and allocates the next number after the deny is removed', async () => {
		...
		const created = await service.createFromContract(actorOwnerA, {
			contractPublicId: executedContractPublicId, invoiceType: 'standard'
		});
		secondInvoicePublicId = created.publicId;
		await service.addLine(actorOwnerA, {
		...
		await service.updateDraft(actorOwnerA, {
			...
			paymentTermPublicId,
			customerPurchaseOrderReference: 'PO-CLIENT-1002'
		});
		...
		await expect(service.issue(actorOwnerA, {
			invoicePublicId: secondInvoicePublicId, deliveryChannel: 'manual'
		})).rejects.toBeInstanceOf(TenantAccessError);
	...
	it('separates read authority from mutation and masks foreign-tenant invoice identity', async () => {
		...
		await expect(service.addLine(actorReadOnlyA, {
			invoicePublicId: secondInvoicePublicId,
			salesItemTypeCode,

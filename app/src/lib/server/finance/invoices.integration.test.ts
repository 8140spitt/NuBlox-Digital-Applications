...
describe('Package 004 operational accounts receivable invoices', () => {
	it('manages payment terms and customer billing defaults through the finance umbrella', async () => {
		...
		expect(workspace.paymentTerms[0]).toMatchObject({
			name: 'Net 30',
			calculationBasis: 'invoice_date',
			daysOffset: 30,
			isDefault: true
		...
		expect(
			workspace.parties.find((party) => party.partyPublicId === customerPartyPublicId)
		).toMatchObject({
			defaultPaymentTermPublicId: paymentTermPublicId,
			defaultCurrencyCode: 'GBP',
			customerAccountReference: 'CUST-AR-001',
		...
		await expect(
			service.createPaymentTerm(actorReadOnlyA, {
				name: 'Forbidden',
	...
	it('creates only from an active executed contract and keeps draft invoices unnumbered', async () => {
		...
		await expect(
			service.createFromContract(actorOwnerA, {
	...
	it('refreshes tax at issue, enforces customer PO policy, snapshots evidence and freezes the invoice', async () => {
		const draftService = new InvoiceService(
			db,
			randomUUID,
		...
		const issueService = new InvoiceService(
			db,
			randomUUID,
			() => new Date('2026-08-16T11:00:00.000Z')
		);
		await expect(
			issueService.issue(actorOwnerA, {
				invoicePublicId: firstInvoicePublicId,
		...
		expect(issued.lines[0]).toMatchObject({
			netAmount: '500.0000',
			taxAmount: '105.0000',
			grossAmount: '605.0000'
		});
		expect(issued.partySnapshots.map((snapshot) => snapshot.snapshotRole).sort()).toEqual([
			'billing',
			'customer'
		]);
		expect(
			issued.partySnapshots.find((snapshot) => snapshot.snapshotRole === 'customer')
		).toMatchObject({
			displayName: `${PREFIX}Client Ltd`,
			referenceIdentifier: 'CUST-AR-001'
		});
		...
		await expect(
			draftService.addLine(actorOwnerA, {
				invoicePublicId: firstInvoicePublicId,
				salesItemTypeCode,
	...
	it('lets an explicit granular deny override finance.manage and allocates the next number after the deny is removed', async () => {
		...
		const issuePermission = await db
			.selectFrom('permissions')
			.select('id')
			.where('permission_key', '=', 'finance.invoice.issue')
			.executeTakeFirstOrThrow();
		await db
	...
	it('separates read authority from mutation and masks foreign-tenant invoice identity', async () => {
		...
		await expect(
			service.addLine(actorReadOnlyA, {
				invoicePublicId: secondInvoicePublicId,
				salesItemTypeCode,
		...
		await expect(service.getWorkspace(actorOwnerB, firstInvoicePublicId)).rejects.toBeInstanceOf(
			RecordNotFoundError
		);
	});
});

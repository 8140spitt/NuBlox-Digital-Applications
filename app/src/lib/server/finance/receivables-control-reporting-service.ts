...
function amount(value: string): bigint {
	return parseScaledDecimal(value, 4, 'Money amount', true);
}
...
export class ReceivablesControlReportingService {
	...
	constructor(
		...
		this.base = new ReceivablesReportingService(db, now);
	}

	private async activeWriteOffAsOf(
		...
		const rows = await this.db
			...
			.where('writeOff.authorised_at', '<', cutoff)
			.execute();
		let total = 0n;
		for (const row of rows)
			if (row.reversedAt === null || row.reversedAt >= cutoff) total += amount(row.amount);
		return money(total);
	}
	...
	private rebuildPosition(position: any) {
		...
		for (const bucket of position.buckets)
			buckets.set(bucket.code, { label: bucket.label, amount: 0n, invoiceCount: 0 });
		let total = 0n;
		for (const invoice of position.invoices) {
			const value = amount(invoice.outstandingAmount);
			total += value;
			const bucket = buckets.get(invoice.bucket);
			if (bucket) {
				bucket.amount += value;
		...
		position.buckets = [...buckets.entries()].map(([code, bucket]) => ({
			code,
			label: bucket.label,
	...
	async getPortfolio(actor: TenantActorContext) {
			...
			for (const position of account.positions) {
				...
				for (const invoice of position.invoices) {
					const document = await this.db
						...
						.where('public_id', '=', invoice.invoicePublicId)
						.where('document_kind', '=', 'invoice')
						.executeTakeFirst();
					if (!document) continue;
					const current = await issuedInvoiceOutstanding(
						this.db,
					...
					adjusted.push({
						...invoice,
						issuedCreditGross: current.issuedCreditGross,
						activeAllocatedAmount: current.activeAllocatedAmount,
						activeWriteOffAmount: current.activeWriteOffAmount,
		...
		for (const account of portfolio.accounts)
			for (const position of account.positions) {
				...
				total.totalOutstanding += amount(position.totalOutstanding);
				for (const bucket of position.buckets) {
					let target = total.buckets.get(bucket.code);
					if (!target) {
						target = { code: bucket.code, label: bucket.label, amount: 0n, invoiceCount: 0 };
						total.buckets.set(bucket.code, target);
					}
					target.amount += amount(bucket.amount);
					target.invoiceCount += bucket.invoiceCount;
				}
			}
		portfolio.totals = [...totals.values()]
			.map((total) => ({
				currencyCode: total.currencyCode,
				issuedInvoiceCount: total.issuedInvoiceCount,
				openInvoiceCount: total.openInvoiceCount,
				totalOutstanding: money(total.totalOutstanding),
				buckets: [...total.buckets.values()].map((bucket: any) => ({
					...bucket,
					amount: money(bucket.amount)
				})),
				invoices: []
	...
	async getCustomerStatement(
		...
		const workspace: any = await this.base.getCustomerStatement(
			actor,
		...
		const cutoff = zonedStartOfDay(
			addDateOnlyDays(workspace.period.to, 1),
			workspace.period.timezone
		);
		const writeOffRows = await this.db
			...
			.where('invoice.customer_party_id', '=', customer.id)
			.where('writeOff.authorised_at', '<', cutoff)
			.orderBy('writeOff.authorised_at', 'asc')
			.execute();
		const statementByCurrency = new Map(
			workspace.statements.map((statement: any) => [statement.currencyCode, statement])
		);
		for (const row of writeOffRows) {
			...
			if (!statement) {
				...
				statementByCurrency.set(row.currencyCode, statement);
			}
			let opening = amount(statement.openingBalance);
			if (row.authorisedAt < fromInstant) opening -= amount(row.amount);
			else
				statement.movements.push({
					id: `write-off:${row.id}`,
					occurredAt: row.authorisedAt,
					kind: 'write_off',
					reference: row.invoiceNumber ?? row.publicId,
					description: `Bad-debt write-off — ${row.reason}`,
			...
			if (row.reversedAt && row.reversedAt < cutoff) {
				if (row.reversedAt < fromInstant) opening += amount(row.amount);
				else
					statement.movements.push({
						id: `write-off-reversal:${row.id}`,
						occurredAt: row.reversedAt,
						kind: 'write_off_reversal',
						reference: row.invoiceNumber ?? row.publicId,
						description: row.reversalReason
							? `Write-off reversal — ${row.reversalReason}`
							: 'Write-off reversal',
						invoicePublicId: row.invoicePublicId,
		...
		for (const statement of workspace.statements) {
			...
			for (const movement of statement.movements) {
				running += amount(movement.debitAmount) - amount(movement.creditAmount);
				movement.runningBalance = money(running);
			}
		...
		for (const position of workspace.aging) {
			...
			for (const invoice of position.invoices) {
				const document = await this.db
					...
					.executeTakeFirst();
				if (!document) continue;
				const writeOffAmount = await this.activeWriteOffAsOf(
					actor.organisationId,
					document.id,
					cutoff
				);
				...
				adjusted.push({
					...invoice,
					activeWriteOffAmount: writeOffAmount,

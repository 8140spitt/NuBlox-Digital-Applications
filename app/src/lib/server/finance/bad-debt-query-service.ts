...
export class BadDebtQueryService {
	...
	private async assertView(
		actor: import('$lib/server/auth/tenant-actor-context').TenantActorContext
	): Promise<void> {
		...
		const [financeView, badDebtView] = await Promise.all([
			...
			access.badDebtViewDecision(actor)
		]);
		if (!financeView.allowed || !badDebtView.allowed)
			throw new TenantAccessError('Bad-debt viewing is not permitted.');
	}

	private async caseSummary(
		organisationId: string,
		caseRecord: BadDebtCaseRecord
	): Promise<BadDebtCaseSummary> {
		...
		const position =
			invoice.lifecycleStatus === 'issued'
				? await issuedInvoiceOutstanding(this.db, organisationId, invoice.id)
				: null;
		return {
			...
			outstandingAmount:
				position &&
				parseScaledDecimal(position.outstandingAmount, 4, 'Outstanding amount', true) > 0n
	...
	private async recommendations(
		organisationId: string,
		caseId: string
	): Promise<BadDebtRecommendationSummary[]> {
		const rows = await this.db
			.selectFrom('receivable_bad_debt_recommendations as recommendation')
			.leftJoin('receivable_write_offs as writeOff', (join) =>
				join
					.onRef('writeOff.recommendation_id', '=', 'recommendation.id')
					.onRef('writeOff.organisation_id', '=', 'recommendation.organisation_id')
			)
			.select([
				'recommendation.public_id as publicId',
				'recommendation.recommended_amount as amount',
				'recommendation.reason',
				'recommendation.recommended_at as recommendedAt',
				'writeOff.id as writeOffId'
			])
			.where('recommendation.organisation_id', '=', organisationId)
			.where('recommendation.bad_debt_case_id', '=', caseId)
			.orderBy('recommendation.recommended_at', 'asc')
			.orderBy('recommendation.id', 'asc')
			.execute();
		return rows.map((row) => ({
	...
	private async writeOffs(
		organisationId: string,
		caseId: string
	): Promise<BadDebtWriteOffSummary[]> {
		const rows = await this.db
			.selectFrom('receivable_write_offs as writeOff')
			.innerJoin('receivable_bad_debt_recommendations as recommendation', (join) =>
				join
					.onRef('recommendation.id', '=', 'writeOff.recommendation_id')
					.onRef('recommendation.organisation_id', '=', 'writeOff.organisation_id')
			)
			...
			.select([
				...
				'recommendation.public_id as recommendationPublicId',
			...
			.where('writeOff.bad_debt_case_id', '=', caseId)
			.orderBy('writeOff.authorised_at', 'asc')
			.orderBy('writeOff.id', 'asc')
			.execute();
		const result: BadDebtWriteOffSummary[] = [];
		for (const row of rows)
			result.push({
				publicId: row.publicId,
				recommendationPublicId: row.recommendationPublicId,
				amount: row.amount,
				taxTreatmentPolicy: row.taxTreatmentPolicy,
				reason: row.reason,
				authorisedAt: row.authorisedAt,
				isReversed: row.reversedAt !== null,
				reversedAt: row.reversedAt,
				reversalReason: row.reversalReason,
				activeRecoveryAmount: await activeRecoveryAmountForWriteOff(this.db, organisationId, row.id)
			});
		return result;
	}
	...
	private async recoveries(
		organisationId: string,
		caseId: string
	): Promise<BadDebtRecoverySummary[]> {
		const rows = await this.db
			...
			.innerJoin('payments as payment', (join) =>
				join
					.onRef('payment.id', '=', 'recovery.payment_id')
					.onRef('payment.organisation_id', '=', 'recovery.organisation_id')
			)
			...
			.where('writeOff.bad_debt_case_id', '=', caseId)
			.orderBy('recovery.recovered_at', 'asc')
	...
	private async paymentCandidates(
		organisationId: string,
		currencyCode: string
	): Promise<BadDebtPaymentCandidate[]> {
		const rows = await this.db
			.selectFrom('payments')
			.select([
				'id',
		...
		for (const payment of rows) {
			...
			if (parseScaledDecimal(availability.availableAmount, 4, 'Available payment', true) <= 0n)
				continue;
			result.push({
	...
	async getPortfolio(
		...
		for (const row of invoiceRows) {
			...
			if (parseScaledDecimal(position.outstandingAmount, 4, 'Outstanding amount', true) <= 0n)
				continue;
			const activeCase = await this.db
				.selectFrom('receivable_bad_debt_cases')
				.select('id')
				.where('organisation_id', '=', actor.organisationId)
	...
	async getWorkspace(
		...
		const [
			...
			recommendDecision,
			authoriseDecision,
			reverseWriteOffDecision,
			recoveryDecision,
			reverseRecoveryDecision
		] = await Promise.all([
			this.recommendations(actor.organisationId, caseRecord.id),
			this.writeOffs(actor.organisationId, caseRecord.id),
			this.recoveries(actor.organisationId, caseRecord.id),
			this.paymentCandidates(actor.organisationId, invoice.currencyCode),
			access.mutationDecision(actor, 'finance.bad_debt.case.manage'),
			access.mutationDecision(actor, 'finance.bad_debt.recommend'),
			access.mutationDecision(actor, 'finance.bad_debt.write_off.authorise'),
			access.mutationDecision(actor, 'finance.bad_debt.write_off.reverse'),
			access.mutationDecision(actor, 'finance.bad_debt.recovery.record'),
			access.mutationDecision(actor, 'finance.bad_debt.recovery.reverse')
		]);
		return {
			case: await this.caseSummary(actor.organisationId, caseRecord),
			recommendations,
			writeOffs,
			recoveries,
			paymentCandidates,
			canManageCase: caseDecision.allowed && caseRecord.status === 'open',
			canRecommend: recommendDecision.allowed && caseRecord.status === 'open',
			canAuthoriseWriteOff: authoriseDecision.allowed && caseRecord.status === 'open',
			canReverseWriteOff: reverseWriteOffDecision.allowed,
			canRecordRecovery: recoveryDecision.allowed,
			canReverseRecovery: reverseRecoveryDecision.allowed
		};
	}
}

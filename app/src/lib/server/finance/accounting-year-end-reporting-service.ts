...
import { AccountingReportingService, type AccountingReportingWorkspace, type FinancialReportRow } from './accounting-reporting-service';

type ReportAccount = { publicId: string; code: string; name: string; type: 'revenue' | 'expense'; periodDebit: bigint; periodCredit: bigint; yearDebit: bigint; yearCredit: bigint };
...
function reportRow(account: ReportAccount, amount: bigint): FinancialReportRow {
	return { accountPublicId: account.publicId, accountCode: account.code, name: account.name, amount: moneyText(amount) };
}
	...
	async getWorkspace(
		...
		const closeRows = await this.db
			...
			.leftJoin('accounting_year_end_close_reversals as reversal', (join) =>
				join.onRef('reversal.year_end_close_id', '=', 'close.id').onRef('reversal.organisation_id', '=', 'close.organisation_id')
			)
			.select(['close.closing_journal_entry_id as closingJournalId', 'reversal.reversal_journal_entry_id as reversalJournalId'])
			...
			.execute();
		const excludedJournalIds = closeRows.flatMap((row) => [row.closingJournalId, row.reversalJournalId].filter((value): value is string => value !== null));
		...
		if (excludedJournalIds.length > 0) query = query.where('journal.id', 'not in', excludedJournalIds);
		...
		for (const row of rows) {
			...
			if (row.accountingDate >= workspace.selectedPeriod.startsOn && row.accountingDate <= workspace.selectedPeriod.endsOn) {
		...
		const periodRevenue = revenue.reduce((sum, account) => sum + account.periodCredit - account.periodDebit, 0n);
		const periodExpenses = expenses.reduce((sum, account) => sum + account.periodDebit - account.periodCredit, 0n);
		const yearRevenue = revenue.reduce((sum, account) => sum + account.yearCredit - account.yearDebit, 0n);
		const yearExpenses = expenses.reduce((sum, account) => sum + account.yearDebit - account.yearCredit, 0n);
		workspace.profitAndLoss = {
			revenue: revenue.map((account) => reportRow(account, account.periodCredit - account.periodDebit)),
			expenses: expenses.map((account) => reportRow(account, account.periodDebit - account.periodCredit)),

...
beforeAll(async () => {
	db = getDatabase();
	await cleanup();
	userId = insertedId(await db.insertInto('users').values({
		public_id: randomUUID(),
		display_name: `${PREFIX}User`,
		status: 'active'
	}).executeTakeFirstOrThrow());
	organisationId = insertedId(await db.insertInto('organisations').values({
		public_id: randomUUID(),
	...
	memberId = insertedId(await db.insertInto('organisation_members').values({
	...
	customerId = insertedId(await db.insertInto('parties').values({
		organisation_id: organisationId,
		public_id: randomUUID(),
	...
	await db.insertInto('financial_document_items').values({
		organisation_id: organisationId,
		financial_document_id: invoiceDocumentId,
	...
	const policyId = insertedId(await db.insertInto('receivable_credit_policies').values({
		organisation_id: organisationId,
		public_id: randomUUID(),
		customer_party_id: customerId,
		currency_code: 'GBP',
	...
	await db.insertInto('receivable_credit_policy_revisions').values({
		organisation_id: organisationId,
		public_id: randomUUID(),
...
describe.sequential('Package 004I credit-control serialization', () => {
	it('waits for a concurrently locked draft invoice and re-evaluates after it becomes issued', async () => {
		...
		const releaseInvoicePromise = new Promise<void>((resolve) => { releaseInvoice = resolve; });
		...
		const issuingTransaction = db.transaction().execute(async (trx) => {
			...
			await trx.updateTable('financial_documents')
		...
		const gateResult = db.transaction().execute(async (trx) => {
			await new CreditControlService(db).enforceCommitment(actor, {
		...
		const persistedOverride = await db.selectFrom('receivable_credit_control_overrides')
			.select('id')
			.where('organisation_id', '=', organisationId)

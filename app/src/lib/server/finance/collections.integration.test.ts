...
async function createCustomer(organisationId: string, memberId: string, name: string): Promise<{ id: string; publicId: string }> {
	const publicId = randomUUID();
	const id = insertedId(await db.insertInto('parties').values({ organisation_id: organisationId, public_id: publicId, party_kind: 'organisation', account_owner_member_id: memberId, status: 'active' }).executeTakeFirstOrThrow());
	...
	await db.insertInto('party_billing_settings').values({ party_id: id, organisation_id: organisationId, default_payment_term_id: null, default_currency_code: 'GBP', customer_account_reference: `COL-${id}`, purchase_order_required: 0 }).executeTakeFirstOrThrow();
	return { id, publicId };
}

async function createIssuedInvoice(input: { organisationId: string; customerId: string; memberId: string; number: string; amount: string; dueOn: string; issuedAt: string; currency?: string }): Promise<{ id: string; publicId: string }> {
	const publicId = randomUUID();
	...
	await db.insertInto('invoices').values({ financial_document_id: id, organisation_id: input.organisationId, payment_term_id: null, invoice_type: 'standard', due_date: new Date(`${input.dueOn}T00:00:00.000Z`), customer_purchase_order_reference: null }).executeTakeFirstOrThrow();
	await db.insertInto('financial_document_items').values({ organisation_id: input.organisationId, financial_document_id: id, source_quotation_item_id: null, sales_item_type_id: salesItemTypeId, sales_catalog_item_id: null, unit_of_measure_id: null, line_number: 1, description: `${PREFIX}${input.number}`, quantity: '1.000000', unit_rate: input.amount }).executeTakeFirstOrThrow();
	await db.insertInto('financial_document_issue_events').values({ organisation_id: input.organisationId, financial_document_id: id, issue_sequence: 1, issued_by_member_id: input.memberId, delivery_channel: 'manual', issued_at: new Date(input.issuedAt), note: null }).executeTakeFirstOrThrow();
	return { id, publicId };
}
...
beforeAll(async () => {
	...
	futureCustomerPublicId = (await createCustomer(organisationAId, ownerAMemberId, 'Future Due Client Ltd')).publicId;
	foreignCustomerPublicId = (await createCustomer(organisationBId, ownerBMemberId, 'Foreign Client Ltd')).publicId;
	invoicePublicId = (await createIssuedInvoice({ organisationId: organisationAId, customerId: customerPartyId, memberId: ownerAMemberId, number: 'INV-COL-001', amount: '120.0000', dueOn: '2026-06-01', issuedAt: '2026-05-01T09:00:00.000Z' })).publicId;
	secondCustomerInvoicePublicId = (await createIssuedInvoice({ organisationId: organisationAId, customerId: secondCustomerPartyId, memberId: ownerAMemberId, number: 'INV-COL-002', amount: '80.0000', dueOn: '2026-06-15', issuedAt: '2026-05-15T09:00:00.000Z' })).publicId;
	const futureCustomer = await db.selectFrom('parties').select('id').where('organisation_id', '=', organisationAId).where('public_id', '=', futureCustomerPublicId).executeTakeFirstOrThrow();
	await createIssuedInvoice({ organisationId: organisationAId, customerId: futureCustomer.id, memberId: ownerAMemberId, number: 'INV-COL-FUTURE', amount: '50.0000', dueOn: '2026-09-30', issuedAt: '2026-08-01T09:00:00.000Z' });
});
...
describe('Package 004G controlled collections and dunning', () => {
	it('lists only overdue accounts and opens one idempotent active collections case with immutable opening evidence', async () => {
		...
		expect(portfolio.accounts.map((account) => account.customerPartyPublicId)).toContain(customerPartyPublicId);
		expect(portfolio.accounts.map((account) => account.customerPartyPublicId)).not.toContain(futureCustomerPublicId);
		...
		expect(workspace.actions.filter((action) => action.actionType === 'case_opened')).toHaveLength(1);
	});
	...
	it('records immutable collection evidence and promise lifecycle without changing the receivable ledger', async () => {
		...
		await service.recordAction(actorFinanceA, { casePublicId: activeCasePublicId, actionType: 'reminder', deliveryChannel: 'email', subject: 'Invoice overdue', messageBody: 'Please arrange payment.', outcome: 'Reminder recorded for delivery evidence.' });
		openPromisePublicId = (await service.recordPromise(actorFinanceA, { casePublicId: activeCasePublicId, invoicePublicId, amount: '60', currencyCode: 'GBP', dueOn: '2026-08-24' })).publicId;
		await expect(service.setCaseStatus(actorFinanceA, activeCasePublicId, 'closed', 'Paid')).rejects.toThrow('Resolve open promises and disputes');
		await service.resolvePromise(actorFinanceA, { casePublicId: activeCasePublicId, promisePublicId: openPromisePublicId, status: 'kept', note: 'Customer confirmed transfer.' });
		const workspace = await service.getWorkspace(actorFinanceA, customerPartyPublicId);
		expect(workspace.promises[0]).toMatchObject({ publicId: openPromisePublicId, status: 'kept', promisedAmount: '60.0000', currencyCode: 'GBP' });
	...
	it('enforces same-customer invoice context for promises and disputes, and records dispute resolution evidence', async () => {
		const service = new CollectionsService(db, randomUUID, () => NOW);
		await expect(service.recordPromise(actorFinanceA, { casePublicId: activeCasePublicId, invoicePublicId: secondCustomerInvoicePublicId, amount: '10', currencyCode: 'GBP', dueOn: '2026-08-25' })).rejects.toBeInstanceOf(RecordNotFoundError);
		await expect(service.openDispute(actorFinanceA, { casePublicId: activeCasePublicId, invoicePublicId: secondCustomerInvoicePublicId, disputedAmount: '10', currencyCode: 'GBP', reason: 'Wrong customer invoice' })).rejects.toBeInstanceOf(RecordNotFoundError);
		openDisputePublicId = (await service.openDispute(actorFinanceA, { casePublicId: activeCasePublicId, invoicePublicId, disputedAmount: '20', currencyCode: 'GBP', reason: 'Customer challenges part of the charge.' })).publicId;
		await expect(service.setCaseStatus(actorFinanceA, activeCasePublicId, 'closed', 'Resolved')).rejects.toThrow('Resolve open promises and disputes');
		await service.resolveDispute(actorFinanceA, { casePublicId: activeCasePublicId, disputePublicId: openDisputePublicId, status: 'resolved', note: 'Supporting records accepted by customer.' });
		const workspace = await service.getWorkspace(actorFinanceA, customerPartyPublicId);
		expect(workspace.disputes[0]).toMatchObject({ publicId: openDisputePublicId, status: 'resolved', disputedAmount: '20.0000', currencyCode: 'GBP' });
	...
	it('lets an explicit granular deny override the finance.manage umbrella', async () => {
		const permission = await db.selectFrom('permissions').select('id').where('permission_key', '=', 'finance.collections.case.manage').executeTakeFirstOrThrow();
		await db.insertInto('member_permission_overrides').values({ organisation_id: organisationAId, organisation_member_id: ownerAMemberId, permission_id: permission.id, effect: 'deny' }).executeTakeFirstOrThrow();
		const service = new CollectionsService(db, randomUUID, () => NOW);
		await expect(service.setCaseStatus(actorOwnerA, activeCasePublicId, 'paused')).rejects.toBeInstanceOf(TenantAccessError);
	...
	it('closes only after commitments and disputes are resolved, while preserving historical collection evidence', async () => {
		const service = new CollectionsService(db, randomUUID, () => NOW);
		await service.setCaseStatus(actorFinanceA, activeCasePublicId, 'closed', 'Collections activity complete.');
		const workspace = await service.getWorkspace(actorFinanceA, customerPartyPublicId);
		expect(workspace.case).toMatchObject({ status: 'closed', closeReason: 'Collections activity complete.' });
		expect(workspace.actions.some((action) => action.actionType === 'case_closed')).toBe(true);
		await expect(service.recordAction(actorFinanceA, { casePublicId: activeCasePublicId, actionType: 'note', messageBody: 'Should not mutate closed case.' })).rejects.toBeInstanceOf(FinanceValidationError);
	});

	it('rejects case creation without an overdue receivable and masks foreign-tenant customer identities', async () => {
		const service = new CollectionsService(db, randomUUID, () => NOW);
		await expect(service.startCase(actorFinanceA, futureCustomerPublicId)).rejects.toThrow('overdue receivable');
		await expect(service.getWorkspace(actorOwnerB, customerPartyPublicId)).rejects.toBeInstanceOf(RecordNotFoundError);
		await expect(service.getWorkspace(actorOwnerA, foreignCustomerPublicId)).rejects.toBeInstanceOf(RecordNotFoundError);
	});
	...
	it('requires collections read authority in addition to finance.view', async () => {
		...
		const actor: TenantActorContext = { organisationId: organisationAId, userId, memberId, correlationId: randomUUID() };
		await expect(new CollectionsService(db, randomUUID, () => NOW).getPortfolio(actor)).rejects.toBeInstanceOf(TenantAccessError);
	});
});

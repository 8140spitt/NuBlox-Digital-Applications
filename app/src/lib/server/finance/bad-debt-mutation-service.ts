...
export class BadDebtMutationService {
	...
	async recommendWriteOff(actor: TenantActorContext, input: { casePublicId: string; amount: string; reason: string }): Promise<{ publicId: string }> {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const invoiceIdentity = await badDebtInvoiceById(trx, actor.organisationId, caseIdentity.invoiceDocumentId);
			...
			if (!invoice || invoice.lifecycleStatus !== 'issued') throw new FinanceValidationError('The source invoice is no longer available for bad-debt assessment.');
			const caseRecord = await badDebtCaseByPublicId(trx, actor.organisationId, casePublicId, true);
			if (!caseRecord || caseRecord.invoiceDocumentId !== invoice.id || caseRecord.customerPartyId !== invoice.customerPartyId) throw new RecordNotFoundError('Bad-debt case not found.');
	...
	async authoriseWriteOff(actor: TenantActorContext, input: { casePublicId: string; recommendationPublicId: string; taxTreatmentPolicy: string; reason: string }): Promise<{ publicId: string }> {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			await trx.selectFrom('parties').select('id').where('organisation_id', '=', actor.organisationId).where('id', '=', invoiceIdentity.customerPartyId).forUpdate().executeTakeFirstOrThrow();
			const invoice = await badDebtInvoiceById(trx, actor.organisationId, invoiceIdentity.id, true);
			if (!invoice || invoice.lifecycleStatus !== 'issued') throw new FinanceValidationError('The source invoice is no longer available for write-off.');
			const caseRecord = await badDebtCaseByPublicId(trx, actor.organisationId, casePublicId, true);
			if (!caseRecord || caseRecord.invoiceDocumentId !== invoice.id || caseRecord.customerPartyId !== invoice.customerPartyId) throw new RecordNotFoundError('Bad-debt case not found.');
			...
			const existing = await trx.selectFrom('receivable_write_offs').select('public_id as publicId').where('organisation_id', '=', actor.organisationId).where('recommendation_id', '=', recommendation.id).forUpdate().executeTakeFirst();
			if (existing) return { publicId: existing.publicId };
			const position = await issuedInvoiceOutstanding(trx, actor.organisationId, invoice.id, true);
	...
	async recordRecovery(actor: TenantActorContext, input: { casePublicId: string; writeOffPublicId: string; paymentPublicId: string; amount: string; reason: string }): Promise<{ publicId: string }> {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (await paymentIsReversed(trx, actor.organisationId, payment.id, true)) throw new FinanceValidationError('A reversed payment cannot fund a bad-debt recovery.');
			...
			const writeOff = await trx.selectFrom('receivable_write_offs').select(['id', 'write_off_amount as amount']).where('organisation_id', '=', actor.organisationId).where('bad_debt_case_id', '=', caseRecord.id).where('public_id', '=', writeOffPublicId).forUpdate().executeTakeFirst();
			if (!writeOff) throw new RecordNotFoundError('Write-off not found.');
			if (await trx.selectFrom('receivable_write_off_reversals').select('write_off_id').where('organisation_id', '=', actor.organisationId).where('write_off_id', '=', writeOff.id).forUpdate().executeTakeFirst()) throw new FinanceValidationError('A reversed write-off cannot receive recovery.');
			const availability = await availablePaymentAmount(trx, actor.organisationId, payment.id, payment.amount, true);
			...
			const activeWriteOffRecovery = await activeRecoveryAmountForWriteOff(trx, actor.organisationId, writeOff.id, true);

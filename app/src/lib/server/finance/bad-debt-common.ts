...
export async function lockBadDebtCustomerThenInvoice(
	...
	if (!locked || locked.customerPartyId !== identity.customerPartyId)
		throw new RecordNotFoundError('Invoice not found.');
	return locked;
}
...
export async function badDebtCaseByPublicId(
	...
	let query = db
		...
		.select([
			...
			'invoice_document_id as invoiceDocumentId',
			'status',
...
export async function paymentIsReversed(
	db: DatabaseExecutor,
	organisationId: string,
	paymentId: string,
	currentRead = false
...
export async function availablePaymentAmount(
	...
	return {
		...
		availableAmount: subtractMoney(
			subtractMoney(paymentAmount, activeAllocatedAmount),
			activeRecoveryAmount
		)
	};
}

export async function assertPositiveBadDebtOutstanding(
	db: DatabaseExecutor,
	organisationId: string,
	invoiceDocumentId: string
): Promise<string> {
	...
	if (parseScaledDecimal(position.outstandingAmount, 4, 'Outstanding amount', true) <= 0n)
		throw new Error('NO_BAD_DEBT_OUTSTANDING');

...
async function cleanup() {
	...
	await db
		...
		.execute();
	await db
		.deleteFrom('receivable_write_offs')
		.where('organisation_id', '=', organisationId)
		.execute();
	await db
		...
		.execute();
	await db
		.deleteFrom('receivable_bad_debt_cases')
		.where('organisation_id', '=', organisationId)
		.execute();
	await db
		.deleteFrom('payment_allocation_reversals')
		.where('organisation_id', '=', organisationId)
		.execute();
	await db
...
beforeAll(async () => {
	...
	invoiceId = insertedId(
		await db
			...
			.values({
				...
				document_number: 'INV-CONCURRENT-001',
	...
	await db
		...
		.values({
			...
			line_number: 1,
			description: 'Concurrent write-off invoice',
			quantity: '1.000000',
			unit_rate: '100.0000'
		})
		.executeTakeFirstOrThrow();
	...
	paymentId = insertedId(
		await db
			...
			.values({
				...
				amount: '50.0000',
				currency_code: 'GBP',
				payment_reference: 'CONCURRENT-CASH',
				created_by_member_id: memberId
			})
			.executeTakeFirstOrThrow()
	);
	...
	casePublicId = (
		await mutations.startCase(actor, { invoicePublicId, reason: 'Concurrency assessment.' })
	).publicId;
	recommendationPublicId = (
		await mutations.recommendWriteOff(actor, {
			casePublicId,
			amount: '60.0000',
			reason: 'Recommend sixty before concurrent payment allocation.'
		})
	).publicId;
});
...
describe.sequential(
	'Package 004J bad-debt concurrency',
	() => {
		it('waits for a concurrent allocation and rejects a now-excessive write-off instead of using a stale snapshot', async () => {
			...
			const allocationMayCommit = new Promise<void>((resolve) => {
				releaseAllocation = resolve;
			});
			let allocationReady!: () => void;
			const allocationReadyPromise = new Promise<void>((resolve) => {
			...
			const allocationTransaction = db.transaction().execute(async (trx) => {
				...
				await trx
					...
					.values({
						...
						allocated_by_member_id: memberId,
						allocated_at: NOW
					})
					.executeTakeFirstOrThrow();
				allocationReady();
			...
			const authorisation = new BadDebtMutationService(db, randomUUID, () => NOW).authoriseWriteOff(
				actor,
				{
					casePublicId,
					recommendationPublicId,
			...
			const writeOffs = await db
				...
				.select('id')
				.where('organisation_id', '=', organisationId)
				.where('invoice_document_id', '=', invoiceId)
				.execute();

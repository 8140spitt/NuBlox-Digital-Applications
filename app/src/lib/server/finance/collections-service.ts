...
function asPromiseStatus(value: string): PromiseStatus {
	if (value === 'open' || value === 'kept' || value === 'broken' || value === 'cancelled')
		return value;
...
export class CollectionsService {
	...
	private publicCase(
		row: Awaited<ReturnType<CollectionsService['activeCase']>>
	): CollectionCaseSummary | null {
		if (!row) return null;
		return {
			id: row.id,
	...
	async getPortfolio(actor: TenantActorContext): Promise<CollectionsPortfolio> {
		...
		const receivables = await new ReceivablesReportingService(this.db, this.now).getPortfolio(
			actor
		);
		...
		for (const account of receivables.accounts) {
			...
			accounts.push({
				...
				activeCase: this.publicCase(
					await this.activeCase(this.db, actor.organisationId, customer.id)
				)
			});
		}
	...
	async getWorkspace(
		...
		await this.assertRead(actor);
		const receivable = await new ReceivablesReportingService(
			this.db,
			this.now
		).getCustomerStatement(actor, customerPartyPublicId);
			...
			const [actionRows, promiseRows, disputeRows] = await Promise.all([
				this.db
					...
					.leftJoin('financial_documents as invoice', (join) =>
						join
							.onRef('invoice.id', '=', 'action.invoice_document_id')
							.onRef('invoice.organisation_id', '=', 'action.organisation_id')
					)
					.leftJoin('receivable_promises_to_pay as promise', (join) =>
						join
							.onRef('promise.id', '=', 'action.promise_to_pay_id')
							.onRef('promise.organisation_id', '=', 'action.organisation_id')
					)
					.leftJoin('receivable_disputes as dispute', (join) =>
						join
							.onRef('dispute.id', '=', 'action.dispute_id')
							.onRef('dispute.organisation_id', '=', 'action.organisation_id')
					)
					.select([
						'action.public_id as publicId',
						'action.action_type as actionType',
						'action.delivery_channel as deliveryChannel',
						'action.occurred_at as occurredAt',
						'action.recorded_by_member_id as recordedByMemberId',
						'invoice.public_id as invoicePublicId',
						'promise.public_id as promisePublicId',
						'dispute.public_id as disputePublicId',
						'action.subject as subject',
						'action.message_body as messageBody',
				...
				this.db
					...
					.leftJoin('financial_documents as invoice', (join) =>
						join
							.onRef('invoice.id', '=', 'promise.invoice_document_id')
							.onRef('invoice.organisation_id', '=', 'promise.organisation_id')
					)
					.select([
						'promise.public_id as publicId',
						'invoice.public_id as invoicePublicId',
						...
						'promise.recorded_at as recordedAt',
						'promise.resolved_at as resolvedAt',
						'promise.resolution_note as resolutionNote'
					])
					.where('promise.organisation_id', '=', actor.organisationId)
					.where('promise.collection_case_id', '=', caseRow.id)
					.orderBy('promise.recorded_at', 'desc')
					.execute(),
				this.db
					.selectFrom('receivable_disputes as dispute')
					.leftJoin('financial_documents as invoice', (join) =>
						join
							.onRef('invoice.id', '=', 'dispute.invoice_document_id')
							.onRef('invoice.organisation_id', '=', 'dispute.organisation_id')
					)
					.select([
						'dispute.public_id as publicId',
						'invoice.public_id as invoicePublicId',
						'dispute.disputed_amount as disputedAmount',
						'dispute.currency_code as currencyCode',
						'dispute.reason as reason',
						'dispute.status as status',
						'dispute.opened_at as openedAt',
						'dispute.resolved_at as resolvedAt',
						'dispute.resolution_note as resolutionNote'
					])
					.where('dispute.organisation_id', '=', actor.organisationId)
					.where('dispute.collection_case_id', '=', caseRow.id)
					.orderBy('dispute.opened_at', 'desc')
					.execute()
			]);
			actions.push(
				...actionRows.map((row) => ({ ...row, actionType: asActionType(row.actionType) }))
			);
		...
		const hasOverdue = receivable.aging.some((position) =>
			position.invoices.some((invoice) => invoice.daysOverdue > 0)
		);
		return {
			receivable,
	...
	async startCase(
		...
		if (
			!receivable.aging.some((position) =>
				position.invoices.some((invoice) => invoice.daysOverdue > 0)
			)
		) {
			throw new FinanceValidationError(
				'Collections can only start when the customer has an overdue receivable.'
			);
		}
		return this.db.transaction().execute(async (trx) => {
			...
			if (!decision.allowed)
				throw new TenantAccessError('Collections case management is not permitted.');
			...
			const currentReceivable = await new ReceivablesReportingService(
				this.db,
				this.now
			).getCustomerStatement(actor, customerPartyPublicId);
			if (
				!currentReceivable.aging.some((position) =>
					position.invoices.some((invoice) => invoice.daysOverdue > 0)
				)
			) {
				throw new FinanceValidationError(
			...
			const caseId = insertedId(
				await trx
					.insertInto('receivable_collection_cases')
					.values({
	...
	async setCaseStatus(
		...
		const closeReason =
			status === 'closed' ? cleanFinanceText(reason, 1000, 'Close reason', true)! : null;
		await this.db.transaction().execute(async (trx) => {
			...
			if (row.status === 'closed')
				throw new FinanceValidationError('Closed collections cases are immutable.');
			if (status === 'closed') {
				const [promise, dispute] = await Promise.all([
					trx
						...
						.select('id')
						.where('organisation_id', '=', actor.organisationId)
						.where('collection_case_id', '=', row.id)
						.where('status', '=', 'open')
						.executeTakeFirst(),
					trx
				...
				if (promise || dispute)
					throw new FinanceValidationError(
						'Resolve open promises and disputes before closing the collections case.'
					);
			}
			if (status === row.status) return;
			await trx
				.updateTable('receivable_collection_cases')
				.set(
					status === 'closed'
						? {
								status,
						...
						: { status, closed_by_member_id: null, closed_at: null, close_reason: null }
				)
				.where('id', '=', row.id)
				.where('organisation_id', '=', actor.organisationId)
				.executeTakeFirstOrThrow();
			const actionType: CollectionActionType =
				status === 'paused' ? 'case_paused' : status === 'open' ? 'case_resumed' : 'case_closed';
			await this.appendAction(trx, {
				organisationId: actor.organisationId,
				caseId: row.id,
				actionType,
			...
			await this.audit(
				...
				actor,
				membership.id,
				`finance.collections.case.${status}`,
				'receivable_collection_case',
				row.public_id,
				{ status, reason: closeReason }
			);
		});
	}
	...
	async recordAction(
		...
		if (!USER_ACTION_TYPES.has(actionType))
			throw new FinanceValidationError('Collection action type is invalid.');
		const channel = cleanFinanceText(input.deliveryChannel, 24, 'Delivery channel');
		if (channel && !DELIVERY_CHANNELS.has(channel))
			throw new FinanceValidationError('Delivery channel is invalid.');
		...
		await this.db.transaction().execute(async (trx) => {
			...
			if (!decision.allowed)
				throw new TenantAccessError('Recording collection actions is not permitted.');
			...
			await this.audit(
				...
				'receivable_collection_case',
				row.public_id,
				{ actionType, deliveryChannel: channel, subject }
			);
		});
	}

	private async invoiceForCustomer(
		db: DatabaseExecutor,
		actor: TenantActorContext,
		caseRow: { customer_party_id: string },
		invoicePublicIdInput?: string | null
	) {
		...
		const row = await db
			...
			.where('document.customer_party_id', '=', caseRow.customer_party_id)
			.where('document.document_kind', '=', 'invoice')
			.executeTakeFirst();
	...
	async recordPromise(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (!decision.allowed)
				throw new TenantAccessError('Promise-to-pay management is not permitted.');
			...
			if (invoice && invoice.currencyCode !== currencyCode)
				throw new FinanceValidationError(
					'Promise currency must match the linked invoice currency.'
				);
			const publicId = this.publicIdFactory();
			const promiseId = insertedId(
				await trx
					.insertInto('receivable_promises_to_pay')
					.values({
						organisation_id: actor.organisationId,
						public_id: publicId,
						collection_case_id: caseRow.id,
						invoice_document_id: invoice?.id ?? null,
			...
			await this.appendAction(trx, {
				organisationId: actor.organisationId,
				caseId: caseRow.id,
				actionType: 'promise_recorded',
				recordedByMemberId: membership.id,
				invoiceDocumentId: invoice?.id ?? null,
				promiseToPayId: promiseId,
				subject: `Promise to pay ${amount} ${currencyCode}`
			});
			await this.audit(
				trx,
				actor,
				...
				'receivable_promise_to_pay',
				publicId,
				{
					casePublicId: caseRow.public_id,
	...
	async resolvePromise(
		...
		await this.db.transaction().execute(async (trx) => {
			...
			const membership = await policy.assertActiveActor(actor, trx);
			if (
				!(await policy.mutationDecision(actor, 'finance.collections.promise.manage', trx)).allowed
			)
				throw new TenantAccessError('Promise-to-pay management is not permitted.');
			const caseRow = await this.caseByPublicId(trx, actor, input.casePublicId, true);
			if (caseRow.status === 'closed')
				throw new FinanceValidationError('Closed collections cases are immutable.');
			const promise = await trx
				.selectFrom('receivable_promises_to_pay')
				.selectAll()
				.where('organisation_id', '=', actor.organisationId)
				.where('collection_case_id', '=', caseRow.id)
				.where('public_id', '=', input.promisePublicId)
				.forUpdate()
				.executeTakeFirst();
			if (!promise) throw new RecordNotFoundError('Promise to pay not found.');
			if (promise.status !== 'open')
				throw new FinanceValidationError('Only an open promise can be resolved.');
			await trx
				.updateTable('receivable_promises_to_pay')
				.set({
					status: input.status,
					resolved_by_member_id: membership.id,
			...
			await this.appendAction(trx, {
				...
				actionType,
				recordedByMemberId: membership.id,
				invoiceDocumentId: promise.invoice_document_id,
			...
			await this.audit(
				...
				actor,
				membership.id,
				`finance.collections.promise.${input.status}`,
				'receivable_promise_to_pay',
				promise.public_id,
				{ note }
			);
	...
	async openDispute(
		...
		const currencyCode = hasAmount
			? validateCurrencyCode(input.currencyCode, 'Dispute currency')
			: null;
		if (hasAmount && !currencyCode)
			throw new FinanceValidationError('Dispute currency is required when an amount is supplied.');
		return this.db.transaction().execute(async (trx) => {
			const policy = new FinanceAccessPolicy(trx);
			const membership = await policy.assertActiveActor(actor, trx);
			if (
				!(await policy.mutationDecision(actor, 'finance.collections.dispute.manage', trx)).allowed
			)
				throw new TenantAccessError('Receivable dispute management is not permitted.');
			const caseRow = await this.caseByPublicId(trx, actor, input.casePublicId, true);
			if (caseRow.status === 'closed')
				throw new FinanceValidationError('Closed collections cases are immutable.');
			const invoice = await this.invoiceForCustomer(trx, actor, caseRow, input.invoicePublicId);
			if (invoice && currencyCode && invoice.currencyCode !== currencyCode)
				throw new FinanceValidationError(
					'Dispute currency must match the linked invoice currency.'
				);
			const publicId = this.publicIdFactory();
			const disputeId = insertedId(
				await trx
					.insertInto('receivable_disputes')
					.values({
						organisation_id: actor.organisationId,
						public_id: publicId,
						collection_case_id: caseRow.id,
			...
			await this.appendAction(trx, {
				...
				invoiceDocumentId: invoice?.id ?? null,
				disputeId,
				subject: 'Receivable dispute opened',
				messageBody: reason
			});
			await this.audit(
				trx,
	...
	async resolveDispute(
		...
		await this.db.transaction().execute(async (trx) => {
			...
			const dispute = await trx
				...
				.where('organisation_id', '=', actor.organisationId)
				.where('collection_case_id', '=', caseRow.id)
				.where('public_id', '=', input.disputePublicId)
				.forUpdate()
				.executeTakeFirst();
			if (!dispute) throw new RecordNotFoundError('Receivable dispute not found.');
			if (dispute.status !== 'open')
				throw new FinanceValidationError('Only an open dispute can be resolved.');
			await trx
				.updateTable('receivable_disputes')
				.set({
					status: input.status,
					resolved_by_member_id: membership.id,
					resolved_at: this.now(),
			...
			await this.appendAction(trx, {
				...
				actionType,
				recordedByMemberId: membership.id,
				invoiceDocumentId: dispute.invoice_document_id,
			...
			await this.audit(
				...
				actor,
				membership.id,
				`finance.collections.dispute.${input.status}`,
				'receivable_dispute',
				dispute.public_id,
				{ note }
			);

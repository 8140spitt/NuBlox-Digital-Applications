...
beforeAll(async () => {
	...
	await assignRole(preparerMemberId, 'Preparer role', [
		'finance.view',
	...
	await assignRole(authoriserMemberId, 'Authoriser role', [
		'finance.view',
		'finance.accounting.view',
		'finance.accounting.year_end.authorise',
	...
	actorAuthoriser = {
		organisationId,
		userId: authoriserUserId,
		memberId: authoriserMemberId,
		correlationId: randomUUID()
	};
	...
	const yearId = insertedId(
		await db
			...
			.values({
				organisation_id: organisationId,
				public_id: yearPublicId,
				year_code: 'FY26-YE',
				name: 'FY26 Year End',
				starts_on: new Date('2026-08-18T00:00:00.000Z'),
				ends_on: new Date('2026-08-18T00:00:00.000Z'),
	...
	await db
		...
		.executeTakeFirstOrThrow();
	await new AccountingYearEndConfigurationService(db).assignRetainedEarnings(actorPreparer, {
		accountPublicId: retainedPublicId,
		reason: 'Year-end integration mapping.'
	});

	const saleJournalId = insertedId(
		await db
			...
			.values({
				...
				accounting_date: new Date('2026-08-18T00:00:00.000Z'),
				currency_code: 'GBP',
				memo: 'Sale',
	...
	await db
		...
		.execute();
	const expenseJournalId = insertedId(
		await db
			.insertInto('accounting_journal_entries')
			.values({
				organisation_id: organisationId,
				public_id: randomUUID(),
				journal_number: 'JRN-000002',
				source_type: 'payment_receipt',
				source_public_id: randomUUID(),
				source_event_at: NOW,
				source_amount: '60.0000',
	...
	await db
		...
		.values([
			{
				organisation_id: organisationId,
				journal_entry_id: expenseJournalId,
				accounting_account_id: expenseId,
				line_number: 1,
				description: 'Expense',
				debit_amount: '60.0000',
				credit_amount: '0.0000'
			},
			{
				organisation_id: organisationId,
...
describe.sequential('Package 004O controlled year-end close', () => {
	it('requires a different authoriser, posts retained earnings and preserves P&L reporting', async () => {
		...
		const prepared = await service.prepare(actorPreparer, {
			financialYearPublicId: yearPublicId,
			currencyCode: 'GBP',
			reason: 'Prepare FY26 close.'
		});
		await expect(
			service.authorise(actorPreparer, {
				preparationPublicId: prepared.publicId,
				reason: 'Self authorise.'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
		const authorised = await service.authorise(actorAuthoriser, {
			preparationPublicId: prepared.publicId,
			reason: 'Independent approval.'
		});
		expect(authorised.journalNumber).toBe('JRN-000003');
		const closeJournal = await db
			.selectFrom('accounting_journal_entries')
			.select('id')
			.where('organisation_id', '=', organisationId)
			.where('public_id', '=', authorised.journalPublicId)
			.executeTakeFirstOrThrow();
		const lines = await db
			...
			.where('line.journal_entry_id', '=', closeJournal.id)
		...
		const report = await new AccountingYearEndReportingService(db).getWorkspace(actorAuthoriser, {
			periodPublicId,
			currencyCode: 'GBP'
		});
	...
	it('reverses the close additively and restores unclosed earnings without changing P&L', async () => {
		const close = await db
			...
			.orderBy('id', 'desc')
			.executeTakeFirstOrThrow();
		const reversed = await new AccountingYearEndService(db, randomUUID, () => NOW).reverse(
			actorAuthoriser,
			{ closePublicId: close.publicId, reason: 'Controlled correction cycle.' }
		);
	...
	it('serializes concurrent authorisation so only one active close can win', async () => {
		...
		const prepared = await service.prepare(actorPreparer, {
			financialYearPublicId: yearPublicId,
			currencyCode: 'GBP',
			reason: 'Concurrent close preparation.'
		});
		const results = await Promise.allSettled([
			service.authorise(actorAuthoriser, {
				preparationPublicId: prepared.publicId,
				reason: 'Concurrent authorisation A.'
			}),
			service.authorise(actorAuthoriser, {
				preparationPublicId: prepared.publicId,
				reason: 'Concurrent authorisation B.'
			})
		]);

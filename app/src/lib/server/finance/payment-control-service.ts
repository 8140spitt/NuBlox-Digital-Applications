...
export class PaymentControlService {
	...
	async allocate(actor: TenantActorContext, input: { paymentPublicId: string; invoicePublicId: string; amount: string }): Promise<void> {
		...
		await this.db.transaction().execute(async (trx) => {
			...
			if (await trx.selectFrom('payment_reversals').select('payment_id').where('organisation_id', '=', actor.organisationId).where('payment_id', '=', payment.id).forUpdate().executeTakeFirst()) throw new FinanceValidationError('A reversed payment cannot be allocated.');
	...
	async reversePayment(actor: TenantActorContext, input: { paymentPublicId: string; reason: string }): Promise<void> {
		...
		await this.db.transaction().execute(async (trx) => {
			...
			const payment = await trx.selectFrom('payments').select(['id', 'public_id as publicId', 'amount', 'currency_code as currencyCode']).where('organisation_id', '=', actor.organisationId).where('public_id', '=', paymentPublicId).forUpdate().executeTakeFirst();
			if (!payment) throw new RecordNotFoundError('Payment not found.');
			if (await trx.selectFrom('payment_reversals').select('payment_id').where('organisation_id', '=', actor.organisationId).where('payment_id', '=', payment.id).forUpdate().executeTakeFirst()) throw new FinanceValidationError('The payment is already reversed.');
			const activeRecovery = await activeRecoveryAmountForPayment(trx, actor.organisationId, payment.id, true);
			...
			if (allocations.length > 0) {
				const reversalRows = await trx.selectFrom('payment_allocation_reversals').select('payment_allocation_id as paymentAllocationId').where('organisation_id', '=', actor.organisationId).where('payment_allocation_id', 'in', allocations.map((allocation) => allocation.id)).forUpdate().execute();

...
export class PaymentControlService {
	...
	async getPortfolio(actor: TenantActorContext): Promise<PaymentPortfolio> {
		...
		for (const payment of portfolio.payments) {
			...
			const recoveryAmount = await activeRecoveryAmountForPayment(
				this.db,
				actor.organisationId,
				payment.id
			);
			payment.unallocatedAmount = positiveOrZero(
				subtractMoney(payment.unallocatedAmount, recoveryAmount)
			);
		}
		return portfolio;
	}
	...
	async recordPayment(
		...
		input: Parameters<PaymentService['recordPayment']>[1]
	) {
	...
	async getWorkspace(
		...
		if (!workspace.payment.isReversed) {
			const recoveryAmount = await activeRecoveryAmountForPayment(
				this.db,
				actor.organisationId,
				workspace.payment.id
			);
			workspace.payment.unallocatedAmount = positiveOrZero(
				subtractMoney(workspace.payment.unallocatedAmount, recoveryAmount)
			);
		}
		const candidates = [];
		for (const candidate of workspace.invoiceCandidates) {
			const invoice = await this.db
				...
				.where('public_id', '=', candidate.invoicePublicId)
		...
		workspace.canAllocate =
			workspace.canAllocate &&
			parseScaledDecimal(workspace.payment.unallocatedAmount, 4, 'Available payment', true) > 0n;
		return workspace;
	}
	...
	async allocate(
		...
		await this.db.transaction().execute(async (trx) => {
			...
			if (!(await policy.mutationDecision(actor, 'finance.payment.allocate', trx)).allowed)
				throw new TenantAccessError('Payment allocation is not permitted.');
			...
			const [allocatedAmount, recoveryAmount] = await Promise.all([
				activeAllocatedAmountForPayment(trx, actor.organisationId, payment.id, true),
				activeRecoveryAmountForPayment(trx, actor.organisationId, payment.id, true)
			]);
			const available = subtractMoney(
				subtractMoney(payment.amount, allocatedAmount),
				recoveryAmount
			);
			if (
				parseScaledDecimal(amount, 4) > parseScaledDecimal(available, 4, 'Available payment', true)
			)
				throw new FinanceValidationError(
					`Allocation exceeds the remaining ${positiveOrZero(available)} available on the payment.`
				);
			const position = await issuedInvoiceOutstanding(trx, actor.organisationId, invoice.id, true);
			if (parseScaledDecimal(position.outstandingAmount, 4, 'Invoice outstanding', true) <= 0n)
				throw new FinanceValidationError('The invoice has no remaining outstanding balance.');
			if (
				parseScaledDecimal(amount, 4) >
				parseScaledDecimal(position.outstandingAmount, 4, 'Invoice outstanding', true)
			)
				throw new FinanceValidationError(
					`Allocation exceeds the invoice outstanding balance of ${position.outstandingAmount}.`
				);
	...
	async reverseAllocation(
		actor: TenantActorContext,
		input: Parameters<PaymentService['reverseAllocation']>[1]
	) {
	...
	async reversePayment(
		...
		await this.db.transaction().execute(async (trx) => {
			const policy = new FinanceAccessPolicy(trx);
			const membership = await policy.assertActiveActor(actor, trx);
			if (!(await policy.mutationDecision(actor, 'finance.payment.reverse', trx)).allowed)
				throw new TenantAccessError('Payment reversal is not permitted.');
			const payment = await trx
				.selectFrom('payments')
				.select(['id', 'public_id as publicId', 'amount', 'currency_code as currencyCode'])
				.where('organisation_id', '=', actor.organisationId)
			...
			const activeRecovery = await activeRecoveryAmountForPayment(
				trx,
				actor.organisationId,
				payment.id,
				true
			);
			if (parseScaledDecimal(activeRecovery, 4) > 0n)
				throw new FinanceValidationError(
					'Reverse active bad-debt recovery usage before reversing the payment.'
				);

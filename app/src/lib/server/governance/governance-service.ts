...
function optionalText(value: string | null | undefined, maximum: number): string | null {
	...
	if (normalized.length > maximum)
		throw new GovernanceValidationError(`Text must not exceed ${maximum} characters.`);
	return normalized;
}
...
function coded(value: string, label: string): string {
	...
	if (!CODE.test(normalized))
		throw new GovernanceValidationError(`${label} has an invalid format.`);
	return normalized;
}
...
function domain(value: string): string {
	...
	if (!DOMAIN.test(normalized))
		throw new GovernanceValidationError('Subject domain has an invalid format.');
	return normalized;
}
...
function positiveInteger(value: number | string, label: string): number {
	...
	if (!Number.isInteger(parsed) || parsed < 1)
		throw new GovernanceValidationError(`${label} must be a positive whole number.`);
	return parsed;
}
...
function currency(value: string | null | undefined): string | null {
	...
	if (!/^[A-Z]{3}$/.test(normalized))
		throw new GovernanceValidationError('Currency must be a three-letter ISO code.');
	return normalized;
}
...
function referenceTriple(
	...
	const values = [
		sourceDomain?.trim() ?? '',
		sourceRecordType?.trim() ?? '',
		sourcePublicId?.trim() ?? ''
	];
	const supplied = values.filter(Boolean).length;
	if (supplied !== 0 && supplied !== 3) {
		throw new GovernanceValidationError(
			'Canonical source reference requires domain, record type and public ID together.'
		);
	}
	return {
...
export class GovernanceService {
	...
	private async permissionFlags(actor: TenantActorContext) {
		...
		if (!view.allowed && !manage.allowed && !approve.allowed) {
			throw new RecordNotFoundError(
				'Corporate governance workspace not found in the active scope.'
			);
		}
		return {
	...
	private async requirePermission(
		...
		if (!decision.allowed)
			throw new TenantAccessError('Corporate governance action is not permitted.');
	}
	...
	private async activeMember(
		...
		const row = await db
			...
			.executeTakeFirst();
		if (!row)
			throw new GovernanceValidationError(`${label} must be an active organisation member.`);
		return normalized;
	}
	...
	private async requireFramework(
		...
		const framework = await new GovernanceRepository(db).findFrameworkByPublicId(
			organisationId,
			publicId.trim()
		);
	...
	private async requireBody(
		...
		const body = await new GovernanceRepository(db).findBodyByPublicId(
			organisationId,
			publicId.trim()
		);
	...
	async createFramework(
		actor: TenantActorContext,
		input: GovernanceFrameworkInput
	): Promise<GovernanceFrameworkRecord> {
		...
		if (effectiveTo && effectiveTo < effectiveFrom)
			throw new GovernanceValidationError('Effective to must not be before effective from.');
		return this.db.transaction().execute(async (trx) => {
			...
			if (await repository.findLatestFrameworkVersion(actor.organisationId, frameworkCode)) {
				throw new GovernanceValidationError(
					'A governance framework with this code already exists; create a controlled revision instead.'
				);
			}
			const ownerMemberId = await this.activeMember(
				trx,
				actor.organisationId,
				input.ownerMemberId,
				'Owner'
	...
	async addBody(
		...
		await this.requirePermission(actor, 'governance.manage');
		return this.db.transaction().execute(async (trx) => {
			const framework = await this.requireFramework(
				trx,
				actor.organisationId,
				input.frameworkPublicId,
				'draft'
			);
			const chairMemberId = await this.activeMember(
				trx,
				actor.organisationId,
				input.chairMemberId,
				'Chair'
			);
			const secretaryMemberId = await this.activeMember(
				trx,
				actor.organisationId,
			...
			await this.appendEvidence(
				trx,
				actor,
				'governance.body.create',
				'governance_body',
				body.public_id,
				{ bodyCode: body.body_code, bodyType: body.body_type },
				['F02.01', 'F02.04', 'F02.05']
			);
			return body;
		});
	}

	async appointBodyMember(
		actor: TenantActorContext,
		input: BodyMembershipInput
	): Promise<GovernanceBodyMembershipRecord> {
		await this.requirePermission(actor, 'governance.manage');
		return this.db.transaction().execute(async (trx) => {
			const body = await this.requireBody(trx, actor.organisationId, input.bodyPublicId);
			await this.requireFramework(
				trx,
				actor.organisationId,
				(
					await trx
						...
						.select('public_id')
						.where('id', '=', body.governance_framework_id)
						.executeTakeFirstOrThrow()
				).public_id,
				'draft'
			);
			const memberId = await this.activeMember(
				trx,
				actor.organisationId,
				input.memberId,
				'Appointee'
			...
			await this.appendEvidence(
				trx,
				actor,
				'governance.body.member.appoint',
				'governance_body_membership',
				membership.public_id,
				{
					bodyPublicId: body.public_id,
					governanceRole: membership.governance_role,
					votingRights: Boolean(membership.voting_rights)
				},
				['F02.01', 'F02.04', 'F02.05']
			);
			return membership;
	...
	async addAuthorityRule(
		...
		if ((minAmount || maxAmount) && !currencyCode)
			throw new GovernanceValidationError('Currency is required for financial authority limits.');
		if (!minAmount && !maxAmount && currencyCode)
			throw new GovernanceValidationError(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const body = await this.requireBody(trx, actor.organisationId, input.bodyPublicId);
			if (body.governance_framework_id !== framework.id)
				throw new GovernanceValidationError(
					'Authority body must belong to the selected governance framework.'
				);
			const effectiveFrom = dateOnly(input.effectiveFrom, 'Effective from');
			const effectiveTo = optionalDateOnly(input.effectiveTo, 'Effective to');
			if (effectiveTo && effectiveTo < effectiveFrom)
				throw new GovernanceValidationError(
					'Authority effective-to date must not be before effective-from date.'
				);
			...
			await this.appendEvidence(
				trx,
				actor,
				'governance.authority.create',
				'governance_authority_rule',
				rule.public_id,
				{
					subjectDomain: rule.subject_domain,
	...
	async approveFramework(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const repository = new GovernanceRepository(trx);
			const bodies = (await repository.listBodies(framework.id)).filter(
				(body) => body.lifecycle_status === 'active'
			);
			if (!bodies.some((body) => body.body_type === 'board'))
				throw new GovernanceValidationError(
			...
			for (const body of bodies) {
				...
				if (voters.length < body.quorum_count)
					throw new GovernanceValidationError(
						`${body.title} requires at least ${body.quorum_count} active voting appointments before approval.`
					);
				if (
					body.chair_member_id &&
					!bodyMemberships.some(
						(membership) =>
							membership.organisation_member_id === body.chair_member_id &&
							membership.governance_role === 'chair'
					)
				)
					throw new GovernanceValidationError(
						`${body.title} chair must have an active chair appointment.`
					);
				if (
					body.secretary_member_id &&
					!bodyMemberships.some(
						(membership) =>
							membership.organisation_member_id === body.secretary_member_id &&
							membership.governance_role === 'secretary'
					)
				)
					throw new GovernanceValidationError(
						`${body.title} secretary must have an active secretary appointment.`
					);
			}
			const rules = (await repository.listAuthorityRules(framework.id)).filter(
				(rule) => rule.lifecycle_status === 'active'
			);
			if (rules.length === 0)
				throw new GovernanceValidationError(
					'Governance approval requires at least one active delegation-of-authority rule.'
				);
			if (framework.supersedes_governance_framework_id) {
				await repository.updateFramework(
					actor.organisationId,
					framework.supersedes_governance_framework_id,
			...
			await this.appendEvidence(
				...
				approved.public_id,
				{
					...
					authorityRuleCount: rules.length
				},
	...
	async reviseFramework(
		...
		await this.requirePermission(actor, 'governance.manage');
		return this.db.transaction().execute(async (trx) => {
			const source = await this.requireFramework(
				trx,
				actor.organisationId,
				frameworkPublicId,
				'approved'
			...
			const repository = new GovernanceRepository(trx);
			const latest = await repository.findLatestFrameworkVersion(
				actor.organisationId,
				source.framework_code
			);
			if (latest && latest.id !== source.id && latest.lifecycle_status === 'draft')
				throw new GovernanceValidationError('A draft governance revision already exists.');
			...
			for (const body of bodies) {
				const clone = await repository.insertBody({
					...body,
					id: undefined,
					public_id: this.publicIdFactory(),
					governance_framework_id: revision.id,
					created_by_member_id: actor.memberId,
			...
			for (const membership of memberships) {
				...
				await repository.insertBodyMembership({
					...membership,
					id: undefined,
					public_id: this.publicIdFactory(),
					governance_body_id: cloneBody.id,
					created_by_member_id: actor.memberId,
					created_at: undefined,
					updated_at: undefined
				});
			...
			for (const rule of await repository.listAuthorityRules(source.id)) {
				...
				if (!cloneBody) continue;
				await repository.insertAuthorityRule({
					...rule,
					id: undefined,
					public_id: this.publicIdFactory(),
					governance_framework_id: revision.id,
					authority_body_id: cloneBody.id,
					created_by_member_id: actor.memberId,
					created_at: undefined,
					updated_at: undefined
				});
			...
			await this.appendEvidence(
				trx,
				actor,
				'governance.framework.revise',
				'governance_framework',
				revision.public_id,
				{ versionNumber: revision.version_number, supersedesPublicId: source.public_id },
				['F02.02', 'F02.03']
			);
			return revision;
		});
	...
	async createMeeting(
		...
		return this.db.transaction().execute(async (trx) => {
			const body = await this.requireBody(trx, actor.organisationId, input.bodyPublicId);
			const framework = await trx
				.selectFrom('governance_frameworks')
				.selectAll()
				.where('id', '=', body.governance_framework_id)
				.where('organisation_id', '=', actor.organisationId)
				.executeTakeFirstOrThrow();
			if (framework.lifecycle_status !== 'approved' || body.lifecycle_status !== 'active')
				throw new GovernanceValidationError(
					'Meetings require an active body under an approved governance framework.'
				);
			...
			await this.appendEvidence(
				...
				'governance_meeting',
				meeting.public_id,
				{ bodyPublicId: body.public_id, meetingCode: meeting.meeting_code },
				['F02.01', 'F02.04', 'F02.05']
			);
			return meeting;
		});
	...
	async setMeetingAttendance(
		...
		await this.db.transaction().execute(async (trx) => {
			const repository = new GovernanceRepository(trx);
			const meeting = await repository.findMeetingByPublicId(
				actor.organisationId,
				input.meetingPublicId.trim()
			);
			if (!meeting) throw new RecordNotFoundError('Governance meeting not found.');
			if (meeting.lifecycle_status !== 'draft')
				throw new GovernanceValidationError(
					'Attendance can only be prepared while the meeting is draft.'
				);
			const memberId = await this.activeMember(
				trx,
				actor.organisationId,
			...
			if (!membership)
				throw new GovernanceValidationError(
					'Attendee must hold an active appointment to this governance body.'
				);
			await repository.upsertMeetingAttendee({
				organisation_id: actor.organisationId,
				governance_meeting_id: meeting.id,
				organisation_member_id: memberId,
				attendance_status: input.attendanceStatus,
				voting_eligible: membership.voting_rights,
				created_by_member_id: actor.memberId
			});
			await this.appendEvidence(
				trx,
				...
				meeting.public_id,
				{
					...
					votingEligible: Boolean(membership.voting_rights)
				},
				['F02.01', 'F02.04', 'F02.05']
			);
		});
	...
	async addAgendaItem(
		...
		const currencyCode = currency(input.currencyCode);
		if (decisionAmount && !currencyCode)
			throw new GovernanceValidationError('Currency is required for a decision amount.');
		if (!decisionAmount && currencyCode)
			throw new GovernanceValidationError('Currency is only valid with a decision amount.');
		return this.db.transaction().execute(async (trx) => {
			const repository = new GovernanceRepository(trx);
			const meeting = await repository.findMeetingByPublicId(
				actor.organisationId,
			...
			if (meeting.lifecycle_status !== 'draft')
				throw new GovernanceValidationError(
					'Agenda items can only be added while the meeting is draft.'
				);
			const item = await repository.insertAgendaItem({
				...
				authority_action_key: input.authorityActionKey?.trim()
					? keyed(input.authorityActionKey, 'Authority action key')
					: null,
			...
			await this.appendEvidence(
				trx,
				actor,
				'governance.agenda.create',
				'governance_agenda_item',
				item.public_id,
				{
	...
	async conveneMeeting(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const meeting = await repository.findMeetingByPublicId(
				actor.organisationId,
				meetingPublicId.trim()
			);
			if (!meeting) throw new RecordNotFoundError('Governance meeting not found.');
			if (meeting.lifecycle_status !== 'draft')
				throw new GovernanceValidationError('Only a draft meeting can be convened.');
			const attendees = await repository.listMeetingAttendees([meeting.id]);
			const presentVoters = attendees.filter(
				(attendee) => attendee.attendance_status === 'present' && Boolean(attendee.voting_eligible)
			);
			if (presentVoters.length < meeting.quorum_required_count)
				throw new GovernanceValidationError(
					`Meeting requires ${meeting.quorum_required_count} present voting members for quorum.`
				);
			if (
				meeting.chaired_by_member_id &&
				!attendees.some(
					(attendee) =>
						attendee.organisation_member_id === meeting.chaired_by_member_id &&
						attendee.attendance_status === 'present'
				)
			)
				throw new GovernanceValidationError(
					'The appointed chair must be present before the meeting can be convened.'
				);
			const agendaItems = await repository.listAgendaItems([meeting.id]);
			if (agendaItems.length === 0)
				throw new GovernanceValidationError(
					'Meeting requires at least one agenda item before it can be convened.'
				);
			const convened = await repository.updateMeeting(actor.organisationId, meeting.id, {
				lifecycle_status: 'convened',
				quorum_met: 1
			});
			await this.appendEvidence(
				trx,
				actor,
				'governance.meeting.convene',
				'governance_meeting',
				meeting.public_id,
				{ presentVotingCount: presentVoters.length, quorumRequired: meeting.quorum_required_count },
				['F02.01', 'F02.04', 'F02.05']
			);
			return convened;
		});
	...
	private ruleCoversDecision(
		rule: GovernanceAuthorityRuleRecord,
		bodyId: string,
		item: GovernanceAgendaItemRecord,
		decidedAt: Date
	): boolean {
	...
	async recordDecision(
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new GovernanceRepository(trx);
			const item = await repository.findAgendaItemByPublicId(
				actor.organisationId,
				input.agendaItemPublicId.trim()
			);
			if (!item) throw new RecordNotFoundError('Governance agenda item not found.');
			if (await repository.findDecisionByAgendaItemId(item.id))
				throw new GovernanceValidationError(
					'This agenda item already has an immutable recorded decision.'
				);
			const meeting = await trx
				.selectFrom('governance_meetings')
				.selectAll()
				.where('id', '=', item.governance_meeting_id)
				.where('organisation_id', '=', actor.organisationId)
				.executeTakeFirstOrThrow();
			if (meeting.lifecycle_status !== 'convened' || !meeting.quorum_met)
				throw new GovernanceValidationError(
					'Decision recording requires a convened meeting with proven quorum.'
				);
			const body = await trx
				.selectFrom('governance_bodies')
				.selectAll()
				.where('id', '=', meeting.governance_body_id)
				.where('organisation_id', '=', actor.organisationId)
				.executeTakeFirstOrThrow();
			const actorAttendance = await trx
				.selectFrom('governance_meeting_attendees')
				.selectAll()
				.where('governance_meeting_id', '=', meeting.id)
				.where('organisation_member_id', '=', actor.memberId)
				.where('attendance_status', '=', 'present')
				.executeTakeFirst();
			if (!actorAttendance)
				throw new GovernanceValidationError(
					'Decision recorder must be a present member of the governance body.'
				);
			const now = this.now();
			let authorityRuleId: string | null = null;
			if (input.decisionOutcome !== 'noted') {
				if (!item.authority_action_key)
					throw new GovernanceValidationError(
						'A decision authority action key is required for an approve/reject/defer resolution.'
					);
				const framework = await trx
					.selectFrom('governance_frameworks')
					.selectAll()
				...
				if (framework.lifecycle_status !== 'approved')
					throw new GovernanceValidationError(
						'Decision authority must come from an approved governance framework.'
					);
				const rules = await repository.listAuthorityRules(framework.id);
				const matchingRule = rules.find((rule) =>
					this.ruleCoversDecision(rule, body.id, item, now)
				);
				if (!matchingRule)
					throw new GovernanceValidationError(
						'No approved delegation-of-authority rule covers this decision, body, domain and amount.'
					);
			...
			await this.appendEvidence(
				...
				decision.public_id,
				{
					...
					sourcePublicId: item.source_public_id
				},
				['F02.01', 'F02.03', 'F02.04', 'F02.05']
			);
			return decision;
		});
	}
	...
	async createAction(
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new GovernanceRepository(trx);
			const decision = await repository.findDecisionByPublicId(
				actor.organisationId,
				input.decisionPublicId.trim()
			);
			if (!decision) throw new RecordNotFoundError('Governance decision not found.');
			if (decision.decision_outcome !== 'approved')
				throw new GovernanceValidationError(
					'Actions can only be created from an approved governance decision.'
				);
			const ownerMemberId = await this.activeMember(
				trx,
				actor.organisationId,
			...
			const action = await repository.insertAction({
				...
				public_id: this.publicIdFactory(),
				action_code: coded(input.actionCode, 'Action code'),
				title: requiredText(input.title, 'Action title', 255),
				description: requiredText(input.description, 'Action description', 20_000),
				owner_member_id: ownerMemberId,
				due_date: dateOnly(input.dueDate, 'Due date'),
			...
			await this.appendEvidence(
				...
				'governance_action',
				action.public_id,
				{ decisionPublicId: decision.public_id, ownerMemberId },
				['F02.01', 'F02.04', 'F02.05']
			);
			return action;
		});
	...
	async completeAction(
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new GovernanceRepository(trx);
			const action = await repository.findActionByPublicId(
				actor.organisationId,
				actionPublicId.trim()
			);
			if (!action) throw new RecordNotFoundError('Governance action not found.');
			if (action.lifecycle_status === 'completed')
				throw new GovernanceValidationError('Governance action is already completed.');
			const completed = await repository.updateAction(actor.organisationId, action.id, {
				lifecycle_status: 'completed',
				completion_evidence: requiredText(completionEvidence, 'Completion evidence', 20_000),
			...
			await this.appendEvidence(
				trx,
				actor,
				'governance.action.complete',
				'governance_action',
				completed.public_id,
				{ decisionId: completed.governance_decision_id },
				['F02.01', 'F02.04', 'F02.05']
			);
			return completed;
		});
	...
	async closeMeeting(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (!meeting) throw new RecordNotFoundError('Governance meeting not found.');
			if (meeting.lifecycle_status !== 'convened')
				throw new GovernanceValidationError('Only a convened meeting can be closed.');
			const closed = await repository.updateMeeting(actor.organisationId, meeting.id, {
				lifecycle_status: 'closed',
				minutes_text: requiredText(minutesText, 'Minutes', 100_000),
			...
			await this.appendEvidence(
				trx,
				actor,
				'governance.meeting.close',
				'governance_meeting',
				closed.public_id,
				{ quorumMet: Boolean(closed.quorum_met) },
				['F02.01', 'F02.04', 'F02.05']
			);
			return closed;
		});
	...
	async createPolicy(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (body.governance_framework_id !== framework.id || body.lifecycle_status !== 'active')
				throw new GovernanceValidationError(
					'Policy approval body must be active under the selected approved governance framework.'
				);
			const policyCode = coded(input.policyCode, 'Policy code');
			const repository = new GovernanceRepository(trx);
			if (await repository.findLatestPolicyVersion(actor.organisationId, policyCode))
				throw new GovernanceValidationError(
					'A policy with this code already exists; create a controlled revision instead.'
				);
			const ownerMemberId = await this.activeMember(
				trx,
				actor.organisationId,
			...
			if (reviewDueOn < effectiveFrom)
				throw new GovernanceValidationError(
					'Policy review date must not be before its effective date.'
				);
			const policy = await repository.insertPolicy({
				organisation_id: actor.organisationId,
				governance_framework_id: framework.id,
				approval_body_id: body.id,
				public_id: this.publicIdFactory(),
				policy_code: policyCode,
				version_number: 1,
				title: requiredText(input.title, 'Policy title', 255),
				policy_category: input.policyCategory,
				scope_text: requiredText(input.scopeText, 'Policy scope', 20_000),
				policy_text: requiredText(input.policyText, 'Policy text', 100_000),
				effective_from: effectiveFrom,
				review_due_on: reviewDueOn,
				lifecycle_status: 'draft',
				supersedes_governance_policy_id: null,
				owner_member_id: ownerMemberId,
				created_by_member_id: actor.memberId,
				approved_by_member_id: null,
				approved_at: null
			...
			await this.appendEvidence(
				...
				'governance_policy',
				policy.public_id,
				{ policyCode, approvalBodyPublicId: body.public_id },
				['F02.06']
			);
			return policy;
		});
	...
	async approvePolicy(
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new GovernanceRepository(trx);
			const policy = await repository.findPolicyByPublicId(
				actor.organisationId,
				policyPublicId.trim()
			);
			...
			if (policy.supersedes_governance_policy_id)
				await repository.updatePolicy(
					actor.organisationId,
					policy.supersedes_governance_policy_id,
					{ lifecycle_status: 'superseded' }
				);
	...
	async revisePolicy(
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new GovernanceRepository(trx);
			const source = await repository.findPolicyByPublicId(
				actor.organisationId,
				policyPublicId.trim()
			);
			if (!source) throw new RecordNotFoundError('Governance policy not found.');
			if (source.lifecycle_status !== 'approved')
				throw new GovernanceValidationError('Only an approved policy can be revised.');
			const latest = await repository.findLatestPolicyVersion(
				actor.organisationId,
				source.policy_code
			);
			if (latest && latest.id !== source.id && latest.lifecycle_status === 'draft')
				throw new GovernanceValidationError('A draft policy revision already exists.');
			const revision = await repository.insertPolicy({
				...source,
				id: undefined,
				public_id: this.publicIdFactory(),
				version_number: source.version_number + 1,
				lifecycle_status: 'draft',
				supersedes_governance_policy_id: source.id,
				created_by_member_id: actor.memberId,
				approved_by_member_id: null,
				approved_at: null,
				created_at: undefined,
	...
	async attestPolicy(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (policy.lifecycle_status !== 'approved')
				throw new GovernanceValidationError('Only an approved policy can be attested.');
			if (await repository.findPolicyAttestation(policy.id, actor.memberId))
				throw new GovernanceValidationError(
					'This member has already attested this immutable policy version.'
				);
			const attestation = await repository.insertPolicyAttestation({
				organisation_id: actor.organisationId,
				governance_policy_id: policy.id,
				public_id: this.publicIdFactory(),
				organisation_member_id: actor.memberId,
				attestation_status: status,
				commentary: optionalText(commentary, 20_000),
				attested_at: this.now()
			});
			await this.appendEvidence(
				trx,
				actor,
				'governance.policy.attest',
				'governance_policy_attestation',
				attestation.public_id,
				{ policyPublicId: policy.public_id, status },
				['F02.06']
			);
			return attestation;
		});
	...
	async declareConflict(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (input.policyPublicId?.trim()) {
				const policy = await new GovernanceRepository(trx).findPolicyByPublicId(
					actor.organisationId,
					input.policyPublicId.trim()
				);
				if (!policy || policy.lifecycle_status !== 'approved')
					throw new GovernanceValidationError(
						'Conflict declaration can only reference an approved policy.'
					);
				policyId = policy.id;
			}
			const conflict = await new GovernanceRepository(trx).insertConflict({
				organisation_id: actor.organisationId,
				public_id: this.publicIdFactory(),
				organisation_member_id: actor.memberId,
				governance_policy_id: policyId,
				declaration_type: input.declarationType,
				subject_text: requiredText(input.subjectText, 'Conflict subject', 255),
				details: requiredText(input.details, 'Conflict details', 20_000),
				declared_on: dateOnly(input.declaredOn, 'Declared on'),
				lifecycle_status: 'open',
			...
			await this.appendEvidence(
				...
				'governance_conflict_declaration',
				conflict.public_id,
				{ declarationType: conflict.declaration_type },
				['F02.07']
			);
			return conflict;
		});
	...
	async reviewConflict(
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new GovernanceRepository(trx);
			const conflict = await repository.findConflictByPublicId(
				actor.organisationId,
			...
			const updated = await repository.updateConflict(actor.organisationId, conflict.id, {
				...
				review_outcome: requiredText(outcome, 'Review outcome', 20_000),
				management_action: requiredText(managementAction, 'Management action', 20_000),
	...
	async createEthicsCase(
		actor: TenantActorContext,
		input: EthicsCaseInput
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (input.policyPublicId?.trim()) {
				...
				if (!policy || policy.lifecycle_status !== 'approved')
					throw new GovernanceValidationError('Ethics case can only reference an approved policy.');
				policyId = policy.id;
			}
			const ownerMemberId = await this.activeMember(
				trx,
			...
			const ethicsCase = await new GovernanceRepository(trx).insertEthicsCase({
				organisation_id: actor.organisationId,
				public_id: this.publicIdFactory(),
				case_code: coded(input.caseCode, 'Case code'),
				governance_policy_id: policyId,
				reporter_member_id: input.anonymous ? null : actor.memberId,
				subject_text: requiredText(input.subjectText, 'Case subject', 255),
				description: requiredText(input.description, 'Case description', 20_000),
			...
			await this.appendEvidence(
				trx,
				actor,
				'governance.ethics.case.create',
				'governance_ethics_case',
				ethicsCase.public_id,
				{ severity: ethicsCase.severity, anonymous: Boolean(input.anonymous) },
				['F02.07']
			);
			return ethicsCase;
		});
	...
	async resolveEthicsCase(
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new GovernanceRepository(trx);
			const ethicsCase = await repository.findEthicsCaseByPublicId(
				actor.organisationId,
				casePublicId.trim()
			);
			if (!ethicsCase) throw new RecordNotFoundError('Ethics case not found.');
			if (ethicsCase.lifecycle_status === 'resolved' || ethicsCase.lifecycle_status === 'closed')
				throw new GovernanceValidationError('Ethics case is already resolved.');
			const resolved = await repository.updateEthicsCase(actor.organisationId, ethicsCase.id, {
				lifecycle_status: 'resolved',
				resolution_text: requiredText(resolutionText, 'Resolution', 20_000),
			...
			await this.appendEvidence(
				trx,
				actor,
				'governance.ethics.case.resolve',
				'governance_ethics_case',
				resolved.public_id,

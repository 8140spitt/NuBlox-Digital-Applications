	...
	return insertedId(
		await db
			...
			.values({
				...
				public_id: randomUUID(),
				status: 'active',
				joined_at: new Date('2026-08-20T13:00:00.000Z')
			})
			.executeTakeFirstOrThrow()
	);
}
...
beforeAll(async () => {
	...
	actorOwner = {
		...
		correlationId: `slice6-owner-${randomUUID()}`
	};
	actorViewer = {
		organisationId,
		userId: viewerUserId,
		memberId: viewerMemberId,
		correlationId: `slice6-viewer-${randomUUID()}`
	};
});
...
describe('V1 assets, facilities and maintenance activation', () => {
	it('keeps read-only defaults separate from field completion and administration permissions', async () => {
		...
		expect(await grantsFor(readOnlyRoleId)).toEqual([
			'assets.view',
			'compliance.view',
			'facilities.view',
			'maintenance.view'
		]);
		expect(await grantsFor(fieldRoleId)).toEqual(
			[...ASSETS_MAINTENANCE_STANDARD_ROLE_PERMISSIONS['Field Worker']].sort()
		);
	});
	...
	it('registers tenant-owned facility hierarchy and prevents cross-facility asset placement', async () => {
		...
		facilityPublicId = await service.createFacility(actorOwner, {
			facilityCode: `FAC-${randomUUID().slice(0, 6)}`,
			name: 'Operations campus',
			timezone: 'Europe/London',
			commissionedOn: '2026-01-01',
			openedOn: '2026-01-15'
		});
		buildingPublicId = await service.createBuilding(actorOwner, {
			facilityPublicId,
			buildingCode: 'BLDG-A',
			name: 'Main building'
		});
		levelPublicId = await service.createLevel(actorOwner, {
			buildingPublicId,
			levelCode: 'L01',
			name: 'Level 01',
			sortOrder: '1'
		});
		spacePublicId = await service.createSpace(actorOwner, {
			buildingPublicId,
			levelPublicId,
			spaceCode: 'PLANT-01',
			name: 'Electrical plant room',
			spaceType: 'plant'
		});
		const assetTypePublicId = await service.createAssetType(actorOwner, {
			categoryCode: 'electrical',
			code: `DB-${randomUUID().slice(0, 5)}`,
			name: 'Distribution board'
		});
		assetPublicId = await service.createAsset(actorOwner, {
			facilityPublicId,
			assetTypePublicId,
			buildingPublicId,
			levelPublicId,
			spacePublicId,
			assetTag: `DB-${randomUUID().slice(0, 6)}`,
			name: 'Main LV distribution board',
			criticality: 'high'
		});
		const secondFacility = await service.createFacility(actorOwner, {
			facilityCode: `FAC-${randomUUID().slice(0, 6)}`,
			name: 'Remote site'
		});
		await expect(
			service.createAsset(actorOwner, {
				facilityPublicId: secondFacility,
				assetTypePublicId,
				buildingPublicId,
				assetTag: `BAD-${randomUUID().slice(0, 6)}`,
				name: 'Cross-facility asset',
				criticality: 'medium'
			})
		).rejects.toBeInstanceOf(AssetsMaintenanceValidationError);
		const asset = await db
			.selectFrom('assets')
			.select(['organisation_id', 'lifecycle_status', 'facility_space_id'])
	...
	it('records attributable asset lifecycle, reactive work completion and service history', async () => {
		const service = new AssetsMaintenanceService(db);
		await service.transitionAsset(
			actorOwner,
			assetPublicId,
			'isolated',
			'Safe isolation for inspection.'
		);
		await service.transitionAsset(
			actorOwner,
			assetPublicId,
			'active',
			'Inspection complete and supply restored.'
		);
		const requestPublicId = await service.createMaintenanceRequest(actorOwner, {
			facilityPublicId,
			assetPublicId,
			priorityCode: 'urgent',
			requestType: 'fault',
			title: 'Intermittent breaker trip',
			description: 'Investigate intermittent outgoing breaker trip.'
		});
		const workOrderPublicId = await service.createReactiveWorkOrder(
			actorOwner,
			requestPublicId,
			assetPublicId
		);
		await service.completeWorkOrder(
			actorOwner,
			workOrderPublicId,
			'Breaker tested, termination remade and circuit restored.'
		);
		const serviceEventPublicId = await service.recordServiceEvent(actorOwner, {
			assetPublicId,
			workOrderPublicId,
			serviceTypeCode: 'reactive_repair',
			performedAt: '2026-08-20T13:30',
			resultCode: 'completed',
			conditionRating: 'good',
			notes: 'Thermal check and functional test passed.',
			recommendedNextServiceOn: '2027-08-20'
		});
		const workOrder = await db
			.selectFrom('work_orders')
			.select(['work_order_status', 'completed_by_member_id'])
			.where('public_id', '=', workOrderPublicId)
			.executeTakeFirstOrThrow();
		expect(workOrder).toEqual({
			work_order_status: 'completed',
			completed_by_member_id: ownerMemberId
		});
		const lifecycle = await db
			.selectFrom('asset_lifecycle_events')
			.select(['event_type', 'acted_by_member_id'])
		...
		expect(lifecycle.map((row) => row.event_type)).toEqual(
			expect.arrayContaining(['in_service', 'isolated', 'returned_to_service'])
		);
		expect(lifecycle.every((row) => row.acted_by_member_id === ownerMemberId)).toBe(true);
		expect(
			await db
				.selectFrom('asset_service_events')
				.select('public_id')
				.where('public_id', '=', serviceEventPublicId)
				.executeTakeFirst()
		).toBeTruthy();
	});
	...
	it('generates planned work from an exact source task and blocks duplicate active generation', async () => {
		const service = new AssetsMaintenanceService(db);
		const planPublicId = await service.createMaintenancePlan(actorOwner, {
			facilityPublicId,
			assetPublicId,
			planTypeCode: 'ppm',
			name: 'Annual LV inspection',
			taskTitle: 'Inspect and torque LV terminations',
			instructions: 'Isolate, inspect, torque and record condition.',
			intervalValue: '12',
			intervalUnit: 'month',
			startsOn: '2026-09-01'
		});
		...
		const workOrderPublicId = await service.generatePlannedWorkOrder(
			actorOwner,
			task.id,
			assetPublicId
		);
		await expect(
			service.generatePlannedWorkOrder(actorOwner, task.id, assetPublicId)
		).rejects.toBeInstanceOf(AssetsMaintenanceValidationError);
		const workOrder = await db
			.selectFrom('work_orders')
			.select(['source_maintenance_plan_task_id', 'work_order_status'])
			.where('public_id', '=', workOrderPublicId)
			.executeTakeFirstOrThrow();
		expect(workOrder).toMatchObject({
			source_maintenance_plan_task_id: task.id,
	...
	it('binds compliance outcomes to the exact published requirement version', async () => {
		const service = new AssetsMaintenanceService(db);
		const requirementPublicId = await service.createComplianceRequirement(actorOwner, {
			categoryCode: 'electrical',
			requirementCode: `ELEC-${randomUUID().slice(0, 6)}`,
			name: 'Periodic LV inspection',
			requirementText: 'Inspect the LV distribution board and record condition.',
			intervalValue: '12',
			intervalUnit: 'month'
		});
		const assignmentId = await service.assignComplianceToAsset(
			actorOwner,
			assetPublicId,
			requirementPublicId,
			'2026-08-20'
		);
		const eventPublicId = await service.recordComplianceEvent(actorOwner, {
			assignmentId,
			performedAt: '2026-08-20T14:00',
			outcome: 'pass',
			findingsSummary: 'Inspection passed with no defects.',
			recommendedNextDueOn: '2027-08-20'
		});
		const event = await db
			.selectFrom('compliance_events as event')
			.innerJoin(
				'compliance_requirement_versions as version',
				'version.id',
				'event.compliance_requirement_version_id'
			)
			.select([
			...
			.where('event.public_id', '=', eventPublicId)
			.executeTakeFirstOrThrow();
	...
	it('constrains exact information evidence to an explicitly linked facility-project context', async () => {
		...
		const project = await new ProjectWorkspaceService(db).createProject(actorOwner, {
			projectNumber: `S6-${randomUUID().slice(0, 7)}`,
			name: 'Asset upgrade project'
		});
		...
		const containerPublicId = await information.createDocument(actorOwner, {
			projectPublicId,
			typeCode: type.code,
			containerNumber: `S6-EVID-${randomUUID().slice(0, 6)}`,
			title: 'LV distribution board service certificate',
			revisionCode: 'P01'
		});
		...
		await information.issueRevision(actorOwner, {
			versionPublicId,
			channel: 'portal',
			note: 'Issued asset evidence.'
		});
		await service.linkEvidence(actorOwner, {
			subjectType: 'asset',
			subjectPublicId: assetPublicId,
			informationVersionPublicId: versionPublicId,
			linkRole: 'certificate'
		});
		const link = await db
			.selectFrom('asset_information_links as link')
			.innerJoin(
				'information_container_versions as version',
				'version.id',
				'link.information_container_version_id'
			)
			.select(['link.link_role as role', 'version.public_id as versionPublicId'])
			.where('version.public_id', '=', versionPublicId)
			.executeTakeFirstOrThrow();

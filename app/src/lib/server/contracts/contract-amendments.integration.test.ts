...
async function cleanup(): Promise<void> {
	...
	await db.deleteFrom('contract_amendment_key_date_changes').where('organisation_id', 'in', ids).execute();
	await db.deleteFrom('contract_amendment_value_adjustments').where('organisation_id', 'in', ids).execute();
...
async function createContractFixture(input: {
	...
	const type = await db.selectFrom('contract_types').select('id').where('code', '=', 'construction_contract').executeTakeFirstOrThrow();
	...
	const versionId = insertedId(
		await db.insertInto('contract_versions').values({
			organisation_id: input.organisationId,
	...
	if (input.executed) {
		...
		const contingency = await db.selectFrom('contract_value_component_types').select('id').where('code', '=', 'contingency').executeTakeFirstOrThrow();
...
beforeAll(async () => {
	...
	await assignRole(organisationAId, ownerAMemberId, 'Owner A', ['contract.view', 'contract.manage']);
	await assignRole(organisationAId, readOnlyAMemberId, 'Read A', ['contract.view']);
	await assignRole(organisationBId, ownerBMemberId, 'Owner B', ['contract.view', 'contract.manage']);
	...
	executedContractPublicId = await createContractFixture({ organisationId: organisationAId, ownerMemberId: ownerAMemberId, contractNumber: 'CON-AMD-001', executed: true });
	draftContractPublicId = await createContractFixture({ organisationId: organisationAId, ownerMemberId: ownerAMemberId, contractNumber: 'CON-AMD-002', executed: false });
});
...
describe('controlled Package 004 contract amendments', () => {
	it('requires an active executed baseline and exposes the derived baseline value', async () => {
		...
		await expect(service.create(actorOwnerA, {
	...
	it('creates and maintains a draft with signed value and key-date changes without changing current value', async () => {
		const service = new ContractAmendmentService(db, randomUUID, () => new Date('2026-08-16T01:20:00.000Z'));
		...
		const workspace = await service.getWorkspace(actorOwnerA, executedContractPublicId, amendmentPublicId);
		expect(workspace.valueAdjustments.map((row) => row.adjustmentAmount)).toEqual(['250.0000', '-50.0000']);
	...
	it('enforces contract authority and masks foreign-tenant amendment identity', async () => {
		const service = new ContractAmendmentService(db);
		await expect(service.addValueAdjustment(actorReadOnlyA, {
			contractPublicId: executedContractPublicId,
			amendmentPublicId,
			typeCode: 'base_scope',
			adjustmentAmount: '10.0000'
		})).rejects.toBeInstanceOf(TenantAccessError);
		await expect(service.getWorkspace(actorOwnerB, executedContractPublicId, amendmentPublicId)).rejects.toBeInstanceOf(RecordNotFoundError);
	});

	it('issues an immutable amendment then agreement updates the derived current contract value', async () => {
		const issueService = new ContractAmendmentService(db, randomUUID, () => new Date('2026-08-16T01:30:00.000Z'));
		await issueService.issue(actorOwnerA, executedContractPublicId, amendmentPublicId);
		const issued = await issueService.getWorkspace(actorOwnerA, executedContractPublicId, amendmentPublicId);
		expect(issued.amendment.lifecycleStatus).toBe('issued');
		expect(issued.amendment.issuedAt).not.toBeNull();
		await expect(issueService.updateDraft(actorOwnerA, {
			contractPublicId: executedContractPublicId,
			amendmentPublicId,
		...
		const decisionService = new ContractAmendmentService(db, randomUUID, () => new Date('2026-08-16T01:40:00.000Z'));
		await decisionService.decide(actorOwnerA, executedContractPublicId, amendmentPublicId, 'agreed');
		const agreed = await decisionService.getWorkspace(actorOwnerA, executedContractPublicId, amendmentPublicId);
	...
	it('keeps rejected and withdrawn amendments as history without changing current contract value', async () => {
		const service = new ContractAmendmentService(db, randomUUID, () => new Date('2026-08-16T01:50:00.000Z'));
		...
		const list = await service.listForContract(actorOwnerA, executedContractPublicId);
		expect(list.items.find((row) => row.publicId === rejected.publicId)?.lifecycleStatus).toBe('rejected');
		expect(list.items.find((row) => row.publicId === withdrawn.publicId)?.lifecycleStatus).toBe('withdrawn');

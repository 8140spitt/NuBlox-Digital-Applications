...
async function cleanup(): Promise<void> {
	...
	if (organisationIds.length > 0) {
		...
		await db
			.deleteFrom('sales_catalog_item_prices')
			.where('organisation_id', 'in', organisationIds)
			.execute();
		await db
			.deleteFrom('sales_catalog_items')
			.where('organisation_id', 'in', organisationIds)
			.execute();
		await db
			.deleteFrom('tax_category_rates')
			.where('organisation_id', 'in', organisationIds)
			.execute();
...
async function createOpportunity(
	...
	clientName: string
): Promise<{ opportunityPublicId: string; clientPublicId: string }> {
...
beforeAll(async () => {
	...
	const client = await db
		...
		.where('public_id', '=', clientAPublicId)
		.executeTakeFirstOrThrow();
...
describe('estimates and quotations', () => {
	it('separates commercial read and mutation permissions', async () => {
		...
		await expect(
			new CommercialService(db).createEstimate(actorViewerA, {
				opportunityPublicId: opportunityAPublicId,
				title: 'Denied'
			})
	...
	it('creates an estimate only from a same-tenant CRM opportunity', async () => {
		...
		const estimate = await service.createEstimate(actorManagerA, {
			opportunityPublicId: opportunityAPublicId,
			title: 'Electrical works',
			currencyCode: 'GBP',
			notes: 'Initial build-up'
		});
		expect(estimate.estimateNumber).toMatch(/^EST-20260816-/);
		expect(estimate.opportunityPublicId).toBe(opportunityAPublicId);
		await expect(
			service.createEstimate(actorManagerA, {
				opportunityPublicId: opportunityBPublicId,
				title: 'Cross tenant'
			})
	...
	it('builds estimate lines and internal cost components with fixed-point totals then freezes the final version', async () => {
		...
		await service.addEstimateItem(actorManagerA, {
			estimatePublicId: estimate.publicId,
			versionNumber: 1,
			salesItemTypeId: labour.id,
			unitOfMeasureId: hour.id,
			description: 'Install distribution board',
			quantity: '2.000000',
			sellUnitRate: '125.5000'
		});
		await service.addEstimateCostComponent(actorManagerA, {
			estimatePublicId: estimate.publicId,
			versionNumber: 1,
			lineNumber: 10,
			salesItemTypeId: labour.id,
		...
		await expect(
			service.addEstimateItem(actorManagerA, {
				estimatePublicId: estimate.publicId,
				versionNumber: 1,
				salesItemTypeId: labour.id,
				unitOfMeasureId: hour.id,
	...
	it('creates a quotation from a final estimate without duplicating the CRM customer and snapshots tax with decimal arithmetic', async () => {
		...
		await service.addEstimateItem(actorManagerA, {
			estimatePublicId: estimate.publicId,
			versionNumber: 1,
			salesItemTypeId: serviceType.id,
			unitOfMeasureId: lump.id,
			description: 'Design and installation',
			quantity: '1.000000',
			sellUnitRate: '999.9900'
		});
		await service.finaliseEstimate(actorManagerA, estimate.publicId, 1);
		const quotation = await service.createQuotationFromEstimate(actorManagerA, {
			estimatePublicId: estimate.publicId,
			versionNumber: 1,
			title: 'Proposal',
		...
		await service.setQuotationLineTax(
			actorManagerA,
			quotation.publicId,
			1,
			10,
	...
	it('locks an issued quotation, creates party/address snapshots and rejects post-issue draft mutation', async () => {
		const service = new CommercialService(
			db,
			randomUUID,
			() => new Date('2026-08-16T00:15:00.000Z')
		);
		const estimate = await service.createEstimate(actorManagerA, {
			opportunityPublicId: opportunityAPublicId,
			title: 'Issue test',
		...
		await service.addEstimateItem(actorManagerA, {
			estimatePublicId: estimate.publicId,
			versionNumber: 1,
			salesItemTypeId: type.id,
			unitOfMeasureId: lump.id,
			description: 'Professional service',
			quantity: '1',
			sellUnitRate: '500.0000'
		});
		await service.finaliseEstimate(actorManagerA, estimate.publicId, 1);
		const quotation = await service.createQuotationFromEstimate(actorManagerA, {
			estimatePublicId: estimate.publicId,
			versionNumber: 1,
			title: 'Issue test quotation'
		});
		await service.issueQuotation(actorManagerA, {
			quotationPublicId: quotation.publicId,
			versionNumber: 1,
			deliveryChannel: 'manual',
			recipientName: 'Client A contact',
			recipientEmail: 'client-a@example.test'
		});
		...
		const snapshots = await db
			.selectFrom('quotation_party_snapshots')
			.select('id')
			.where('organisation_id', '=', organisationAId)
			.where('quotation_version_id', '=', issued.version.id)
			.execute();
		...
		await expect(
			service.updateQuotationDraft(actorManagerA, {
				quotationPublicId: quotation.publicId,
				versionNumber: 1,
				title: 'Illegal post-issue edit'
			})
		).rejects.toBeInstanceOf(CommercialValidationError);
	});
	...
	it('records one accepted customer response only for an issued version and resolves accepted effective status', async () => {
		...
		await service.addEstimateItem(actorManagerA, {
			...
			versionNumber: 1,
			salesItemTypeId: type.id,
			unitOfMeasureId: itemUnit.id,
			description: 'Accepted item',
			quantity: '1',
			sellUnitRate: '50'
		});
		await service.finaliseEstimate(actorManagerA, estimate.publicId, 1);
		...
		await expect(
			service.recordQuotationResponse(actorManagerA, {
				quotationPublicId: quotation.publicId,
				versionNumber: 1,
		...
		await service.recordQuotationResponse(actorManagerA, {
			quotationPublicId: quotation.publicId,
			versionNumber: 1,
			responseType: 'accepted',
	...
	it('masks foreign-tenant estimate and quotation public IDs', async () => {
		...
		const estimateB = await serviceB.createEstimate(actorManagerB, {
			opportunityPublicId: opportunityBPublicId,
			title: 'Tenant B estimate'
		});
		await expect(serviceA.getEstimate(actorManagerA, estimateB.publicId)).rejects.toBeInstanceOf(
			RecordNotFoundError
		);
		const wb = await serviceB.getEstimate(actorManagerB, estimateB.publicId);
		const type = wb.salesItemTypes[0];
		const unit = wb.units[0];
		await serviceB.addEstimateItem(actorManagerB, {
			estimatePublicId: estimateB.publicId,
			versionNumber: 1,
			salesItemTypeId: type.id,
			unitOfMeasureId: unit.id,
			description: 'Tenant B line',
			quantity: '1',
			sellUnitRate: '10'
		});
		await serviceB.finaliseEstimate(actorManagerB, estimateB.publicId, 1);
		const quoteB = await serviceB.createQuotationFromEstimate(actorManagerB, {
			estimatePublicId: estimateB.publicId,
			versionNumber: 1
		});
		await expect(serviceA.getQuotation(actorManagerA, quoteB.publicId)).rejects.toBeInstanceOf(
			RecordNotFoundError
		);
	});
});

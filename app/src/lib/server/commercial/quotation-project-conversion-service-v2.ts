...
import { CreditControlBlockedError, CreditControlService, type CreditCommitmentPreview } from '$lib/server/finance/credit-control-service';
...
export type QuotationProjectConversionWorkspace = {
	...
	creditControl: CreditCommitmentPreview;
...
export class QuotationProjectConversionService {
	...
	private async findAcceptedResponse(
		...
		let query = db
			...
			.select(['id', 'public_id as publicId', 'responded_at as respondedAt', 'respondent_name as respondentName', 'respondent_email as respondentEmail'])
	...
	private async findAcceptedVersionNumber(actor: TenantActorContext, quotationPublicId: string): Promise<number | null> {
		const row = await this.db
			.selectFrom('quotations as quotation')
			.innerJoin('quotation_responses as response', (join) => join.onRef('response.quotation_id', '=', 'quotation.id').onRef('response.organisation_id', '=', 'quotation.organisation_id').on('response.response_type', '=', 'accepted'))
			.innerJoin('quotation_versions as version', (join) => join.onRef('version.id', '=', 'response.quotation_version_id').onRef('version.organisation_id', '=', 'quotation.organisation_id'))
	...
	private async findConvertedProject(db: DatabaseExecutor, organisationId: string, responseId: string): Promise<ConvertedProject | null> {
		const row = await db
			.selectFrom('quotation_project_conversions as conversion')
			.innerJoin('projects as project', (join) => join.onRef('project.id', '=', 'conversion.project_id').onRef('project.owning_organisation_id', '=', 'conversion.organisation_id'))
			.select(['project.id as id', 'project.public_id as publicId', 'project.project_number as projectNumber', 'project.name as name', 'project.status as status'])
	...
	private async listSourceEstimates(db: DatabaseExecutor, organisationId: string, quotationVersionId: string, lock = false): Promise<QuotationConversionSourceEstimate[]> {
		const links = await db
			.selectFrom('quotation_version_estimates as link')
			.innerJoin('estimate_versions as version', (join) => join.onRef('version.id', '=', 'link.estimate_version_id').onRef('version.organisation_id', '=', 'link.organisation_id'))
		...
		let query = db
			...
			.leftJoin('projects as project', (join) => join.onRef('project.id', '=', 'estimate.project_id').onRef('project.owning_organisation_id', '=', 'estimate.organisation_id'))
			.select(['estimate.id as id', 'estimate.public_id as publicId', 'estimate.estimate_number as estimateNumber', 'estimate.title as title', 'estimate.project_id as projectId', 'project.public_id as projectPublicId', 'project.project_number as projectNumber'])
	...
	async getWorkspace(actor: TenantActorContext, quotationPublicIdInput: string, requestedVersionNumber?: number): Promise<QuotationProjectConversionWorkspace> {
		...
		return {
			...
			creditControl,
			...permissions,
			canConvert: baseEligible && (!creditControl.blocked || creditControl.canOverride)
		};
	}

	async convert(actor: TenantActorContext, quotationPublicIdInput: string, versionNumberInput: number, creditOverrideReasonInput?: string | null): Promise<ConvertedProject> {
		const quotationPublicId = validatePublicId(quotationPublicIdInput);
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (!permissions.hasCommercialConvertPermission) throw new TenantAccessError('Quotation conversion authority is required.');
			if (!permissions.hasProjectCreatePermission) throw new TenantAccessError('Project creation authority is required.');

			const quotation = await trx
				.selectFrom('quotations')
				.select(['id', 'public_id as publicId', 'quotation_number as quotationNumber', 'customer_party_id as customerPartyId', 'project_id as projectId', 'lifecycle_status as lifecycleStatus'])
			...
			if (quotation.lifecycleStatus !== 'active') throw new CommercialValidationError('Only an active quotation can be converted to a project.');

			const version = await trx
				.selectFrom('quotation_versions')
				.select(['id', 'version_number as versionNumber', 'title', 'currency_code as currencyCode', 'version_status as versionStatus', 'locked_at as lockedAt'])
				.where('organisation_id', '=', actor.organisationId)
			...
			if (version.versionStatus !== 'issued' || !version.lockedAt) throw new CommercialValidationError('Only an issued and locked quotation version can be converted.');

			const acceptedResponse = await this.findAcceptedResponse(trx, actor.organisationId, quotation.id, version.id, true);
			if (!acceptedResponse) throw new CommercialValidationError('The selected quotation version does not have an accepted customer response.');

			const existingProject = await this.findConvertedProject(trx, actor.organisationId, acceptedResponse.id);
			if (existingProject) {
				if (quotation.projectId !== existingProject.id) throw new Error('Quotation conversion evidence is inconsistent with quotations.project_id.');
				return existingProject;
			}
			if (quotation.projectId !== null) throw new CommercialValidationError('This quotation is already linked to a project without matching conversion evidence.');
			...
			try {
				...
				if (cause instanceof CreditControlBlockedError) throw new CommercialValidationError(cause.message);
				throw cause;
			}
			...
			if (await projectRepository.findOwnedByProjectNumber(actor.organisationId, projectNumber)) throw new CommercialValidationError(`Project number ${projectNumber} already exists without quotation conversion evidence.`);

			const createdAt = this.now();
			const projectPublicId = this.publicIdFactory();
			const projectId = await projectRepository.insert({ owningOrganisationId: actor.organisationId, publicId: projectPublicId, projectNumber, name: version.title, description: `Created from accepted quotation ${quotation.quotationNumber}.`, createdByMemberId: membership.id });
			...
			await trx.insertInto('quotation_project_conversions').values({ organisation_id: actor.organisationId, quotation_response_id: acceptedResponse.id, project_id: projectId, created_by_member_id: membership.id }).executeTakeFirstOrThrow();
			await trx.updateTable('quotations').set({ project_id: projectId }).where('organisation_id', '=', actor.organisationId).where('id', '=', quotation.id).where('project_id', 'is', null).executeTakeFirstOrThrow();

...
async function cleanup(): Promise<void> {
	...
	if (projectIds.length) {
		await db
			.deleteFrom('project_activity_progress_measurements')
		...
		await db
			...
			.execute();
		await db
			.deleteFrom('project_earned_value_baselines')
			.where('project_id', 'in', projectIds)
			.execute();
...
beforeAll(async () => {
	...
	await assignPermissionRole(organisationAId, viewerMemberId, 'Progress viewer', [
		'project.view',
		'project.progress.view',
		'commercial.forecast.view'
	]);
	await assignPermissionRole(organisationBId, externalMemberId, 'External progress viewer', [
		'project.view',
		'project.progress.view',
		'commercial.forecast.view'
	]);
	...
	owner = {
		...
		correlationId: `progress-owner-${randomUUID()}`
	};
	viewer = {
		organisationId: organisationAId,
		userId: viewerUserId,
		memberId: viewerMemberId,
		correlationId: `progress-viewer-${randomUUID()}`
	};
	external = {
		organisationId: organisationBId,
		userId: externalUserId,
		memberId: externalMemberId,
		correlationId: `progress-external-${randomUUID()}`
	};
	...
	await db
		...
		.values({
			organisation_id: organisationAId,
			project_budget_version_id: versionId,
	...
	await db
		...
		.values({
			...
			project_cost_code_id: costCodeId,
			public_id: randomUUID(),
			direct_cost_number: 'PGR-DC-001',
			entry_type: 'actual',
			transaction_date: new Date('2026-08-15T00:00:00.000Z'),
			party_id: null,
			description: 'Actual cost through data date',
			amount: '200.0000',
			currency_code: 'GBP',
			source_system: null,
			...
			created_by_member_id: ownerMemberId,
			posted_by_member_id: ownerMemberId,
			posted_at: new Date('2026-08-15T12:00:00.000Z')
		})
		.executeTakeFirstOrThrow();

	const plan = new ProjectPlanService(db);
	const wbs = await plan.createWbsNode(owner, {
		projectPublicId,
		wbsCode: '1',
		name: 'Delivery',
		sortOrder: 1
	});
	const activityA = await plan.createActivity(owner, {
		projectPublicId,
		wbsNodePublicId: wbs.publicId,
		activityCode: 'A100',
		name: 'Foundations',
		activityKind: 'activity',
		plannedStartOn: new Date('2026-08-01T00:00:00.000Z'),
		plannedFinishOn: new Date('2026-08-20T00:00:00.000Z'),
		plannedDurationDays: 20
	});
	const activityB = await plan.createActivity(owner, {
		projectPublicId,
		wbsNodePublicId: wbs.publicId,
		activityCode: 'A200',
		name: 'Structure',
		activityKind: 'activity',
		plannedStartOn: new Date('2026-08-21T00:00:00.000Z'),
		plannedFinishOn: new Date('2026-08-31T00:00:00.000Z'),
		plannedDurationDays: 11
	});
	activityAPublicId = activityA.publicId;
	activityBPublicId = activityB.publicId;
	const baseline = await plan.captureBaseline(owner, {
		projectPublicId,
...
describe('project progress and earned value', () => {
	it('creates and approves a performance baseline that reconciles to control budget', async () => {
		...
		const evBaselinePublicId = await service.createEarnedValueBaseline(owner, {
			projectPublicId,
			planBaselinePublicId,
			name: 'Performance baseline 1'
		});
		await service.setEarnedValueAllocation(owner, {
			projectPublicId,
			earnedValueBaselinePublicId: evBaselinePublicId,
			activityPublicId: activityAPublicId,
			budgetAtCompletionAmount: '600.00'
		});
		await expect(
			service.approveEarnedValueBaseline(owner, projectPublicId, evBaselinePublicId)
		).rejects.toThrow(ProjectProgressValidationError);
		await service.setEarnedValueAllocation(owner, {
			projectPublicId,
			earnedValueBaselinePublicId: evBaselinePublicId,
			activityPublicId: activityBPublicId,
			budgetAtCompletionAmount: '400.00'
		});
		...
		const workspace = await service.getWorkspace(viewer, projectPublicId, {
			baselinePublicId: evBaselinePublicId,
			dataDate: new Date('2026-08-15T00:00:00.000Z')
		});
	...
	it('records, submits and approves immutable official progress and derives PV EV AC CPI and SPI', async () => {
		...
		const periodPublicId = await service.createProgressPeriod(owner, {
			projectPublicId,
			label: 'Mid-August progress',
			dataDate: new Date('2026-08-15T00:00:00.000Z')
		});
		await service.recordActivityProgress(owner, {
			projectPublicId,
			periodPublicId,
			activityPublicId: activityAPublicId,
			measurementMethod: 'manual_percent',
			percentComplete: 50,
			actualStartOn: new Date('2026-08-01T00:00:00.000Z'),
			remainingDurationDays: 10,
			commentary: 'Measured foundations installed.'
		});
		await service.recordActivityProgress(owner, {
			projectPublicId,
			periodPublicId,
		...
		await expect(
			service.recordActivityProgress(owner, {
				projectPublicId,
				periodPublicId,
				activityPublicId: activityAPublicId,
				measurementMethod: 'manual_percent',
	...
	it('allows external progress visibility but contains owner financial performance and mutations', async () => {
		...
		await expect(
			service.createProgressPeriod(external, {
				projectPublicId,
				label: 'External attempt',
				dataDate: new Date('2026-08-31T00:00:00.000Z')
			})

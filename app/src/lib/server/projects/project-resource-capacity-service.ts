...
function calendarAssignmentForDate(
	...
	const active = patterns.filter((pattern) => dateWithin(dateText, pattern.validFrom, pattern.validTo));
...
function localWorkInterval(
	...
	const start = zonedLocalToUtc(formatLocalDateTime(dateText, pattern.localStartTime), pattern.timezone);
	const end = zonedLocalToUtc(formatLocalDateTime(endDateText, pattern.localEndTime), pattern.timezone);
...
function overlapMinutes(
	...
	const ranges = periods
		.map((period) => [Math.max(start.getTime(), period.startsAt.getTime()), Math.min(end.getTime(), period.endsAt.getTime())] as const)
...
function effortHoursToMinutes(value: string): number {
	...
	if (scaled <= 0n) {
		throw new ProjectResourceCapacityValidationError('Planned effort must be greater than zero hours.');
	}
	if (scaled > 10_000_000n) {
		throw new ProjectResourceCapacityValidationError('Planned effort must not exceed 100,000 hours.');
	}
...
function workingDatesForAllocation(
	...
	const scheduled = dates.filter((date) => scheduleForDate(calendarPatterns, date).pattern !== null);
...
export class ProjectResourceCapacityService {
	...
	private async resolveProjectAccess(
		...
		if (!projectViewDecision.allowed || (!viewDecision.allowed && !manageDecision.allowed)) {
			throw new RecordNotFoundError('Project resource capacity not found in the active member scope.');
		}
		return {
			project,
	...
	private normaliseRange(
		...
		if (!from) {
			from = activeActivities.length
				? new Date(Math.min(...activeActivities.map((activity) => activity.plannedStartOn.getTime())))
				: new Date(new Date().toISOString().slice(0, 10) + 'T00:00:00.000Z');
		}
		if (!to) {
			const latest = activeActivities.length
				? new Date(Math.max(...activeActivities.map((activity) => activity.plannedFinishOn.getTime())))
				: addDays(from, 27);
		...
		for (const workerId of workerIds) {
			...
			for (const date of dates) {
				...
				const projectCapacityMinutes = projectAssignment
					? Math.round(
							(availableMinutes * allocationPercent(projectAssignment.plannedAllocationPercent)) / 100
						)
					: 0;
			...
			const projectCapacityMinutes = days.reduce(
				(sum, day) => sum + day.projectCapacityMinutes,
				0
			);
			const plannedLoadMinutes = days.reduce((sum, day) => sum + day.plannedLoadMinutes, 0);
			const varianceMinutes = configuredAnyDay
				? projectCapacityMinutes - plannedLoadMinutes
				: null;
			workers.push({
				workerId,
	...
	async createAllocation(
		...
		if (loadFinishOn < loadStartOn) {
			throw new ProjectResourceCapacityValidationError('Load finish must be on or after load start.');
		}
		if (
			loadStartOn < activity.plannedStartOn ||
			loadFinishOn > activity.plannedFinishOn
		) {
			throw new ProjectResourceCapacityValidationError(
		...
		await this.db.transaction().execute(async (transaction) => {
			...
			await transactionRepository.lockProjectResourceAssignment(
				project.id,

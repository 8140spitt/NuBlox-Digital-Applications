...
export class ProjectExternalCollaborationAccessError extends Error {
	...
	constructor(message = 'The project collaboration invitation is invalid, expired or unavailable.') {
		super(message);
...
function normaliseEmail(value: string): string {
	...
	if (!email || email.length > 320 || !email.includes('@')) {
		throw new ProjectExternalCollaborationValidationError('The collaboration contact requires a valid email address.');
	}
	return email;
}
...
function normaliseRoleKeys(values: readonly string[]): string[] {
	...
	if (keys.length === 0) throw new ProjectExternalCollaborationValidationError('Choose at least one project role.');
	if (keys.length > 20) throw new ProjectExternalCollaborationValidationError('Too many project roles were selected.');
	return keys;
}
...
export class ProjectExternalCollaborationService {
	...
	private async requireOwnedProjectManage(actor: TenantActorContext, projectPublicIdInput: string, db: DatabaseExecutor = this.db) {
		...
		if (TERMINAL_PROJECT_STATUSES.has(project.status)) {
			throw new ProjectExternalCollaborationValidationError('This project can no longer accept collaborators.');
		}
		...
		if (!decision.allowed) throw new TenantAccessError('Project collaborator management is not permitted.');
	...
	private async roleIds(db: DatabaseExecutor, roleKeysInput: readonly string[]) {
		...
		if (rows.length !== roleKeys.length) {
			throw new ProjectExternalCollaborationValidationError('One or more project roles are unavailable.');
		}
		return { roleKeys, rows };
	}

	async getManagementView(actor: TenantActorContext, projectPublicId: string): Promise<ExternalCollaborationManagementView> {
		...
		const collaboratorRows = await this.db
			...
			.leftJoin('party_organisations as company', 'company.party_id', 'collaborator.crm_organisation_party_id')
			.select([
		...
		const collaboratorRoleRows = collaboratorIds.length
			? await this.db
					...
					.select(['assigned.project_external_collaborator_id as collaboratorId', 'role.role_key as roleKey', 'role.name'])
		...
		const invitationRows = await this.db
			...
			.innerJoin('party_persons as person', 'person.party_id', 'person_party.id')
			.leftJoin('party_organisations as company', 'company.party_id', 'invitation.crm_organisation_party_id')
			.select([
				'invitation.id as id',
		...
		const invitationRoleRows = invitationIds.length
			? await this.db
					...
					.innerJoin('project_role_types as role', 'role.id', 'assigned.project_role_type_id')
					.select(['assigned.project_collaboration_invitation_id as invitationId', 'role.role_key as roleKey', 'role.name'])
					.where('assigned.project_id', '=', project.id)
		...
		if (crmDecision.allowed) {
			const rows = await this.db
				...
				.innerJoin('party_persons as person', (join) =>
					join.onRef('person.party_id', '=', 'person_party.id').onRef('person.organisation_id', '=', 'person_party.organisation_id')
				)
				.innerJoin('party_email_addresses as email', (join) =>
					join.onRef('email.party_id', '=', 'person_party.id').onRef('email.organisation_id', '=', 'person_party.organisation_id').on('email.is_primary', '=', 1)
				)
				.leftJoin('party_organisation_contacts as contact', (join) =>
					join.onRef('contact.person_party_id', '=', 'person_party.id').onRef('contact.organisation_id', '=', 'person_party.organisation_id').on('contact.ended_on', 'is', null)
				)
				.leftJoin('parties as company_party', (join) =>
					join.onRef('company_party.id', '=', 'contact.organisation_party_id').onRef('company_party.organisation_id', '=', 'person_party.organisation_id').on('company_party.status', '=', 'active')
				)
				.leftJoin('party_organisations as company', (join) =>
					join.onRef('company.party_id', '=', 'company_party.id').onRef('company.organisation_id', '=', 'company_party.organisation_id')
				)
				.select([
					'person_party.public_id as personPublicId',
					'person.preferred_name as preferredName',
			...
			for (const row of rows) {
				candidates.push({
					...
					organisationName: row.companyLegalName ? displayOrganisation({ legalName: row.companyLegalName, tradingName: row.companyTradingName }) : null,
	...
	async invite(actor: TenantActorContext, input: {
		projectPublicId: string;
		personPartyPublicId: string;
		organisationPartyPublicId?: string | null;
		...
		const summary = await this.db.transaction().execute(async (trx) => {
			const { membership, project } = await this.requireOwnedProjectManage(actor, input.projectPublicId, trx);
			const crmDecision = await new PermissionService(trx).decide(actor, 'crm.view');
			if (!crmDecision.allowed) throw new TenantAccessError('CRM access is required to invite a CRM contact.');
			const person = await trx
				...
				.forUpdate()
				.executeTakeFirst();
			if (!person) throw new ProjectExternalCollaborationValidationError('Select an active CRM person with a primary email.');

			let company: { id: string; publicId: string; legalName: string; tradingName: string | null } | null = null;
			if (companyPublicId) {
				company = await trx
					.selectFrom('party_organisation_contacts as contact')
					.innerJoin('parties as company_party', (join) =>
						join.onRef('company_party.id', '=', 'contact.organisation_party_id').onRef('company_party.organisation_id', '=', 'contact.organisation_id')
					)
				...
				if (!company) throw new ProjectExternalCollaborationValidationError('The selected person is not an active contact of that CRM organisation.');
			}
			...
			const existing = await trx
				...
				.where('status', '=', 'active')
				.executeTakeFirst();
			if (existing) throw new ProjectExternalCollaborationValidationError('This person already collaborates on the project.');
			...
			await trx
				...
				.values(roles.map((role) => ({ project_id: project.id, project_collaboration_invitation_id: invitationId, project_role_type_id: role.id })))
				.execute();
			...
			await new AuditRepository(trx).append({
				...
				changeSummary: { personPartyPublicId: person.publicId, organisationPartyPublicId: company?.publicId ?? null, email, roleKeys }
			});
			return {
				publicId,
		...
		const invitationUrl = new URL(`/collaborate/${encodeURIComponent(token)}`, applicationBaseUrl()).toString();
	...
	private async invitationByToken(rawToken: string) {
		...
		return this.db
			...
			.innerJoin('parties as person_party', 'person_party.id', 'invitation.crm_contact_party_id')
			.innerJoin('party_persons as person', 'person.party_id', 'person_party.id')
			.leftJoin('parties as company_party', 'company_party.id', 'invitation.crm_organisation_party_id')
			.leftJoin('party_organisations as company', 'company.party_id', 'company_party.id')
			.select([
				'invitation.id as id', 'invitation.public_id as publicId', 'invitation.project_id as projectId',
				'invitation.inviting_organisation_id as ownerId', 'invitation.crm_contact_party_id as personPartyId',
				'invitation.crm_organisation_party_id as companyPartyId', 'invitation.invite_email as email',
				'invitation.status as status', 'invitation.auth_user_id as authUserId', 'invitation.invited_by_member_id as invitedByMemberId',
				'invitation.expires_at as expiresAt', 'invitation.accepted_at as acceptedAt',
				'project.public_id as projectPublicId', 'project.project_number as projectNumber', 'project.name as projectName', 'project.status as projectStatus',
				'owner.legal_name as ownerLegalName', 'owner.trading_name as ownerTradingName',
				'person_party.public_id as personPublicId', 'person.preferred_name as preferredName', 'person.given_names as givenNames', 'person.family_name as familyName',
				'company_party.public_id as companyPublicId', 'company.legal_name as companyLegalName', 'company.trading_name as companyTradingName'
			])
			.where('invitation.token_hash', '=', tokenHash)
			.executeTakeFirst();
	}

	async getPendingInvitation(rawToken: string): Promise<ProjectExternalCollaborationInvitationSummary | null> {
		...
		return {
			...
			invitingOrganisationName: displayOrganisation({ legalName: row.ownerLegalName, tradingName: row.ownerTradingName }),
			personPartyPublicId: row.personPublicId,
	...
	async activateVerifiedAuthUser(input: { authUserId: string; email: string; displayName: string; correlationId?: string }): Promise<string | null> {
		...
		return this.finaliseByAuth(input.authUserId, identity.userId, input.email, input.correlationId ?? randomUUID());
	}
	...
	async acceptExistingUser(rawToken: string, actor: Actor, correlationId: string): Promise<string> {
		...
		if (!invitation || normaliseEmail(invitation.email) !== normaliseEmail(actor.email)) {
			throw new ProjectExternalCollaborationAccessError('This invitation is addressed to a different verified email address.');
		}
	...
	private async finaliseByAuth(authUserId: string, userId: string, emailInput: string, correlationId: string): Promise<string> {
		...
		return this.db.transaction().execute(async (trx) => {
			const invitation = await trx
				...
				.select([
					...
					'invitation.crm_organisation_party_id as companyPartyId', 'invitation.invite_email as email',
					'invitation.status as status', 'invitation.auth_user_id as authUserId', 'invitation.invited_by_member_id as invitedByMemberId',
					'invitation.expires_at as expiresAt', 'project.public_id as projectPublicId', 'project.status as projectStatus',
					'person_party.public_id as personPublicId'
				])
				...
				.forUpdate()
				.executeTakeFirst();
			if (!invitation || invitation.expiresAt <= this.now()) throw new ProjectExternalCollaborationAccessError();
			...
			const roleRows = await trx
				...
				.execute();
			if (roleRows.length === 0) throw new ProjectExternalCollaborationValidationError('This invitation no longer has a project role.');
			...
			if (!collaborator) {
				...
				const inserted = await trx
					...
					.executeTakeFirstOrThrow();
				if (inserted.insertId === undefined) throw new Error('External collaborator insert did not return an ID.');
				collaborator = { id: inserted.insertId.toString(), publicId: collaboratorPublicId, status: 'active' };
			...
			await new AuditRepository(trx).append({
				...
				correlationId,
				changeSummary: { personPartyPublicId: invitation.personPublicId, invitationPublicId: invitation.publicId }
			});
			return invitation.projectPublicId;
		});
	}

	async revokeInvitation(actor: TenantActorContext, projectPublicId: string, invitationPublicIdInput: string): Promise<void> {
		const invitationPublicId = normalisePublicId(invitationPublicIdInput, 'Invitation');
		await this.db.transaction().execute(async (trx) => {
			const { membership, project } = await this.requireOwnedProjectManage(actor, projectPublicId, trx);
			const result = await trx
				...
				.where('status', '=', 'pending')
				.executeTakeFirst();
			if (result.numUpdatedRows !== 1n) throw new RecordNotFoundError('Pending collaboration invitation not found.');
			await new AuditRepository(trx).append({
				eventPublicId: this.publicIdFactory(), actingOrganisationId: actor.organisationId,
				actorUserId: actor.userId, actorMemberId: membership.id, projectId: project.id,
				actionKey: 'project.external_collaboration.invitation_revoked', subjectType: 'project_collaboration_invitation',
				subjectPublicId: invitationPublicId, correlationId: actor.correlationId
			});
		});
	}

	async removeCollaborator(actor: TenantActorContext, projectPublicId: string, collaboratorPublicIdInput: string): Promise<void> {
		const collaboratorPublicId = normalisePublicId(collaboratorPublicIdInput, 'External collaborator');
		await this.db.transaction().execute(async (trx) => {
			const { membership, project } = await this.requireOwnedProjectManage(actor, projectPublicId, trx);
			const result = await trx
				.updateTable('project_external_collaborators')
				...
				.where('status', '=', 'active')
				.executeTakeFirst();
			if (result.numUpdatedRows !== 1n) throw new RecordNotFoundError('Active external collaborator not found.');
			await new AuditRepository(trx).append({
				eventPublicId: this.publicIdFactory(), actingOrganisationId: actor.organisationId,
				actorUserId: actor.userId, actorMemberId: membership.id, projectId: project.id,
				actionKey: 'project.external_collaboration.revoked', subjectType: 'project_external_collaborator',
				subjectPublicId: collaboratorPublicId, correlationId: actor.correlationId
			});
		});
	}
	...
	async listExternalPortalProjects(authUserId: string): Promise<ExternalPortalProject[]> {
		const rows = await this.db
			...
			.leftJoin('party_organisations as company', 'company.party_id', 'collaborator.crm_organisation_party_id')
			.select([
				'collaborator.id as collaboratorId', 'collaborator.public_id as collaboratorPublicId',
				'project.public_id as projectPublicId', 'project.project_number as projectNumber', 'project.name as projectName', 'project.status as projectStatus',
				'owner.legal_name as ownerLegalName', 'owner.trading_name as ownerTradingName',
				'company.legal_name as companyLegalName', 'company.trading_name as companyTradingName'
			])
		...
		return rows.map((row) => ({
			...
			owningOrganisationName: displayOrganisation({ legalName: row.ownerLegalName, tradingName: row.ownerTradingName }),
			crmOrganisationName: row.companyLegalName ? displayOrganisation({ legalName: row.companyLegalName, tradingName: row.companyTradingName }) : null,

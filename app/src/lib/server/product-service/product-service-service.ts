...
function code(value: string, label: string) {
	...
	if (!CODE.test(normalized)) throw new ProductServiceValidationError(`${label} has an invalid format.`);
	return normalized;
}

function token(value: string, label: string) {
	const normalized = value.trim().toLowerCase();
	if (!TOKEN.test(normalized)) throw new ProductServiceValidationError(`${label} has an invalid format.`);
	return normalized;
}
...
export class ProductServiceService {
	...
	private async requirePermission(
		...
		if (!decision.allowed) throw new TenantAccessError('Product/service lifecycle action is not permitted.');
	}
	...
	private async activeMember(db: DatabaseExecutor, organisationId: string, memberId: string) {
		const row = await db
			...
			.where('status', '=', 'active')
			.executeTakeFirst();
		if (!row) throw new ProductServiceValidationError('Owner must be an active organisation member.');
		return row.id;
	}
	...
	async getWorkspace(actor: TenantActorContext): Promise<ProductServiceWorkspace> {
		...
		return { portfolios, offerings, needs, ideas, businessCases, assumptions, scenarios, ...flags };
	}
	...
	async createPortfolio(actor: TenantActorContext, input: PortfolioInput) {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			await this.evidence(trx, actor, 'product_service.portfolio.created', 'product_service_portfolio', created.public_id, { portfolioCode: created.portfolio_code, portfolioType: created.portfolio_type }, ['F05.01']);
			return created;
		});
	}
	...
	async createOffering(actor: TenantActorContext, input: OfferingInput) {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const portfolio = await repository.findPortfolioByPublicId(actor.organisationId, input.portfolioPublicId.trim());
			...
			await this.evidence(trx, actor, 'product_service.offering.created', 'product_service_offering', created.public_id, { offeringCode: created.offering_code, portfolioPublicId: portfolio.public_id }, ['F05.01']);
			return created;
		});
	}
	...
	async createNeed(actor: TenantActorContext, input: NeedInput) {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const portfolio = input.portfolioPublicId?.trim()
				? await repository.findPortfolioByPublicId(actor.organisationId, input.portfolioPublicId.trim())
				: null;
			...
			await this.evidence(trx, actor, 'product_service.need.created', 'product_service_need', created.public_id, { needCode: created.need_code, sourceDomain: created.source_domain }, ['F05.02']);
			return created;
		});
	}
	...
	async createIdea(actor: TenantActorContext, input: IdeaInput) {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const need = input.needPublicId?.trim()
				...
				: null;
			if (input.needPublicId?.trim() && !need) throw new RecordNotFoundError('Product/service need not found.');
			const owner = await this.activeMember(trx, actor.organisationId, input.ownerMemberId);
			...
			await this.evidence(trx, actor, 'product_service.idea.created', 'product_service_idea', created.public_id, { ideaCode: created.idea_code, needPublicId: need?.public_id ?? null }, ['F05.03']);
			return created;
		});
	}
	...
	async scoreIdea(actor: TenantActorContext, ideaPublicId: string, input: IdeaScoreInput) {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const values = [input.strategicFit, input.customerValue, input.feasibility, input.commercialValue];
			...
			const updated = await repository.updateIdea(actor.organisationId, idea.id, {
				strategic_fit_score: score(input.strategicFit, 'Strategic fit score'),
				customer_value_score: score(input.customerValue, 'Customer value score'),
				feasibility_score: score(input.feasibility, 'Feasibility score'),
				commercial_value_score: score(input.commercialValue, 'Commercial value score'),
				risk_score: score(input.risk, 'Risk score'),
				overall_score: score(overall, 'Overall score'),
				stage: 'triage'
			});
			await this.evidence(trx, actor, 'product_service.idea.scored', 'product_service_idea', updated.public_id, { overallScore: updated.overall_score }, ['F05.03']);
			return updated;
		});
	}
	...
	async createBusinessCase(actor: TenantActorContext, input: BusinessCaseInput) {
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new ProductServiceRepository(trx);
			const idea = await repository.findIdeaByPublicId(actor.organisationId, input.ideaPublicId.trim());
			if (!idea) throw new RecordNotFoundError('Product/service idea not found.');
			const offering = input.offeringPublicId?.trim()
				? await repository.findOfferingByPublicId(actor.organisationId, input.offeringPublicId.trim())
				: null;
			...
			const predecessor = predecessors.find((row) => row.business_case_code === businessCaseCode) ?? null;
			const created = await repository.insertBusinessCase({
				...
				currency_code: requiredText(input.currencyCode, 'Currency code', 3).toUpperCase(),
			...
			await this.evidence(trx, actor, 'product_service.business_case.created', 'product_service_business_case', created.public_id, { businessCaseCode: created.business_case_code, versionNumber: created.version_number, ideaPublicId: idea.public_id }, ['F05.04']);
			return created;
		});
	}
	...
	async approveBusinessCase(actor: TenantActorContext, businessCasePublicId: string) {
		...
		return this.db.transaction().execute(async (trx) => {
			const repository = new ProductServiceRepository(trx);
			const businessCase = await repository.findBusinessCaseByPublicId(actor.organisationId, businessCasePublicId.trim());
			...
			if (businessCase.supersedes_business_case_id) {
				await repository.updateBusinessCase(actor.organisationId, businessCase.supersedes_business_case_id, { lifecycle_status: 'superseded' });
			}
			...
			await this.evidence(trx, actor, 'product_service.business_case.approved', 'product_service_business_case', approved.public_id, { businessCaseCode: approved.business_case_code, versionNumber: approved.version_number }, ['F05.04']);
			return approved;
		});
	}
}

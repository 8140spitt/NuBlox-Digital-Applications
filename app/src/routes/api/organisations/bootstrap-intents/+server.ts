...
export const POST: RequestHandler = async ({ request, cookies, locals }) => {
	if (locals.actor) {
		throw error(409, 'You are already signed in. Create the organisation from the organisation setup page.');
	}
	...
	try {
		...
		const remainingSeconds = Math.max(60, Math.floor((intent.expiresAt.getTime() - Date.now()) / 1000));

...
<section class="workspace">
	<header>
		...
		<p>
			Originate, evaluate and govern strategic transactions through one evidence-backed deal thread.
		</p>
	</header>
					...
					{#each data.valuations as valuation}
						<article class="card">
							...
							{#if data.canApprove && valuation.lifecycle_status === 'draft'}
								<form method="POST" action="?/approveValuation">
			...
			{#if data.canManage}
				<section class="panel">
					...
					<form method="POST" action="?/createValuation" class="form-grid">
						...
						><label>EV high <input name="enterpriseValueHigh" inputmode="decimal" /></label>
						<label>Equity low <input name="equityValueLow" inputmode="decimal" /></label><label
							>Equity base <input name="equityValueBase" inputmode="decimal" /></label
						><label>Equity high <input name="equityValueHigh" inputmode="decimal" /></label>
						<label class="wide"

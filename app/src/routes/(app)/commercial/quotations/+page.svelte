		...
		{#each data.quotations as quote}
			<article class="card">
				...
				{#if quote.effectiveStatus==='accepted' && quote.latestVersionNumber}
					<div class="card-actions"><a href={`/commercial/quotations/${quote.publicId}/convert?version=${quote.latestVersionNumber}`}>Project conversion</a></div>
				{/if}
			</article>
		{/each}
	</div></section>

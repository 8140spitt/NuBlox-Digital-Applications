...
<section class="page-header">
	...
	{#if data.canManage && selectedProject}<a class="header-action" href="#create-valuation"
			>New supplier application</a
		>{/if}
</section>
					...
					<article class="record-card valuation-card">
						<header>
							<div>
								...
								<p class="muted">
									{valuation.kind.replaceAll('_', ' ')} · Valuation date {date(
						...
						<p class="money-value">
							{money(valuation.totalGrossValueToDate, valuation.currencyCode)}
						</p>
			...
			<form method="POST" action="?/createSupplierApplication" class="stack-form">
				...
				<label>
					...
					<select name="purchaseOrderPublicId" required>
						...
						{#each data.purchaseOrders as order}<option value={order.publicId}
				...
				<label>
					...
					<select name="costCodePublicId">
						...
						{#each data.costCodes as costCode}<option value={costCode.publicId}
				...
				<div class="two-up">
					...
					<label
						>Gross value to date<input
							name="grossValueToDate"
							inputmode="decimal"
							required
						/></label
					>

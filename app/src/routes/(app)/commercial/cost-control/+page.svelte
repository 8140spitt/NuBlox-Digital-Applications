...
<section class="page-header">
	...
	{#if data.canManageCostCodes && selectedProject}<a class="header-action" href="#create-cost-code"
			>New cost code</a
		>{/if}
</section>
...
{:else}
	...
	{#if data.position && selectedProject}
		<section class="position" aria-label="Project commercial position">
			...
			<div class="metrics">
				<article>
					<span>Approved baseline budget</span><strong
						>{money(data.position.approvedBaselineBudget, data.position.currencyCode)}</strong
					>
				</article>
				<article>
					<span>Issued PO commitment</span><strong
						>{money(
							data.position.issuedPurchaseOrderCommitment,
							data.position.currencyCode
						)}</strong
					>
				</article>
				<article>
					<span>Cost-code classified</span><strong
						>{money(data.position.classifiedCommitment, data.position.currencyCode)}</strong
					>
				</article>
				<article>
					<span>Accepted receipt cost</span><strong
						>{money(data.position.acceptedReceiptCost, data.position.currencyCode)}</strong
					>
				</article>
				<article>
					<span>Approved change</span><strong
						>{money(data.position.approvedChange, data.position.currencyCode)}</strong
					>
				</article>
				<article>
					<span>Pending change exposure</span><strong
						>{money(data.position.pendingChangeExposure, data.position.currencyCode)}</strong
					>
				</article>
				<article>
					<span>Budget headroom</span><strong
						>{money(data.position.budgetHeadroom, data.position.currencyCode)}</strong
					>
				</article>
				<article>
					<span>Exposed headroom</span><strong
						>{money(data.position.exposedHeadroom, data.position.currencyCode)}</strong
					>
				</article>
			</div>
		</section>
	...
	<section class="workspace-section" id="cost-code-register">
		<div class="section-heading">
			<div>
				...
				<h2>Project cost codes</h2>
			</div>
			<span class="count">{selectedCostCodes.length}</span>
		</div>
		{#if selectedCostCodes.length === 0}
			<div class="empty-state">
				<h3>No project cost codes</h3>
				<p>Create the first classification before setting a budget or classifying commitments.</p>
			</div>
		{:else}
			<div class="register-list">
				{#each selectedCostCodes as costCode}
					<article class="record-card">
						<header>
							<div>
								<p class="reference">{costCode.code}</p>
								<h3>{costCode.name}</h3>
								<p class="muted">{costCode.categoryName}</p>
							</div>
							<span class="status">{costCode.isActive ? 'active' : 'inactive'}</span>
						</header>
	...
	<section class="workspace-section" id="budget-register">
		<div class="section-heading">
			...
			<span class="count">{selectedBudgets.length}</span>
		</div>
		{#if selectedBudgets.length === 0}
			<div class="empty-state">
				<h3>No controlled budget</h3>
				<p>Create a draft budget and approve it to establish the commercial baseline.</p>
			</div>
		{:else}
			<div class="register-list">
				{#each selectedBudgets as budget}
					<article class="record-card budget-card">
						<header>
							<div>
								<p class="reference">{budget.budgetNumber}</p>
								<h3>{budget.name}</h3>
							</div>
							<span class="status">{budget.latestVersion?.status ?? budget.status}</span>
						</header>
						<p class="money-value">
							{money(budget.total, budget.latestVersion?.currencyCode ?? 'GBP')}
						</p>
						<p class="muted">
							Version {budget.latestVersion?.versionNumber ?? '—'} · {budget.latestVersion
								?.currencyCode ?? '—'}
						</p>
	...
	<section class="workspace-section" id="commitment-register">
		<div class="section-heading">
			<div>
				<p class="eyebrow">Authoritative source facts</p>
				<h2>Purchase-order commitments</h2>
			</div>
			<span class="count">{selectedOrders.length}</span>
		</div>
		<p class="section-copy">
			Purchase-order value stays authoritative in Procurement. Commercial control only classifies
			the issued line against a project cost code.
		</p>
		{#if selectedOrders.length === 0}
			<div class="empty-state">
				<h3>No project purchase orders</h3>
				<p>Issue a purchase order in Purchasing before classifying its commitment.</p>
			</div>
		{:else}
			<div class="register-list">
					...
					<article class="record-card commitment-card">
						<header>
							<div>
								...
								<p class="muted">{order.typeName} · {order.currencyCode}</p>
							</div>
						...
						{#if data.canManageCostCodes && selectedCostCodes.length > 0 && selectedProject}
							<form method="POST" action="?/allocatePurchaseOrderLine" class="inline-form">
								...
								>
								<label
									>Cost code<select name="costCodePublicId" required
										><option value="">Select cost code</option
										>{#each selectedCostCodes as costCode}<option value={costCode.publicId}
												>{costCode.code} · {costCode.name}</option
											>{/each}</select
									></label
								>
	...
	<section class="workspace-section" id="variation-register">
		<div class="section-heading">
			<div>
				...
				<h2>Commercial variations</h2>
			</div>
			<span class="count">{selectedVariations.length}</span>
		</div>
		{#if selectedVariations.length === 0}
			<div class="empty-state">
				<h3>No commercial variations</h3>
				<p>
					Create a cost, revenue or internal variation and issue a locked version before recording a
					decision.
				</p>
			</div>
		{:else}
			<div class="register-list">
				{#each selectedVariations as variation}
					<article class="record-card variation-card">
						<header>
							<div>
								<p class="reference">{variation.variationNumber}</p>
								<h3>{variation.title}</h3>
								<p class="muted">{variation.typeName} · {variation.commercialSide}</p>
							</div>
						...
						{#if variation.latestDecision}<p class="decision">
								Decision · <strong>{variation.latestDecision}</strong>{#if variation.decisionAmount}
									· {money(variation.decisionAmount, variation.currencyCode)}{/if}
							</p>{/if}
								...
								<form method="POST" action="?/decideVariation" class="stack-form compact-form">
									...
									<label
										>Decision<select name="decision" required
											><option value="accepted">Accepted</option><option value="partially_accepted"
												>Partially accepted</option
											><option value="pending">Pending</option><option value="rejected"
												>Rejected</option
											><option value="withdrawn">Withdrawn</option></select
										></label
									>
									<label
										>Decision amount <small>blank = full amount for accepted decisions</small><input
											name="decisionAmount"
											inputmode="decimal"
										/></label
									>
	...
	{#if selectedProject && data.canManageCostCodes}
		<section class="action-panel" id="create-cost-code">
			<p class="eyebrow">Classification</p>
			<h2>Create project cost code</h2>
			<form method="POST" action="?/createCostCode" class="stack-form">
				<input type="hidden" name="projectPublicId" value={selectedProject.publicId} />
				<label
					>Project<input
						value={`${selectedProject.projectNumber} · ${selectedProject.name}`}
						disabled
					/></label
				>
				<div class="three-up">
					<label
						>Cost category<select name="categoryCode" required
							><option value="">Select category</option
							>{#each data.costCategories as category}<option value={category.code}
	...
	{#if selectedProject && data.canManageBudgets && selectedCostCodes.length > 0}
		<section class="action-panel" id="create-budget">
			<p class="eyebrow">Controlled baseline</p>
			<h2>Create project budget</h2>
			<form method="POST" action="?/createBudget" class="stack-form">
				<input type="hidden" name="projectPublicId" value={selectedProject.publicId} />
				<label
					>Cost code<select name="costCodePublicId" required
						><option value="">Select cost code</option>{#each selectedCostCodes as costCode}<option
								value={costCode.publicId}>{costCode.code} · {costCode.name}</option
							>{/each}</select
					></label
				>
				<div class="three-up">
	...
	{#if selectedProject && data.canManageVariations}
		<section class="action-panel" id="create-variation">
			<p class="eyebrow">Controlled change</p>
			<h2>Create commercial variation</h2>
			<form method="POST" action="?/createVariation" class="stack-form">
				<input type="hidden" name="projectPublicId" value={selectedProject.publicId} />
				<div class="two-up">
					<label
						>Variation type<select name="variationTypeCode" required
							><option value="">Select type</option>{#each data.variationTypes as type}<option
									value={type.code}>{type.name}</option
								>{/each}</select
						></label
					>
					<label
						>Commercial side<select name="commercialSide" required
							><option value="cost">Cost / downstream</option><option value="revenue"
								>Revenue / upstream</option
				...
				<div class="two-up">
					<label
						>Cost code <small>optional</small><select name="costCodePublicId"
					...
					>
					<label
						>Purchase order <small>cost-side only, optional</small><select
							name="purchaseOrderPublicId"
							><option value="">No linked purchase order</option
							>{#each selectedOrders as order}<option value={order.publicId}
									>{order.purchaseOrderNumber} · {order.supplierName}</option
								>{/each}</select
						></label
					>
				</div>

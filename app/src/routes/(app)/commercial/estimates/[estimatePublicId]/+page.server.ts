...
export const load: PageServerLoad = async ({ locals, params, url }) => {
	...
	try {
		return await new CommercialService(getDatabase()).getEstimate(
			actor,
			params.estimatePublicId,
			versionFromUrl(url)
		);
	} catch (error) {
...
export const actions: Actions = {
	...
	removeItem: async ({ request, locals, params }) => {
		...
		const data = await request.formData();
		try {
			await new CommercialService(getDatabase()).removeEstimateItem(
				actor,
				params.estimatePublicId,
	...
	finalise: async ({ request, locals, params }) => {
		...
		const data = await request.formData();
		try {
			await new CommercialService(getDatabase()).finaliseEstimate(
				actor,
				params.estimatePublicId,
				positiveInt(data.get('versionNumber'), 'Version number')
			);
	...
	createQuotation: async ({ request, locals, params }) => {
		...
		const data = await request.formData();
		try {
			const quotation = await new CommercialService(getDatabase()).createQuotationFromEstimate(
				actor,
				{
					estimatePublicId: params.estimatePublicId,
					versionNumber: positiveInt(data.get('versionNumber'), 'Version number'),

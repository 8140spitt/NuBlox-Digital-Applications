...
<section class="page-heading">
	<div>
		...
		<h1>Collections</h1>
		<p>
			Work overdue customer accounts without changing the invoice, credit-note or payment ledger.
		</p>
	</div>
...
<section class="panel">
	<div class="section-heading">
		<div>
			...
			<h2>Overdue customer accounts</h2>
			<p class="muted">
				Only invoices with positive outstanding value past their due date enter this queue.
			</p>
		</div>
				...
				<a class="account-row" href={`/finance/collections/${account.customerPartyPublicId}`}>
					...
					<div class="currency-list">
						{#each account.overduePositions as position}
							<span
								><strong>{money(overdueAmount(position), position.currencyCode)}</strong><small
									>{position.invoices.length} overdue invoice{position.invoices.length === 1
										? ''
										: 's'}</small
								></span
							>
					...
					<span
						class={`status ${account.activeCase ? `status-${account.activeCase.status}` : 'status-new'}`}
						>{account.activeCase?.status ?? 'not started'}</span
					>
				</a>
			{/each}
		</div>

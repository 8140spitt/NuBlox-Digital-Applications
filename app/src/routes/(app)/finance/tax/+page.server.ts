...
function actionFailure(cause: unknown) {
	...
	if (cause instanceof RecordNotFoundError) return fail(404, { actionError: 'The requested tax category is unavailable.' });
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try {
		...
		if (cause instanceof TenantAccessError) throw httpError(403, 'Tax settings access is not permitted.');
		throw cause;
	}
};

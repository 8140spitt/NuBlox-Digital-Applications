...
<nav class="breadcrumbs" aria-label="Breadcrumb">
	<a href="/finance/receivables">Receivables</a><span aria-hidden="true">/</span><span>Credit Control</span>
</nav>
...
<div class="layout">
	<div class="stack">
		<section class="panel">
			<p class="eyebrow">Live policy</p><h2>Credit limits</h2>
					...
					{#each data.policies as policy}
						<article class:blocked={policy.limitExhausted} class="record">
							<div class="record-head">
								<span><strong>{policy.customerDisplayName}</strong><small>{policy.currencyCode} · revision {policy.versionNumber}</small></span>
								<em>{policy.isEnabled ? policy.limitExhausted ? 'Exhausted' : 'Enabled' : 'Disabled'}</em>
							</div>
							<dl>
								<div><dt>Limit</dt><dd>{policy.isEnabled ? money(policy.creditLimitAmount, policy.currencyCode) : 'Disabled'}</dd></div>
								<div><dt>Outstanding</dt><dd>{money(policy.outstandingAmount, policy.currencyCode)}</dd></div>
								<div><dt>Available</dt><dd>{policy.isEnabled ? money(policy.availableAmount, policy.currencyCode) : '—'}</dd></div>
							...
							{#if data.canManagePolicies && policy.isEnabled}
								<form method="POST" action="?/disableLimit" class="inline-action">
									<input type="hidden" name="customerPartyPublicId" value={policy.customerPartyPublicId}/>
		...
		<section class="panel">
			<p class="eyebrow">Stop trading</p><h2>Credit holds</h2>
				...
				<div class="cards">
					{#each data.holds as hold}
						<article class:blocked={hold.status === 'active'} class="record">
							<div class="record-head"><span><strong>{hold.customerDisplayName}</strong><small>Placed {new Date(hold.placedAt).toLocaleString()}</small></span><em>{hold.status}</em></div>
							<p>{hold.placedReason}</p>
							{#if hold.status === 'released'}
								<p class="muted">Released {hold.releasedAt ? new Date(hold.releasedAt).toLocaleString() : ''}: {hold.releasedReason}</p>
		...
		<section class="panel">
			<p class="eyebrow">Exceptional authority</p><h2>Override history</h2>
	...
	<aside class="stack">
		{#if data.canManagePolicies}
			<section class="panel action-panel">
				<p class="eyebrow">Limit policy</p><h2>Set or revise a limit</h2>
				<p class="muted">Each change creates a new immutable revision. Existing history is never overwritten.</p>
				<form method="POST" action="?/setLimit">
					<label>Customer<select name="customerPartyPublicId" required><option value="">Select customer</option>{#each data.customers as customer}<option value={customer.publicId}>{customer.displayName}</option>{/each}</select></label>
		...
		{#if data.canManageHolds}
			<section class="panel hold-panel">
				<p class="eyebrow">Stop trading</p><h2>Place a credit hold</h2>
				<p class="muted">A hold blocks new accepted-quotation conversion and contract execution regardless of currency.</p>
		...
		<section class="panel">
			...
			<p class="muted">Override authority: {data.canOverride ? 'granted to you' : 'not granted to you'}.</p>
		</section>
	</aside>
</div>

<style>
	.breadcrumbs{display:flex;gap:.55rem;align-items:center;margin-bottom:1rem;color:#666;font-size:.9rem}.breadcrumbs a{color:inherit;font-weight:650}.page-heading{display:flex;justify-content:space-between;gap:1rem;align-items:start;margin-bottom:1rem}.page-heading h1{margin:.15rem 0 .3rem;font-size:clamp(2rem,5vw,2.8rem);letter-spacing:-.04em}.page-heading p{margin:0;color:#666}.eyebrow{margin:0;text-transform:uppercase;letter-spacing:.1em;font-size:.72rem;font-weight:760;color:#666}.secondary{padding:.55rem .72rem;border:1px solid #cfcfc8;border-radius:.46rem;color:#222;text-decoration:none;font-weight:700}.boundary{display:grid;gap:.2rem;padding:.85rem 1rem;margin-bottom:1rem;border:1px solid #c8d4e6;border-radius:.65rem;background:#f5f8fc}.boundary span,.muted{color:#666;line-height:1.5}.layout{display:grid;grid-template-columns:minmax(0,1.35fr) minmax(20rem,.65fr);gap:1rem;align-items:start}.stack,.cards,form{display:grid;gap:.8rem}.panel{background:#fff;border:1px solid #d9d9d2;border-radius:.8rem;padding:1.1rem}.panel h2{margin:.3rem 0 .8rem}.action-panel{border-color:#b9cbe6}.hold-panel{border-color:#dfb36a}.record{padding:.85rem;border:1px solid #e1e1db;border-radius:.6rem}.record.blocked{border-color:#dfb36a;background:#fffaf0}.record-head{display:flex;justify-content:space-between;gap:.8rem;align-items:start}.record-head span{display:grid;gap:.15rem}.record-head small{color:#666}.record-head em{font-style:normal;text-transform:capitalize;font-size:.76rem;font-weight:760}.record p{line-height:1.45}dl{display:grid;gap:.45rem;margin:.7rem 0}dl div{display:grid;grid-template-columns:7.5rem 1fr;gap:.6rem}dt{font-size:.82rem;color:#666}dd{margin:0}.inline-action{margin-top:.7rem;padding-top:.7rem;border-top:1px solid #ecece7;grid-template-columns:1fr auto;align-items:end}label{display:grid;gap:.3rem;font-weight:650}input,select,textarea{width:100%;box-sizing:border-box;padding:.58rem;border:1px solid #c9c9c2;border-radius:.45rem;background:#fff;font:inherit}button{width:max-content;padding:.62rem .8rem;border:0;border-radius:.46rem;background:#111;color:white;font:inherit;font-weight:750;cursor:pointer}.quiet{background:#6f342d}.error{color:#8a3025}.banner{padding:.7rem .8rem;background:#fff0ed;border:1px solid #e1b1aa;border-radius:.5rem}ul{padding-left:1.2rem;line-height:1.5}@media(max-width:900px){.layout{grid-template-columns:1fr}.page-heading{display:grid}.inline-action{grid-template-columns:1fr}dl div{grid-template-columns:1fr;gap:.1rem}}
</style>

...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try { return await new ReceivablesControlReportingService(getDatabase()).getPortfolio(actor); }
	catch (cause) { if (cause instanceof TenantAccessError) throw httpError(403, 'Receivables reporting is not permitted.'); throw cause; }
};

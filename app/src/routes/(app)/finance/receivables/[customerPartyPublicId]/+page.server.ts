...
export const load: PageServerLoad = async ({ locals, params, url }) => {
	...
	try {
		return await new ReceivablesControlReportingService(getDatabase()).getCustomerStatement(actor, params.customerPartyPublicId, { from: url.searchParams.get('from'), to: url.searchParams.get('to') });
	} catch (cause) {
		if (cause instanceof RecordNotFoundError) throw httpError(404, 'Customer receivable account not found.');

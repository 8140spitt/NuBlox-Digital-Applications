...
<section class="panel">
	<div class="section-heading">
		<div>
			<p class="eyebrow">Source invoices</p>
			<h2>Issued invoices</h2>
			<p class="muted">
				Credit notes reduce an issued receivable. Invoice void is reserved for invalid documents
				with no correction or active allocation history.
			</p>
		</div>
			...
			{#each data.invoices as invoice}
				<article class="source-card">
					<div class="source-summary">
						<div>
							<strong>{invoice.invoiceNumber}</strong><small>{invoice.customerDisplayName}</small>
						</div>
						<div class="amount">
							<strong>{money(invoice.remainingGross, invoice.currencyCode)} remaining</strong><small
								>{money(invoice.invoiceGross, invoice.currencyCode)} issued · {money(
					...
					<div class="actions-grid">
						{#if data.canCreate && invoice.canCredit}
							<form method="POST" action="?/create" class="action-form">
								...
								<label
									>Credit-note reason<textarea
										name="reason"
										rows="2"
										required
										placeholder="Why is this receivable being corrected?"></textarea></label
								>
							...
							<form method="POST" action="?/voidInvoice" class="action-form danger-form">
								...
								<label
									>Void reason<input
										name="reason"
										required
										placeholder="Duplicate, invalid issue, etc."
									/></label
								>
...
<section class="panel">
	<div class="section-heading">
		...
		<span>{data.creditNotes.length}</span>
	</div>
		...
		<div class="credit-list">
			{#each data.creditNotes as credit}
				<a class="credit-row" href={`/finance/credit-notes/${credit.publicId}`}>
					<div>
						<strong>{credit.documentNumber ?? 'Draft credit note'}</strong><small
							>{credit.originalInvoiceNumber} · {credit.customerDisplayName}</small
						>
					</div>
					<div class="amount">
						<strong>{money(credit.grossTotal, credit.currencyCode)}</strong><small
							>{credit.reason}</small
						>
					</div>

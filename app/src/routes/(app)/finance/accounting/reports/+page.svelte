			...
			<select name="period">
				{#each data.periods as period}
					<option
						...
						selected={data.selectedPeriod?.publicId === period.publicId}
					>
						{period.financialYearCode} · {period.periodNumber} · {period.name} · {statusText(
							period.status
						)}
					</option>
				{/each}
...
{:else}
	...
	<section class="panel">
		<div class="section-heading">
			<div>
				<p class="eyebrow">Double-entry control</p>
				<h2>Trial balance</h2>
			</div>
			<div class="balance-badges">
				<span class:bad={!data.trialBalance.openingBalanced}
					>Opening {data.trialBalance.openingBalanced ? 'balanced' : 'out of balance'}</span
				>
				<span class:bad={!data.trialBalance.periodBalanced}
					>Period {data.trialBalance.periodBalanced ? 'balanced' : 'out of balance'}</span
				>
				<span class:bad={!data.trialBalance.closingBalanced}
					>Closing {data.trialBalance.closingBalanced ? 'balanced' : 'out of balance'}</span
				>
			</div>
		</div>
	...
	<div class="report-grid">
		<section class="panel">
			<div class="section-heading">
				<div>
					<p class="eyebrow">Account-type presentation</p>
			...
			<div class="statement-result">
				<span>Period profit / (loss)</span><strong>{money(data.profitAndLoss.periodProfit)}</strong>
			</div>
			<div class="ytd">
		...
		<section class="panel">
			<div class="section-heading">
				<div>
					<p class="eyebrow">Closing position</p>
					<h2>Balance sheet view</h2>
				</div>
				<span class:bad={!data.balanceSheet.balanced}
					>{data.balanceSheet.balanced ? 'Balanced' : 'Out of balance'}</span
				>
			</div>
			<h3>Assets</h3>
			<div class="statement-lines">
			...
			<div class="statement-total">
				<span>Unclosed earnings from journal history</span><strong
					>{money(data.balanceSheet.unclosedEarnings)}</strong
				>
			</div>
			<div class="statement-result">
				<span>Liabilities + equity + unclosed earnings</span><strong
					>{money(data.balanceSheet.liabilitiesEquityAndEarningsTotal)}</strong
				>
			</div>
			<p class="explain">
...
<style>
	...
	.filters form {
		display: grid;
		grid-template-columns: minmax(16rem, 2fr) minmax(8rem, 1fr) auto;
		gap: 0.75rem;
		align-items: end;
	}
	.notice {
	...
	.balance-badges span.bad,
	.section-heading > span.bad {
		background: #fef3f2;
		color: #b42318;
	}
	.table-wrap {

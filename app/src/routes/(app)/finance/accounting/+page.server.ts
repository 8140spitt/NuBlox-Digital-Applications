...
function actionFailure(cause: unknown) {
	...
	if (cause instanceof RecordNotFoundError) return fail(404, { actionError: 'The requested accounting record is unavailable.' });
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try {
		...
		if (cause instanceof TenantAccessError) throw httpError(403, 'Accounting access is not permitted.');
		throw cause;
	}
};

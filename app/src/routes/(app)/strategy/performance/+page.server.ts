...
async function runAction(
	...
	try {
		...
		if (error instanceof PerformanceForesightValidationError) return fail(400, { error: error.message });
		if (error instanceof TenantAccessError) return fail(403, { error: 'You do not have access to this strategy performance action.' });
...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	try {
		...
		const [workspace, members] = await Promise.all([
			...
			db.selectFrom('organisation_members as member')
		...
		if (error instanceof RecordNotFoundError) throw httpError(404, 'Strategy performance workspace not found in the active scope.');
		if (error instanceof TenantAccessError) throw httpError(403, 'You do not have access to the strategy performance workspace.');
		throw error;
	}
};
...
export const actions: Actions = {
	createKpi: async ({ request, locals }) => {
		...
		return runAction(locals, frameworkPublicId, (service, actor) => service.createKpi(actor, {
			frameworkPublicId,
			...
			direction: text(data, 'direction') as 'higher_is_better' | 'lower_is_better' | 'target_is_best' | 'band',
			aggregationMethod: text(data, 'aggregationMethod') as 'latest' | 'sum' | 'average' | 'minimum' | 'maximum' | 'ratio',
	...
	approveKpi: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.approveKpi(actor, text(data, 'kpiPublicId')));
	},
	reviseKpi: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.reviseKpi(actor, text(data, 'kpiPublicId')));
	},
	recordObservation: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.recordObservation(actor, {
	...
	createAction: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.createAction(actor, {
			kpiPublicId: text(data, 'kpiPublicId'),
	...
	completeAction: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.completeAction(actor, text(data, 'actionPublicId'), text(data, 'completionNote')));
	},
	createReview: async ({ request, locals }) => {
		const data = await request.formData();
		const frameworkPublicId = text(data, 'frameworkPublicId');
		return runAction(locals, frameworkPublicId, (service, actor) => service.createReview(actor, {
			frameworkPublicId,
	...
	addReviewKpi: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.addReviewKpi(
			actor,
	...
	approveReview: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.approveReview(actor, text(data, 'reviewPublicId')));
	},
	createScenario: async ({ request, locals }) => {
		const data = await request.formData();
		const frameworkPublicId = text(data, 'frameworkPublicId');
		return runAction(locals, frameworkPublicId, (service, actor) => service.createScenario(actor, {
			frameworkPublicId,
			scenarioCode: text(data, 'scenarioCode'),
			title: text(data, 'title'),
			scenarioType: text(data, 'scenarioType') as 'baseline' | 'upside' | 'downside' | 'stress' | 'custom',
	...
	addScenarioAssumption: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.addScenarioAssumption(actor, text(data, 'scenarioPublicId'), {
	...
	addScenarioProjection: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.addScenarioProjection(actor, text(data, 'scenarioPublicId'), text(data, 'kpiPublicId'), text(data, 'projectionDate'), text(data, 'projectedValue'), text(data, 'rationale')));
	},
	approveScenario: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.approveScenario(actor, text(data, 'scenarioPublicId')));
	},
	reviseScenario: async ({ request, locals }) => {
		const data = await request.formData();
		return runAction(locals, text(data, 'frameworkPublicId'), (service, actor) => service.reviseScenario(actor, text(data, 'scenarioPublicId')));
	}
};

...
<nav class="breadcrumbs" aria-label="Breadcrumb">
	<a href="/more">More</a><span>/</span><a href="/strategy">Strategy</a><span>/</span><span
		>Performance & foresight</span
	>
</nav>
...
<section class="page-heading">
	<div>
		...
		<p>
			Govern KPI definitions, actuals, variance, corrective action, strategic review and scenarios
			as one evidence chain. Canonical source identifiers remain visible so management reporting can
			drill back to operational truth.
		</p>
	</div>
...
<section class="principle-strip">
	<div><strong>Objective</strong><span>Approved strategic intent</span></div>
	<div class="arrow">→</div>
...
{#if data.frameworks.length}
	<section class="panel version-strip">
		<div>
			<p class="eyebrow">Approved strategy</p>
		...
		<div class="version-links">
			{#each data.frameworks as framework}
				<a
					...
					href={`/strategy/performance?framework=${framework.public_id}`}
				>
					<span>{framework.framework_code} · v{framework.version_number}</span><small
						>{framework.title}</small
					>
				</a>
			{/each}
		</div>
...
{:else}
	<section class="summary-grid">
		<div class="metric">
			<span>KPIs</span><strong
				>{data.kpis.filter((kpi) => kpi.lifecycle_status === 'approved').length}</strong
			><small>approved definitions</small>
		</div>
		<div class="metric">
			<span>Observations</span><strong>{data.observations.length}</strong><small
				>append-only actuals</small
			>
		</div>
		<div class="metric">
			<span>Actions</span><strong
				>{data.actions.filter((action) => ['open', 'in_progress'].includes(action.lifecycle_status))
					.length}</strong
			><small>open responses</small>
		</div>
		<div class="metric">
			<span>Reviews</span><strong
				>{data.reviews.filter((review) => review.lifecycle_status === 'approved').length}</strong
			><small>approved evidence</small>
		</div>
		<div class="metric">
			<span>Scenarios</span><strong
				>{data.scenarios.filter((scenario) => scenario.lifecycle_status === 'approved')
					.length}</strong
			><small>approved versions</small>
		</div>
	</section>
	...
	<section class="panel">
		<div class="section-heading">
			<div>
				<p class="eyebrow">F01.06 · Goal & KPI management</p>
				<h2>KPI register</h2>
			</div>
		</div>
		{#if data.kpis.length}
				...
				{#each data.kpis as kpi}
					<article class="record-card">
						<div class="record-head">
							<div>
								<small>{kpi.kpi_code} · v{kpi.version_number}</small>
								<h3>{kpi.title}</h3>
							</div>
							<span class="pill">{label(kpi.lifecycle_status)}</span>
						</div>
						<p>{kpi.description}</p>
						<div class="facts">
							<span
							...
							<span
								><b>Source</b>{kpi.source_mode === 'canonical'
									? `${kpi.source_domain} · ${kpi.source_record_type} · ${kpi.source_measure_key}`
									: 'manual evidence'}</span
							>
						</div>
						...
						{#if data.canApprove && kpi.lifecycle_status === 'draft'}
							<form method="POST" action="?/approveKpi" class="inline-form">
								<input
									type="hidden"
									name="frameworkPublicId"
									value={data.selectedFramework.public_id}
								/><input type="hidden" name="kpiPublicId" value={kpi.public_id} /><button
									type="submit">Approve KPI version {kpi.version_number}</button
								>
							</form>
						{/if}
						{#if data.canManage && kpi.lifecycle_status === 'approved'}
							<form method="POST" action="?/reviseKpi" class="inline-form">
								<input
									type="hidden"
									name="frameworkPublicId"
									value={data.selectedFramework.public_id}
								/><input type="hidden" name="kpiPublicId" value={kpi.public_id} /><button
									class="secondary"
									type="submit">Create KPI revision</button
								>
							</form>
						{/if}
					</article>
	...
	{#if data.canManage}
		<section class="panel split">
			<div>
				...
				<form method="POST" action="?/createKpi" class="form-grid">
					...
					<label
						>Strategic objective<select name="objectivePublicId" required
							>{#each data.objectives.filter((objective) => objective.lifecycle_status === 'active') as objective}<option
									value={objective.public_id}>{objective.objective_code} · {objective.title}</option
								>{/each}</select
						></label
					>
					...
					<label
						>Accountable owner<select name="ownerMemberId" required
							><option value="">Select owner</option>{#each data.members as member}<option
					...
					<label
						>Direction<select name="direction"
							...
							><option value="target_is_best">Target is best</option><option value="band"
								>Target band</option
							></select
						></label
					>
					<label
						>Aggregation<select name="aggregationMethod"
							><option value="latest">Latest</option><option value="sum">Sum</option><option
								value="average">Average</option
							><option value="minimum">Minimum</option><option value="maximum">Maximum</option
							><option value="ratio">Ratio</option></select
						></label
					>
					...
					<label
						>Critical threshold<input type="number" step="any" name="criticalThreshold" /></label
					>
					...
					<label
						>Source record type<input
							...
							placeholder="accounting_report"
						/></label
					>
					<label
						>Source measure key<input
							name="sourceMeasureKey"
							placeholder="gross_margin_percent"
						/></label
					>
					<label class="wide"
						>KPI description<textarea name="description" rows="3" required></textarea></label
					>
		...
		<section class="panel">
				...
				<div class="card-grid">
					{#each data.actions as item}
						<article class="record-card">
							<div class="record-head">
								<div>
									<small>{kpiFor(item.strategy_kpi_id)?.kpi_code ?? '—'}</small>
									<h3>{item.action_code} · {item.title}</h3>
								</div>
								<span class="pill">{label(item.lifecycle_status)}</span>
							</div>
							<p>{item.action_text}</p>
							<small
								>{memberFor(item.owner_member_id)?.display_name ?? '—'} · due {dateValue(
									item.due_date
								)}</small
							>
							{#if item.lifecycle_status === 'open' || item.lifecycle_status === 'in_progress'}
								<form method="POST" action="?/completeAction" class="inline-form compact">
									<input
										type="hidden"
										name="frameworkPublicId"
										value={data.selectedFramework.public_id}
									/>
									<input type="hidden" name="actionPublicId" value={item.public_id} />
									<input
										name="completionNote"
			...
			<form method="POST" action="?/createAction" class="form-grid compact-top">
				...
				>
				<label
					>Observation<select name="observationPublicId"
						><option value="">No specific observation</option
				...
				>
				<label
					>Action owner<select name="ownerMemberId" required
				...
				<label class="wide">Action<textarea name="actionText" rows="3" required></textarea></label>
				<div class="form-actions"><button type="submit">Create corrective action</button></div>
			</form>
		</section>
	{/if}

	<section class="panel">
		<p class="eyebrow">F01.07 · Strategic review</p>
		<h2>Management review evidence</h2>
		<div class="card-grid">
			{#each data.reviews as review}
				<article class="record-card">
					<div class="record-head">
						<div>
							<small>{review.review_code} · {dateValue(review.review_date)}</small>
							<h3>{review.title}</h3>
						</div>
						<span class="pill">{label(review.lifecycle_status)}</span>
					</div>
					<p>{review.summary}</p>
					...
					{#if data.canApprove && review.lifecycle_status === 'draft'}<form
						...
						>
							<input
								...
								name="frameworkPublicId"
								value={data.selectedFramework.public_id}
							/><input type="hidden" name="reviewPublicId" value={review.public_id} /><button
								type="submit">Approve strategic review</button
							>
						</form>{/if}
				</article>
			{/each}
		...
		{#if data.canManage}
			<div class="split compact-top">
				<form method="POST" action="?/createReview" class="form-grid">
					<input type="hidden" name="frameworkPublicId" value={data.selectedFramework.public_id} />
					<h3 class="wide">New strategic review</h3>
					<label>Review code<input name="reviewCode" required /></label><label
						>Review date<input type="date" name="reviewDate" required /></label
					><label class="wide">Review title<input name="title" required /></label><label
						class="wide">Review summary<textarea name="summary" rows="3" required></textarea></label
					><label class="wide">Decisions<textarea name="decisionsText" rows="3"></textarea></label>
					<div class="form-actions"><button type="submit">Create review draft</button></div>
				</form>
				<form method="POST" action="?/addReviewKpi" class="form-grid">
					<input type="hidden" name="frameworkPublicId" value={data.selectedFramework.public_id} />
					<h3 class="wide">Snapshot KPI into review</h3>
					<label
						>Draft review<select name="reviewPublicId" required
							>{#each data.reviews.filter((review) => review.lifecycle_status === 'draft') as review}<option
									value={review.public_id}>{review.review_code} · {review.title}</option
								>{/each}</select
						></label
					><label
						>Approved KPI<select name="kpiPublicId" required
						...
						></label
					><label
						>KPI observation<select name="observationPublicId" required
							>{#each data.observations as observation}<option value={observation.public_id}
									>{kpiFor(observation.strategy_kpi_id)?.kpi_code} · {dateValue(
										observation.observed_on
									)} · {numberValue(observation.actual_value)}</option
					...
					><label
						>Assessment<select name="assessment"
							><option value="on_track">On track</option><option value="watch">Watch</option><option
								value="off_track">Off track</option
							><option value="not_measured">Not measured</option></select
						></label
					><label class="wide"
						>Review commentary<textarea name="commentary" rows="3"></textarea></label
					>
					<div class="form-actions"><button type="submit">Add KPI snapshot</button></div>
				</form>
			</div>
		{/if}
	</section>
	...
	<section class="panel">
		<p class="eyebrow">F01.08 · Scenario & foresight planning</p>
			...
			{#each data.scenarios as scenario}
				<article class="record-card">
					<div class="record-head">
						<div>
							<small
								>{scenario.scenario_code} · v{scenario.version_number} · {label(
									scenario.scenario_type
								)}</small
							>
							<h3>{scenario.title}</h3>
						</div>
						<span class="pill">{label(scenario.lifecycle_status)}</span>
					</div>
					...
					{#if data.canApprove && scenario.lifecycle_status === 'draft'}<form
						...
						>
							<input
								...
								name="frameworkPublicId"
								value={data.selectedFramework.public_id}
							/><input type="hidden" name="scenarioPublicId" value={scenario.public_id} /><button
								type="submit">Approve scenario version {scenario.version_number}</button
							>
						</form>{/if}
					{#if data.canManage && scenario.lifecycle_status === 'approved'}<form
							method="POST"
							action="?/reviseScenario"
							class="inline-form"
						>
							<input
								type="hidden"
							...
							/><input type="hidden" name="scenarioPublicId" value={scenario.public_id} /><button
								class="secondary"
								type="submit">Create scenario revision</button
							>
						</form>{/if}
				</article>
			{/each}
		...
		{#if data.canManage}
			<div class="split compact-top">
				<form method="POST" action="?/createScenario" class="form-grid">
					<input type="hidden" name="frameworkPublicId" value={data.selectedFramework.public_id} />
					<h3 class="wide">New scenario</h3>
					<label>Scenario code<input name="scenarioCode" required /></label><label
						>Scenario type<select name="scenarioType"
							><option value="baseline">Baseline</option><option value="upside">Upside</option
							><option value="downside">Downside</option><option value="stress">Stress</option
							><option value="custom">Custom</option></select
						></label
					><label>Scenario title<input name="title" required /></label><label
						>Horizon start<input type="date" name="horizonStart" required /></label
					><label>Horizon end<input type="date" name="horizonEnd" required /></label><label
						class="wide"
						>Scenario narrative<textarea name="narrative" rows="3" required></textarea></label
					>
					<div class="form-actions"><button type="submit">Create scenario draft</button></div>
				</form>
				<form method="POST" action="?/addScenarioAssumption" class="form-grid">
					<input type="hidden" name="frameworkPublicId" value={data.selectedFramework.public_id} />
					<h3 class="wide">Scenario assumption</h3>
					<label
						>Draft scenario<select name="scenarioPublicId" required
							>{#each data.scenarios.filter((scenario) => scenario.lifecycle_status === 'draft') as scenario}<option
									value={scenario.public_id}
									>{scenario.scenario_code} · v{scenario.version_number}</option
								>{/each}</select
						></label
					><label>Assumption code<input name="assumptionCode" required /></label><label
						>Assumption title<input name="title" required /></label
					><label>Variable key<input name="variableKey" required /></label><label
						>Unit<input name="unitLabel" required /></label
					><label
						>Baseline value<input type="number" step="any" name="baselineValue" required /></label
					><label
						>Scenario value<input type="number" step="any" name="scenarioValue" required /></label
					><label
						>Sensitivity percent<input
							type="number"
							step="any"
							min="0"
						...
						/></label
					><label class="wide"
						>Assumption description<textarea name="description" rows="3" required></textarea></label
					>
					<div class="form-actions"><button type="submit">Add scenario assumption</button></div>
				</form>
			</div>
			<form method="POST" action="?/addScenarioProjection" class="form-grid compact-top">
				<input type="hidden" name="frameworkPublicId" value={data.selectedFramework.public_id} />
				<h3 class="wide">KPI scenario projection</h3>
				<label
					>Draft scenario<select name="scenarioPublicId" required
						>{#each data.scenarios.filter((scenario) => scenario.lifecycle_status === 'draft') as scenario}<option
								value={scenario.public_id}
				...
				><label
					>Approved KPI<select name="kpiPublicId" required
						>{#each data.kpis.filter((kpi) => kpi.lifecycle_status === 'approved') as kpi}<option
							...
							>{/each}</select
					></label
				><label>Projection date<input type="date" name="projectionDate" required /></label><label
					>Projected value<input type="number" step="any" name="projectedValue" required /></label
				><label class="wide"
					>Projection rationale<textarea name="rationale" rows="3" required></textarea></label
				>
				<div class="form-actions"><button type="submit">Add KPI projection</button></div>
			</form>
		{/if}
	</section>
{/if}
...
<style>
	...
	.breadcrumbs a {
		color: var(--color-text-muted, #5d6673);
	}
	.page-heading {
		display: flex;
	...
	.snapshot {
		...
		margin-top: 0.7rem;
		border-radius: 0.5rem;
		background: var(--color-surface-subtle, #f6f7f8);
	}
	.form-grid {
		display: grid;
		grid-template-columns: repeat(2, minmax(0, 1fr));
	...
	.empty-state {
		text-align: center;
		padding: 3rem;
		border: 1px dashed var(--color-border, #dfe3e8);
		border-radius: 0.75rem;
	}
	.button-link,
	button {
		...
		background: var(--color-primary, #1f5eff);
		color: #fff;
		text-decoration: none;
		cursor: pointer;
	}

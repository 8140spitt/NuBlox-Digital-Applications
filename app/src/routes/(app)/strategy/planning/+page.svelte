	...
	<section class="panel version-strip">
		...
		<div class="version-links">
			{#each data.plans as plan}
				<a
					class:active={data.selectedPlan?.public_id === plan.public_id}
					href={`/strategy/planning?plan=${plan.public_id}`}
				>
	...
	{#if data.canManage && data.approvedFrameworks.length}
		<section class="panel">
			...
			<form method="POST" action="?/createPlan" class="form-grid">
				<label>
					...
					<select name="frameworkPublicId" required>
						{#each data.approvedFrameworks as framework}
							<option value={framework.public_id}
								>{framework.framework_code} · v{framework.version_number} — {framework.title}</option
							>
						{/each}
					</select>
				</label>
				...
				<label
					>Title <input name="title" placeholder="2027 Enterprise Business Plan" required /></label
				>
				<label>
					Accountable owner
				...
				>
				<label
					>Planned operating expenditure <input
						type="number"
						min="0"
						step="0.01"
				...
				>
				<label
					>Planned capital expenditure <input
						type="number"
						min="0"
						step="0.01"
				...
				>
				<label class="wide"
					>Plan narrative <textarea name="narrative" rows="4" required></textarea></label
				>
...
{:else}
	...
	<section class="plan-summary">
		<div>
			<span>Business plan</span><strong>{plan.plan_code} · version {plan.version_number}</strong>
		</div>
		<div>
			<span>Status</span><strong class:approved={plan.lifecycle_status === 'approved'}
				>{label(plan.lifecycle_status)}</strong
			>
		</div>
		<div>
			<span>Period</span><strong
				>{dateValue(plan.period_start)} → {dateValue(plan.period_end)}</strong
			>
		</div>
		<div>
			<span>Strategy source</span><strong
				>{data.selectedFramework?.framework_code ?? '—'} · v{data.selectedFramework
					?.version_number ?? '—'}</strong
			>
		</div>
	</section>
	...
	<section class="financial-grid">
		<article>
			<span>Revenue plan</span><strong
				>{money(plan.planned_revenue_amount, plan.currency_code)}</strong
			>
		</article>
		<article>
			<span>Operating expenditure</span><strong
				>{money(plan.planned_opex_amount, plan.currency_code)}</strong
			>
		</article>
		<article>
			<span>Capital expenditure</span><strong
				>{money(plan.planned_capex_amount, plan.currency_code)}</strong
			>
		</article>
		<article><span>Strategic initiatives</span><strong>{data.initiatives.length}</strong></article>
		<article>
			<span>Target-model components</span><strong>{data.operatingModelComponents.length}</strong>
		</article>
		<article>
			<span>Execution links</span><strong
				>{data.initiatives.filter((initiative) => initiative.project_id).length}</strong
			>
		</article>
	</section>
	...
	<section class="panel trace-panel">
		...
		<div class="trace-line">
			<span>Strategy v{data.selectedFramework?.version_number ?? '—'}</span><b>→</b><span
				>{data.objectives.length} objectives</span
			><b>→</b><span>{data.initiatives.length} initiatives</span><b>→</b><span
				>{data.operatingModelComponents.length} target components</span
			><b>→</b><span>canonical execution</span>
		</div>
	</section>
	...
	{#if plan.lifecycle_status === 'approved'}
		<section class="notice success">
			<div>
				...
				<span
					>This version, its initiative commitments and target operating model are immutable.
					Changes proceed through a controlled revision.</span
				>
			</div>
			{#if data.canManage}
					...
					<article class="initiative-card">
						<div class="record-meta">
							<span>{initiative.initiative_code}</span><span
								>{label(initiative.lifecycle_status)}</span
							><span>priority {initiative.priority_rank}</span>
						</div>
						<h3>{initiative.title}</h3>
						<p>{initiative.outcome_text}</p>
						<div class="source-thread">
							<strong>Objective</strong><span
								>{objective?.objective_code ?? '—'} · {objective?.title ??
									'Unknown objective'}</span
							>
						</div>
						<div class="metric-row">
							<span
								><b>{money(initiative.planned_investment_amount, initiative.currency_code)}</b> planned
								investment</span
							>
						...
						{#if project}
							<div class="execution-link">
								<strong>Execution</strong><span>{project.projectNumber} · {project.name}</span
								>{#if budget}<span>{budget.budgetNumber} · approved v{budget.approvedVersion}</span
									>{/if}
							</div>
						{/if}
						{#if initiative.benefit_statement}<small>Benefit: {initiative.benefit_statement}</small
							>{/if}
						...
						<div class="milestones">
							{#each data.milestones.filter((milestone) => milestone.strategy_initiative_id === initiative.id) as milestone}
								<span
									>{milestone.milestone_code} · {milestone.title} · {dateValue(
			...
			{#if plan.lifecycle_status === 'draft' && data.canManage}
				<details class="add-record">
					...
					<form method="POST" action="?/addInitiative" class="stack-form">
							...
							<label
								>Strategic objective <select name="objectivePublicId" required
									>{#each data.objectives.filter((objective) => objective.lifecycle_status === 'active') as objective}<option
											value={objective.public_id}
											>{objective.objective_code} · {objective.title}</option
										>{/each}</select
								></label
							>
							<label
								>Initiative code <input
									name="initiativeCode"
									placeholder="INIT-01"
									required
								/></label
						...
						<div class="form-grid compact">
							<label
								>Accountable owner <select name="ownerMemberId"
							...
							>
							<label
								>Executive sponsor <select name="sponsorMemberId"
									><option value="">Unassigned</option>{#each data.members as member}<option
											value={member.id}>{member.display_name}</option
										>{/each}</select
								></label
						...
						<label
							>Resource assumptions <textarea name="resourceAssumptions" rows="2"></textarea></label
						>
						...
						<div class="execution-selector">
							...
							<div class="form-grid compact">
								<label
									>Execution project <select name="projectPublicId"
										><option value="">No project link</option
										>{#each data.executionProjects as project}<option value={project.publicId}
												>{project.projectNumber} · {project.name}</option
											>{/each}</select
									></label
								>
								<label
							...
							<small
								>NuBlox validates that any budget belongs to the selected project and has an
								approved canonical version.</small
							>
						</div>
				...
				{#if data.initiatives.length}
					<details class="add-record">
						...
						<form method="POST" action="?/addMilestone" class="form-grid compact">
							<input type="hidden" name="planPublicId" value={plan.public_id} />
							<label
								>Initiative <select name="initiativePublicId" required
									>{#each data.initiatives as initiative}<option value={initiative.public_id}
											>{initiative.initiative_code} · {initiative.title}</option
										>{/each}</select
								></label
							>
							<label
								>Milestone code <input name="milestoneCode" placeholder="MS-01" required /></label
							>
					...
					<details class="add-record">
						...
						<form method="POST" action="?/addDependency" class="form-grid compact">
							...
							<label
								>Initiative <select name="initiativePublicId" required
									>{#each data.initiatives as initiative}<option value={initiative.public_id}
											>{initiative.initiative_code}</option
										>{/each}</select
								></label
							>
							<label
								>Depends on <select name="dependsOnInitiativePublicId" required
									>{#each data.initiatives as initiative}<option value={initiative.public_id}
											>{initiative.initiative_code}</option
										>{/each}</select
								></label
							>
							<label
								>Dependency type <select name="dependencyType"
									><option value="finish_to_start">Finish to start</option><option
										value="governance">Governance</option
									><option value="resource">Resource</option><option value="external"
										>External</option
									></select
								></label
							>
			...
			<div class="record-list">
				{#each data.operatingModelComponents as component}
					<article class="component-card">
						<div class="record-meta">
							<span>{component.component_code}</span><span>{label(component.component_type)}</span
							><span>{label(component.lifecycle_status)}</span>
						</div>
						<h3>{component.title}</h3>
						{#if component.current_state_text}<small>Current: {component.current_state_text}</small
							>{/if}
						...
						<div class="accountability-list">
							{#each data.accountabilities.filter((item) => item.operating_model_component_id === component.id) as accountability}
								<div>
									<span>{label(accountability.accountability_type)}</span><strong
										>{accountability.position_label}</strong
									>{#if accountability.member_id}<small
											>{memberFor(accountability.member_id)?.display_name ?? 'Named member'}</small
										>{/if}
								</div>
							{/each}
						</div>
						<div class="change-links">
							{#each data.initiativeComponentLinks.filter((link) => link.operating_model_component_id === component.id) as link}
								<span
									>{initiativeFor(link.initiative_id)?.initiative_code ?? 'Initiative'} → {label(
										link.change_role
									)}</span
								>
							{/each}
				...
				{/each}
				{#if !data.operatingModelComponents.length}<p class="muted">
						No target operating-model components defined yet.
					</p>{/if}
			</div>
			...
			{#if plan.lifecycle_status === 'draft' && data.canManage}
				<details class="add-record">
					...
					<form method="POST" action="?/addComponent" class="stack-form">
						...
						<div class="form-grid compact">
							<label
								>Component code <input name="componentCode" placeholder="CAP-01" required /></label
							>
							<label
								>Component type <select name="componentType"
									><option value="business_capability">Business capability</option><option
										value="value_stream">Value stream</option
									><option value="organisation_design">Organisation design</option><option
										value="process">Process</option
									><option value="governance">Governance</option><option value="information"
										>Information</option
									><option value="technology">Technology</option><option value="partner_ecosystem"
										>Partner ecosystem</option
									><option value="location">Location</option></select
								></label
							>
							<label
								>Parent component <select name="parentComponentPublicId"
									><option value="">No parent</option
									>{#each data.operatingModelComponents as component}<option
											value={component.public_id}
											>{component.component_code} · {component.title}</option
										>{/each}</select
								></label
							>
						</div>
						...
						<label
							>Target state <textarea name="targetStateText" rows="3" required></textarea></label
						>
				...
				{#if data.operatingModelComponents.length}
					<details class="add-record">
						...
						<form method="POST" action="?/addAccountability" class="stack-form">
							...
							<div class="form-grid compact">
								<label
									>Component <select name="componentPublicId" required
										>{#each data.operatingModelComponents as component}<option
												value={component.public_id}
												>{component.component_code} · {component.title}</option
											>{/each}</select
								...
								>
								<label
									>Accountability <select name="accountabilityType"
										><option value="accountable">Accountable</option><option value="responsible"
											>Responsible</option
										><option value="consulted">Consulted</option><option value="informed"
											>Informed</option
										><option value="assured">Assured</option></select
									></label
								>
								<label
									>Business position label <input
										name="positionLabel"
								...
								>
								<label
									>Named member (optional) <select name="memberId"
										><option value="">Position only</option>{#each data.members as member}<option
												value={member.id}>{member.display_name}</option
											>{/each}</select
									></label
								>
							...
							<small
								>Business accountability is deliberately separate from NuBlox access roles and
								permissions.</small
							>
				...
				{#if data.operatingModelComponents.length && data.initiatives.length}
					<details class="add-record">
						...
						<form method="POST" action="?/linkInitiativeComponent" class="form-grid compact">
							...
							>
							<label
								>Target component <select name="componentPublicId" required
									>{#each data.operatingModelComponents as component}<option
											value={component.public_id}
											>{component.component_code} · {component.title}</option
										>{/each}</select
							...
							>
							<label
								>Change role <select name="changeRole"
									><option value="transform">Transform</option><option value="create">Create</option
									><option value="enable">Enable</option><option value="consume">Consume</option
		...
		<section class="panel">
			...
			<div class="dependency-list">
				{#each data.dependencies as dependency}
					<span
						>{initiativeFor(dependency.initiative_id)?.initiative_code ?? '—'} depends on {initiativeFor(
							dependency.depends_on_initiative_id
						)?.initiative_code ?? '—'} · {label(dependency.dependency_type)}</span
					>
				{/each}
			</div>
		</section>
	...
	{#if plan.lifecycle_status === 'draft' && data.canApprove}
		<section class="approval-panel">
			<div>
				...
				<p>
					Approval requires initiatives, accountable target-model components, an initiative-to-model
					change link for every initiative, an acyclic dependency network and valid canonical

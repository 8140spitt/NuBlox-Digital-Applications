...
<section class="page-header">
	<div>
		...
		<h1>Time</h1>
		<p>
			Record time against staffed projects or assigned work, then submit it through a controlled
			approval lifecycle.
		</p>
	</div>
	{#if data.currentWorker}
		<div class="identity-card">
			<span>Recording as</span><strong>{data.currentWorker.displayName}</strong>
		</div>
	{/if}
</section>
...
{#if !data.canView}
	<section class="notice">
		<h2>Timesheet access is not enabled</h2>
		<p>Your current role does not grant timesheet visibility.</p>
	</section>
{:else}
	<section class="time-layout">
		<div class="main-column">
			<section class="panel" aria-labelledby="my-timesheets-heading">
				<div class="section-heading">
					<div>
						<p class="eyebrow">Personal time</p>
						<h2 id="my-timesheets-heading">My timesheets</h2>
					</div>
				...
				{#if !data.currentWorker}
					<div class="empty-state">
						<h3>No workforce identity linked</h3>
						<p>
							An authorised workforce manager must link your organisation membership to a worker
							record before you can record time.
						</p>
					</div>
				{:else if data.ownTimesheets.length === 0}
					<div class="empty-state">
						<h3>No timesheets yet</h3>
						<p>Create a period, add time entries and submit when complete.</p>
					</div>
				{:else}
						...
						{#each data.ownTimesheets as timesheet}
							<article class="timesheet-card">
								<div class="timesheet-heading">
									<div>
										<h3>{dateText(timesheet.periodStart)} → {dateText(timesheet.periodEnd)}</h3>
										<p>
											{timesheet.entries.length} time {timesheet.entries.length === 1
												? 'entry'
												: 'entries'}
										</p>
									</div>
								...
								{#if timesheet.entries.length > 0}
									<div class="entry-list">
										{#each timesheet.entries as entry}
											<div class="entry-row">
												<div>
													<strong>{dateText(entry.workDate)}</strong><span
														>{entry.projectName ?? 'General / non-project time'}</span
													>
												</div>
								...
								{#if data.canManageOwn && canEdit(timesheet.status)}
									<details class="entry-form-wrap">
										...
										<form method="POST" action="?/addEntry" class="entry-form">
											...
											<label
												>Project / job<select name="projectPublicId"
													><option value="">General / non-project</option
													>{#each data.projectAssignments as assignment}<option
															value={assignment.projectPublicId}
															>{assignment.projectNumber} · {assignment.projectName}</option
														>{/each}</select
												></label
											>
											<label
												>Assigned work<select name="scheduleEventPublicId"
													><option value="">No schedule event</option
													>{#each data.assignedScheduleEvents as event}<option
															value={event.publicId}
															>{event.title} · {dateText(event.startsAt)}</option
														>{/each}</select
												></label
											>
											<label>Description<input name="description" maxlength="1000" /></label>
											<label class="check"
												><input type="checkbox" name="isBillable" checked /> Billable time</label
											>
			...
			{#if data.canApprove}
				<section class="panel" aria-labelledby="approval-heading">
					<div class="section-heading">
						...
						<span class="count">{data.approvalQueue.length}</span>
					</div>
							...
							{#each data.approvalQueue as timesheet}
								<article class="approval-card">
									<div class="approval-heading">
										<div>
											<strong>{timesheet.workerName}</strong><span
												>{dateText(timesheet.periodStart)} → {dateText(timesheet.periodEnd)}</span
											>
										</div>
										<span class="status status-submitted">Submitted</span>
									</div>
									<div class="approval-entries">
										{#each timesheet.entries as entry}<p>
												<strong>{dateText(entry.workDate)}</strong> · {hoursText(
													entry.workedMinutes
												)} · {entry.projectName ?? 'General'} · {entry.description ??
													'No description'}
											</p>{/each}
									</div>
		...
		<aside class="side-column">
			...
			<section class="panel scope-panel">
				<p class="eyebrow">Recording scope</p>
				<h2>Available work</h2>
				<dl>
					<div>
						<dt>Staffed projects</dt>
						<dd>{data.projectAssignments.length}</dd>
					</div>
					<div>
						<dt>Assigned schedule events</dt>
						<dd>{data.assignedScheduleEvents.length}</dd>
					</div>
				</dl>
				<p class="muted">
					Project time is accepted only while your workforce record has an active project resource
					assignment. Schedule-linked time must reference work assigned to you.
				</p>
			</section>
		</aside>
	</section>
...
<style>
	...
	.entry-row {
		display: grid;
		grid-template-columns: minmax(10rem, 1fr) auto minmax(10rem, 1.2fr);
		gap: 0.7rem;
		align-items: center;

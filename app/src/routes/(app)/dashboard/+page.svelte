...
<div class="grid">
	<section class="panel">
		...
		<dl>
			<div><dt>Organisation</dt><dd>{data.organisation.name}</dd></div>
			<div><dt>Signed in as</dt><dd>{data.actor.displayName}</dd></div>
			<div><dt>Account</dt><dd>{data.actor.email}</dd></div>
		</dl>
	</section>
	...
	<section class="panel">
		...
		{#if data.canInviteMembers}
			...
			<form class="stack" onsubmit={inviteMember}>
				<label>
					<span>Email</span>
					<input bind:value={email} type="email" required maxlength="320" placeholder="name@example.com" />
				</label>
		...
		{:else}
			<p class="muted">You do not have the <code>member.invite</code> permission in this organisation.</p>
		{/if}
	</section>
</div>
...
<style>
	...
	.eyebrow { margin: 0 0 0.35rem; text-transform: uppercase; letter-spacing: 0.1em; font-size: 0.75rem; font-weight: 750; color: #61615b; }
	...
	.grid { display: grid; grid-template-columns: minmax(0, 1fr) minmax(20rem, 1fr); gap: 1rem; align-items: start; }
	.panel { background: white; border: 1px solid #d9d9d2; border-radius: 0.8rem; padding: 1.25rem; }
	.panel h2 { margin-top: 0; }
	dl { margin: 0; display: grid; gap: 0.8rem; }
	dl div { display: grid; grid-template-columns: 8rem 1fr; gap: 1rem; }
	dt { color: #6a6a64; }
	dd { margin: 0; font-weight: 650; overflow-wrap: anywhere; }
	.stack { display: grid; gap: 1rem; }
	label:not(.check) { display: grid; gap: 0.4rem; font-weight: 650; }
	input[type='email'] { font: inherit; border: 1px solid #b9b9b1; border-radius: 0.5rem; padding: 0.72rem; }
	fieldset { border: 0; padding: 0; margin: 0; display: grid; gap: 0.55rem; }
	legend { font-weight: 700; margin-bottom: 0.55rem; }
	.check { display: flex; align-items: flex-start; gap: 0.65rem; padding: 0.65rem; border: 1px solid #deded8; border-radius: 0.5rem; }
	.check span { display: grid; gap: 0.2rem; }
	.check small { color: #666; }
	button { justify-self: start; font: inherit; font-weight: 700; border: 1px solid #111; border-radius: 0.5rem; padding: 0.7rem 1rem; background: #111; color: white; cursor: pointer; }
	button:disabled { opacity: 0.55; cursor: wait; }
	.error { color: #9b1c1c; margin: 0; }
	.success { color: #1f6a32; margin: 0; }

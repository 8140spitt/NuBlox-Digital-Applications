...
function actionError(error: unknown) {
	if (error instanceof ProductServiceValidationError) return fail(400, { error: error.message });
	if (error instanceof TenantAccessError) {
		return fail(403, { error: 'You do not have access to this product/service lifecycle action.' });
	}
...
async function runLifecycleAction(
	...
	try {
		...
		throw redirect(303, '/product-service/lifecycle');
	} catch (error) {
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try {
		...
		const [core, lifecycle, members] = await Promise.all([

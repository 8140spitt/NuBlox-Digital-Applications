...
<section class="page-header">
	<div>
		...
		<p>
			Review issued enquiries, price every requested line and submit the commercial return directly
			to the buyer.
		</p>
	</div>
...
{#if data.quotes.length === 0}
	<section class="empty">
		...
		<p>
			If you were sent an RFQ invitation, open the original invitation link and make sure you sign
			...
			<article class="quote-card">
				...
				{#if quote.status === 'submitted'}
					...
					<div class="line-table" role="table" aria-label={`${quote.rfqNumber} submitted prices`}>
						<div class="line-row head" role="row">
							<span>Line</span><span>Description</span><span>Qty</span><span>Unit rate</span><span
								>Lead</span
							>
						</div>
						{#each quote.lines as line}
							<div class="line-row" role="row">
								<span>{line.lineNumber}</span><span>{line.description}</span><span
									>{line.offeredQuantity ?? '—'}</span
								><span>{money(line.unitRate, quote.currencyCode)}</span><span
									>{line.leadTimeDays === null ? '—' : `${line.leadTimeDays} days`}</span
								>
							</div>
						{/each}
					</div>
				{:else}
					<form method="POST" action="?/submit" class="quote-form">
						...
						<div class="pricing-lines">
							{#each quote.lines as line}
								<fieldset>
									...
									<p class="request">
										Requested {line.requestedQuantity} · Required {date(line.requiredByDate)}
									</p>
									<div class="line-inputs">
										<label
											>Offered quantity<input
												name={`offeredQuantity:${line.itemId}`}
												inputmode="decimal"
										...
										>
										<label
											>Unit rate ({quote.currencyCode})<input
												name={`unitRate:${line.itemId}`}
												inputmode="decimal"
												required
											/></label
										>
									...
									<label
										>Qualification / note<textarea
											name={`qualificationNote:${line.itemId}`}
											rows="2"
						...
						<div class="submit-row">
							<p>
								Submitting locks this response as the supplier's returned quotation for this
								invitation.
							</p>
...
<style>
	...
	.line-row {
		display: grid;
		grid-template-columns: 4rem minmax(12rem, 1fr) 7rem 9rem 8rem;
		gap: 0.6rem;

...
<div class="workspace">
	...
	<section class="context-grid" aria-label="Quotation context">
		...
		<div>
			<span>Project</span><strong>{data.quote.projectNumber} · {data.quote.projectName}</strong>
		</div>
			...
			{#each data.quote.lines as line}
				<article class="line-card">
					...
					<label class="note-field">
						...
						<textarea name={`note:${line.rfqItemId}`} rows="2" maxlength="4000" disabled={submitted}
							>{line.qualificationNote}</textarea
						>
					</label>
				</article>
			{/each}

...
import {
	...
	type ExternalProjectAssignmentOptions
...
function teamFailure(error: unknown, teamAction: string) {
	if (error instanceof RecordNotFoundError) {
		return fail(
			404,
			actionFailure({ teamError: 'Project or requested record was not found.', teamAction })
		);
	}
	...
	if (
		...
		error instanceof ExternalProjectActionValidationError
	) {
	...
	if (error instanceof ConcurrentUpdateError) {
		return fail(
			409,
			actionFailure({
				teamError: 'The project team changed concurrently. Reload and try again.',
				teamAction
			})
		);
	}
...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	try {
		...
		let externalActionOptions: ExternalProjectAssignmentOptions = {
			canAssignRfis: false,
			canAssignSubmittals: false,
			canAssignInstructions: false,
			rfis: [],
			submittals: [],
			instructions: []
		};
		if (team.canManageParticipants) {
			try {
				...
				if (externalCollaboration.canManage) {
					externalActionOptions = await new ExternalProjectActionService(db).listAssignmentOptions(
						actor,
						params.projectPublicId
					);
				}
				...
				if (
					...
					!(cause instanceof ExternalProjectActionValidationError)
				)
					throw cause;
			}
		}
		return { ...workspace, team, externalCollaboration, externalActionOptions };
	} catch (cause) {
...
export const actions: Actions = {
	transition: async ({ request, locals, params }) => {
		...
		if (!actor) {
			return fail(
				401,
				actionFailure({ transitionError: 'Authentication and organisation context are required.' })
			);
		}
		...
		if (!PROJECT_STATUSES.has(rawStatus as ProjectLifecycleStatus)) {
			return fail(
				400,
				actionFailure({ transitionError: 'The requested project status is invalid.' })
			);
		}
		...
		try {
			...
			if (cause instanceof TenantAccessError) {
				return fail(
					403,
					actionFailure({
						transitionError: 'You do not have permission to manage this project lifecycle.'
					})
				);
			}
			if (cause instanceof InvalidLifecycleTransitionError) {
				return fail(
					...
					actionFailure({
						transitionError:
							'That lifecycle transition is not allowed from the current project state.'
					})
				);
			}
			if (cause instanceof ConcurrentUpdateError) {
				return fail(
					409,
					actionFailure({
						transitionError: 'The project changed concurrently. Reload and try again.'
					})
				);
			}
			throw cause;
	...
	assignExternalRfi: async ({ request, locals, params }) => {
		const actor = actorFromLocals(locals);
		const data = await request.formData();
		const collaboratorPublicId = String(data.get('collaboratorPublicId') ?? '');
		const marker = `external-work-${collaboratorPublicId}`;
		if (!actor)
			return fail(
				401,
		...
		try {
			await new ExternalProjectActionService(getDatabase()).assignRfi(actor, {
				projectPublicId: params.projectPublicId,
				collaboratorPublicId,
				rfiPublicId: String(data.get('rfiPublicId') ?? '')
			});
		} catch (cause) {
	...
	assignExternalSubmittal: async ({ request, locals, params }) => {
		const actor = actorFromLocals(locals);
		const data = await request.formData();
		const collaboratorPublicId = String(data.get('collaboratorPublicId') ?? '');
		const marker = `external-work-${collaboratorPublicId}`;
		...
		try {
			await new ExternalProjectActionService(getDatabase()).assignSubmittal(actor, {
				projectPublicId: params.projectPublicId,
				collaboratorPublicId,
				submittalPublicId: String(data.get('submittalPublicId') ?? '')
			});
		} catch (cause) {
			return teamFailure(cause, marker);
		}
	...
	assignExternalInstruction: async ({ request, locals, params }) => {
		const actor = actorFromLocals(locals);
		const data = await request.formData();
		const collaboratorPublicId = String(data.get('collaboratorPublicId') ?? '');
		const marker = `external-work-${collaboratorPublicId}`;
		...
		try {
			await new ExternalProjectActionService(getDatabase()).assignInstruction(actor, {
				projectPublicId: params.projectPublicId,
				collaboratorPublicId,
				instructionPublicId: String(data.get('instructionPublicId') ?? '')
			});
		} catch (cause) {
			return teamFailure(cause, marker);
		}
	...
	addMember: async ({ request, locals, params }) => {
		...
		if (!actor)
			return fail(
				401,
				actionFailure({ teamError: 'Authentication is required.', teamAction: 'add-member' })
			);
		const data = await request.formData();
		try {
	...
	leaveProject: async ({ locals, params }) => {
		...
		if (!actor)
			return fail(
				401,
				actionFailure({ teamError: 'Authentication is required.', teamAction: 'leave-project' })
			);
		try {

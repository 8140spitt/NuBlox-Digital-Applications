	...
	{#if data.canManage}
		<section class="panel">
			...
			<form method="POST" action="?/createFramework" class="form-grid">
				...
				<label
					>Title <input name="title" placeholder="2027–2031 Enterprise Strategy" required /></label
				>
				...
				<label class="wide"
					>Mission <textarea name="missionText" rows="3" required></textarea></label
				>
				<label>
					Accountable owner
...
{:else}
	...
	<section class="strategy-summary">
		...
		<div>
			<span>Status</span>
			<strong class:approved={framework.lifecycle_status === 'approved'}
				>{label(framework.lifecycle_status)}</strong
			>
		</div>
		<div>
			<span>Horizon</span>
		...
		<div>
			<span>Evidence</span>
			<strong
				>{data.environmentFactors.length} factors · {data.options.length} options · {data.objectives
					.length} objectives</strong
			>
		</div>
	</section>
			...
			<form method="POST" action="?/updateFramework" class="form-grid">
				...
				<label>
					...
					<select name="ownerMemberId">
						<option value="">Unassigned</option>
						{#each data.members as member}
							<option value={member.id} selected={framework.owner_member_id === member.id}
				...
				<label
					>Horizon start <input
						type="date"
						name="horizonStart"
						value={dateValue(framework.horizon_start)}
						required
					/></label
				>
				<label
					>Horizon end <input
						type="date"
						name="horizonEnd"
						value={dateValue(framework.horizon_end)}
						required
					/></label
				>
				<label class="wide"
					>Purpose <textarea name="purposeText" rows="3" required>{framework.purpose_text}</textarea
					></label
				>
				<label class="wide"
					>Vision <textarea name="visionText" rows="3" required>{framework.vision_text}</textarea
					></label
				>
				<label class="wide"
					>Mission <textarea name="missionText" rows="3" required>{framework.mission_text}</textarea
					></label
				>
	...
	<div class="workspace-grid">
				...
				{#each data.environmentFactors as factor}
					<article>
						<div class="record-meta">
							<span>{factor.context_scope}</span><span>{factor.dimension}</span><span
						...
						{#if factor.likelihood_score || factor.impact_score}
							<small
								>Likelihood {factor.likelihood_score ?? '—'} / 5 · impact {factor.impact_score ??
									'—'} / 5</small
							>
						{/if}
					</article>
				{/each}
				{#if !data.environmentFactors.length}<p class="muted">
						No environmental evidence captured yet.
					</p>{/if}
			</div>
				...
				<details class="add-record">
					...
					<form method="POST" action="?/addEnvironmentFactor" class="stack-form">
						...
						<div class="form-grid compact">
							<label
								>Scope <select name="contextScope"
									><option value="external">External</option><option value="internal"
										>Internal</option
									></select
								></label
							>
							<label
								>Dimension <select name="dimension"
									><option value="economic">Economic</option><option value="competitive"
										>Competitive</option
									><option value="market">Market</option><option value="technology"
										>Technology</option
							...
							<label
								>Direction <select name="direction"
									><option value="opportunity">Opportunity</option><option value="threat"
										>Threat</option
									><option value="strength">Strength</option><option value="weakness"
										>Weakness</option
						...
						<label
							>Evidence/source reference <textarea name="evidenceReference" rows="2"
							></textarea></label
						>
						<div class="form-grid compact">
							<label
		...
		<section class="panel">
			<div class="section-heading">
				...
				<span
					>{data.options.filter((option) => option.decision_status === 'selected').length} selected</span
				>
			</div>
			<div class="record-list">
				{#each data.options as option}
					<article>
						<div class="record-meta">
							<span>{label(option.decision_status)}</span>{#if option.priority_rank}<span
									>priority {option.priority_rank}</span
								>{/if}
						</div>
						<h3>{option.title}</h3>
						...
						{#if framework.lifecycle_status === 'draft' && data.canApprove}
							<form method="POST" action="?/decideOption" class="decision-form">
								...
								<select name="decisionStatus"
									><option value="selected">Select</option><option value="rejected">Reject</option
									></select
								>
		...
		<div class="objective-grid">
			{#each data.objectives as objective}
				<article>
					...
					<div>
						<div class="record-meta">
							<span>{objective.objective_code}</span><span>{label(objective.lifecycle_status)}</span
							>
						</div>
		...
		{#if framework.lifecycle_status === 'draft' && data.canManage}
			<details class="add-record">
				...
				<form method="POST" action="?/addObjective" class="form-grid">
					...
					<label class="wide"
						>Description <textarea name="description" rows="4" required></textarea></label
					>
	...
	{#if framework.lifecycle_status === 'draft' && data.canApprove}
		<section class="approval-panel">
			<div>
				...
				<p>
					Approval requires environmental evidence, at least one strategic objective, all options to
					be decided and at least one option selected. Any previously approved version is then
					superseded.
				</p>
			</div>

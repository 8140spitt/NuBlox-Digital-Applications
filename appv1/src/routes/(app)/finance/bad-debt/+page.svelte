...
<section class="page-heading">
	<div>
		...
		<h1>Bad debt</h1>
		<p>
			Assess doubtful receivables, recommend write-off, authorise loss recognition and record later
			cash recovery without rewriting the original invoice.
		</p>
	</div>
...
<section class="panel">
	<div class="section-heading">
		<div>
			...
			<h2>Invoices with receivable remaining</h2>
		</div>
	</div>
				...
				<article class="card">
					<div>
						<strong>{invoice.invoiceNumber}</strong><small
							>{invoice.customerDisplayName} · due {date(invoice.dueDate)}</small
						>
					</div>
					...
					{#if data.canStartCase}
						<form method="POST" action="?/start" class="action-form">
							...
							<label
								>Assessment reason<textarea
									name="reason"
									maxlength="1000"
									required
									placeholder="Why is this receivable being assessed as doubtful?"
								></textarea></label
							>
		...
		<div class="case-list">
			{#each data.cases as item}
				<a class="case-row" href={`/finance/bad-debt/${item.publicId}`}>
					<div>
						<strong>{item.invoiceNumber}</strong><small
							>{item.customerDisplayName} · opened {date(item.openedAt)} · {item.status}</small
						><span>{item.openingReason}</span>
					</div>

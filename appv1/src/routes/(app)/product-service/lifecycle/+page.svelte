...
<section class="workspace">
		...
		{#if data.canManage && data.offerings.length}
			<section class="panel">
				...
				<form method="POST" action="?/createDesign" class="form-grid">
					<label
						>Offering<select name="offeringPublicId" required
					...
					>
					<label
						>Approved business case<select name="businessCasePublicId"
							><option value="">None</option
							>{#each data.businessCases.filter((item) => item.lifecycle_status === 'approved') as item}<option
									value={item.public_id}>{item.business_case_code} · {item.title}</option
								>{/each}</select
						></label
					>
					...
					<label class="wide"
						>Customer outcomes<textarea name="customerOutcomes" required></textarea></label
					>
					<label class="wide"
						>Functional requirements<textarea name="functionalRequirements" required
						></textarea></label
					>
					<label class="wide"
						>Non-functional requirements<textarea name="nonFunctionalRequirements"
						></textarea></label
					>
					<label class="wide"
						>Acceptance criteria<textarea name="acceptanceCriteria" required></textarea></label
					>
	...
	{#if data.canManage && data.designs.length}
		<section class="panel">
			...
			<form method="POST" action="?/addDesignReview" class="form-grid">
				<label
					>Design<select name="designPublicId" required
						>{#each data.designs as design}<option value={design.public_id}
								>{design.design_code} · {design.title}</option
							>{/each}</select
					></label
				>
				...
				<label
					>Review type<select name="reviewType"
						><option value="gate">Gate</option><option value="customer">Customer</option><option
							value="technical">Technical</option
						><option value="commercial">Commercial</option><option value="operational"
							>Operational</option
						><option value="compliance">Compliance</option><option value="sustainability"
				...
				<label
					>Outcome<select name="outcome"
						><option value="pass">Pass</option><option value="conditional">Conditional</option
						><option value="fail">Fail</option></select
					></label
				>
				<label>Review date<input name="reviewDate" type="date" required /></label>
				<label
					>Reviewer<select name="reviewerMemberId" required
						>{#each data.members as member}<option value={member.id}>{member.display_name}</option
							>{/each}</select
					></label
				>
		...
		<section class="panel">
			...
			{#each data.developmentPlans as plan}
				<article class="card">
					...
					<p>
						{plan.lifecycle_status}{plan.project_public_id ? ` · D5 ${plan.project_public_id}` : ''}
					</p>
					{#if data.canManage && plan.lifecycle_status !== 'completed'}
						<form method="POST" action="?/completeDevelopmentPlan">
							<input type="hidden" name="developmentPublicId" value={plan.public_id} /><button
								type="submit">Complete development</button
							>
						</form>
					{/if}
				</article>
		...
		{#if data.canManage && data.designs.some((item) => item.lifecycle_status === 'approved')}
			<section class="panel">
				...
				<form method="POST" action="?/createDevelopmentPlan" class="form-grid">
					<label
						>Approved design<select name="designPublicId" required
							>{#each data.designs.filter((item) => item.lifecycle_status === 'approved') as design}<option
									value={design.public_id}>{design.design_code} · {design.title}</option
								>{/each}</select
						></label
					>
					...
					<label>Evidence ID<input name="evidencePublicId" /></label>
					<label class="wide"
						>Delivery approach<textarea name="deliveryApproach" required></textarea></label
					>
					<label class="wide">Scope<textarea name="scopeText" required></textarea></label>
					<label class="wide"
						>Definition of done<textarea name="definitionOfDone" required></textarea></label
					>
		...
		<section class="panel">
			...
			{#each data.launchPlans as plan}
				<article class="card">
					...
					{#if data.canApprove && plan.lifecycle_status === 'planning'}<form
							method="POST"
							action="?/approveLaunch"
						>
							<input type="hidden" name="launchPublicId" value={plan.public_id} /><button
								type="submit">Approve launch</button
							>
						</form>{/if}
					{#if data.canManage && plan.lifecycle_status === 'approved'}<form
							method="POST"
							action="?/markLaunched"
						>
							<input type="hidden" name="launchPublicId" value={plan.public_id} /><button
								type="submit">Mark launched</button
							>
						</form>{/if}
				</article>
			{/each}
		...
		{#if data.canManage && data.offerings.length}
			<section class="panel">
				...
				<form method="POST" action="?/createLaunchPlan" class="form-grid">
					...
					>
					<label
						>Completed development<select name="developmentPlanPublicId"
							><option value="">None</option
							>{#each data.developmentPlans.filter((item) => item.lifecycle_status === 'completed') as plan}<option
									value={plan.public_id}>{plan.development_code} · {plan.title}</option
								>{/each}</select
						></label
					>
					...
					<label class="wide"
						>Target segments<textarea name="targetSegments" required></textarea></label
					>
					<label class="wide"
						>Commercial readiness<textarea name="commercialReadiness" required></textarea></label
					>
					<label class="wide"
						>Operational readiness<textarea name="operationalReadiness" required></textarea></label
					>
					<label class="wide"
						>Customer readiness<textarea name="customerReadiness" required></textarea></label
					>
					<label class="wide"
						>Support readiness<textarea name="supportReadiness" required></textarea></label
					>
	...
	<div class="grid two">
		<section class="panel">
			...
			{#each data.lifecycleReviews as review}<article class="card">
					<strong>{review.review_code} · {review.lifecycle_phase}</strong>
		...
		{#if data.canManage && data.offerings.length}
			<section class="panel">
				...
				<form method="POST" action="?/recordLifecycleReview" class="form-grid">
					...
					<label>Review date<input name="reviewDate" type="date" required /></label>
					<label
						>Phase<select name="lifecyclePhase"
							><option value="launch">Launch</option><option value="growth">Growth</option><option
								value="maturity">Maturity</option
							><option value="decline">Decline</option><option value="end_of_life"
								>End of life</option
							></select
						></label
					>
					<label
						>Recommendation<select name="recommendation"
							><option value="continue">Continue</option><option value="improve">Improve</option
							><option value="reposition">Reposition</option><option value="invest">Invest</option
					...
					<label class="wide"
						>Performance summary<textarea name="performanceSummary" required></textarea></label
					>
					<label class="wide"
						>Customer summary<textarea name="customerSummary" required></textarea></label
					>
					<label class="wide"
						>Financial summary<textarea name="financialSummary" required></textarea></label
					>
	...
	<div class="grid two">
		<section class="panel">
			...
			{#each data.retirementPlans as plan}
				<article class="card">
					<strong>{plan.retirement_code} · {plan.title}</strong>
					<p>{plan.lifecycle_status}</p>
					{#if data.canApprove && plan.lifecycle_status === 'draft'}<form
							method="POST"
							action="?/approveRetirement"
						>
							<input type="hidden" name="retirementPublicId" value={plan.public_id} /><button
								type="submit">Approve retirement</button
							>
						</form>{/if}{#if data.canManage && plan.lifecycle_status === 'approved'}<form
							method="POST"
							action="?/completeRetirement"
						>
							<input type="hidden" name="retirementPublicId" value={plan.public_id} /><button
								type="submit">Complete retirement</button
							>
						</form>{/if}
				</article>
			{/each}
		...
		{#if data.canManage && data.offerings.length}
			<section class="panel">
				...
				<form method="POST" action="?/createRetirementPlan" class="form-grid">
					...
					>
					<label
						>Retirement review<select name="lifecycleReviewPublicId"
							><option value="">None</option
							>{#each data.lifecycleReviews.filter((item) => item.recommendation === 'retire') as review}<option
									value={review.public_id}>{review.review_code}</option
								>{/each}</select
						></label
					>
					...
					<label>F02 governance decision ID<input name="governanceDecisionPublicId" /></label>
					<label class="wide"
						>Retirement rationale<textarea name="retirementRationale" required></textarea></label
					>
					<label class="wide"
						>Customer transition plan<textarea name="customerTransitionPlan" required
						></textarea></label
					>
					<label class="wide"
						>Operational transition plan<textarea name="operationalTransitionPlan" required
						></textarea></label
					>
					<label class="wide"
						>Financial impact<textarea name="financialImpactSummary" required></textarea></label
					>
					<label class="wide"
						>Data/record retention plan<textarea name="dataRecordRetentionPlan" required
						></textarea></label
					>
	...
	<div class="grid two">
		<section class="panel">
			...
			{#each data.innovationExperiments as experiment}
				<article class="card">
					<strong>{experiment.experiment_code} · {experiment.title}</strong>
					<p>{experiment.lifecycle_status}{experiment.outcome ? ` · ${experiment.outcome}` : ''}</p>
					<p>{experiment.hypothesis}</p>
					{#if data.canManage && experiment.lifecycle_status !== 'completed'}<form
							method="POST"
							action="?/closeInnovationExperiment"
							class="form-grid"
						>
							<input type="hidden" name="experimentPublicId" value={experiment.public_id} /><label
								>Outcome<select name="outcome"
									><option value="validated">Validated</option><option value="invalidated"
										>Invalidated</option
									><option value="inconclusive">Inconclusive</option></select
								></label
							><label>Evidence ID<input name="evidencePublicId" /></label><label class="wide"
								>Learning summary<textarea name="learningSummary" required></textarea></label
							><button type="submit">Close experiment</button>
						</form>{/if}
				</article>
			{/each}
		...
		{#if data.canManage}
			<section class="panel">
				...
				<form method="POST" action="?/createInnovationExperiment" class="form-grid">
					<label
						>Portfolio<select name="portfolioPublicId"
							><option value="">None</option>{#each data.portfolios as portfolio}<option
									value={portfolio.public_id}>{portfolio.portfolio_code}</option
								>{/each}</select
						></label
					>
					<label
						>Idea<select name="ideaPublicId"
							><option value="">None</option>{#each data.ideas as idea}<option
									value={idea.public_id}>{idea.idea_code}</option
								>{/each}</select
						></label
					>
					<label
						>Offering<select name="offeringPublicId"
							><option value="">None</option>{#each data.offerings as offering}<option
									value={offering.public_id}>{offering.offering_code}</option
								>{/each}</select
						></label
					>
					...
					<label class="wide"
						>Experiment method<textarea name="experimentMethod" required></textarea></label
					>
					<label class="wide"
						>Success measure<textarea name="successMeasure" required></textarea></label
					>

...
<div class="metrics">
	...
	{#if data.canInvite}
		<div>
			<strong
				>{data.invitations.filter((invitation) => invitation.status === 'pending').length}</strong
			>
	...
	{#if data.canManageOrganisation}
		<div>
			<strong>{data.roles.filter((role) => role.isActive).length}</strong><span>active roles</span>
		</div>
	{/if}
</div>
	...
	<section class="panel" aria-labelledby="invite-heading">
		...
		<form method="POST" action="?/invite" class="form-grid">
					...
					<div class="choice-grid">
						{#each data.roles.filter((role) => role.isActive) as role}
							<label class="choice">
								...
								<span
									><strong>{role.name}</strong>{#if role.description}<small
			...
			{:else if !data.canManageMembers}
				<p class="muted wide">
					You can invite members, but role assignment requires <code>member.manage</code>.
				</p>
			{/if}
...
{#if data.canManageMembers}
	<section class="panel" id="members" aria-labelledby="members-heading">
		<div class="panel-heading">
			...
			<p>
				Membership state and organisation roles are independent controls. Self-demotion is blocked.
			</p>
		</div>
			...
			{#each data.members as member}
				<article class="record">
					<div class="record-main">
						<div>
							<h3>
								{member.displayName}{#if member.isCurrent}
									<span class="you">You</span>{/if}
							</h3>
							<p>{member.email ?? 'No primary email recorded'}</p>
						</div>
						<span class:inactive={member.status !== 'active'} class="status"
					...
					<div class="record-actions">
						<form method="POST" action="?/setMemberStatus" class="inline-form">
							...
							<button type="submit" class="secondary" disabled={member.isCurrent}
									...
									{#each data.roles.filter((role) => role.isActive) as role}
										<label class="choice">
											<input
												...
												name="rolePublicId"
												value={role.publicId}
												checked={member.roles.some(
													(assigned) => assigned.publicId === role.publicId
												)}
			...
			{#each data.invitations as invitation}
				<article class="record">
					<div class="record-main">
						...
						<span class:inactive={invitation.status !== 'pending'} class="status"
							>{statusLabel(invitation.status)}</span
						>
					</div>
					<div class="record-meta">
...
{#if data.canManageOrganisation}
	<section class="panel" id="roles" aria-labelledby="roles-heading">
		<div class="panel-heading">
			...
			<p>
				Permission definitions are platform-controlled. Organisations decide which permissions each
				organisation role grants.
			</p>
		</div>
			...
			{#each data.roles as role}
				<article class="record">
					<div class="record-main">
						...
						<span class:inactive={!role.isActive} class="status"
					...
					<div class="record-meta">
						...
						<span
							>{role.permissionKeys.length} permission grant{role.permissionKeys.length === 1
								? ''
								: 's'}</span
						>
					</div>
					...
					<details>
						...
						<form method="POST" action="?/updateRole" class="stack role-form">
							...
							<label class="choice standalone">
								...
								<span
									><strong>Role is active</strong><small
										>Inactive roles do not grant permissions.</small
									></span
								>
							</label>
							<fieldset>

...
function validatePublicId(value: string): string {
	...
	if (!result || result.length > 64)
		throw new CommercialValidationError('Quotation ID is required.');
	return result;
}
...
export class QuotationProjectConversionService {
	...
	async getWorkspace(
		...
		const commercial = await new CommercialService(this.db).getQuotation(
			actor,
			quotationPublicId,
			selectedVersionNumber
		);
		const commitmentAmount = await quotationCommitmentAmount(
			this.db,
			actor.organisationId,
			commercial.version.id
		);
		const [acceptedResponse, permissions, sourceEstimates, creditControl] = await Promise.all([
			this.findAcceptedResponse(
				this.db,
				actor.organisationId,
				commercial.quotation.id,
				commercial.version.id
			),
			this.permissionState(actor),
			this.listSourceEstimates(this.db, actor.organisationId, commercial.version.id),
			new CreditControlService(this.db, this.publicIdFactory, this.now).commitmentPreview(
				actor,
				commercial.quotation.customerPartyId,
				commercial.version.currencyCode,
				commitmentAmount
			)
		]);
	...
	async convert(
		...
		creditOverrideReasonInput?: string | null
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (quotation.lifecycleStatus !== 'active')
				throw new CommercialValidationError(
					'Only an active quotation can be converted to a project.'
				);
			...
			if (version.versionStatus !== 'issued' || !version.lockedAt)
				throw new CommercialValidationError(
					'Only an issued and locked quotation version can be converted.'
				);
			...
			if (!acceptedResponse)
				throw new CommercialValidationError(
					'The selected quotation version does not have an accepted customer response.'
				);

			const existingProject = await this.findConvertedProject(
				trx,
				actor.organisationId,
				acceptedResponse.id
			);
			if (existingProject) {
				if (quotation.projectId !== existingProject.id)
					throw new Error(
						'Quotation conversion evidence is inconsistent with quotations.project_id.'
					);
				return existingProject;
			}
			if (quotation.projectId !== null)
				throw new CommercialValidationError(
					'This quotation is already linked to a project without matching conversion evidence.'
				);

			const sourceEstimates = await this.listSourceEstimates(
				trx,
				actor.organisationId,
				version.id,
				true
			...
			try {
				...
				if (cause instanceof CreditControlBlockedError)
					throw new CommercialValidationError(cause.message);
				throw cause;
			}
			...
			if (await projectRepository.findOwnedByProjectNumber(actor.organisationId, projectNumber))
				throw new CommercialValidationError(
					`Project number ${projectNumber} already exists without quotation conversion evidence.`
				);
			...
			if (sourceEstimates.length > 0)
				await trx
					...
					.where(
						'id',
						'in',
						sourceEstimates.map((estimate) => estimate.id)
					)
					.where('project_id', 'is', null)
					.execute();
			...
			await audit.append({
				...
				changeSummary: {
					...
					commitmentAmount,
					creditControlOverrideRequested: Boolean(creditOverrideReasonInput?.trim())
				}
			});
			await audit.append({
				eventPublicId: this.publicIdFactory(),

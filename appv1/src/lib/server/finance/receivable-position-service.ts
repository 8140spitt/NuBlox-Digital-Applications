...
export class ReceivablePositionService {
	...
	async getInvoicePosition(
		actor: TenantActorContext,
		invoicePublicIdInput: string
	): Promise<InvoiceReceivablePosition> {
		const invoicePublicId = invoicePublicIdInput.trim();
		if (!invoicePublicId || invoicePublicId.length > 64)
		...
		const outstanding = parseScaledDecimal(

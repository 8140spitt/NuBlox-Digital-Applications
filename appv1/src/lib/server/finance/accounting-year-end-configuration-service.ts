...
export class AccountingYearEndConfigurationService {
	...
	async getConfiguration(actor: TenantActorContext) {
		...
		if (!financeView.allowed || !accountingView.allowed)
			throw new TenantAccessError('Year-end accounting configuration is not permitted.');
		const [accounts, mapping] = await Promise.all([
			this.db
				.selectFrom('accounting_accounts')
				.select(['public_id as publicId', 'account_code as accountCode', 'name'])
		...
		return {
			retainedEarningsAccounts: accounts,
			retainedEarningsMapping: mapping ?? null,
			canConfigureRetainedEarnings: configure.allowed
		};
	}

	async assignRetainedEarnings(
		actor: TenantActorContext,
		input: { accountPublicId: string; reason: string }
	): Promise<void> {
		...
		await this.db.transaction().execute(async (trx) => {
			...
			if (account.accountType !== 'equity' || account.isActive !== 1)
				throw new FinanceValidationError('Retained earnings must map to an active equity account.');
			await trx
				.insertInto('accounting_account_mappings')
				.values({
					organisation_id: actor.organisationId,
					mapping_key: 'retained_earnings',
			...
			await new AuditRepository(trx).append({
				...
				subjectPublicId: 'retained_earnings',

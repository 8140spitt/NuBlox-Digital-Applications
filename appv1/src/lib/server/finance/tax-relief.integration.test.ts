...
async function cleanup() {
	...
	await db
		.deleteFrom('receivable_vat_return_posting_reversals')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('receivable_vat_return_postings')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('receivable_vat_bad_debt_repayment_reversals')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('receivable_vat_bad_debt_repayments')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('receivable_vat_bad_debt_claim_reversals')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('receivable_vat_bad_debt_claim_authorisations')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('receivable_vat_bad_debt_claim_lines')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('receivable_vat_bad_debt_claims')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
...
beforeAll(async () => {
	...
	invoiceId = insertedId(
		await db
			...
			.values({
				...
				document_number: 'INV-VAT-BDR-001',
	...
	invoiceItemId = insertedId(
		await db
			...
			.values({
				...
				line_number: 1,
				description: 'VAT bad-debt relief test invoice',
				quantity: '1.000000',
				unit_rate: '100.0000'
			})
			.executeTakeFirstOrThrow()
	...
	await db
		...
		.values({
			organisation_id: organisationAId,
			financial_document_item_id: invoiceItemId,
			tax_category_id: taxCategoryId,
			sort_order: 1,
	...
	await db
		...
		.values({
			...
			issued_at: new Date('2026-01-01T09:00:00.000Z'),
			note: null
		})
		.executeTakeFirstOrThrow();

	badDebtCasePublicId = randomUUID();
	const badDebtCaseId = insertedId(
		await db
			.insertInto('receivable_bad_debt_cases')
			.values({
				organisation_id: organisationAId,
				public_id: badDebtCasePublicId,
				customer_party_id: customerId,
				invoice_document_id: invoiceId,
				status: 'open',
				opening_reason: 'Customer insolvency.',
				opened_by_member_id: financeAMemberId,
				opened_at: new Date('2026-07-15T09:00:00.000Z')
			})
			.executeTakeFirstOrThrow()
	);
	const recommendationId = insertedId(
		await db
			.insertInto('receivable_bad_debt_recommendations')
			.values({
				organisation_id: organisationAId,
				public_id: randomUUID(),
				bad_debt_case_id: badDebtCaseId,
				invoice_document_id: invoiceId,
				recommended_amount: '60.0000',
				reason: 'Partial write-off.',
				recommended_by_member_id: financeAMemberId,
				recommended_at: new Date('2026-07-16T09:00:00.000Z')
			})
			.executeTakeFirstOrThrow()
	);
	writeOffPublicId = randomUUID();
	writeOffId = insertedId(
		await db
			...
			.values({
				...
				invoice_document_id: invoiceId,
				write_off_amount: '60.0000',
				tax_treatment_policy: 'separate_tax_adjustment_required',
				reason: 'Authorised with separate VAT treatment.',
				authorised_by_member_id: ownerAMemberId,
				authorised_at: new Date('2026-07-16T10:00:00.000Z')
			})
			.executeTakeFirstOrThrow()
	);
	...
	paymentId = insertedId(
		await db
			...
			.values({
				...
				amount: '30.0000',
				currency_code: 'GBP',
				payment_reference: 'LATE-RECOVERY',
...
describe.sequential('Package 004K controlled VAT bad-debt relief', () => {
	it('allows delegated preparation, binds eligibility to the issued due date, calculates VAT from immutable source tax, and enforces the six-month gate', async () => {
		...
		await expect(
			service.prepareClaim(actorFinanceA, {
				writeOffPublicId,
				...
				lines: [
					{ sourceInvoiceItemId: invoiceItemId, taxCategoryId, considerationBasisAmount: '60.0000' }
				]
			})
		).rejects.toThrow('Payment due date must match the issued invoice due date of 2026-01-31.');

		claimPublicId = (
			await service.prepareClaim(actorFinanceA, {
				writeOffPublicId,
				supplyDate: '2026-01-01',
		...
		await expect(
			service.authoriseClaim(actorFinanceA, {
				claimPublicId,
				reason: 'Delegated preparer must not authorise.'
			})
		).rejects.toBeInstanceOf(TenantAccessError);
		await expect(
			new ControlledTaxReliefService(db, randomUUID, () => BEFORE_ELIGIBLE).authoriseClaim(
				actorOwnerA,
				{ claimPublicId, reason: 'Too early.' }
			)
		).rejects.toBeInstanceOf(FinanceValidationError);
		await service.authoriseClaim(actorOwnerA, {
			claimPublicId,
			reason: 'Eligibility and source evidence revalidated.'
		});
		...
		await expect(
			service.prepareClaim(actorFinanceA, {
				...
				reliefSchemeApplicable: true,
				lines: [
					{ sourceInvoiceItemId: invoiceItemId, taxCategoryId, considerationBasisAmount: '0.0001' }
				]
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
	});
	...
	it('records Box 4 posting evidence and honours explicit granular deny above finance.manage', async () => {
		...
		const permission = await db
			.selectFrom('permissions')
			.select('id')
			.where('permission_key', '=', 'finance.tax_relief.post')
			.executeTakeFirstOrThrow();
		await db
			...
			.executeTakeFirstOrThrow();
		await expect(
			service.recordReturnPosting(actorOwnerA, {
				sourceKind: 'relief_claim',
				sourcePublicId: claimPublicId,
				vatReturnPeriodReference: '2026-Q3',
				vatReturnPeriodStart: '2026-07-01',
				vatReturnPeriodEnd: '2026-09-30',
				externalReference: 'VAT-Q3-DRAFT',
		...
		await db
			...
			.execute();
		claimPostingPublicId = (
			await service.recordReturnPosting(actorOwnerA, {
				sourceKind: 'relief_claim',
				sourcePublicId: claimPublicId,
				vatReturnPeriodReference: '2026-Q3',
				vatReturnPeriodStart: '2026-07-01',
				vatReturnPeriodEnd: '2026-09-30',
				externalReference: 'VAT-Q3-001',
				reason: 'Record inclusion in VAT return evidence.'
			})
		).publicId;
		const posting = (await service.getWorkspace(actorOwnerA)).claims
			.find((row) => row.publicId === claimPublicId)
			?.postings.find((row) => row.publicId === claimPostingPublicId);
		expect(posting?.vatReturnBox).toBe(4);
		expect(posting?.amount).toBe('10.0000');
		await expect(
			service.reverseClaim(actorOwnerA, {
				claimPublicId,
				reason: 'Posting must be reversed first.'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
		await expect(
			new BadDebtMutationService(db).reverseWriteOff(actorOwnerA, {
				casePublicId: badDebtCasePublicId,
				writeOffPublicId,
				reason: 'VAT claim must be reversed first.'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
	});
	...
	it('records proportional VAT repayment from later bad-debt recovery and requires the VAT period containing the recovery receipt date', async () => {
		...
		await db
			...
			.values({
				...
				recovered_amount: '30.0000',
				reason: 'Late customer recovery after VAT relief.',
				recorded_by_member_id: financeAMemberId,
				recovered_at: NOW
			})
			.executeTakeFirstOrThrow();
		const service = new ControlledTaxReliefService(db, randomUUID, () => NOW);
		await expect(
			service.recordRepayment(actorFinanceA, {
				claimPublicId,
				recoveryPublicId,
				considerationPaymentAmount: '30.0000',
				reason: 'Delegated preparation must not record VAT repayment.'
			})
		).rejects.toBeInstanceOf(TenantAccessError);
		repaymentPublicId = (
			await service.recordRepayment(actorOwnerA, {
				claimPublicId,
				recoveryPublicId,
				considerationPaymentAmount: '30.0000',
				reason: 'Repay VAT proportionally after recovery.'
			})
		).publicId;
		const claim = (await service.getWorkspace(actorOwnerA)).claims.find(
			(row) => row.publicId === claimPublicId
		);
		const repayment = claim?.repayments.find((row) => row.publicId === repaymentPublicId);
		expect(repayment?.vatRepaymentAmount).toBe('5.0000');
		await expect(
			service.recordReturnPosting(actorOwnerA, {
				sourceKind: 'relief_repayment',
				sourcePublicId: repaymentPublicId,
				vatReturnPeriodReference: '2026-Q2',
				vatReturnPeriodStart: '2026-04-01',
				vatReturnPeriodEnd: '2026-06-30',
				externalReference: 'WRONG-PERIOD',
				reason: 'Must not post recovery repayment in the wrong period.'
			})
		).rejects.toThrow(
			'VAT repayment posting period must include the bad-debt recovery receipt date of 2026-08-18.'
		);
		repaymentPostingPublicId = (
			await service.recordReturnPosting(actorOwnerA, {
				sourceKind: 'relief_repayment',
				sourcePublicId: repaymentPublicId,
				vatReturnPeriodReference: '2026-Q3',
				vatReturnPeriodStart: '2026-07-01',
				vatReturnPeriodEnd: '2026-09-30',
				externalReference: 'VAT-Q3-RECOVERY-001',
				reason: 'Record repayment in VAT return evidence.'
			})
		).publicId;
		const repaymentPosting = (await service.getWorkspace(actorOwnerA)).claims
			.find((row) => row.publicId === claimPublicId)
			?.postings.find((row) => row.publicId === repaymentPostingPublicId);
	...
	it('enforces additive reversal ordering across VAT posting, VAT repayment and operational recovery evidence', async () => {
		...
		await expect(
			badDebt.reverseRecovery(actorOwnerA, {
				casePublicId: badDebtCasePublicId,
				recoveryPublicId,
				reason: 'VAT repayment must be reversed first.'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
		await expect(
			service.reverseRepayment(actorOwnerA, {
				repaymentPublicId,
				reason: 'Posting must be reversed first.'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
		await service.reverseReturnPosting(actorOwnerA, {
			postingPublicId: repaymentPostingPublicId,
			reason: 'Correct the VAT repayment posting.'
		});
		await service.reverseRepayment(actorOwnerA, {
			repaymentPublicId,
			reason: 'Reverse VAT repayment before operational recovery correction.'
		});
		await badDebt.reverseRecovery(actorOwnerA, {
			casePublicId: badDebtCasePublicId,
			recoveryPublicId,
			reason: 'Operational recovery can now be reversed.'
		});
			...
			.where(
				...
				(
					await db
						...
						.select('id')
						.where('organisation_id', '=', organisationAId)
						.where('public_id', '=', recoveryPublicId)
						.executeTakeFirstOrThrow()
				).id
			)
			.executeTakeFirst();
	...
	it('reverses VAT claim evidence before allowing the operational write-off reversal', async () => {
		const service = new ControlledTaxReliefService(db, randomUUID, () => NOW);
		await service.reverseReturnPosting(actorOwnerA, {
			postingPublicId: claimPostingPublicId,
			reason: 'Correct the VAT relief posting.'
		});
		await service.reverseClaim(actorOwnerA, {
			claimPublicId,
			reason: 'Reverse VAT relief before reversing the write-off.'
		});
		await new BadDebtMutationService(db, randomUUID, () => NOW).reverseWriteOff(actorOwnerA, {
			casePublicId: badDebtCasePublicId,
			writeOffPublicId,
			reason: 'Tax and recovery dependencies are now reversed.'
		});
		const reversal = await db
	...
	it('masks VAT relief claim identities across tenants', async () => {
		await expect(
			new ControlledTaxReliefService(db, randomUUID, () => NOW).authoriseClaim(actorOwnerB, {
				claimPublicId,
				reason: 'Foreign tenant must not see claim.'
			})

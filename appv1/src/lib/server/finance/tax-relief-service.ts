...
function addMonthsUtc(date: Date, months: number): Date {
	...
	return new Date(
		Date.UTC(first.getUTCFullYear(), first.getUTCMonth(), Math.min(day, endOfTargetMonth))
	);
}
...
function vatForConsiderationBasis(
	basisAmount: string,
	taxableAmount: string,
	taxAmount: string
): string {
	...
	if (basis <= 0n || tax <= 0n || gross <= 0n || basis > gross) {
		throw new FinanceValidationError(
			'VAT relief basis is incompatible with the source tax snapshot.'
		);
	}
...
async function activeClaimTotalsForWriteOff(
	db: DatabaseExecutor,
	...
	let query = db
		...
		.innerJoin('receivable_vat_bad_debt_claim_authorisations as authorisation', (join) =>
			join
				.onRef('authorisation.claim_id', '=', 'claim.id')
				.onRef('authorisation.organisation_id', '=', 'claim.organisation_id')
		)
		.leftJoin('receivable_vat_bad_debt_claim_reversals as reversal', (join) =>
			join
				.onRef('reversal.claim_id', '=', 'claim.id')
				.onRef('reversal.organisation_id', '=', 'claim.organisation_id')
		)
		.innerJoin('receivable_vat_bad_debt_claim_lines as line', (join) =>
			join
				.onRef('line.claim_id', '=', 'claim.id')
				.onRef('line.organisation_id', '=', 'claim.organisation_id')
		)
		.select([
			'claim.id as claimId',
			'line.consideration_basis_amount as basisAmount',
			'line.vat_relief_amount as vatAmount'
		])
...
async function activeClaimBasisForTaxSource(
	...
	let query = db
		...
		.innerJoin('receivable_vat_bad_debt_claims as claim', (join) =>
			join
				.onRef('claim.id', '=', 'line.claim_id')
				.onRef('claim.organisation_id', '=', 'line.organisation_id')
		)
		.innerJoin('receivable_vat_bad_debt_claim_authorisations as authorisation', (join) =>
			join
				.onRef('authorisation.claim_id', '=', 'claim.id')
...
async function activeRepaymentTotalsForClaim(
	db: DatabaseExecutor,
	organisationId: string,
	claimId: string
) {
	const rows = await db
		...
		.leftJoin('receivable_vat_bad_debt_repayment_reversals as reversal', (join) =>
			join
				.onRef('reversal.repayment_id', '=', 'repayment.id')
				.onRef('reversal.organisation_id', '=', 'repayment.organisation_id')
		)
		.select([
			'repayment.consideration_payment_amount as considerationAmount',
			'repayment.vat_repayment_amount as vatAmount'
		])
...
async function activeRepaymentConsiderationForRecovery(
	db: DatabaseExecutor,
	organisationId: string,
	recoveryId: string
) {
...
async function activePostingsForSource(
	db: DatabaseExecutor,
	organisationId: string,
	source: { claimId?: string; repaymentId?: string }
) {
	let query = db
		.selectFrom('receivable_vat_return_postings as posting')
		.leftJoin('receivable_vat_return_posting_reversals as reversal', (join) =>
			join
				.onRef('reversal.posting_id', '=', 'posting.id')
				.onRef('reversal.organisation_id', '=', 'posting.organisation_id')
		)
...
export class TaxReliefService {
	...
	private async assertView(actor: TenantActorContext): Promise<FinanceAccessPolicy> {
		...
		if (
			...
			!(await policy.taxReliefViewDecision(actor)).allowed
		) {
	...
	async getWorkspace(actor: TenantActorContext): Promise<TaxReliefWorkspace> {
		...
		const writeOffRows = await this.db
			...
			.innerJoin('invoices as invoiceDetail', (join) =>
				join
					.onRef('invoiceDetail.financial_document_id', '=', 'invoice.id')
					.onRef('invoiceDetail.organisation_id', '=', 'invoice.organisation_id')
			)
			.innerJoin('parties as customer', (join) =>
				join
					.onRef('customer.id', '=', 'invoice.customer_party_id')
					.onRef('customer.organisation_id', '=', 'invoice.organisation_id')
			)
			.leftJoin('party_organisations as customerOrganisation', (join) =>
				join
					.onRef('customerOrganisation.party_id', '=', 'customer.id')
					.onRef('customerOrganisation.organisation_id', '=', 'customer.organisation_id')
			)
			...
			.select([
				'writeOff.id',
				'writeOff.public_id as writeOffPublicId',
				'writeOff.write_off_amount as writeOffAmount',
				'writeOff.authorised_at as authorisedAt',
				'invoice.id as invoiceId',
				'invoice.public_id as invoicePublicId',
				'invoice.document_number as invoiceNumber',
				'invoiceDetail.due_date as invoiceDueDate',
				'customerOrganisation.legal_name as customerLegalName'
			])
		...
		for (const row of writeOffRows) {
			const sourceRows = await this.db
				...
				.innerJoin('financial_document_item_taxes as tax', (join) =>
					join
						.onRef('tax.financial_document_item_id', '=', 'item.id')
						.onRef('tax.organisation_id', '=', 'item.organisation_id')
				)
			...
			for (const source of sourceRows) {
				...
				const activeBasis = await activeClaimBasisForTaxSource(
					this.db,
					actor.organisationId,
					source.sourceInvoiceItemId,
			...
			const recovery = money(
				await activeRecoveryAmountForWriteOff(this.db, actor.organisationId, row.id)
			);
			const claimed = (await activeClaimTotalsForWriteOff(this.db, actor.organisationId, row.id))
				.basis;
			const writeOffAmount = money(row.writeOffAmount);
			const available =
				writeOffAmount > recovery + claimed ? writeOffAmount - recovery - claimed : 0n;
			candidates.push({
		...
		const claimRows = await this.db
			...
			.innerJoin('receivable_write_offs as writeOff', (join) =>
				join
					.onRef('writeOff.id', '=', 'claim.write_off_id')
					.onRef('writeOff.organisation_id', '=', 'claim.organisation_id')
			)
			.innerJoin('financial_documents as invoice', (join) =>
				join
					.onRef('invoice.id', '=', 'claim.invoice_document_id')
					.onRef('invoice.organisation_id', '=', 'claim.organisation_id')
			)
			...
			.select([
				'claim.id',
				'claim.public_id as publicId',
				'claim.write_off_id as writeOffId',
				'writeOff.public_id as writeOffPublicId',
				'claim.invoice_document_id as invoiceId',
				'invoice.public_id as invoicePublicId',
				'invoice.document_number as invoiceNumber',
				'claim.prepared_at as preparedAt',
				'claim.supply_date as supplyDate',
				'claim.payment_due_date as paymentDueDate',
				'claim.relevant_date as relevantDate',
				'claim.eligible_from as eligibleFrom',
				'claim.claim_deadline as claimDeadline',
				'claim.original_vat_period_reference as originalVatPeriodReference',
				'claim.reason',
				'authorisation.authorised_at as authorisedAt',
		...
		for (const claim of claimRows) {
			const lineRows = await this.db
				...
				.innerJoin('financial_document_items as item', (join) =>
					join
						.onRef('item.id', '=', 'line.source_invoice_item_id')
						.onRef('item.organisation_id', '=', 'line.organisation_id')
				)
				.innerJoin('financial_document_item_taxes as tax', (join) =>
					join
						.onRef('tax.financial_document_item_id', '=', 'line.source_invoice_item_id')
						.onRef('tax.organisation_id', '=', 'line.organisation_id')
						.onRef('tax.tax_category_id', '=', 'line.tax_category_id')
				)
				.innerJoin('tax_categories as category', (join) =>
					join
						.onRef('category.id', '=', 'line.tax_category_id')
						.onRef('category.organisation_id', '=', 'line.organisation_id')
				)
				.select([
					'line.source_invoice_item_id as sourceInvoiceItemId',
					'item.line_number as lineNumber',
					'item.description',
					'line.tax_category_id as taxCategoryId',
					'category.code as taxCategoryCode',
					'tax.applied_rate_percent as appliedRatePercent',
					'tax.taxable_amount as taxableAmount',
					'tax.tax_amount as taxAmount',
					'line.consideration_basis_amount as claimBasis',
					'line.vat_relief_amount as claimVat'
				])
			...
			const repayments = await this.db
				...
				.select([
					'repayment.id',
					'repayment.public_id as publicId',
					'recovery.public_id as recoveryPublicId',
					'repayment.consideration_payment_amount as considerationPaymentAmount',
					'repayment.vat_repayment_amount as vatRepaymentAmount',
					'repayment.recorded_at as recordedAt',
					'reversal.reversed_at as reversedAt'
				])
			...
			const postings = await this.db
				...
				.select([
					'posting.public_id as publicId',
					'posting.posting_kind as postingKind',
					'posting.vat_return_box as vatReturnBox',
					'posting.vat_return_period_reference as vatReturnPeriodReference',
					'posting.vat_return_period_start as vatReturnPeriodStart',
					'posting.vat_return_period_end as vatReturnPeriodEnd',
					'posting.amount',
					'posting.external_reference as externalReference',
					'posting.posted_at as postedAt',
					'reversal.reversed_at as reversedAt'
				])
				...
				.where((eb) =>
					eb.or([
						...
						eb(
							'posting.repayment_id',
							'in',
							repayments.map((repayment) => repayment.id).length
								? repayments.map((repayment) => repayment.id)
								: ['0']
						)
					])
				)
	...
	async prepareClaim(
		...
		if (!supplyDate || !paymentDueDate)
			throw new FinanceValidationError('Supply date and payment due date are required.');
		const originalVatPeriodReference = cleanFinanceText(
			input.originalVatPeriodReference,
			80,
			'Original VAT period reference',
			true
		)!;
		const reason = cleanFinanceText(
			input.reason,
			1000,
			'VAT bad-debt relief preparation reason',
			true
		)!;
		if (
			!input.vatAccountedAndPaid ||
			!input.debtNotSoldOrFactored ||
			!input.sellingPriceConditionMet ||
			!input.reliefSchemeApplicable
		) {
			throw new FinanceValidationError(
				'All VAT bad-debt relief eligibility attestations must be confirmed before preparation.'
			);
		}
		if (input.lines.length === 0)
			throw new FinanceValidationError('At least one source VAT line is required.');
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const invoiceIdentity = await badDebtInvoiceById(
				trx,
				actor.organisationId,
				identity.invoiceDocumentId
			);
			...
			await trx
				...
				.select('id')
				.where('organisation_id', '=', actor.organisationId)
				.where('id', '=', invoiceIdentity.customerPartyId)
				.forUpdate()
				.executeTakeFirstOrThrow();
			const invoice = await badDebtInvoiceById(
				trx,
				actor.organisationId,
				identity.invoiceDocumentId,
				true
			...
			if (!invoice || invoice.lifecycleStatus !== 'issued')
				throw new FinanceValidationError('VAT bad-debt relief requires an issued source invoice.');
			const writeOff = await trx
				...
				.select([
					'writeOff.id',
					'writeOff.invoice_document_id as invoiceDocumentId',
					'writeOff.write_off_amount as amount',
					'writeOff.tax_treatment_policy as taxTreatmentPolicy',
					'reversal.write_off_id as reversedId'
				])
				.where('writeOff.organisation_id', '=', actor.organisationId)
				.where('writeOff.public_id', '=', writeOffPublicId)
				.forUpdate()
				.executeTakeFirst();
			if (!writeOff || writeOff.invoiceDocumentId !== invoice.id)
				throw new RecordNotFoundError('Write-off not found.');
			if (writeOff.reversedId)
				throw new FinanceValidationError(
					'A reversed write-off cannot support VAT bad-debt relief.'
				);
			if (writeOff.taxTreatmentPolicy !== 'separate_tax_adjustment_required') {
				throw new FinanceValidationError(
					'The write-off was not marked for separate tax adjustment.'
				);
			}
			const recovery = money(
				await activeRecoveryAmountForWriteOff(trx, actor.organisationId, writeOff.id, true)
			);
			const activeClaims = await activeClaimTotalsForWriteOff(
				trx,
				actor.organisationId,
				writeOff.id
			);
			const availableWriteOffBasis =
				money(writeOff.amount) > recovery + activeClaims.basis
					? money(writeOff.amount) - recovery - activeClaims.basis
					: 0n;
			let totalBasis = 0n;
			...
			for (const line of input.lines) {
				const sourceInvoiceItemId = cleanFinanceText(
					line.sourceInvoiceItemId,
					32,
					'Source invoice item ID',
					true
				)!;
				...
				if (seen.has(key))
					throw new FinanceValidationError(
						'A source VAT line can appear only once in one preparation.'
					);
				seen.add(key);
				const basisAmount = validateMoneyAmount(
					line.considerationBasisAmount,
					'Consideration basis'
				);
				...
				if (money(source.taxAmount) <= 0n)
					throw new FinanceValidationError(
						'Only source lines with VAT charged can support VAT bad-debt relief.'
					);
				const sourceGross = money(source.taxableAmount) + money(source.taxAmount);
				const alreadyClaimed = await activeClaimBasisForTaxSource(
					trx,
					actor.organisationId,
					sourceInvoiceItemId,
					taxCategoryId
				);
				if (money(basisAmount) > sourceGross - alreadyClaimed)
					throw new FinanceValidationError(
						'Consideration basis exceeds the remaining source VAT-line capacity.'
					);
				const vatAmount = vatForConsiderationBasis(
					basisAmount,
			...
			if (totalBasis > availableWriteOffBasis)
				throw new FinanceValidationError(
					`Prepared consideration exceeds the currently available written-off amount of ${moneyText(availableWriteOffBasis)}.`
				);
			const publicId = this.publicIdFactory();
			const result = await trx
				.insertInto('receivable_vat_bad_debt_claims')
				.values({
			...
			await trx
				.insertInto('receivable_vat_bad_debt_claim_lines')
				.values(
			...
			await new AuditRepository(trx).append({
				...
				projectId: null,
				actionKey: 'finance.tax_relief.claim.prepared',
				subjectType: 'vat_bad_debt_claim',
				subjectPublicId: publicId,
				correlationId: actor.correlationId,
				changeSummary: {
					writeOffPublicId,
					invoicePublicId: invoice.publicId,
					considerationBasisAmount: moneyText(totalBasis),
					eligibleFrom,
					claimDeadline,
					originalVatPeriodReference
				}
			});
			return { publicId };
		});
	...
	async authoriseClaim(
		actor: TenantActorContext,
		input: { claimPublicId: string; reason: string }
	): Promise<void> {
		...
		await this.db.transaction().execute(async (trx) => {
			const policy = new FinanceAccessPolicy(trx);
			const membership = await policy.assertActiveActor(actor, trx);
			if (!(await policy.mutationDecision(actor, 'finance.tax_relief.authorise', trx)).allowed)
				throw new TenantAccessError('VAT bad-debt relief authorisation is not permitted.');
			const claimIdentity = await trx
				.selectFrom('receivable_vat_bad_debt_claims')
				.select(['id', 'write_off_id as writeOffId', 'invoice_document_id as invoiceId'])
				.where('organisation_id', '=', actor.organisationId)
			...
			if (!invoice || invoice.lifecycleStatus !== 'issued')
				throw new FinanceValidationError(
					'The source invoice is no longer eligible for VAT bad-debt relief.'
				);
			const claim = await trx
				.selectFrom('receivable_vat_bad_debt_claims')
				.selectAll()
				.where('organisation_id', '=', actor.organisationId)
				.where('id', '=', claimIdentity.id)
				.forUpdate()
				.executeTakeFirstOrThrow();
			if (
				await trx
					.selectFrom('receivable_vat_bad_debt_claim_authorisations')
					.select('claim_id')
					.where('organisation_id', '=', actor.organisationId)
			...
			if (
				await trx
					.selectFrom('receivable_vat_bad_debt_claim_reversals')
					.select('claim_id')
					.where('organisation_id', '=', actor.organisationId)
					.where('claim_id', '=', claim.id)
					.forUpdate()
				...
				throw new FinanceValidationError(
					'A reversed VAT bad-debt relief claim cannot be authorised.'
				);
			const writeOff = await trx
				.selectFrom('receivable_write_offs as writeOff')
				.leftJoin('receivable_write_off_reversals as reversal', (join) =>
				...
				.where('writeOff.organisation_id', '=', actor.organisationId)
				.where('writeOff.id', '=', claim.write_off_id)
				.forUpdate()
				.executeTakeFirst();
			if (!writeOff || writeOff.reversedId)
				throw new FinanceValidationError('The source write-off is no longer active.');
			if (writeOff.taxTreatmentPolicy !== 'separate_tax_adjustment_required')
				throw new FinanceValidationError(
					'The source write-off no longer requires separate tax adjustment.'
				);
			const today = dateOnly(this.now());
			if (today < claim.eligible_from)
				throw new FinanceValidationError(
					`VAT bad-debt relief is not eligible before ${claim.eligible_from.toISOString().slice(0, 10)}.`
				);
			if (today > claim.claim_deadline)
				throw new FinanceValidationError(
					`VAT bad-debt relief claim deadline passed on ${claim.claim_deadline.toISOString().slice(0, 10)}.`
				);
			const totals = await claimTotals(trx, actor.organisationId, claim.id);
			if (totals.basis <= 0n || totals.vat <= 0n)
				throw new FinanceValidationError('VAT bad-debt relief claim has no source VAT lines.');
			const recovery = money(
				await activeRecoveryAmountForWriteOff(trx, actor.organisationId, claim.write_off_id, true)
			);
			const otherClaims = await activeClaimTotalsForWriteOff(
				trx,
				actor.organisationId,
				claim.write_off_id,
				claim.id
			...
			const availableWriteOffBasis =
				money(writeOff.amount) > recovery + otherClaims.basis
					? money(writeOff.amount) - recovery - otherClaims.basis
					: 0n;
			if (totals.basis > availableWriteOffBasis)
				throw new FinanceValidationError(
					`Prepared consideration now exceeds the available written-off amount of ${moneyText(availableWriteOffBasis)}.`
				);
			...
			for (const line of lines) {
				...
				const otherBasis = await activeClaimBasisForTaxSource(
					trx,
					actor.organisationId,
					line.sourceInvoiceItemId,
					line.taxCategoryId,
				...
				if (money(line.basisAmount) > sourceGross - otherBasis)
					throw new FinanceValidationError(
						'A source VAT line no longer has enough unclaimed consideration capacity.'
					);
				const expectedVat = vatForConsiderationBasis(
					line.basisAmount,
					line.taxableAmount,
					line.taxAmount
				);
				if (money(expectedVat) !== money(line.vatAmount))
					throw new FinanceValidationError(
						'Prepared VAT relief no longer matches the immutable source tax snapshot.'
					);
			}
			const authorisedAt = this.now();
			await trx
				.insertInto('receivable_vat_bad_debt_claim_authorisations')
				.values({
					claim_id: claim.id,
					organisation_id: actor.organisationId,
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: null,
				actionKey: 'finance.tax_relief.claim.authorised',
				subjectType: 'vat_bad_debt_claim',
				subjectPublicId: claimPublicId,
				correlationId: actor.correlationId,
				changeSummary: {
					considerationBasisAmount: moneyText(totals.basis),
					vatReliefAmount: moneyText(totals.vat),
	...
	async reverseClaim(
		...
		await this.db.transaction().execute(async (trx) => {
			const policy = new FinanceAccessPolicy(trx);
			const membership = await policy.assertActiveActor(actor, trx);
			if (!(await policy.mutationDecision(actor, 'finance.tax_relief.reverse', trx)).allowed)
				throw new TenantAccessError('VAT bad-debt relief reversal is not permitted.');
			const claim = await trx
				.selectFrom('receivable_vat_bad_debt_claims')
			...
			if (
				...
				throw new FinanceValidationError(
					'Only an authorised VAT bad-debt relief claim can be reversed.'
				);
			if (
				await trx
					.selectFrom('receivable_vat_bad_debt_claim_reversals')
				...
				throw new FinanceValidationError('The VAT bad-debt relief claim is already reversed.');
			if ((await activePostingsForSource(trx, actor.organisationId, { claimId: claim.id })).length)
				throw new FinanceValidationError(
					'Reverse active VAT-return posting evidence before reversing the relief claim.'
				);
			const activeRepayments = await activeRepaymentTotalsForClaim(
				trx,
				actor.organisationId,
				claim.id
			);
			if (activeRepayments.consideration > 0n)
				throw new FinanceValidationError(
					'Reverse active VAT relief repayment evidence before reversing the relief claim.'
				);
			const reversedAt = this.now();
			await trx
				.insertInto('receivable_vat_bad_debt_claim_reversals')
				.values({
					claim_id: claim.id,
					organisation_id: actor.organisationId,
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: null,
				actionKey: 'finance.tax_relief.claim.reversed',
				subjectType: 'vat_bad_debt_claim',
				subjectPublicId: claimPublicId,
				correlationId: actor.correlationId,
				changeSummary: { reason, reversedAt }
			});
		});
	}
	...
	async recordRepayment(
		...
		const considerationPaymentAmount = validateMoneyAmount(
			input.considerationPaymentAmount,
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const membership = await policy.assertActiveActor(actor, trx);
			if (
				!(await policy.mutationDecision(actor, 'finance.tax_relief.repayment.record', trx)).allowed
			)
				throw new TenantAccessError('VAT bad-debt relief repayment recording is not permitted.');
			const claim = await trx
				.selectFrom('receivable_vat_bad_debt_claims')
				.select(['id', 'write_off_id as writeOffId'])
				.where('organisation_id', '=', actor.organisationId)
				.where('public_id', '=', claimPublicId)
				.forUpdate()
				.executeTakeFirst();
			...
			if (
				...
				throw new FinanceValidationError('VAT repayment requires an authorised relief claim.');
			if (
				await trx
					.selectFrom('receivable_vat_bad_debt_claim_reversals')
					.select('claim_id')
				...
				throw new FinanceValidationError(
					'A reversed VAT relief claim cannot receive repayment evidence.'
				);
			const recovery = await trx
				...
				.select([
					'recovery.id',
					'recovery.write_off_id as writeOffId',
					'recovery.recovered_amount as amount',
					'reversal.recovery_id as reversedId'
				])
				.where('recovery.organisation_id', '=', actor.organisationId)
				.where('recovery.public_id', '=', recoveryPublicId)
				.forUpdate()
				.executeTakeFirst();
			if (!recovery || recovery.writeOffId !== claim.writeOffId)
				throw new RecordNotFoundError('Bad-debt recovery not found for this relief claim.');
			if (recovery.reversedId)
				throw new FinanceValidationError(
					'A reversed recovery cannot create VAT repayment evidence.'
				);
			const recoveryUsed = await activeRepaymentConsiderationForRecovery(
				trx,
				actor.organisationId,
				recovery.id
			);
			if (money(considerationPaymentAmount) > money(recovery.amount) - recoveryUsed)
				throw new FinanceValidationError(
					'Recovered consideration exceeds the unused amount of the selected bad-debt recovery.'
				);
			const totals = await claimTotals(trx, actor.organisationId, claim.id);
			...
			if (consideration > totals.basis - repaid.consideration)
				throw new FinanceValidationError(
					'Recovered consideration exceeds the remaining consideration covered by this VAT relief claim.'
				);
			...
			if (vatRepayment <= 0n)
				throw new FinanceValidationError('VAT repayment amount rounds to zero.');
			const publicId = this.publicIdFactory();
			await trx
				.insertInto('receivable_vat_bad_debt_repayments')
				.values({
					organisation_id: actor.organisationId,
					public_id: publicId,
					claim_id: claim.id,
					write_off_id: claim.writeOffId,
					recovery_id: recovery.id,
					consideration_payment_amount: considerationPaymentAmount,
					vat_repayment_amount: moneyText(vatRepayment),
					reason,
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: null,
				actionKey: 'finance.tax_relief.repayment.recorded',
				subjectType: 'vat_bad_debt_claim',
				subjectPublicId: claimPublicId,
				correlationId: actor.correlationId,
				changeSummary: {
					repaymentPublicId: publicId,
					recoveryPublicId,
					considerationPaymentAmount,
					vatRepaymentAmount: moneyText(vatRepayment),
					reason
				}
			});
			return { publicId };
	...
	async reverseRepayment(
		...
		await this.db.transaction().execute(async (trx) => {
			...
			const membership = await policy.assertActiveActor(actor, trx);
			if (
				!(await policy.mutationDecision(actor, 'finance.tax_relief.repayment.reverse', trx)).allowed
			)
				throw new TenantAccessError('VAT bad-debt relief repayment reversal is not permitted.');
			const repayment = await trx
				.selectFrom('receivable_vat_bad_debt_repayments')
				.select(['id', 'claim_id as claimId'])
				.where('organisation_id', '=', actor.organisationId)
				.where('public_id', '=', repaymentPublicId)
				.forUpdate()
				.executeTakeFirst();
			if (!repayment) throw new RecordNotFoundError('VAT bad-debt relief repayment not found.');
			if (
				await trx
					.selectFrom('receivable_vat_bad_debt_repayment_reversals')
					.select('repayment_id')
					.where('organisation_id', '=', actor.organisationId)
				...
				throw new FinanceValidationError('The VAT repayment is already reversed.');
			if (
				(await activePostingsForSource(trx, actor.organisationId, { repaymentId: repayment.id }))
					.length
			)
				throw new FinanceValidationError(
					'Reverse active VAT-return posting evidence before reversing the VAT repayment.'
				);
			const reversedAt = this.now();
			await trx
				.insertInto('receivable_vat_bad_debt_repayment_reversals')
				.values({
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: null,
				actionKey: 'finance.tax_relief.repayment.reversed',
				subjectType: 'vat_bad_debt_repayment',
				subjectPublicId: repaymentPublicId,
				correlationId: actor.correlationId,
				changeSummary: { reason, reversedAt }
			});
		});
	...
	async recordReturnPosting(
		...
		const vatReturnPeriodReference = cleanFinanceText(
			input.vatReturnPeriodReference,
			80,
			'VAT return period reference',
			true
		)!;
		const vatReturnPeriodStart = validateFinanceDate(
			input.vatReturnPeriodStart,
			'VAT return period start'
		);
		const vatReturnPeriodEnd = validateFinanceDate(
			input.vatReturnPeriodEnd,
			'VAT return period end'
		);
		if (!vatReturnPeriodStart || !vatReturnPeriodEnd || vatReturnPeriodEnd < vatReturnPeriodStart)
			throw new FinanceValidationError('VAT return period is invalid.');
		const externalReference = cleanFinanceText(
		...
		return this.db.transaction().execute(async (trx) => {
			const policy = new FinanceAccessPolicy(trx);
			const membership = await policy.assertActiveActor(actor, trx);
			if (!(await policy.mutationDecision(actor, 'finance.tax_relief.post', trx)).allowed)
			...
			if (input.sourceKind === 'relief_claim') {
				const claim = await trx
					.selectFrom('receivable_vat_bad_debt_claims')
					.select(['id', 'eligible_from as eligibleFrom', 'claim_deadline as claimDeadline'])
					.where('organisation_id', '=', actor.organisationId)
				...
				if (
					...
					throw new FinanceValidationError(
						'Only an authorised VAT bad-debt relief claim can be posted.'
					);
				if (
					await trx
						.selectFrom('receivable_vat_bad_debt_claim_reversals')
					...
					throw new FinanceValidationError('A reversed VAT relief claim cannot be posted.');
				if (vatReturnPeriodEnd < claim.eligibleFrom)
					throw new FinanceValidationError(
						'VAT return period ends before bad-debt relief becomes eligible.'
					);
				if (vatReturnPeriodEnd > claim.claimDeadline)
					throw new FinanceValidationError(
						'VAT return period ends after the recorded bad-debt relief claim deadline.'
					);
				claimId = claim.id;
				amount = (await claimTotals(trx, actor.organisationId, claim.id)).vat;
				vatReturnBox = 4;
				if ((await activePostingsForSource(trx, actor.organisationId, { claimId })).length)
					throw new FinanceValidationError(
						'This VAT relief claim already has active VAT-return posting evidence.'
					);
			} else {
				const repayment = await trx
					.selectFrom('receivable_vat_bad_debt_repayments')
					.select(['id', 'vat_repayment_amount as vatAmount'])
					.where('organisation_id', '=', actor.organisationId)
					.where('public_id', '=', sourcePublicId)
					.forUpdate()
					.executeTakeFirst();
				...
				if (
					...
					throw new FinanceValidationError('A reversed VAT repayment cannot be posted.');
				repaymentId = repayment.id;
				amount = money(repayment.vatAmount);
				vatReturnBox = 1;
				if ((await activePostingsForSource(trx, actor.organisationId, { repaymentId })).length)
					throw new FinanceValidationError(
						'This VAT repayment already has active VAT-return posting evidence.'
					);
			}
			if (amount <= 0n)
				throw new FinanceValidationError('VAT return posting amount must be positive.');
			const publicId = this.publicIdFactory();
			await trx
				.insertInto('receivable_vat_return_postings')
				.values({
					organisation_id: actor.organisationId,
					public_id: publicId,
					posting_kind: input.sourceKind,
					claim_id: claimId,
					repayment_id: repaymentId,
					vat_return_box: vatReturnBox,
					vat_return_period_reference: vatReturnPeriodReference,
					vat_return_period_start: vatReturnPeriodStart,
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: null,
				actionKey: 'finance.tax_relief.return_posting.recorded',
				subjectType: input.sourceKind,
				subjectPublicId: sourcePublicId,
				correlationId: actor.correlationId,
				changeSummary: {
					postingPublicId: publicId,
					vatReturnBox,
					vatReturnPeriodReference,
					vatReturnPeriodStart,
					vatReturnPeriodEnd,
					amount: moneyText(amount),
					externalReference,
					reason
	...
	async reverseReturnPosting(
		actor: TenantActorContext,
		input: { postingPublicId: string; reason: string }
	): Promise<void> {
		const postingPublicId = cleanPublicId(input.postingPublicId, 'VAT return posting ID');
		const reason = cleanFinanceText(
			input.reason,
			1000,
			'VAT return posting reversal reason',
			true
		)!;
		await this.db.transaction().execute(async (trx) => {
			const policy = new FinanceAccessPolicy(trx);
			const membership = await policy.assertActiveActor(actor, trx);
			if (!(await policy.mutationDecision(actor, 'finance.tax_relief.post.reverse', trx)).allowed)
				throw new TenantAccessError('VAT return posting reversal is not permitted.');
			const posting = await trx
				.selectFrom('receivable_vat_return_postings')
				.select(['id', 'posting_kind as postingKind', 'amount'])
				.where('organisation_id', '=', actor.organisationId)
				.where('public_id', '=', postingPublicId)
				.forUpdate()
				.executeTakeFirst();
			...
			if (
				...
				throw new FinanceValidationError('The VAT return posting is already reversed.');
			const reversedAt = this.now();
			await trx
				.insertInto('receivable_vat_return_posting_reversals')
				.values({
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: null,
				actionKey: 'finance.tax_relief.return_posting.reversed',
				subjectType: 'vat_return_posting',
				subjectPublicId: postingPublicId,
				correlationId: actor.correlationId,
				changeSummary: {

...
async function createOpenDayPeriod(actor: TenantActorContext, code: string): Promise<string> {
	...
	const year = await service.createFinancialYear(actor, {
		yearCode: code,
		name: `${code} year`,
		startsOn: '2026-08-18',
		endsOn: '2026-08-18'
...
beforeAll(async () => {
	...
	const taxCategoryId = insertedId(
		await db
			...
			.values({
				organisation_id: organisationAId,
				public_id: randomUUID(),
				code: 'VAT_ACCOUNTING_20',
				name: 'Accounting VAT 20%',
				treatment: 'taxable',
				is_active: 1
			})
			.executeTakeFirstOrThrow()
	...
	const itemId = insertedId(
		await db
			...
			.values({
				...
				line_number: 1,
				description: 'Accounting test invoice',
				quantity: '1.000000',
				unit_rate: '100.0000'
			})
			.executeTakeFirstOrThrow()
	...
	await db
		...
		.values({
			...
			issued_at: new Date('2026-08-18T10:00:00.000Z'),
			note: null
		})
		.executeTakeFirstOrThrow();
});
...
describe.sequential('Package 004L controlled accounting posting and export evidence', () => {
	it('keeps Finance/Commercial view-only while Owner configures typed semantic account mappings', async () => {
		...
		await expect(
			financeService.createAccount(actorFinanceA, {
				accountCode: '1100',
		...
		for (const [accountCode, name, accountType, mappingKey] of accountSpecs) {
			const account = await ownerService.createAccount(actorOwnerA, {
				accountCode,
				name,
				accountType
			});
			await ownerService.assignMapping(actorOwnerA, {
				mappingKey,
				accountPublicId: account.publicId,
				reason: 'Package 004L integration mapping.'
			});
		}
		const workspace = await ownerService.getWorkspace(actorOwnerA);
		expect(workspace.mappings).toHaveLength(7);
		const invoice = workspace.candidates.find(
			(candidate) =>
				candidate.sourceType === 'invoice_issue' && candidate.sourcePublicId === invoicePublicId
		);
	...
	it('honours explicit granular post deny above finance.manage then posts one balanced source-derived journal', async () => {
		...
		const permission = await db
			.selectFrom('permissions')
			.select('id')
			.where('permission_key', '=', 'finance.accounting.post')
			.executeTakeFirstOrThrow();
		await db
			...
			.values({
				...
				permission_id: permission.id,
				effect: 'deny',
				reason: 'Accounting integration explicit deny.'
			})
			.executeTakeFirstOrThrow();
		await expect(
			service.postSource(actorOwnerA, {
		...
		await db
			...
			.execute();
		const posted = await service.postSource(actorOwnerA, {
			sourceType: 'invoice_issue',
			sourcePublicId: invoicePublicId
		});
		...
		const journal = await db
			...
			.select(['id', 'source_amount as sourceAmount', 'source_fingerprint as fingerprint'])
			.where('organisation_id', '=', organisationAId)
		...
		const lines = await db
			...
			.select(['debit_amount as debitAmount', 'credit_amount as creditAmount'])
			.where('organisation_id', '=', organisationAId)
	...
	it('reverses journals additively, preserves the original and permits a controlled repost', async () => {
		const service = new AccountingService(db, randomUUID, () => NOW);
		const reversed = await service.reverseJournal(actorOwnerA, {
			journalPublicId,
			accountingDate: '2026-08-18',
			reason: 'Correct accounting mapping after review.'
		});
		reversalJournalPublicId = reversed.publicId;
		expect(reversed.journalNumber).toBe('JRN-000002');
		const original = await db
			.selectFrom('accounting_journal_entries')
			.select(['id', 'journal_number as journalNumber'])
			.where('organisation_id', '=', organisationAId)
			.where('public_id', '=', journalPublicId)
			.executeTakeFirstOrThrow();
		const reversalLink = await db
			.selectFrom('accounting_journal_entry_reversals')
			.select('reversal_journal_entry_id as reversalJournalId')
			.where('organisation_id', '=', organisationAId)
		...
		const reversalJournal = await db
			...
			.where('organisation_id', '=', organisationAId)
			.where('public_id', '=', reversalJournalPublicId)
			.executeTakeFirstOrThrow();
		...
		const reposted = await service.postSource(actorOwnerA, {
			sourceType: 'invoice_issue',
			sourcePublicId: invoicePublicId,
			memo: 'Repost after additive reversal.'
		});
	...
	it('creates checksum-backed generic CSV export evidence, prevents duplicate active export, then re-enables after additive export reversal', async () => {
		const service = new AccountingService(db, randomUUID, () => NOW);
		await new AccountingPeriodService(db, randomUUID, () => NOW).softClose(
			actorOwnerA,
			periodAPublicId,
			'Ready for accounting export.'
		);
		const created = await service.createExport(actorOwnerA, {
			periodStart: '2026-08-18',
			periodEnd: '2026-08-18',
			reason: 'Export posted accounting evidence for external review.'
		});
		...
		await expect(
			service.createExport(actorOwnerA, {
				periodStart: '2026-08-18',
				periodEnd: '2026-08-18',
				reason: 'No duplicate active export.'
			})
		).rejects.toBeInstanceOf(FinanceValidationError);
		await service.reverseExport(actorOwnerA, {
			exportPublicId,
			reason: 'External export was withdrawn before import.'
		});
		const replacement = await service.createExport(actorOwnerA, {
			periodStart: '2026-08-18',
			periodEnd: '2026-08-18',
			reason: 'Replacement export after explicit reversal.'
		});
	...
	it('masks accounting journal and export identities across tenants', async () => {
		...
		await expect(serviceB.getExportContent(actorOwnerB, exportPublicId)).rejects.toBeInstanceOf(
			RecordNotFoundError
		);
		await expect(
			serviceB.reverseJournal(actorOwnerB, {
				journalPublicId: repostJournalPublicId,
				accountingDate: '2026-08-18',
				reason: 'Foreign tenant must not see journal.'
			})

...
async function createCredit(
	...
	await db
		...
		.values({
			...
			reason: 'Historic account correction'
		})
		.executeTakeFirstOrThrow();
	const itemId = insertedId(
		await db
...
beforeAll(async () => {
	...
	foreignCustomerPublicId = (
		await createCustomer(organisationBId, ownerBMemberId, 'Foreign Client Ltd', 'AR-B-001')
	).publicId;
	salesItemTypeId = (
		await db
	...
	const invoiceOne = await createInvoice({
		number: 'INV-AR-001',
		currency: 'GBP',
		amount: '100.0000',
	...
	await createAllocation({
		...
		amount: '30.0000',
		allocatedAt: '2026-06-15T10:00:00.000Z',
		reference: 'PAY-AR-001',
		reversedAt: '2026-08-10T08:00:00.000Z'
	});
	const invoiceTwo = await createInvoice({
		number: 'INV-AR-002',
		currency: 'GBP',
		amount: '200.0000',
	...
	await createAllocation({
		...
		amount: '50.0000',
		allocatedAt: '2026-08-12T11:00:00.000Z',
		reference: 'PAY-AR-002'
	});
	await createInvoice({
		number: 'INV-AR-EUR',
		currency: 'EUR',
	...
	await createInvoice({
		...
		dueOn: '2026-05-20',
		voidedAt: '2026-06-20T10:00:00.000Z',
...
describe('Package 004F customer statements and aged receivables', () => {
	it('derives current aged receivables per currency without cross-currency aggregation', async () => {
		...
		const account = portfolio.accounts.find(
			(row) => row.customerPartyPublicId === customerPartyPublicId
		);
	...
	it('reconstructs a historical statement and historical aging before a later allocation reversal', async () => {
		const service = new ReceivablesReportingService(db, () => new Date('2026-08-17T12:00:00.000Z'));
		const workspace = await service.getCustomerStatement(actorFinanceA, customerPartyPublicId, {
			from: '2026-06-01',
			to: '2026-07-31'
		});
		const gbp = workspace.statements.find((statement) => statement.currencyCode === 'GBP');
		expect(gbp?.openingBalance).toBe('160.0000');
		expect(gbp?.movements.map((movement) => movement.kind)).toEqual([
			'credit_note',
			'payment_allocation',
			'invoice_void'
		]);
		...
		expect(gbpAging?.invoices[0]).toMatchObject({
			invoicePublicId: invoiceOnePublicId,
	...
	it('carries opening balances into the current period and reconciles closing balance to current aging', async () => {
		...
		expect(
			workspace.aging.find((position) => position.currencyCode === 'GBP')?.totalOutstanding
		).toBe('230.0000');
	});
	...
	it('masks foreign-tenant customer accounts and validates statement periods', async () => {
		const service = new ReceivablesReportingService(db, () => new Date('2026-08-17T12:00:00.000Z'));
		await expect(
			service.getCustomerStatement(actorFinanceA, foreignCustomerPublicId)
		).rejects.toBeInstanceOf(RecordNotFoundError);
		await expect(
			service.getCustomerStatement(actorOwnerB, customerPartyPublicId)
		).rejects.toBeInstanceOf(RecordNotFoundError);
		await expect(
			service.getCustomerStatement(actorFinanceA, customerPartyPublicId, {
				from: '2026-08-18',
				to: '2026-08-17'
			})

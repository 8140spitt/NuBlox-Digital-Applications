...
beforeAll(async () => {
	...
	const year = await periodService.createFinancialYear(actor, {
		yearCode: 'FY26-CON',
		name: 'FY26 Concurrency',
		startsOn: '2026-08-18',
		endsOn: '2026-08-18'
	});
	await periodService.createPeriod(actor, {
	...
	const ar = await service.createAccount(actor, {
		accountCode: '1100',
		name: 'Trade Receivables',
		accountType: 'asset'
	});
	const revenue = await service.createAccount(actor, {
		accountCode: '4000',
		name: 'Sales Revenue',
		accountType: 'revenue'
	});
	await service.assignMapping(actor, {
		mappingKey: 'accounts_receivable',
		accountPublicId: ar.publicId,
	...
	const invoiceId = insertedId(
		await db
			...
			.values({
				...
				document_number: 'INV-ACC-CON-001',
	...
	await db
		...
		.values({
			...
			line_number: 1,
			description: 'Concurrency source',
			quantity: '1.000000',
			unit_rate: '25.0000'
		})
		.executeTakeFirstOrThrow();
...
afterAll(async () => {
	if (db && organisationId) {
		await db
			.deleteFrom('accounting_export_reversals')
		...
		await db
			...
			.execute();
		await db
			.deleteFrom('accounting_export_batches')
			.where('organisation_id', '=', organisationId)
			.execute();
		await db
			...
			.execute();
		await db
			.deleteFrom('accounting_journal_lines')
			.where('organisation_id', '=', organisationId)
			.execute();
		await db
			.deleteFrom('accounting_journal_entries')
			.where('organisation_id', '=', organisationId)
			.execute();
		await db
			.deleteFrom('accounting_account_mappings')
			.where('organisation_id', '=', organisationId)
			.execute();
		await db
			.deleteFrom('accounting_accounts')
			.where('organisation_id', '=', organisationId)
			.execute();
		await db
...
describe('Package 004L accounting posting concurrency', () => {
	it('serialises two attempts so only one active journal is created for the same source event', async () => {
		const first = new AccountingService(db, randomUUID, () => NOW).postSource(actor, {
			sourceType: 'invoice_issue',
			sourcePublicId: invoicePublicId
		});
		const second = new AccountingService(db, randomUUID, () => NOW).postSource(
			{ ...actor, correlationId: randomUUID() },
			{ sourceType: 'invoice_issue', sourcePublicId: invoicePublicId }
		);

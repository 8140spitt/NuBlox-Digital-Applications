...
async function deriveSnapshot(
	...
	const periods = await db
		...
		.execute();
	if (periods.length === 0)
		throw new FinanceValidationError('The financial year has no configured accounting periods.');
	if (periods.some((period) => period.status !== 'hard_closed')) {
		throw new FinanceValidationError(
			'Every accounting period in the financial year must be hard closed before year-end close.'
		);
	}
	if (
		periods[0]!.startsOn.getTime() !== year.startsOn.getTime() ||
		periods.at(-1)!.endsOn.getTime() !== year.endsOn.getTime()
	) {
		throw new FinanceValidationError(
			'Accounting periods must cover the complete financial year before year-end close.'
		);
	}
	for (let index = 1; index < periods.length; index += 1) {
		...
		if (expected.getTime() !== current.startsOn.getTime()) {
			throw new FinanceValidationError(
				'Accounting periods must cover the financial year without gaps before year-end close.'
			);
		}
	}
	...
	if (rows.length === 0)
		throw new FinanceValidationError(
			'No revenue or expense journal movement exists for this financial year and currency.'
		);
	...
	for (const [accountId, account] of [...accounts.entries()].sort((a, b) =>
		a[1].code.localeCompare(b[1].code)
	)) {
		const natural =
			account.type === 'revenue' ? account.credit - account.debit : account.debit - account.credit;
		if (natural === 0n) continue;
		if (account.type === 'revenue') revenueTotal += natural;
		else expenseTotal += natural;
		const debit =
			account.type === 'revenue' ? (natural > 0n ? natural : 0n) : natural < 0n ? -natural : 0n;
		const credit =
			account.type === 'expense' ? (natural > 0n ? natural : 0n) : natural < 0n ? -natural : 0n;
	...
	const fingerprintPayload = {
		...
		periods: periods.map((period) => [
			...
			period.startsOn.toISOString().slice(0, 10),
			period.endsOn.toISOString().slice(0, 10),
			period.status
		]),
...
export class AccountingYearEndService {
	...
	private async assertView(actor: TenantActorContext): Promise<void> {
		...
		if (!financeView.allowed || !accountingView.allowed)
			throw new TenantAccessError('Year-end accounting viewing is not permitted.');
	}

	async getWorkspace(actor: TenantActorContext) {
		await this.assertView(actor);
		const permissions = new PermissionService(this.db);
		const [prepareDecision, authoriseDecision, reverseDecision, years, preparations, closes] =
			await Promise.all([
				permissions.decideWithUmbrella(
					actor,
				...
				this.db
					...
					.select([
						'close.public_id as publicId',
						'close.financial_year_id as financialYearId',
						'close.preparation_id as preparationId',
						'close.currency_code as currencyCode',
						'close.close_sequence as closeSequence',
						'close.authorised_at as authorisedAt',
						'reversal.reversed_at as reversedAt'
					])
					.where('close.organisation_id', '=', actor.organisationId)
					.orderBy('close.authorised_at', 'desc')
					.limit(100)
					.execute()
			]);
		return {
	...
	async prepare(
		...
		input: { financialYearPublicId: string; currencyCode: string; reason: string }
	) {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (
				!(
					await new PermissionService(trx).decideWithUmbrella(
						actor,
						'finance.accounting.year_end.prepare',
						'finance.manage'
					)
				...
				throw new TenantAccessError('Year-end close preparation is not permitted.');
			await lockOrganisation(trx, actor.organisationId);
			const year = await trx
				.selectFrom('accounting_financial_years')
				.select(['id', 'public_id as publicId', 'starts_on as startsOn', 'ends_on as endsOn'])
				.where('organisation_id', '=', actor.organisationId)
			...
			const active = await trx
				...
				.where('close.currency_code', '=', currencyCode)
				.where('reversal.year_end_close_id', 'is', null)
				.forUpdate()
				.executeTakeFirst();
			if (active)
				throw new FinanceValidationError(
					'An active authorised year-end close already exists for this financial year and currency.'
				);
			const retained = await trx
				...
				.where('mapping.mapping_key', '=', 'retained_earnings')
				.forUpdate()
				.executeTakeFirst();
			if (!retained || retained.accountType !== 'equity' || retained.isActive !== 1)
				throw new FinanceValidationError(
					'An active equity account must be mapped to retained earnings before year-end close.'
				);
			const snapshot = await deriveSnapshot(trx, actor.organisationId, year, currencyCode);
			const prior = await trx
				.selectFrom('accounting_year_end_close_preparations')
				.select('preparation_sequence as sequence')
				.where('organisation_id', '=', actor.organisationId)
				.where('financial_year_id', '=', year.id)
				.where('currency_code', '=', currencyCode)
				.orderBy('preparation_sequence', 'desc')
				.forUpdate()
				.executeTakeFirst();
			const publicId = this.publicIdFactory();
			await trx
				.insertInto('accounting_year_end_close_preparations')
				.values({
					organisation_id: actor.organisationId,
					public_id: publicId,
					financial_year_id: year.id,
					preparation_sequence: Number(prior?.sequence ?? 0) + 1,
					currency_code: currencyCode,
					revenue_total: moneyText(snapshot.revenueTotal),
					expense_total: moneyText(snapshot.expenseTotal),
					profit_loss_amount: moneyText(snapshot.profitLoss),
					closing_debit_total: moneyText(snapshot.closingDebitTotal),
					closing_credit_total: moneyText(snapshot.closingCreditTotal),
			...
			await new AuditRepository(trx).append({
				...
				projectId: null,
				actionKey: 'finance.accounting.year_end.prepared',
				subjectType: 'accounting_year_end_close_preparation',
				subjectPublicId: publicId,
				correlationId: actor.correlationId,
				changeSummary: {
					financialYearPublicId,
					currencyCode,
					revenueTotal: moneyText(snapshot.revenueTotal),
					expenseTotal: moneyText(snapshot.expenseTotal),
					profitLossAmount: moneyText(snapshot.profitLoss),
	...
	async authorise(
		actor: TenantActorContext,
		input: { preparationPublicId: string; reason: string }
	) {
		const preparationPublicId = cleanFinanceText(
			input.preparationPublicId,
			64,
			'Preparation ID',
			true
		)!;
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (
				...
				throw new TenantAccessError('Year-end close authorisation is not permitted.');
			await lockOrganisation(trx, actor.organisationId);
			const preparation = await trx
				.selectFrom('accounting_year_end_close_preparations as preparation')
				.innerJoin('accounting_financial_years as year', (join) =>
					join
						.onRef('year.id', '=', 'preparation.financial_year_id')
						.onRef('year.organisation_id', '=', 'preparation.organisation_id')
				)
				.select([
					'preparation.id',
					'preparation.public_id as publicId',
					'preparation.financial_year_id as financialYearId',
					'preparation.currency_code as currencyCode',
					'preparation.source_fingerprint as sourceFingerprint',
					'preparation.prepared_by_member_id as preparedByMemberId',
					'year.public_id as financialYearPublicId',
					'year.starts_on as startsOn',
					'year.ends_on as endsOn'
				])
				.where('preparation.organisation_id', '=', actor.organisationId)
				.where('preparation.public_id', '=', preparationPublicId)
				.forUpdate()
				.executeTakeFirst();
			if (!preparation) throw new RecordNotFoundError('Year-end close preparation not found.');
			if (preparation.preparedByMemberId === membership.id)
				throw new FinanceValidationError(
					'The member who prepared a year-end close cannot authorise the same preparation.'
				);
			if (
				await trx
					.selectFrom('accounting_year_end_closes')
					.select('id')
					.where('organisation_id', '=', actor.organisationId)
				...
				throw new FinanceValidationError('This year-end preparation has already been authorised.');
			const active = await trx
				.selectFrom('accounting_year_end_closes as close')
				.leftJoin('accounting_year_end_close_reversals as reversal', (join) =>
					join
				...
				.select('close.id')
				.where('close.organisation_id', '=', actor.organisationId)
				.where('close.financial_year_id', '=', preparation.financialYearId)
				.where('close.currency_code', '=', preparation.currencyCode)
				.where('reversal.year_end_close_id', 'is', null)
				.forUpdate()
				.executeTakeFirst();
			if (active)
			...
			if (!retained || retained.accountType !== 'equity' || retained.isActive !== 1)
				throw new FinanceValidationError(
					'An active equity account must be mapped to retained earnings before authorisation.'
				);
			const snapshot = await deriveSnapshot(
				trx,
				actor.organisationId,
				{
					id: preparation.financialYearId,
					startsOn: preparation.startsOn,
					endsOn: preparation.endsOn
				},
				preparation.currencyCode
			);
			if (snapshot.fingerprint !== preparation.sourceFingerprint)
				throw new FinanceValidationError(
					'The preparation is stale because governed accounting evidence changed; prepare the year-end close again.'
				);
			...
			const journalResult = await trx
				...
				.values({
					organisation_id: actor.organisationId,
					public_id: journalPublicId,
					journal_number: journalNumber,
					source_type: 'year_end_close',
					source_public_id: preparation.publicId,
					source_event_at: this.now(),
					source_amount: moneyText(snapshot.closingDebitTotal),
					source_fingerprint: snapshot.fingerprint,
					accounting_date: preparation.endsOn,
					currency_code: preparation.currencyCode,
					memo: `Year-end close ${preparation.financialYearPublicId}: ${reason}`,
					posted_by_member_id: membership.id,
					posted_at: this.now()
				})
				.executeTakeFirstOrThrow();
			const journalId = insertedId(journalResult);
			const journalLines = snapshot.lines.map((line, index) => ({
				organisation_id: actor.organisationId,
				journal_entry_id: journalId,
				accounting_account_id: line.accountId,
				line_number: index + 1,
				description: `Year-end close ${line.accountCode} · ${line.accountName}`,
				debit_amount: moneyText(line.debit),
				credit_amount: moneyText(line.credit)
			}));
			journalLines.push({
				organisation_id: actor.organisationId,
				journal_entry_id: journalId,
				accounting_account_id: retained.id,
				line_number: journalLines.length + 1,
				description: `Year-end retained earnings · ${retained.accountCode} · ${retained.name}`,
				debit_amount: moneyText(snapshot.profitLoss < 0n ? -snapshot.profitLoss : 0n),
				credit_amount: moneyText(snapshot.profitLoss > 0n ? snapshot.profitLoss : 0n)
			});
			await trx.insertInto('accounting_journal_lines').values(journalLines).execute();
			const prior = await trx
				.selectFrom('accounting_year_end_closes')
				.select('close_sequence as sequence')
				.where('organisation_id', '=', actor.organisationId)
				.where('financial_year_id', '=', preparation.financialYearId)
				.where('currency_code', '=', preparation.currencyCode)
				.orderBy('close_sequence', 'desc')
				.forUpdate()
				.executeTakeFirst();
			const closePublicId = this.publicIdFactory();
			await trx
				.insertInto('accounting_year_end_closes')
				.values({
					organisation_id: actor.organisationId,
					public_id: closePublicId,
					financial_year_id: preparation.financialYearId,
					preparation_id: preparation.id,
					close_sequence: Number(prior?.sequence ?? 0) + 1,
					currency_code: preparation.currencyCode,
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: null,
				actionKey: 'finance.accounting.year_end.authorised',
				subjectType: 'accounting_year_end_close',
				subjectPublicId: closePublicId,
				correlationId: actor.correlationId,
				changeSummary: {
					preparationPublicId,
					journalPublicId,
	...
	async reverse(actor: TenantActorContext, input: { closePublicId: string; reason: string }) {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (
				...
				throw new TenantAccessError('Year-end close reversal is not permitted.');
			await lockOrganisation(trx, actor.organisationId);
			const close = await trx
				.selectFrom('accounting_year_end_closes as close')
				.innerJoin('accounting_journal_entries as journal', (join) =>
					join
						.onRef('journal.id', '=', 'close.closing_journal_entry_id')
						.onRef('journal.organisation_id', '=', 'close.organisation_id')
				)
				.select([
					'close.id',
					'close.public_id as publicId',
				...
				.where('close.organisation_id', '=', actor.organisationId)
				.where('close.public_id', '=', closePublicId)
				.forUpdate()
				.executeTakeFirst();
			if (!close) throw new RecordNotFoundError('Year-end close not found.');
			if (
				await trx
					.selectFrom('accounting_year_end_close_reversals')
					.select('year_end_close_id')
					.where('organisation_id', '=', actor.organisationId)
					.where('year_end_close_id', '=', close.id)
					.forUpdate()
					.executeTakeFirst()
			)
				throw new FinanceValidationError('The year-end close is already reversed.');
			const lines = await trx
				...
				.where('organisation_id', '=', actor.organisationId)
				.where('journal_entry_id', '=', close.journalId)
				.orderBy('line_number')
				.forUpdate()
				.execute();
			const journalNumber = await nextJournalNumber(trx, actor.organisationId);
			const journalPublicId = this.publicIdFactory();
			const reversalFingerprint = createHash('sha256')
				.update(
					JSON.stringify({ closePublicId, sourceFingerprint: close.sourceFingerprint, reason })
				)
				.digest('hex');
			const result = await trx
				...
				.values({
					...
					source_public_id: close.journalPublicId,
					source_event_at: this.now(),
					source_amount: close.sourceAmount,
					source_fingerprint: reversalFingerprint,
					accounting_date: close.accountingDate,
					currency_code: close.currencyCode,
					memo: `Reverse ${close.journalNumber}: ${reason}`,
					posted_by_member_id: membership.id,
					posted_at: this.now()
				})
				.executeTakeFirstOrThrow();
			...
			await trx
				...
				.values(
					lines.map((line) => ({
						...
						accounting_account_id: line.accountId,
						line_number: Number(line.lineNumber),
						description: `Reverse: ${line.description}`,
						debit_amount: line.creditAmount,
						credit_amount: line.debitAmount
					}))
				)
				.execute();
			await trx
				.insertInto('accounting_year_end_close_reversals')
				.values({
					year_end_close_id: close.id,
					organisation_id: actor.organisationId,
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: null,
				actionKey: 'finance.accounting.year_end.reversed',
				subjectType: 'accounting_year_end_close',
				subjectPublicId: close.publicId,
				correlationId: actor.correlationId,
				changeSummary: {
					originalJournalPublicId: close.journalPublicId,
					reversalJournalPublicId: journalPublicId,

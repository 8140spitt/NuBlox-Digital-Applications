...
import { lineAmount, percentageAmount, sumMoney } from '$lib/server/commercial/commercial-decimal';
import {
...
export class InvoiceService {
	...
	private async contractViewAllowed(
	...
	private async partyFacts(
		db: DatabaseExecutor,
		organisationId: string,
		partyId: string
	): Promise<PartyFacts | null> {
		const row = await db
	...
	private async primaryBillingContactId(
		db: DatabaseExecutor,
		organisationId: string,
		customerPartyId: string
	): Promise<string | null> {
	...
	private async currentContractValue(
		db: DatabaseExecutor,
		organisationId: string,
		contractId: string
	): Promise<string> {
		const baseline = await db
			...
			.executeTakeFirst();
		if (!baseline)
			throw new FinanceValidationError('An executed contract baseline is required for invoicing.');
		const values = await db
			.selectFrom('contract_version_value_components')
			.select('amount')
			.where('organisation_id', '=', organisationId)
			.where('contract_version_id', '=', baseline.id)
			.execute();
	...
	private async contractCandidates(actor: TenantActorContext): Promise<EligibleInvoiceContract[]> {
		...
		const rows = await this.db
			...
			.innerJoin('contract_versions as version', (join) =>
				join
					.onRef('version.contract_id', '=', 'contract.id')
					.onRef('version.organisation_id', '=', 'contract.organisation_id')
					.on('version.version_status', '=', 'executed')
			)
			.innerJoin('contract_version_parties as client', (join) =>
				join
					.onRef('client.contract_version_id', '=', 'version.id')
					.onRef('client.organisation_id', '=', 'contract.organisation_id')
			)
			.innerJoin('contract_party_role_types as role', (join) =>
				join
					.onRef('role.id', '=', 'client.contract_party_role_type_id')
					.on('role.code', '=', 'client')
			)
		...
		for (const row of unique.values()) {
			result.push({
				...
				currentContractValue: await this.currentContractValue(
					this.db,
	...
	private async invoiceRecord(
		...
		let query = db
			...
			.innerJoin('parties as customer', (join) =>
				join
					.onRef('customer.id', '=', 'document.customer_party_id')
					.onRef('customer.organisation_id', '=', 'document.organisation_id')
			)
			.leftJoin('party_persons as person', (join) =>
				join
					.onRef('person.party_id', '=', 'customer.id')
					.onRef('person.organisation_id', '=', 'customer.organisation_id')
			)
			.leftJoin('party_organisations as company', (join) =>
				join
					.onRef('company.party_id', '=', 'customer.id')
					.onRef('company.organisation_id', '=', 'customer.organisation_id')
			)
			.leftJoin('contracts as contract', (join) =>
				join
					.onRef('contract.id', '=', 'document.contract_id')
					.onRef('contract.organisation_id', '=', 'document.organisation_id')
			)
			.leftJoin('projects as project', (join) =>
				join
			...
			.leftJoin('payment_terms as term', (join) =>
				join
					.onRef('term.id', '=', 'invoice.payment_term_id')
					.onRef('term.organisation_id', '=', 'document.organisation_id')
			)
			.leftJoin('party_billing_settings as settings', (join) =>
				join
					.onRef('settings.party_id', '=', 'document.customer_party_id')
					.onRef('settings.organisation_id', '=', 'document.organisation_id')
			)
			.select([
				'document.id as id',
				...
				'customer.public_id as customerPartyPublicId',
				...
				'invoice.customer_purchase_order_reference as customerPurchaseOrderReference',
				'term.public_id as paymentTermPublicId',
				'term.name as paymentTermName',
				'term.calculation_basis as paymentTermBasis',
				'term.days_offset as paymentTermDaysOffset',
	...
	async createFromContract(
		...
		const purchaseOrderReference = cleanFinanceText(
			input.customerPurchaseOrderReference,
			160,
		...
		await this.db.transaction().execute(async (trx) => {
			...
			if (!createDecision.allowed)
				throw new TenantAccessError('Invoice creation is not permitted.');
			if (!(await this.contractViewAllowed(actor, trx)))
				throw new TenantAccessError('Contract viewing is required to create a contract invoice.');
			...
			if (contract.lifecycleStatus !== 'active')
				throw new FinanceValidationError('Only an active executed contract can be invoiced.');
			...
			if (!version)
				throw new FinanceValidationError(
					'An executed contract baseline is required before invoicing.'
				);
			const client = await trx
				.selectFrom('contract_version_parties as client')
				.innerJoin(
					'contract_party_role_types as role',
					'role.id',
					'client.contract_party_role_type_id'
				)
				.select('client.source_party_id as customerPartyId')
				.where('client.organisation_id', '=', actor.organisationId)
				.where('client.contract_version_id', '=', version.id)
				.where('role.code', '=', 'client')
				.orderBy('client.sort_order', 'asc')
				.executeTakeFirst();
			if (!client)
				throw new FinanceValidationError(
					'The executed contract has no client party available for invoicing.'
				);
			const billingSettings = await trx
				.selectFrom('party_billing_settings')
				.select(['default_payment_term_id as defaultPaymentTermId'])
				.where('organisation_id', '=', actor.organisationId)
				.where('party_id', '=', client.customerPartyId)
				.executeTakeFirst();
			...
			const billingContactPartyId = await this.primaryBillingContactId(
				trx,
				actor.organisationId,
				client.customerPartyId
			);
			...
			await new AuditRepository(trx).append({
				...
				actionKey: 'finance.invoice.created_from_contract',
				subjectType: 'invoice',
	...
	async getWorkspace(
		...
		if (record.contractId) {
			contractCurrentValue = await this.currentContractValue(
				this.db,
				actor.organisationId,
				record.contractId
			);
			const otherIssued = await this.db
				.selectFrom('financial_documents')
				.select('id')
				.where('organisation_id', '=', actor.organisationId)
				.where('document_kind', '=', 'invoice')
	...
	private async lockDraft(
		...
		if (record.lifecycleStatus !== 'draft')
			throw new FinanceValidationError('Issued invoices are immutable through draft APIs.');
		return record;
	}
	...
	async updateDraft(
		...
		await this.db.transaction().execute(async (trx) => {
			...
			if (!decision.allowed)
				throw new TenantAccessError('Invoice draft management is not permitted.');
			...
			await new AuditRepository(trx).append({
				...
				actionKey: 'finance.invoice.draft.updated',
				subjectType: 'invoice',
				subjectPublicId: record.publicId,
				correlationId: actor.correlationId,
				changeSummary: {
					invoiceType: selectedInvoiceType,
					paymentTermPublicId: termPublicId || null,
					dueDate,
					customerPurchaseOrderReference: purchaseOrderReference
				}
			});
		});
	}
	...
	async addLine(
		...
		const taxCategoryPublicId = cleanFinanceText(
			input.taxCategoryPublicId,
			64,
			'Tax category',
			true
		)!;
		await this.db.transaction().execute(async (trx) => {
			...
			const category = await new CommercialRepository(trx).resolveTaxCategory(
				actor.organisationId,
				taxCategoryPublicId,
				this.now()
			);
			...
			await trx
				...
				.values({
					...
					taxable_amount: net,
					tax_amount: percentageAmount(net, appliedRate)
				})
				.executeTakeFirstOrThrow();
			await new AuditRepository(trx).append({
				eventPublicId: this.publicIdFactory(),
				...
				actorMemberId: membership.id,
				projectId: record.projectId,
				actionKey: 'finance.invoice.line.added',
				subjectType: 'invoice',
				subjectPublicId: record.publicId,
				correlationId: actor.correlationId,
				changeSummary: {
	...
	async removeLine(
		...
		if (!Number.isSafeInteger(lineNumber) || lineNumber <= 0)
			throw new FinanceValidationError('Invoice line is invalid.');
		await this.db.transaction().execute(async (trx) => {
			const policy = new FinanceAccessPolicy(trx);
			const membership = await policy.assertActiveActor(actor, trx);
			const decision = await policy.mutationDecision(actor, 'finance.invoice.draft.manage', trx);
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: record.projectId,
				actionKey: 'finance.invoice.line.removed',
				subjectType: 'invoice',
				subjectPublicId: record.publicId,
				correlationId: actor.correlationId,
	...
	private async dueDateForIssue(
		trx: DatabaseExecutor,
		organisationId: string,
		record: Awaited<ReturnType<InvoiceService['invoiceRecord']>>,
		issuedAt: Date
	): Promise<Date> {
		...
		if (!record.paymentTermPublicId || !record.paymentTermBasis) {
			if (!record.dueDate)
				throw new FinanceValidationError(
					'A due date or active payment term is required before invoice issue.'
				);
			if (record.dueDate < issueDate)
				throw new FinanceValidationError('Invoice due date cannot be before the issue date.');
		...
		if (record.paymentTermBasis === 'end_of_month')
			return addUtcDays(endOfUtcMonth(issueDate), offset);
		if (record.paymentTermBasis === 'manual') {
			if (!record.dueDate)
				throw new FinanceValidationError(
					'Manual payment terms require an explicit due date before issue.'
				);
			if (record.dueDate < issueDate)
				throw new FinanceValidationError('Invoice due date cannot be before the issue date.');
			return record.dueDate;
	...
	private async refreshTaxesForIssue(
		trx: DatabaseExecutor,
		organisationId: string,
		documentId: string,
		issuedAt: Date
		...
		const rows = await trx
			...
			.select([
				'tax.financial_document_item_id as itemId',
				'tax.sort_order as sortOrder',
				'category.public_id as categoryPublicId',
				'item.quantity as quantity',
				'item.unit_rate as unitRate'
			])
			.where('tax.organisation_id', '=', organisationId)
		...
		for (const row of rows) {
			...
			if (!category)
				throw new FinanceValidationError(
					'An invoice tax category is no longer available at issue time.'
				);
			const rate = taxRate(category);
			const taxableAmount = lineAmount(row.quantity, row.unitRate);
			await trx
				.updateTable('financial_document_item_taxes')
				.set({
					applied_rate_percent: rate,
					taxable_amount: taxableAmount,
					tax_amount: percentageAmount(taxableAmount, rate)
				})
				.where('organisation_id', '=', organisationId)
				.where('financial_document_item_id', '=', row.itemId)
				.where('sort_order', '=', row.sortOrder)
				.executeTakeFirstOrThrow();
		}
	}

	private async insertPartySnapshot(
		trx: DatabaseExecutor,
		organisationId: string,
		documentId: string,
		facts: PartyFacts,
		...
		if (facts.address) {
			await trx
				...
				.values({
					...
					line_1: facts.address.line1,
					line_2: facts.address.line2,
					line_3: facts.address.line3,
					locality: facts.address.locality,
					city: facts.address.city,
					region: facts.address.region,
					postal_code: facts.address.postalCode,
	...
	async issue(
		...
		if (!FINANCE_DELIVERY_CHANNELS.has(deliveryChannel))
			throw new FinanceValidationError('Invoice delivery channel is invalid.');
		...
		await this.db.transaction().execute(async (trx) => {
			...
			const items = await trx
				...
				.execute();
			if (items.length === 0)
				throw new FinanceValidationError('Add at least one invoice line before issue.');
			if (record.purchaseOrderRequired === 1 && !record.customerPurchaseOrderReference) {
				throw new FinanceValidationError(
					'This customer requires a purchase order/reference before invoice issue.'
				);
			}
			...
			const customerFacts = await this.partyFacts(
				trx,
				actor.organisationId,
			...
			if (record.billingContactPartyId && record.billingContactPartyId !== record.customerPartyId) {
				billingFacts = await this.partyFacts(
					trx,
					actor.organisationId,
					record.billingContactPartyId
				);
			}
			await this.insertPartySnapshot(
				trx,
				actor.organisationId,
				record.id,
				customerFacts,
				'customer',
				record.customerAccountReference ?? null,
				1
			);
			if (billingFacts)
				await this.insertPartySnapshot(
			...
			await trx
				...
				.values({
					...
					financial_document_id: record.id,
					source_party_id: recipientFacts.id,
			...
			await new AuditRepository(trx).append({
				...
				actorMemberId: membership.id,
				projectId: record.projectId,
				actionKey: 'finance.invoice.issued',
				subjectType: 'invoice',
				subjectPublicId: record.publicId,
				correlationId: actor.correlationId,
				changeSummary: {

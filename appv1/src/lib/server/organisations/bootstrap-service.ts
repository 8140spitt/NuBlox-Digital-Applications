...
const STANDARD_ROLES = [
	{
		...
		permissionKeys: [...OWNER_CORE_KEYS, ...OWNER_FINANCE_KEYS]
	},
	{
		name: 'Administrator',
	...
	},
	{
		name: 'Manager',
		description:
			'Manages ordinary members, invitations, projects and CRM within delegated authority.',
		permissionKeys: [
			'member.invite',
			'member.manage',
			'project.create',
...
function bootstrapSigningKey(): string {
	...
	if (!secret)
		throw new Error('BETTER_AUTH_SECRET is required for organisation bootstrap signing.');
...
export class OrganisationBootstrapService {
	...
	async provisionSignup(input: {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			await trx
				...
				.values({ user_id: userId, email, is_primary: 1, is_verified: 0, verified_at: null })
				.executeTakeFirstOrThrow();
			await trx
	...
	async activateVerifiedAuthUser(input: {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (pending.length !== 1)
				throw new OrganisationBootstrapAccessError(
					'Multiple pending organisation bootstrap records were found.'
				);
	...
	async createForExistingUser(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (!user || user.status !== 'active')
				throw new OrganisationBootstrapAccessError(
					'Only an active NuBlox user can create an organisation.'
				);
			return this.createOrganisationRecords(trx, {
				userId: actor.userId,
				details,

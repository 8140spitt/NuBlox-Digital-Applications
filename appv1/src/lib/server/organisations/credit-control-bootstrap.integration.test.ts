...
const PREFIX = 'Credit Control Bootstrap Integration ';
...
async function permissionKeys(roleName: string): Promise<string[]> {
	const rows = await db
		...
		.where('permission.permission_key', 'like', 'finance.credit_control.%')
...
describe('Package 004I future organisation bootstrap parity', () => {
	it('persists the intended Owner/Admin vs Finance/Commercial credit-control authority split', async () => {
		const created = await new OrganisationBootstrapService(db).createForExistingUser(
			{ userId, correlationId: `credit-control-bootstrap-${randomUUID()}` },
			{
		...
		await expect(permissionKeys('Owner')).resolves.toEqual(allFour);
		await expect(permissionKeys('Administrator')).resolves.toEqual(allFour);

...
describe('CRM opportunities and activity timeline', () => {
	...
	it('creates a tenant-owned opportunity with a primary prospective customer and pipeline stage', async () => {
		...
		await expect(
			new CrmOpportunityService(db).createOpportunity(actorViewerA, {
				title: 'Denied',
				pipelinePublicId: pipelineAPublicId,
				stageName: 'Lead',
				primaryPartyPublicId: client.publicId
	...
	it('rejects cross-tenant customer and pipeline references and masks direct opportunity IDs', async () => {
		const clientB = await new CrmService(db).createParty(actorManagerB, {
			kind: 'organisation',
			legalName: `${PREFIX}Client B`,
			roleCodes: ['prospect']
		});
		await expect(
			new CrmOpportunityService(db).createOpportunity(actorManagerA, {
				title: 'Cross tenant customer',
				pipelinePublicId: pipelineAPublicId,
				stageName: 'Lead',
				primaryPartyPublicId: clientB.publicId
			})
		).rejects.toBeInstanceOf(RecordNotFoundError);
		const ownClient = await new CrmService(db).createParty(actorManagerA, {
			kind: 'organisation',
			legalName: `${PREFIX}Client A2`,
			roleCodes: ['prospect']
		});
		await expect(
			new CrmOpportunityService(db).createOpportunity(actorManagerA, {
				title: 'Cross tenant pipeline',
				pipelinePublicId: pipelineBPublicId,
				stageName: 'Lead',
				primaryPartyPublicId: ownClient.publicId
			})
		).rejects.toBeInstanceOf(CrmOpportunityValidationError);
		const foreignOpportunity = await new CrmOpportunityService(db).createOpportunity(
			actorManagerB,
			{
				title: `${PREFIX}Foreign Opportunity`,
				pipelinePublicId: pipelineBPublicId,
				stageName: 'Lead',
				primaryPartyPublicId: clientB.publicId
			}
		...
		await expect(
			new CrmOpportunityService(db).getWorkspace(actorManagerA, foreignOpportunity.publicId)
		).rejects.toBeInstanceOf(RecordNotFoundError);
	});
	...
	it('updates stage, outcome and primary customer while retaining participant identity', async () => {
		...
		const client = await crm.createParty(actorManagerA, {
			kind: 'organisation',
			legalName: `${PREFIX}Outcome Client`,
			roleCodes: ['prospect']
		});
		...
		expect(
			participants.participants.some(
				(row) => row.partyPublicId === client.publicId && !row.isPrimary
			)
		).toBe(true);
		expect(
			participants.participants.some(
				(row) => row.partyPublicId === replacement.publicId && row.isPrimary
			)
		).toBe(true);
	});
	...
	it('adds/removes non-primary opportunity parties and protects the primary customer', async () => {
		...
		expect(workspace.participants.some((row) => row.partyPublicId === consultant.publicId)).toBe(
			false
		);
		await expect(
			service.removeParticipant(actorManagerA, {
				opportunityPublicId: opportunity.publicId,
				partyPublicId: client.publicId,
				roleCode: 'customer'
			})
		).rejects.toBeInstanceOf(CrmOpportunityValidationError);
	});
	...
	it('logs chronological activities with tenant-safe party and internal-member junctions', async () => {
		...
		const service = new CrmOpportunityService(
			db,
			() => randomUUID(),
			() => new Date('2026-08-15T22:35:00.000Z')
		);
		const opportunity = await service.createOpportunity(actorManagerA, {
		...
		await service.addParticipant(actorManagerA, {
			opportunityPublicId: opportunity.publicId,
			partyPublicId: contact.publicId,
			roleCode: 'contact'
		});
		...
		expect(workspace.activities[0]).toMatchObject({
			typeCode: 'meeting',
			subject: 'Client briefing',
			direction: 'outbound'
		});
		expect(workspace.activities[0]!.parties.map((party) => party.partyPublicId).sort()).toEqual(
			[client.publicId, contact.publicId].sort()
		);
		...
		await expect(
			service.createActivity(actorViewerA, {
				opportunityPublicId: opportunity.publicId,

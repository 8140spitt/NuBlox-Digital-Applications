...
export type PerformanceBenefitMeasurementRecord =
	Selectable<StrategyPerformanceBenefitMeasurements>;
...
async function insertedId(
	result: { insertId?: bigint | undefined },
	label: string
): Promise<string> {
...
export class EnterprisePerformanceRepository {
	...
	async insertFramework(values: Insertable<StrategyPerformanceFrameworks>) {
		const result = await this.db
			.insertInto('strategy_performance_frameworks')
			.values(values)
	...
	async updateFramework(
		...
		id: string,
		values: Updateable<StrategyPerformanceFrameworks>
	) {
		await this.db
	...
	listFrameworkKpis(frameworkId: string): Promise<PerformanceFrameworkKpiRecord[]> {
		return this.db
			.selectFrom('strategy_performance_framework_kpis')
			.selectAll()
			.where('performance_framework_id', '=', frameworkId)
	...
	async insertFrameworkKpi(values: Insertable<StrategyPerformanceFrameworkKpis>) {
		const result = await this.db
			.insertInto('strategy_performance_framework_kpis')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	listPeriods(frameworkId: string): Promise<PerformancePeriodRecord[]> {
		return this.db
			.selectFrom('strategy_performance_periods')
			.selectAll()
			.where('performance_framework_id', '=', frameworkId)
	...
	async insertPeriod(values: Insertable<StrategyPerformancePeriods>) {
		const result = await this.db
			.insertInto('strategy_performance_periods')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	async updatePeriod(
		organisationId: string,
		id: string,
		values: Updateable<StrategyPerformancePeriods>
	) {
		await this.db
			.updateTable('strategy_performance_periods')
			.set(values)
			.where('organisation_id', '=', organisationId)
			.where('id', '=', id)
			.executeTakeFirstOrThrow();
	...
	listPacks(periodIds: string[]): Promise<PerformancePackRecord[]> {
		...
		return this.db
			.selectFrom('strategy_performance_packs')
			.selectAll()
			.where('performance_period_id', 'in', periodIds)
			.orderBy('created_at', 'desc')
			.execute();
	}
	...
	async insertPack(values: Insertable<StrategyPerformancePacks>) {
		const result = await this.db
			.insertInto('strategy_performance_packs')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	async updatePack(
		organisationId: string,
		id: string,
		values: Updateable<StrategyPerformancePacks>
	) {
		await this.db
	...
	listPackKpis(packIds: string[]): Promise<PerformancePackKpiRecord[]> {
		...
		return this.db
			...
			.selectAll()
			.where('performance_pack_id', 'in', packIds)
			.orderBy('id', 'asc')
			.execute();
	}
	...
	async insertPackKpi(values: Insertable<StrategyPerformancePackKpis>) {
		const result = await this.db
			.insertInto('strategy_performance_pack_kpis')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	listVariances(organisationId: string): Promise<PerformanceVarianceRecord[]> {
		return this.db
			.selectFrom('strategy_performance_variances')
			.selectAll()
			.where('organisation_id', '=', organisationId)
			.orderBy('created_at', 'desc')
			.execute();
	...
	async insertVariance(values: Insertable<StrategyPerformanceVariances>) {
		const result = await this.db
			.insertInto('strategy_performance_variances')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	async updateVariance(
		organisationId: string,
		id: string,
		values: Updateable<StrategyPerformanceVariances>
	) {
		await this.db
			.updateTable('strategy_performance_variances')
			.set(values)
			.where('organisation_id', '=', organisationId)
			.where('id', '=', id)
			.executeTakeFirstOrThrow();
	...
	listActions(organisationId: string): Promise<PerformanceActionRecord[]> {
		return this.db
			.selectFrom('strategy_performance_actions')
			.selectAll()
			.where('organisation_id', '=', organisationId)
	...
	async insertAction(values: Insertable<StrategyPerformanceActions>) {
		const result = await this.db
			.insertInto('strategy_performance_actions')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	async updateAction(
		organisationId: string,
		id: string,
		values: Updateable<StrategyPerformanceActions>
	) {
		await this.db
			.updateTable('strategy_performance_actions')
			.set(values)
			.where('organisation_id', '=', organisationId)
			.where('id', '=', id)
			.executeTakeFirstOrThrow();
	...
	async insertReview(values: Insertable<StrategyPerformanceReviews>) {
		const result = await this.db
			.insertInto('strategy_performance_reviews')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	async updateReview(
		organisationId: string,
		id: string,
		values: Updateable<StrategyPerformanceReviews>
	) {
		await this.db
			.updateTable('strategy_performance_reviews')
			.set(values)
			.where('organisation_id', '=', organisationId)
			.where('id', '=', id)
			.executeTakeFirstOrThrow();
	...
	listBenchmarks(organisationId: string): Promise<PerformanceBenchmarkRecord[]> {
		return this.db
			.selectFrom('strategy_performance_benchmarks')
			.selectAll()
			.where('organisation_id', '=', organisationId)
			.orderBy('period_end', 'desc')
			.execute();
	...
	async insertBenchmark(values: Insertable<StrategyPerformanceBenchmarks>) {
		const result = await this.db
			.insertInto('strategy_performance_benchmarks')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	listBenchmarkResults(organisationId: string): Promise<PerformanceBenchmarkResultRecord[]> {
		return this.db
			.selectFrom('strategy_performance_benchmark_results')
			.selectAll()
			.where('organisation_id', '=', organisationId)
			.orderBy('created_at', 'desc')
			.execute();
	...
	async insertBenchmarkResult(values: Insertable<StrategyPerformanceBenchmarkResults>) {
		const result = await this.db
			.insertInto('strategy_performance_benchmark_results')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	listBenefits(organisationId: string): Promise<PerformanceBenefitRecord[]> {
		return this.db
			.selectFrom('strategy_performance_benefits')
			.selectAll()
			.where('organisation_id', '=', organisationId)
	...
	async insertBenefit(values: Insertable<StrategyPerformanceBenefits>) {
		const result = await this.db
			.insertInto('strategy_performance_benefits')
			.values(values)
			.executeTakeFirstOrThrow();
	...
	async updateBenefit(
		organisationId: string,
		id: string,
		values: Updateable<StrategyPerformanceBenefits>
	) {
		await this.db
			.updateTable('strategy_performance_benefits')
			.set(values)
			.where('organisation_id', '=', organisationId)
			.where('id', '=', id)
			.executeTakeFirstOrThrow();
	...
	listBenefitMeasurements(organisationId: string): Promise<PerformanceBenefitMeasurementRecord[]> {
		return this.db
			.selectFrom('strategy_performance_benefit_measurements')
			.selectAll()
			.where('organisation_id', '=', organisationId)
	...
	async insertBenefitMeasurement(values: Insertable<StrategyPerformanceBenefitMeasurements>) {
		const result = await this.db
			.insertInto('strategy_performance_benefit_measurements')
			.values(values)
			.executeTakeFirstOrThrow();

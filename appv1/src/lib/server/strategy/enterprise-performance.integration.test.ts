...
async function cleanup(): Promise<void> {
	...
	await db
		.deleteFrom('strategy_performance_benefit_measurements')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('strategy_performance_benefits')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('strategy_performance_benchmark_results')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('strategy_performance_benchmarks')
		.where('organisation_id', 'in', ids)
		.execute();
	...
	await db
		.deleteFrom('strategy_performance_variances')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('strategy_performance_pack_kpis')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.updateTable('strategy_performance_packs')
		.set({ supersedes_performance_pack_id: null })
		.where('organisation_id', 'in', ids)
		.execute();
	...
	await db
		.deleteFrom('strategy_performance_framework_kpis')
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.updateTable('strategy_performance_frameworks')
		.set({ supersedes_performance_framework_id: null })
		.where('organisation_id', 'in', ids)
		.execute();
	await db
		.deleteFrom('strategy_performance_frameworks')
		.where('organisation_id', 'in', ids)
		.execute();
...
beforeAll(async () => {
	...
	memberId = insertedId(
		await db
			...
			.values({
				...
				public_id: randomUUID(),
				status: 'active',
				joined_at: new Date('2026-09-01T08:00:00.000Z')
			})
			.executeTakeFirstOrThrow()
	);
	...
	const strategyFrameworkId = insertedId(
		await db
			.insertInto('strategy_frameworks')
			.values({
				organisation_id: organisationId,
				public_id: randomUUID(),
				framework_code: 'F03-STRATEGY',
				version_number: 1,
				title: 'F03 approved strategy',
				horizon_start: new Date('2026-01-01T00:00:00.000Z'),
				horizon_end: new Date('2028-12-31T00:00:00.000Z'),
				purpose_text: 'Drive enterprise performance.',
				vision_text: 'Measured outcomes.',
				mission_text: 'Operate a closed performance loop.',
				lifecycle_status: 'approved',
				supersedes_strategy_framework_id: null,
				owner_member_id: memberId,
				created_by_member_id: memberId,
				approved_by_member_id: memberId,
				approved_at: new Date('2026-01-02T10:00:00.000Z')
			})
			.executeTakeFirstOrThrow()
	);
	const objectiveId = insertedId(
		await db
			...
			.values({
				organisation_id: organisationId,
				strategy_framework_id: strategyFrameworkId,
				public_id: randomUUID(),
				objective_code: 'OBJ-F03',
				title: 'Improve operating margin',
				description: 'Create measurable enterprise value.',
				priority_rank: 1,
				owner_member_id: memberId,
				target_date: new Date('2027-12-31T00:00:00.000Z'),
	...
	const kpiId = insertedId(
		await db
			...
			.values({
				...
				description: 'Canonical enterprise operating margin.',
				unit_label: '%',
				...
				source_measure_key: null,
				owner_member_id: memberId,
				lifecycle_status: 'approved',
				supersedes_strategy_kpi_id: null,
				created_by_member_id: memberId,
				approved_by_member_id: memberId,
				approved_at: new Date('2026-01-03T10:00:00.000Z')
			})
			.executeTakeFirstOrThrow()
	);
	observationPublicId = randomUUID();
	await db
		.insertInto('strategy_kpi_observations')
		.values({
			organisation_id: organisationId,
			strategy_kpi_id: kpiId,
			public_id: observationPublicId,
			observed_on: new Date('2026-08-31T00:00:00.000Z'),
			actual_value: '9.5',
			forecast_value: '10.8',
			commentary: 'Below target; intervention required.',
			source_mode: 'manual',
			source_domain: null,
			source_record_type: null,
...
describe('F03 enterprise performance management', () => {
	it('runs framework → reporting → variance/action → review → benchmark → benefit measurement as one governed thread', async () => {
		...
		const framework = await service.createFramework(actor, {
			frameworkCode: 'PERF-2026',
			title: 'Enterprise performance framework',
			purposeText: 'Operate one governed enterprise performance cycle.',
			reportingCadence: 'monthly',
			scopeText: 'Enterprise KPIs and executive interventions.',
			ownerMemberId: memberId,
			effectiveFrom: '2026-01-01'
		});
		await service.addFrameworkKpi(actor, {
			frameworkPublicId: framework.public_id,
			kpiPublicId,
			displayOrder: 1,
			materialityThresholdPercent: '5',
			commentaryRequired: true
		});
		...
		const period = await service.createPeriod(actor, {
			frameworkPublicId: framework.public_id,
			periodCode: '2026-08',
			title: 'August 2026 performance',
			periodStart: '2026-08-01',
			periodEnd: '2026-08-31',
			reportingDate: '2026-09-05'
		});
		const pack = await service.createPack(actor, {
			periodPublicId: period.public_id,
			packCode: 'PACK-2026-08',
			title: 'August executive performance pack',
			executiveSummary: 'Operating margin is below target and requires intervention.'
		});
		...
		const variance = await service.createVariance(actor, {
			packKpiId: packKpi.id,
			varianceCode: 'VAR-MARGIN-01',
			materiality: 'high',
			causeCategory: 'productivity',
			rootCauseText: 'Cost productivity is behind the approved operating plan.',
			impactText: 'Operating margin remains below the approved target.',
			ownerMemberId: memberId
		});
		const action = await service.createCorrectiveAction(actor, {
			variancePublicId: variance.public_id,
			actionCode: 'ACT-MARGIN-01',
			title: 'Recover margin gap',
			actionText: 'Execute the approved cost productivity recovery plan.',
			ownerMemberId: memberId,
			dueDate: '2026-10-31',
			sourceDomain: 'projects',
			sourceRecordType: 'project_action',
			sourcePublicId: 'project-action-f03'
		});
		await service.completeCorrectiveAction(
			actor,
			action.public_id,
			'Recovery work mobilised and verified.'
		);
		const closedVariance = await service.closeVariance(
			actor,
			variance.public_id,
			'Corrective action completed with governed evidence.'
		);
		expect(closedVariance.lifecycle_status).toBe('closed');

		const review = await service.createReview(actor, {
			packPublicId: pack.public_id,
			reviewCode: 'REV-2026-08',
			reviewDate: '2026-09-07',
			title: 'August executive performance review',
			summary: 'Executive review accepted the margin intervention.',
			decisionText: 'Maintain target and monitor recovery weekly.'
		});
		...
		const benchmark = await service.createBenchmark(actor, {
			kpiPublicId,
			benchmarkCode: 'BM-MARGIN-PEER',
			benchmarkType: 'peer',
			title: 'Peer operating margin',
			scopeText: 'Comparable peer benchmark.',
			unitLabel: '%',
			periodStart: '2026-01-01',
			periodEnd: '2026-08-31',
			benchmarkValue: '11.5',
			provenanceText: 'Controlled peer benchmark evidence.',
			sourceReference: 'peer-set-2026'
		});
		const comparison = await service.compareBenchmark(
			actor,
			benchmark.public_id,
			observationPublicId,
			'Current margin trails the peer benchmark and validates the recovery intervention.'
		);
		expect(Number(comparison.gap_value)).toBeCloseTo(-2);

		const benefit = await service.createBenefit(actor, {
			kpiPublicId,
			benefitCode: 'BEN-MARGIN',
			title: 'Margin recovery benefit',
			...
			sourceDomain: 'projects',
			sourceRecordType: 'programme',
			sourcePublicId: 'margin-recovery-programme'
		});
		const measurement = await service.recordBenefitMeasurement(actor, {
			benefitPublicId: benefit.public_id,
			measuredOn: '2026-09-30',
			realisedValue: '10.25',
			confidencePercent: '85',
			evidenceText: 'September finance close confirms the first realised margin improvement.',
			sourceDomain: 'finance',
			sourceRecordType: 'period_close',
			sourcePublicId: '2026-09'
		});
		...
		const auditCount = await db
			...
			.where('action_key', 'like', 'performance.%')
			.executeTakeFirstOrThrow();
		const outboxCount = await db
			.selectFrom('outbox_events')
			.select(({ fn }) => fn.countAll<number>().as('count'))
			.where('organisation_id', '=', organisationId)
			.where('topic', 'like', 'performance.%')
			.executeTakeFirstOrThrow();

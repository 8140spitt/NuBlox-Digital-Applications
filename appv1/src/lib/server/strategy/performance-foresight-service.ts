...
function normalizedCode(value: string, label: string): string {
	...
	if (!CODE.test(normalized))
		throw new PerformanceForesightValidationError(`${label} has an invalid format.`);
	return normalized;
}
...
function dateOnly(value: string | Date, label: string): Date {
	...
	if (Number.isNaN(parsed.getTime()))
		throw new PerformanceForesightValidationError(`${label} is invalid.`);
...
function assertCanonicalSource(
	...
	if (Object.values(parts).some((value) => !value)) {
		throw new PerformanceForesightValidationError(
			'Canonical source evidence requires domain, record and measure identifiers.'
		);
	}
}
...
export class PerformanceForesightService {
	...
	private async permissionFlags(actor: TenantActorContext) {
		...
		if (!view.allowed && !manage.allowed && !approve.allowed)
			throw new RecordNotFoundError(
				'Strategy performance workspace not found in the active scope.'
			);
	...
	private async requirePermission(
		...
		if (!decision.allowed)
			throw new TenantAccessError('Strategy performance action is not permitted.');
	}
	...
	private async approvedFramework(
		db: DatabaseExecutor,
		organisationId: string,
		publicId: string
		...
		if (framework.lifecycle_status !== 'approved')
			throw new PerformanceForesightValidationError(
				'Performance management requires an approved strategy framework.'
			);
		return framework;
	}
	...
	async getWorkspace(
	...
	): Promise<PerformanceForesightWorkspace> {
		...
		if (selectedFrameworkPublicId?.trim()) {
			...
			if (!selectedFramework)
				throw new RecordNotFoundError('Approved strategy framework not found.');
		}
		if (!selectedFramework) {
			return {
				frameworks,
	...
	async createKpi(actor: TenantActorContext, input: KpiInput): Promise<StrategyKpiRecord> {
		...
		return this.db.transaction().execute(async (trx) => {
			const framework = await this.approvedFramework(
				trx,
				actor.organisationId,
				input.frameworkPublicId
			);
			const objective = await trx
				.selectFrom('strategy_objectives')
				.selectAll()
				.where('organisation_id', '=', actor.organisationId)
				.where('strategy_framework_id', '=', framework.id)
				.where('public_id', '=', input.objectivePublicId.trim())
			...
			if (await repository.findLatestKpiVersion(actor.organisationId, kpiCode))
				throw new PerformanceForesightValidationError(
					'A KPI with this code already exists; create a controlled revision instead.'
				);
			...
			await this.appendEvidence(
				...
				'strategy_kpi',
				kpi.public_id,
				{ kpiCode, objectivePublicId: objective.public_id, sourceMode: input.sourceMode },
				['F01.06']
			);
			return kpi;
		});
	...
	async approveKpi(actor: TenantActorContext, kpiPublicId: string): Promise<StrategyKpiRecord> {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (kpi.lifecycle_status !== 'draft')
				throw new PerformanceForesightValidationError('Only draft KPIs can be approved.');
			const approved = await repository.updateKpi(actor.organisationId, kpi.id, {
			...
			await this.appendEvidence(
				...
				'strategy_kpi',
				approved.public_id,
				{ kpiCode: approved.kpi_code, versionNumber: approved.version_number },
				['F01.06']
			);
			return approved;
		});
	...
	async reviseKpi(actor: TenantActorContext, kpiPublicId: string): Promise<StrategyKpiRecord> {
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const kpi = await repository.findKpiByPublicId(actor.organisationId, kpiPublicId.trim());
			if (!kpi) throw new RecordNotFoundError('Strategy KPI not found.');
			if (kpi.lifecycle_status !== 'approved')
				throw new PerformanceForesightValidationError(
					'Only approved KPIs can start a controlled revision.'
				);
			...
			const revision = await repository.insertKpi({
				organisation_id: actor.organisationId,
				strategy_framework_id: kpi.strategy_framework_id,
				strategy_objective_id: kpi.strategy_objective_id,
				public_id: this.publicIdFactory(),
				kpi_code: kpi.kpi_code,
				version_number: kpi.version_number + 1,
				title: kpi.title,
				...
				source_mode: kpi.source_mode,
				source_domain: kpi.source_domain,
				source_record_type: kpi.source_record_type,
				source_measure_key: kpi.source_measure_key,
				owner_member_id: kpi.owner_member_id,
				lifecycle_status: 'draft',
				supersedes_strategy_kpi_id: kpi.id,
				created_by_member_id: actor.memberId,
				approved_by_member_id: null,
				approved_at: null
			});
			await this.appendEvidence(
				...
				revision.public_id,
				{
					kpiCode: revision.kpi_code,
					versionNumber: revision.version_number,
					supersedesPublicId: kpi.public_id
				},
				['F01.06']
			);
			return revision;
	...
	async recordObservation(
		actor: TenantActorContext,
		input: KpiObservationInput
	): Promise<StrategyKpiObservationRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new PerformanceForesightRepository(trx);
			...
			const sourceMeasureKey = sourceToken(input.sourceMeasureKey, 'Source measure key');
			assertCanonicalSource(input.sourceMode, {
				sourceDomain,
				sourceRecordType,
				sourcePublicId,
				sourceMeasureKey
			...
			const observation = await repository.insertObservation({
				...
				public_id: this.publicIdFactory(),
				observed_on: dateOnly(input.observedOn, 'Observed date'),
				actual_value: decimal(input.actualValue, 'Actual value'),
			...
			await this.appendEvidence(
				trx,
				actor,
				'strategy.kpi.observe',
				'strategy_kpi_observation',
				observation.public_id,
				{
					...
					sourceMode: observation.source_mode
				},
				['F01.06', 'F01.07']
			);
			return observation;
	...
	async createAction(
		actor: TenantActorContext,
		input: KpiActionInput
	): Promise<StrategyKpiActionRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new PerformanceForesightRepository(trx);
			const kpi = await repository.findKpiByPublicId(
			...
			if (input.observationPublicId?.trim()) {
				const observation = await repository.findObservationByPublicId(
					actor.organisationId,
					input.observationPublicId.trim()
				);
				if (!observation || observation.strategy_kpi_id !== kpi.id)
					throw new PerformanceForesightValidationError(
						'Corrective-action observation must belong to the selected KPI.'
					);
			...
			const action = await repository.insertAction({
				organisation_id: actor.organisationId,
				strategy_kpi_id: kpi.id,
				strategy_kpi_observation_id: observationId,
				public_id: this.publicIdFactory(),
				action_code: normalizedCode(input.actionCode, 'Action code'),
				title: requiredText(input.title, 'Action title', 255),
				action_text: requiredText(input.actionText, 'Action', 20_000),
				owner_member_id: await this.activeMember(
					trx,
					actor.organisationId,
					input.ownerMemberId,
					'Action owner'
			...
			await this.appendEvidence(
				trx,
				actor,
				'strategy.kpi.action.create',
				'strategy_kpi_action',
				action.public_id,
				{
					kpiPublicId: kpi.public_id,
					actionCode: action.action_code,
					dueDate: action.due_date.toISOString().slice(0, 10)
				},
				['F01.06', 'F01.07']
			);
			return action;
	...
	async completeAction(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (!['open', 'in_progress'].includes(action.lifecycle_status))
				throw new PerformanceForesightValidationError(
					'Only open or in-progress actions can be completed.'
				);
			const completed = await repository.updateAction(actor.organisationId, action.id, {
				lifecycle_status: 'completed',
				completion_note: requiredText(completionNote, 'Completion note', 20_000),
			...
			await this.appendEvidence(
				trx,
				actor,
				'strategy.kpi.action.complete',
				'strategy_kpi_action',
				completed.public_id,
				{ actionCode: completed.action_code, lifecycleStatus: 'completed' },
				['F01.06', 'F01.07']
			);
			return completed;
		});
	...
	async createReview(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			const review = await new PerformanceForesightRepository(trx).insertReview({
				...
				strategy_framework_id: framework.id,
				public_id: this.publicIdFactory(),
				review_code: normalizedCode(input.reviewCode, 'Review code'),
				...
				decisions_text: optionalText(input.decisionsText, 20_000),
				lifecycle_status: 'draft',
				created_by_member_id: actor.memberId,
				approved_by_member_id: null,
				approved_at: null
			...
			await this.appendEvidence(
				...
				'strategy_review',
				review.public_id,
				{ reviewCode: review.review_code, frameworkPublicId: framework.public_id },
				['F01.07']
			);
			return review;
		});
	...
	async addReviewKpi(
		...
		assessment: 'on_track' | 'watch' | 'off_track' | 'not_measured',
		commentary?: string | null
	): Promise<StrategyReviewKpiRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new PerformanceForesightRepository(trx);
			const review = await repository.findReviewByPublicId(
				actor.organisationId,
				reviewPublicId.trim()
			);
			if (!review) throw new RecordNotFoundError('Strategy review not found.');
			if (review.lifecycle_status !== 'draft')
				throw new PerformanceForesightValidationError('Approved strategy reviews are immutable.');
			const kpi = await repository.findKpiByPublicId(actor.organisationId, kpiPublicId.trim());
			if (
				!kpi ||
				kpi.lifecycle_status !== 'approved' ||
				kpi.strategy_framework_id !== review.strategy_framework_id
			)
				throw new PerformanceForesightValidationError(
					'Review KPI must be an approved KPI in the same strategy framework.'
				);
			const observation = await repository.findObservationByPublicId(
				actor.organisationId,
			...
			if (!observation || observation.strategy_kpi_id !== kpi.id)
				throw new PerformanceForesightValidationError(
					'Review observation must belong to the selected KPI.'
				);
			...
			const snapshot = await repository.insertReviewKpi({
				...
				variance_value: String(variance),
				variance_percent: target === 0 ? null : String((variance / Math.abs(target)) * 100),
				assessment,
			...
			await this.appendEvidence(
				...
				review.public_id,
				{
					...
					assessment,
					varianceValue: snapshot.variance_value
				},
				['F01.07']
			);
			return snapshot;
	...
	async approveReview(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (review.lifecycle_status !== 'draft')
				throw new PerformanceForesightValidationError(
					'Only draft strategy reviews can be approved.'
				);
			if (!(await repository.listReviewKpisForReview(review.id)).length)
				throw new PerformanceForesightValidationError(
					'A strategic review requires at least one KPI observation snapshot.'
				);
			...
			await this.appendEvidence(
				...
				approved.public_id,
				{
					...
					reviewDate: approved.review_date.toISOString().slice(0, 10)
				},
				['F01.07']
			);
			return approved;
	...
	async createScenario(
	...
	): Promise<StrategyScenarioRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		...
		if (horizonEnd < horizonStart)
			throw new PerformanceForesightValidationError(
				'Scenario horizon end must not precede its start.'
			);
		return this.db.transaction().execute(async (trx) => {
			const framework = await this.approvedFramework(
				trx,
			...
			if (await repository.findLatestScenarioVersion(actor.organisationId, scenarioCode))
				throw new PerformanceForesightValidationError(
					'A scenario with this code already exists; create a controlled revision instead.'
				);
			const scenario = await repository.insertScenario({
				...
				version_number: 1,
				title: requiredText(input.title, 'Scenario title', 255),
				scenario_type: input.scenarioType,
				horizon_start: horizonStart,
				horizon_end: horizonEnd,
				narrative: requiredText(input.narrative, 'Scenario narrative', 20_000),
				lifecycle_status: 'draft',
				supersedes_strategy_scenario_id: null,
				created_by_member_id: actor.memberId,
				approved_by_member_id: null,
				approved_at: null
			});
	...
	): Promise<StrategyScenarioAssumptionRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new PerformanceForesightRepository(trx);
			const scenario = await repository.findScenarioByPublicId(
				actor.organisationId,
				scenarioPublicId.trim()
			);
			if (!scenario) throw new RecordNotFoundError('Strategy scenario not found.');
			if (scenario.lifecycle_status !== 'draft')
				throw new PerformanceForesightValidationError(
					'Approved scenarios are immutable; create a controlled revision instead.'
				);
			const sensitivity = optionalDecimal(input.sensitivityPercent, 'Sensitivity percent');
			if (sensitivity !== null && Number(sensitivity) < 0)
				throw new PerformanceForesightValidationError('Sensitivity percent must not be negative.');
			const assumption = await repository.insertScenarioAssumption({
				organisation_id: actor.organisationId,
				strategy_scenario_id: scenario.id,
				public_id: this.publicIdFactory(),
				assumption_code: normalizedCode(input.assumptionCode, 'Assumption code'),
				title: requiredText(input.title, 'Assumption title', 255),
				description: requiredText(input.description, 'Assumption description', 20_000),
				variable_key: sourceToken(input.variableKey, 'Variable key') ?? '',
				unit_label: requiredText(input.unitLabel, 'Unit', 64),
				baseline_value: decimal(input.baselineValue, 'Baseline value'),
				scenario_value: decimal(input.scenarioValue, 'Scenario value'),
			...
			await this.appendEvidence(
				trx,
				actor,
				'strategy.scenario.assumption.add',
				'strategy_scenario',
				scenario.public_id,
				{ assumptionCode: assumption.assumption_code, variableKey: assumption.variable_key },
				['F01.08']
			);
			return assumption;
		});
	...
	async addScenarioProjection(
		actor: TenantActorContext,
		scenarioPublicId: string,
		kpiPublicId: string,
		projectionDate: string | Date,
		projectedValue: string | number,
		rationale: string
	): Promise<StrategyScenarioKpiProjectionRecord> {
		await this.requirePermission(actor, 'strategy.manage');
		return this.db.transaction().execute(async (trx) => {
			const repository = new PerformanceForesightRepository(trx);
			const scenario = await repository.findScenarioByPublicId(
			...
			if (
				!kpi ||
				kpi.lifecycle_status !== 'approved' ||
				kpi.strategy_framework_id !== scenario.strategy_framework_id
			)
				throw new PerformanceForesightValidationError(
					'Scenario projection KPI must be approved in the same strategy framework.'
				);
			const date = dateOnly(projectionDate, 'Projection date');
			if (date < scenario.horizon_start || date > scenario.horizon_end)
				throw new PerformanceForesightValidationError(
					'Projection date must sit inside the scenario horizon.'
				);
			const projection = await repository.insertScenarioProjection({
				organisation_id: actor.organisationId,
				strategy_scenario_id: scenario.id,
				strategy_kpi_id: kpi.id,
				public_id: this.publicIdFactory(),
				projection_date: date,
				projected_value: decimal(projectedValue, 'Projected value'),
				rationale: requiredText(rationale, 'Projection rationale', 20_000),
				created_by_member_id: actor.memberId
			});
			await this.appendEvidence(
				trx,
				actor,
				'strategy.scenario.projection.add',
				'strategy_scenario',
				scenario.public_id,
				{
					kpiPublicId: kpi.public_id,
					projectionDate: date.toISOString().slice(0, 10),
	...
	async approveScenario(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (!scenario) throw new RecordNotFoundError('Strategy scenario not found.');
			if (scenario.lifecycle_status !== 'draft')
				throw new PerformanceForesightValidationError('Only draft scenarios can be approved.');
			const [assumptions, projections] = await Promise.all([
				repository.listScenarioAssumptionsForScenario(scenario.id),
				repository.listScenarioProjectionsForScenario(scenario.id)
			]);
			if (!assumptions.length || !projections.length)
				throw new PerformanceForesightValidationError(
					'Scenario approval requires at least one assumption and one KPI projection.'
				);
			const approved = await repository.updateScenario(actor.organisationId, scenario.id, {
				lifecycle_status: 'approved',
				approved_by_member_id: actor.memberId,
				approved_at: this.now()
			});
			await this.appendEvidence(
				...
				approved.public_id,
				{
					scenarioCode: approved.scenario_code,
	...
	async reviseScenario(
		...
		return this.db.transaction().execute(async (trx) => {
			...
			if (!scenario) throw new RecordNotFoundError('Strategy scenario not found.');
			if (scenario.lifecycle_status !== 'approved')
				throw new PerformanceForesightValidationError(
					'Only approved scenarios can start a controlled revision.'
				);
			const [assumptions, projections] = await Promise.all([
				repository.listScenarioAssumptionsForScenario(scenario.id),
				repository.listScenarioProjectionsForScenario(scenario.id)
			]);
			await repository.updateScenario(actor.organisationId, scenario.id, {
				lifecycle_status: 'superseded'
			});
			const revision = await repository.insertScenario({
				organisation_id: actor.organisationId,
				strategy_framework_id: scenario.strategy_framework_id,
				public_id: this.publicIdFactory(),
				scenario_code: scenario.scenario_code,
				version_number: scenario.version_number + 1,
				...
				lifecycle_status: 'draft',
				supersedes_strategy_scenario_id: scenario.id,
				created_by_member_id: actor.memberId,
				approved_by_member_id: null,
				approved_at: null
			});
			for (const assumption of assumptions) {
				await repository.insertScenarioAssumption({
					organisation_id: actor.organisationId,
					strategy_scenario_id: revision.id,
					public_id: this.publicIdFactory(),
					assumption_code: assumption.assumption_code,
					title: assumption.title,
					description: assumption.description,
					variable_key: assumption.variable_key,
					unit_label: assumption.unit_label,
					baseline_value: assumption.baseline_value,
					scenario_value: assumption.scenario_value,
					sensitivity_percent: assumption.sensitivity_percent,
					created_by_member_id: actor.memberId
				});
			}
			for (const projection of projections) {
				await repository.insertScenarioProjection({
					organisation_id: actor.organisationId,
					strategy_scenario_id: revision.id,
					strategy_kpi_id: projection.strategy_kpi_id,
					public_id: this.publicIdFactory(),
			...
			await this.appendEvidence(
				trx,
				actor,
				'strategy.scenario.revise',
				'strategy_scenario',
				revision.public_id,
				{
					scenarioCode: revision.scenario_code,
					versionNumber: revision.version_number,
					supersedesPublicId: scenario.public_id
				},
				['F01.08']
			);
			return revision;
